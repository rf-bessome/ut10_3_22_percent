...
Процедура НеСопоставленныеКлючиАдресовОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "СопоставитьМестоположение" Тогда
		НеСопоставленныеКлючиАдресовСтруктураОтбора(ЭтаФорма);
		...
		ОткрытьФорму(
			...
			Новый ОписаниеОповещения("НеСопоставленныеКлючиОкончаниеРедактирования", ЭтаФорма),
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "СписокМестоположениеПредставление" Тогда
		Если ЗначениеЗаполнено(ДанныеСтроки.Местоположение) Тогда
			...
			ОткрытьФорму("Справочник.КлючиАдресовЗЕРНО.ФормаОбъекта", ПараметрыОткрытия, ЭтаФорма,,,,
				Новый ОписаниеОповещения("МестоположениеОкончаниеРедактирования", ЭтаФорма),
				РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	ИначеЕсли Поле.Имя = "СписокВладелецПартииПредставление" Тогда
		Если ЗначениеЗаполнено(ДанныеСтроки.ВладелецПартии) Тогда
			...
			ОткрытьФорму("Справочник.КлючиРеквизитовОрганизацийЗЕРНО.ФормаОбъекта", ПараметрыОткрытия, ЭтаФорма,,,,
				Новый ОписаниеОповещения("ВладелецПартииОкончаниеРедактирования", ЭтаФорма),
				РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	...
	ИначеЕсли Поле.Имя = "СписокПартия" Или Поле.Имя = "СписокПродукция" Тогда
		Если ЗначениеЗаполнено(ДанныеСтроки.Партия) Тогда
			...
			ОткрытьФорму("Справочник.РеестрПартийЗЕРНО.ФормаОбъекта", ПараметрыОткрытия, ЭтаФорма,,,,
				Новый ОписаниеОповещения("ПартияОкончаниеРедактирования", ЭтаФорма),
				РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗапроситьПартии(Команда)
	...
	ЗапросСДИЗЗавершение = Новый ОписаниеОповещения("ЗапросПартийЗавершение", ЭтаФорма);
...
Процедура НастроитьОтборСпискаДляВыбора()
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры.Местоположение) Тогда
		...
		КонецЕсли;
		ПриИзмененииОтбора(ЭтаФорма, "МестоположениеСписок", Ложь, Ложь);
	КонецЕсли;
...
Процедура УстановитьБыстрыйОтборСервер()
		...
		Если ЗначениеЗаполнено(Организации) Тогда
			...
			Если КоличествоКлючей > 0 Тогда
				...
				Если КоличествоЭлеваторов > 0 И КоличествоЭлеваторов = КоличествоКлючей Тогда
					...
					ПриИзмененииОтбора(ЭтаФорма, "ЭлеваторСписок", Ложь, Ложь);
				Иначе
					ВладелецПартииСписок = Организации;
					ВладелецПартииКлючи.ЗагрузитьЗначения(Ключи);
					ПриИзмененииОтбора(ЭтаФорма, "ВладелецПартииСписок", Ложь, Ложь);
				КонецЕсли;
	...
	ОбновитьНадписьНеСопоставленныеКлючиАдресов(ЭтаФорма);
...
Процедура ПолеОтбораНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ВыбранныеЗначения", ЭтаФорма[ИмяЭлементаОтбораСписок].ВыгрузитьЗначения());
	...
	ОткрытьФорму("РегистрНакопления.ОстаткиПартийЗЕРНО.Форма.ФормаВыбораЗначений", ПараметрыОткрытия, ЭтаФорма,,,,
		Новый ОписаниеОповещения("ПолеОтбораОкончаниеВыбора", ЭтаФорма, ДополнительныеПараметры),
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПолеОтбораОкончаниеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	ЭтаФорма[ИмяЭлементаОтбораСписок].ЗагрузитьЗначения(Результат);
	ПриИзмененииОтбора(ЭтаФорма, ИмяЭлементаОтбораСписок);
...
Функция ДанныеДляСписка(Строки)
	...
	Таблица.Колонки.Добавить("ОрганизацияКонтрагент", ОпределяемыеТипы().ОрганизацияКонтрагентГосИС.Тип);
	Таблица.Колонки.Добавить("Подразделение",         ОпределяемыеТипы().Подразделение.Тип);
...
Процедура ПолеСоставногоТипаПриИзменении(Элемент)
	...
	ПриИзмененииОтбора(ЭтаФорма, Элемент.Имя);
...
Процедура ПолеСоставногоТипаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ЭтаФорма[СтрЗаменить(Элемент.Имя, "Строкой", "Список")].Количество() > 1 Тогда
		СтандартнаяОбработка = Ложь;
		Возврат;
	КонецЕсли;
...
Процедура ПолеСоставногоТипаОкончаниеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	ПриИзмененииОтбора(ЭтаФорма, ДополнительныеПараметры.Элемент.Имя);
