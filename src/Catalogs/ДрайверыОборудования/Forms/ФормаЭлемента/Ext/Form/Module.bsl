...
Процедура УстановитьДрайверКоманда(Команда)
	...
	Если Модифицированность Тогда
		...
		Оповещение = ОписаниеОповещенияИС("УстановитьДрайверКоманда_Завершение", ЭтаФорма);
		ПоказатьВопросИС(Оповещение, Текст, РежимДиалогаВопрос.ДаНет);
	Иначе
		УстановитьДрайвер();
	КонецЕсли
...
Процедура ВыгрузитьФайлДрайвераКоманда(Команда)
	...
	Иначе 
		...
		Если Модифицированность Тогда
			Текст = НСтр("ru='Продолжение операции возможно только после записи данных.
				|Записать данные и продолжить?'");
			Оповещение = ОписаниеОповещенияИС("ВыгрузитьФайлДрайвераКоманда_Завершение", ЭтаФорма);
			ПоказатьВопросИС(Оповещение, Текст, РежимДиалогаВопрос.ДаНет);
		Иначе
...
Процедура ЗагрузитьФайлДрайвераКоманда(Команда)
	...
	Оповещение = ОписаниеОповещенияИС("ЗагрузитьФайлДрайвераКоманда_ВыборФайлаЗавершение", ЭтаФорма);
...
Процедура ПолучитьИнформациюДрайвераПоФайлу(ПолноеИмяФайла) 
	...
	Оповещение = ОписаниеОповещенияИС("ПолучитьИнформациюДрайвераПоФайлу_ИнициализацияФайлаЗавершение", ЭтаФорма, ПараметрыВыполнения);
...
Процедура ПолучитьИнформациюДрайвераПоФайлу_ИнициализацияФайлаЗавершение(ФайлДрайвера, ПараметрыВыполнения) Экспорт 
	...
	Оповещение = ОписаниеОповещенияИС("ПолучитьИнформациюДрайвераПоФайлу_Завершение", ЭтаФорма, ПараметрыВыполнения);
	НачатьПолучениеКаталогаВременныхФайловИС(Оповещение);
...
Процедура ПолучитьИнформациюДрайвераПоФайлу_Завершение(Результат, ПараметрыВыполнения) Экспорт 
	...
	ИначеЕсли ИмяФайлаРасширение = ".ZIP" Тогда
		...
		Для Каждого ЭлементАрхива Из АрхивДрайвера.Элементы Цикл
			...
			Если ВРег(ЭлементАрхива.Имя) = "INFO.XML" Тогда
				...
				НачатьУдалениеФайловИС(, ВременныйКаталог + "INFO.XML");
			КонецЕсли;
	...
	Иначе
		ПоказатьПредупреждениеИС(, НСтр("ru='Неверное расширение файла.'"));
	КонецЕсли;
...
Процедура ЗагрузитьФайлДрайвераВБазу(ПолноеИмяФайла, ИмяФайла) Экспорт 
	...
	Оповещение = ОписаниеОповещенияИС("ЗагрузитьФайлДрайвераВБазу_Завершение", ЭтаФорма, ИмяФайла);
	НачатьПомещениеФайловИС(Оповещение, Неопределено, ПолноеИмяФайла, Ложь) 
...
Процедура ПолучитьОписаниеНовыйЗавершение(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт;
	...
	Попытка
		ОповещениеМетода = ОписаниеОповещенияИС("ПолучитьРевизиюИнтерфейсаЗавершение", ЭтаФорма);
...
Процедура ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, Параметры) Экспорт
	...
	Иначе
		...
		ТекущаяВерсия = "";
		Попытка
			ОповещениеМетода = ОписаниеОповещенияИС("ПолучитьНомерВерсииЗавершение", ЭтаФорма);
		...
		Попытка
			...
			URLЗагрузкиДрайвера       = "";
			ОповещениеМетода = ОписаниеОповещенияИС("ПолучитьОписаниеЗавершение", ЭтаФорма);
		...
		Исключение
			...
			ОписаниеДрайвера = "";
			ОповещениеМетода = ОписаниеОповещенияИС("ПолучитьОписаниеНовыйЗавершение", ЭтаФорма, ОбъектДрайвера);
			Попытка
...
Процедура ОбновитьСтатусДрайвера();
	...
	Оповещение = ОписаниеОповещенияИС("ПолучениеОбъектаДрайвераЗавершение", ЭтаФорма);
...
Процедура УстановитьДрайвер()
	...
	ОповещенияДрайверИзДистрибутиваПриЗавершении = ОписаниеОповещенияИС("УстановитьДрайверИзДистрибутиваПриЗавершении", ЭтаФорма);
	ОповещенияДрайверИзАрхиваПриЗавершении = ОписаниеОповещенияИС("УстановитьДрайверИзАрхиваПриЗавершении", ЭтаФорма);
