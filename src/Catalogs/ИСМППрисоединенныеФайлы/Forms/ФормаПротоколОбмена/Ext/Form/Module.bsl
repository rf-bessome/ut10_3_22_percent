...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияИС.ОтработатьВходящийДокументПротоколаОбмена(ЭтаФорма);
	
	Если НЕ ОпределяемыеТипы().ДокументыИСМП.Тип.СодержитТип(ТипЗнч(Документ)) Тогда
...
Процедура ЗаполнитьДеревоФайлов()
	...
	ЭтоДокументИСМП = ОпределяемыеТипы().ДокументыИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено;
	...
	Если ОпределяемыеТипы().ОснованиеМаркировкаТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаМаркировкаТоваровИСМП(Документ);
		
	КонецЕсли;
	
	Если ОпределяемыеТипы().ОснованиеЗаказНаЭмиссиюКодовМаркировкиИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаЗаказНаЭмиссиюКодовМаркировкиСУЗ(Документ);
		
	КонецЕсли;
	
	Если ОпределяемыеТипы().ОснованиеВыводаИзОборотаИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаВыводИзОборотаИСМП(Документ);
		
	КонецЕсли;
	
	Если ОпределяемыеТипы().ОснованиеСписаниеКодовМаркировкиИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаСписаниеКодовМаркировкиИСМП(Документ);
		
	КонецЕсли;
	
	Если ОпределяемыеТипы().ОснованиеПеремаркировкиТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаПеремаркировкаТоваровИСМП(Документ);
		
	ИначеЕсли ОпределяемыеТипы().ОснованиеВозвратВОборотИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаВозвратВОборотИСМП(Документ);
		
	КонецЕсли;
	
	Если ОпределяемыеТипы().ОснованиеОтгрузкаТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаОтгрузкаТоваровИСМП(Документ);
		
	КонецЕсли;
	
	Если ОпределяемыеТипы().ОснованиеПриемкаТоваровИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
		
		ЗаполнитьПоОснованиюДокументаПриемкаТоваровИСМП(Документ);
		
	КонецЕсли;
	
	Если ОпределяемыеТипы().ОснованиеОтчетИСМП.Тип.Типы().Найти(ТипДокумента) <> Неопределено Тогда
...
Процедура ПоказатьСообщенияОперации(ВыбраннаяСтрока)
	...
	Иначе
		...
		ИнтеграцияИСКлиент.ПоказатьСообщенияОперации(ЭтаФорма, "ИСМП", ВыбраннаяСтрока);
