...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	Пока Не ОтработаныВсеДанныеМЧД003 Цикл
		...
		НаборСсылок = ДанныеМЧД003.ВыгрузитьКолонку("Ссылка");
...
Процедура ДополнитьСписокДоверенностейПоОтбору(Отбор, Доверенности) Экспорт
	...
	Если ТипЗнч(Отбор.Сертификат) = Тип("СертификатКриптографии") Тогда
		СвойстваСубъектаСертификата = МашиночитаемыеДоверенности.СвойстваСубъектаСертификата(Отбор.Сертификат);
	Иначе
		СвойстваСубъектаСертификата = МашиночитаемыеДоверенности.СвойстваСубъектаСертификатаПоСсылке(Отбор.Сертификат);
	КонецЕсли;
...
Функция ПолучитьИмяФайлаМЧД(Ссылка) Экспорт
	...
	ЭлементыИмениФайла = МашиночитаемыеДоверенностиКлиентСервер.ЗначениеВМассиве(МашиночитаемыеДоверенности.ПространствоИмен_МЧД_003());
...
Функция ДанныеДляЗаполненияМЧД(ОбъектМЧД) Экспорт
	...
	СвДовXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Довер.СвДов", ,,Истина);
	Если СвДовXDTO <> Неопределено Тогда
		Сведения = СвДовXDTO;
	КонецЕсли;
	
	СвПереДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПереДовер",,,Истина);
	Если СвПереДоверXDTO <> Неопределено Тогда
		Сведения = СвПереДоверXDTO;
	КонецЕсли;
	
	СвПервДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПервДовер", ,,Истина);
	Если СвПервДоверXDTO <> Неопределено Тогда

		ДанныеДляЗаполнения.НомерРодительскойДоверенности = СвПервДоверXDTO.НомДоверПерв;
		
		СвДоверПервXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПервДовер.СвДоверПерв", ,Истина); 
		...
		Если ТипУчастника = ТипыДоверителей.ЮридическоеЛицо Тогда
			СвРосОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.РосОргДовер");
		...
		ИначеЕсли ТипУчастника = ТипыДоверителей.ИндивидуальныйПредприниматель Тогда	
			ИППерПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.ИПДовер");
		...
		ИначеЕсли ТипУчастника = ТипыДоверителей.ФизическоеЛицо Тогда	
			ФЛПерПолнЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.ФЛДовер");
		...
		ИначеЕсли ТипУчастника = ТипыДоверителей.ИностраннаяОрганизация Тогда	
			СвИнОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.ИнОргДовер");
	...
	ДанныеДляЗаполнения.НомерДоверенности = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Сведения, "НомДовер", "Строка");
	ДанныеДляЗаполнения.ДатаВыдачи = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Сведения, "ДатаВыдДовер", "Дата");
	ДанныеДляЗаполнения.СрокДействия = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Сведения, "СрокДейст", "Дата");
	
	ПрПередов = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Сведения, "ПрПередов");
...
Функция ДанныеДляПодачиЗаявленияНаОтмену(ДанныеДоверенности) Экспорт
	...
	ИначеЕсли ДанныеДоверенности.Документ.Довер <> Неопределено
		...
		Если НаименованиеИП = "" И СведенияОДоверителе.СведФЛ <> Неопределено Тогда
			НаименованиеИП = МашиночитаемыеДоверенностиКлиентСервер.ФИОСтрокой(СведенияОДоверителе.СведФЛ.ФИО);
		КонецЕсли;
...
Функция ПолучитьУчастника_ЮридическоеЛицо(ДанныеXDTO)
	...
	Участник.ИНН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДанныеXDTO, "ИННЮЛ", "Строка");
	Участник.КПП = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДанныеXDTO, "КПП", "Строка");
	Участник.ОГРН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДанныеXDTO, "ОГРН", "Строка");
	Возврат Участник;
...
Функция ПолучитьУчастника_ИндивидуальныйПредприниматель(ДанныеXDTO)
	...
	Участник = НовыеДанныеУчастника();
	Участник.ИНН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДанныеXDTO, "ИННФЛ", "Строка");
	Участник.ОГРН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДанныеXDTO, "ОГРНИП", "Строка");
	Участник.СНИЛС = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДанныеXDTO, "СНИЛС", "Строка");
	Возврат Участник;
...
Процедура ЗаполнитьПредставителей(МассивУчастников, ОбъектМЧД)
	...
	ПредставителиXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Довер.СвУпПред", , Истина, Истина);
	Если ПредставителиXDTO = Неопределено Тогда
	    ПредставителиXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПолучПолн", , Истина, Истина);
	КонецЕсли;
	...
	Для Каждого ПредставительXDTO Из ПредставителиXDTO Цикл
		...
		Если ТипУчастника = ТипыПредставителей.ЮридическоеЛицо Тогда
			СведОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "Пред.СведОрг");
		...
		ИначеЕсли ТипУчастника = ТипыПредставителей.ИндивидуальныйПредприниматель Тогда
			СведИПXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "Пред.СведИП");
		...
		ИначеЕсли ТипУчастника = ТипыПредставителей.ФизическоеЛицо Тогда	
			СведФизЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "Пред.СведФизЛ");
		...
		ИначеЕсли ТипУчастника = ТипыПредставителей.ФилиалЮридическогоЛица Тогда
			СведФилиалXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "Пред.СведФилиал");
		...
		ИначеЕсли ТипУчастника = ТипыПредставителей.ФилиалИностраннойОрганизации Тогда	
			СведИОXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "Пред.СведИО");
...
Процедура ЗаполнитьДоверителей(МассивУчастников, ОбъектМЧД)
	...
	СвДоверитXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Довер.СвДоверит", , Истина, Истина); // СписокXDTO Из См. XDTOПакет.ON_EMCHD_1_928_00_01_01_01.СвДоверитТип
	Если СвДоверитXDTO <> Неопределено Тогда
		Для Каждого ДоверительXDTO Из СвДоверитXDTO Цикл	
			...
			Если ТипУчастника = ТипыДоверителей.ЮридическоеЛицо Тогда
				СвРосОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Доверит.РосОргДовер.СвРосОрг");
				Участник = ПолучитьУчастника_ЮридическоеЛицо(СвРосОргXDTO);
				...
				КонецЕсли;
			ИначеЕсли ТипУчастника = ТипыДоверителей.ИностраннаяОрганизация Тогда
				СвИнОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Доверит.ИнОргДовер.СвИнОрг");
				Участник = ПолучитьУчастника_ИностраннаяОрганизация(СвИнОргXDTO);
			ИначеЕсли ТипУчастника = ТипыДоверителей.ИндивидуальныйПредприниматель Тогда
				ИПДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Доверит.ИПДовер");
				Участник = ПолучитьУчастника_ИндивидуальныйПредприниматель(ИПДоверXDTO);
			ИначеЕсли ТипУчастника = ТипыДоверителей.ФизическоеЛицо Тогда
				ФЛДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Доверит.ФЛДовер");
	...
	СвПередПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПередПолн", , Истина, Истина);
	Если СвПередПолнXDTO <> Неопределено Тогда
		Для Каждого ДоверительXDTO Из СвПередПолнXDTO Цикл
			...
			Если ТипУчастника = ТипыПредставителей.ЮридическоеЛицо Тогда
				СвРосОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.РосОргПерПолн.СвРосОрг");
				Участник = ПолучитьУчастника_ЮридическоеЛицо(СвРосОргXDTO);
			ИначеЕсли ТипУчастника = ТипыПредставителей.ИндивидуальныйПредприниматель Тогда	
				ИППерПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.ИППерПолн");
				Участник = ПолучитьУчастника_ИндивидуальныйПредприниматель(ИППерПолнXDTO);
			ИначеЕсли ТипУчастника = ТипыПредставителей.ФизическоеЛицо Тогда	
				ФЛПерПолнЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.ФЛПерПолн");
				Участник = ПолучитьУчастника_ФизическоеЛицо(ФЛПерПолнЛXDTO);
			ИначеЕсли ТипУчастника = ТипыПредставителей.ФилиалЮридическогоЛица Тогда	
				СвФилXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.ФилПерПолн.СвФил");
				Участник = ПолучитьУчастника_ФилиалЮридическогоЛица(СвФилXDTO);
			ИначеЕсли ТипУчастника = ТипыПредставителей.ФилиалИностраннойОрганизации Тогда	
				СвИнОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ПередПолн.ИнПерПолн.СвИнОрг");
...
Процедура ЗаполнитьДанныеПолномочий(ДанныеДляЗаполнения, ОбъектМЧД)
	...
	СвПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Передов.СвПолн", , Истина, Истина);
	...
	СвПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектМЧД, "Документ.Довер.СвПолн", , Истина, Истина);
	Если СвПолнXDTO <> Неопределено Тогда
	...
	Если СведенияОПолномочиях.ТипПолн = ТипыПолномочий().Текстовые Тогда
		...
		ДанныеДляЗаполнения.ТекстовоеПолномочие = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СведенияОПолномочиях,
			"ТекстПолн", "Строка");
	...
	Иначе
		...
		Если ТипЗнч(МашиночитаемыеПолномочия) <> Тип("СписокXDTO") Тогда
			МашиночитаемыеПолномочия = МашиночитаемыеДоверенностиКлиентСервер.ЗначениеВМассиве(МашиночитаемыеПолномочия);
		КонецЕсли;
		
		Для Каждого Полномочие Из МашиночитаемыеПолномочия Цикл
		
			ДанныеПолномочия = НовыеДанныеМашиночитаемогоПолномочия();
			ДанныеПолномочия.Мнемоника    = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Полномочие, "МнПолн", "Строка");
			ДанныеПолномочия.Код          = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Полномочие, "КодПолн", "Строка");
			ДанныеПолномочия.Наименование = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Полномочие, "НаимПолн", "Строка");
...
Функция ВыгрузитьДанныеДоверенности(Ссылка) Экспорт
	...
	РезультатВыгрузки.Вставить("ОписаниеФайла", МашиночитаемыеДоверенности.НовоеОписаниеФайла());
	...
	Архив = ЭлектронныеДокументыСлужебный.СформироватьЗипАрхивФайлов(ИмяФайлаДоверенность, Файлы);
	
	РезультатВыгрузки.ОписаниеФайла.ИмяФайла = Архив;
	РезультатВыгрузки.ОписаниеФайла.ДвоичныеДанные = Новый ДвоичныеДанные(Архив);
...
Процедура ОбработатьДанные_ЗаполнитьСведенияПоПередоверию(ОбъектМЧД003, Записать) Экспорт
	...
	Если ДоверXDTO <> Неопределено Тогда
		СвПереДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверXDTO, "СвДов", , Истина)[0];
	...
	Если ПередовXDTO <> Неопределено Тогда
		...
		Если ПередовXDTO.СвПервДовер <> Неопределено Тогда
			...
			СвДоверПервXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПередовXDTO, "СвПервДовер.СвДоверПер", , Истина, Истина); 
			ДоверительXDTO = СвДоверПервXDTO[0];
...
Функция ПолучитьОрганизациюДоверителяПоИННКПП(ДанныеДоверителей) Экспорт
	...
	Для Каждого ДанныеДоверителя Из ДанныеДоверителей Цикл
		Организация = ЭлектронныеДокументыПереопределяемый.НайтиСсылкуНаОбъект("Организации", , ДанныеДоверителя[0]);
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПрофилиНастроекЭДО.Организация В (&Организации)";
	Запрос.УстановитьПараметр("Организации", ОрганизацииДоверителей);
	
	РезультатЗапроса = Запрос.Выполнить();
	Выгрузка = РезультатЗапроса.Выгрузить();
	
	ОрганизацииСУчетнойЗаписью = Выгрузка.ВыгрузитьКолонку("Организация");
...
Процедура ДополнитьНомерамиДоверенностейСИННДоверителейПоУчастникам(Результат, ДоверительИНН, ПредставительИНН, ДействительныеНаДату,
	...
	Для Каждого СтрокаТЗ Из Доверенности Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЗ.РодительскаяМЧД) Тогда			
			АтрибутыПроверкиПолномочий = АтрибутыПроверкиПолномочий(СтрокаТЗ.РодительскаяМЧД);
			
			Если Не ВозможнаАвтопроверкаПолномочий(АтрибутыПроверкиПолномочий) Тогда
			...
			Если АтрибутыПроверкиПолномочий.ПолномочияОграничены И ЭлектронныйДокумент <> Неопределено Тогда
				РезультатПроверки = МашиночитаемыеДоверенности.ПроверитьПолномочияДоверенности(
					СтрокаТЗ.РодительскаяМЧД, ЭлектронныйДокумент);
...
Процедура ПолучитьПоследнююВыданнуюДоверенность(Отбор, ЭлектронныйДокумент, Результат) Экспорт
	...
	ЭлектронныеДокументыПереопределяемый.ЗаполнитьРегистрационныеДанныеОрганизации(Организация, РеквизитыОрганизации);
	...
	Если ТипЗнч(Сертификат) = Тип("СертификатКриптографии") Тогда
		СвойстваСубъектаСертификата = МашиночитаемыеДоверенности.СвойстваСубъектаСертификата(Сертификат);
	Иначе
		СвойстваСубъектаСертификата = МашиночитаемыеДоверенности.СвойстваСубъектаСертификатаПоСсылке(Сертификат);
	КонецЕсли;
	...
	Для Каждого СтрокаТЗ Из Доверенности Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТЗ.РодительскаяМЧД) Тогда			
			...
			ЭтоТекстПолныхПолномочий = 
				МашиночитаемыеДоверенностиКлиентСервер.ЭтоТекстПолныхПолномочий(АтрибутыПроверкиПолномочий.ТекстовоеПолномочие);
				
			Если Не ЭтоТекстПолныхПолномочий Тогда
				РезультатПроверки = МашиночитаемыеДоверенности.ПроверитьПолномочияДоверенности(
					СтрокаТЗ.РодительскаяМЧД, ЭлектронныйДокумент);
	...
	Если ОшибкиПроверкиПолномочий.Количество() > 0 Тогда
		ЭлектронныеДокументыСлужебныйВызовСервера.ДополнитьСоответствие(
...
Функция ПолучитьДоверенностиПоОтбору(Доверитель, Представитель, ДействительныеНаДату)
	...
	Запрос.Текст =
		...
		|	ЗНАЧЕНИЕ(Справочник.МЧД003.ПустаяСсылка) КАК РодительскаяМЧД
		...
		|	ТаблицаДоверители.Ссылка КАК РодительскаяМЧД
		|ИЗ
		...
		|	ВыбранныеМЧД.РодительскаяМЧД КАК РодительскаяМЧД,
		|	МЧД003.НомерДоверенности КАК НомерДоверенности,
		|	МЧД003.ДатаВыдачи КАК ДатаВыдачи,
		|	МЧД003.СрокДействия КАК ДатаОкончания,
		|	МЧД003.ДатаПрекращения > ДАТАВРЕМЯ(1, 1, 1) КАК Отозвана,
...
Функция ТабличныйДокументМЧД(ДоверенностьXDTO) Экспорт
	...
	ДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументXDTO, "Довер",,,Истина);
	Если ДоверXDTO <> Неопределено Тогда
		ВывестиДоверенность(ТабличныйДокумент, Макет, ДоверXDTO);
	КонецЕсли; 
	
	ПередовXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументXDTO, "Передов",,,Истина);
	Если ПередовXDTO <> Неопределено Тогда
...
Процедура ВывестиДоверенность(ТабличныйДокумент, Макет, Доверенность)
	...
	СвДоверитXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Доверенность, "СвДоверит", , Истина);
	...
	Для Каждого ДоверительXDTO Из СвДоверитXDTO Цикл	
		...
		Если ДоверительXDTO.ТипДоверит = ТипыДоверителей.ЮридическоеЛицо Тогда
			...
			ЛицоБезДовXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Доверит.РосОргДовер.ЛицоБезДов", , Истина);
		...
		ИначеЕсли ДоверительXDTO.ТипДоверит = ТипыДоверителей.ИностраннаяОрганизация Тогда	
			...
			СвРукОПXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Доверит.ИнОргДовер.СвРукОП", , Истина);
	...
	СвУпПредXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Доверенность, "СвУпПред", , Истина);
	...
	СвНотУдXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Доверенность, "СвНотУд",,,Истина); 
...
Процедура ВывестиПередоверие(ТабличныйДокумент, Макет, Передоверие)
    ...
    СвПередовXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Передоверие, "СвПередов",,,Истина);
	...
	СвПередПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Передоверие, "СвПередПолн", , Истина);
	...
	Для Каждого ПредставительXDTO Из СвПередПолнXDTO Цикл
		...
		Если ПредставительXDTO.ТипПерПолн = ТипыПредставителей.ЮридическоеЛицо Тогда
			...
			СвРосОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.РосОргПерПолн.СвРосОрг");
			ВывестиСведенияЮридическогоЛица(ТабличныйДокумент, Макет, СвРосОргXDTO);
			
			ЛицоБезДовXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.РосОргПерПолн.ЛицоБезДов", , Истина);
		...
		ИначеЕсли ПредставительXDTO.ТипПерПолн = ТипыПредставителей.ИндивидуальныйПредприниматель Тогда	
			...
			ИППерПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.ИППерПолн");
		...
		ИначеЕсли ПредставительXDTO.ТипПерПолн = ТипыПредставителей.ФизическоеЛицо Тогда	
			...
			ФЛПерПолнЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.ФЛПерПолн");
		...
		ИначеЕсли ПредставительXDTO.ТипПерПолн = ТипыПредставителей.ФилиалЮридическогоЛица Тогда	
			...
			СвФилXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.ФилПерПолн.СвФил");
			...
			СвРукФилXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.ФилПерПолн.СвРукФил");
		...
		ИначеЕсли ПредставительXDTO.ТипПерПолн = ТипыПредставителей.ФилиалИностраннойОрганизации Тогда
			...
			СвИнОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.ИнПерПолн.СвИнОрг");
			ВывестиСведенияИностранногоЮридическогоЛица(ТабличныйДокумент, Макет, СвИнОргXDTO);
			
			СвРукОПXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПредставительXDTO, "ПередПолн.ИнПерПолн.СвРукОП", , Истина);
	...
	СвПолучПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Передоверие, "СвПолучПолн", , Истина);
	...
	СвНотУдXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Передоверие, "СвНотУд",,,Истина); 
	Если СвНотУдXDTO <> Неопределено Тогда
...
Процедура ВывестиШапку(ТабличныйДокумент, Макет, Доверенность)
	...
	БезотзывXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Доверенность, "Безотзыв",,,Истина);
	Если БезотзывXDTO <> Неопределено Тогда
		...
		Если БезотзывXDTO.УслОтзыва = "2" Тогда
			...
			УсловиеОтзываБезотзывнойДоверенности.Параметры.УсловиеОтзыва =
				ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(БезотзывXDTO, "ОписУслОт", "");
...
Процедура ВывестиСведенияПервоначальнойДоверенности(ТабличныйДокумент, Макет, ПервоначальнаяДоверенность)
	...
	НомДоверПерв = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПервоначальнаяДоверенность, "НомДоверПерв", "Строка");
	...
	НомДоверN = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПервоначальнаяДоверенность, "НомДоверN", "Строка");
	...
	СвДоверПервXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПервоначальнаяДоверенность, "СвДоверПерв", , Истина, Истина);
	Если СвДоверПервXDTO <> Неопределено Тогда
		Для Каждого ДоверительXDTO Из СвДоверПервXDTO Цикл	
			...
			ИначеЕсли ДоверительXDTO.ТипДоверит = ТипыДоверителей.ИностраннаяОрганизация Тогда	
				...
				СвИнОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.ИнОргДовер.СвИнОрг");
				ВывестиСведенияИностранногоЮридическогоЛица(ТабличныйДокумент, Макет, СвИнОргXDTO);
				
				СвРукОПXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.ИнОргДовер.СвРукОП", , Истина);
			...
			ИначеЕсли ДоверительXDTO.ТипДоверит = ТипыДоверителей.ИндивидуальныйПредприниматель Тогда
				...
				ИПДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.ИПДовер");
				ВывестиСведенияИндивидуальногоПредпринимателя(ТабличныйДокумент, Макет, ИПДоверXDTO);
				
			ИначеЕсли ДоверительXDTO.ТипДоверит = ТипыДоверителей.ФизическоеЛицо Тогда	
				
				ФЛДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "ДоверитПерв.ФЛДовер");
	...
	СвНотДействXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПервоначальнаяДоверенность, "СвНотДейств",,,Истина);
	...
	ВриоНотXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПервоначальнаяДоверенность, "ВриоНот",,,Истина);
...
Процедура ВывестиСведенияЮридическогоЛица(ТабличныйДокумент, Макет, ЮридическоеЛицо)
	...
	СведенияЮридическогоЛица.Параметры.ИНН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ЮридическоеЛицо, "ИННЮЛ", "Строка");
	СведенияЮридическогоЛица.Параметры.КПП = ЮридическоеЛицо.КПП;
	СведенияЮридическогоЛица.Параметры.ОГРН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ЮридическоеЛицо, "ОГРН", "Строка");
	ТабличныйДокумент.Вывести(СведенияЮридическогоЛица);
	
	АдрРегXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ЮридическоеЛицо, "АдрРег",,,Истина);
	Если АдрРегXDTO <> Неопределено Тогда
		ВывестиАдрес(ТабличныйДокумент, Макет, АдрРегXDTO);
	КонецЕсли;
	
	ДокПдтвXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ЮридическоеЛицо, "ДокПдтв",,,Истина);
...
Процедура ВывестиСведенияИностранногоЮридическогоЛица(ТабличныйДокумент, Макет, ИностранноеЮридическоеЛицо)
	...
	СведенияИностранногоЮридическогоЛица.Параметры.ИНН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ИностранноеЮридическоеЛицо, "ИННЮЛ", "Строка");
	СведенияИностранногоЮридическогоЛица.Параметры.КПП = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ИностранноеЮридическоеЛицо, "КПП", "Строка");
	ТабличныйДокумент.Вывести(СведенияИностранногоЮридическогоЛица);
	
	АдрМНФактXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СведенияИностранногоЮридическогоЛица, "АдрМНФакт",,,Истина);
	Если АдрМНФактXDTO <> Неопределено Тогда
		ВывестиАдрес(ТабличныйДокумент, Макет, АдрМНФактXDTO);
	КонецЕсли;
	
	АдрСтрРег = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СведенияИностранногоЮридическогоЛица, "АдрСтрРег", "Строка");
...
Процедура ВывестиСведенияИндивидуальногоПредпринимателя(ТабличныйДокумент, Макет, ИндивидуальныйПредприниматель)
	...
	СведенияИндивидуальногоПредпринимателя.Параметры.Наименование =
		ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ИндивидуальныйПредприниматель, "НаимИП", "Строка");
	...
	СведенияИндивидуальногоПредпринимателя.Параметры.ДатаРождения = 
		ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СведФЛXDTO, "ДатаРожд", "Дата");
	ТабличныйДокумент.Вывести(СведенияИндивидуальногоПредпринимателя);
	
	АдрМЖXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СведФЛXDTO, "АдрМЖ",,,Истина);
	Если АдрМЖXDTO <> Неопределено Тогда
		ВывестиАдрес(ТабличныйДокумент, Макет, АдрМЖXDTO);
	КонецЕсли;
	
	УдЛичнФЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СведФЛXDTO, "УдЛичнФЛ",,,Истина);
	Если УдЛичнФЛXDTO <> Неопределено Тогда
		ВывестиУдостоверениеЛичности(ТабличныйДокумент, Макет, УдЛичнФЛXDTO);
	КонецЕсли;
	
	ДокПдтвXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ИндивидуальныйПредприниматель, "ДокПдтв",,,Истина);
	Если ДокПдтвXDTO <> Неопределено Тогда
		ВывестиДокументПодтверждающийПолномочия(ТабличныйДокумент, Макет, ДокПдтвXDTO);
	КонецЕсли;	
...
Процедура ВывестиСведенияФизическогоЛица(ТабличныйДокумент, Макет, ФизическоеЛицо)
	...
	СведенияФизическогоЛица.Параметры.ДатаРождения = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СведФЛXDTO, "ДатаРожд", "Дата");
	СведенияФизическогоЛица.Параметры.ИНН = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ФизическоеЛицо, "ИННФЛ", "Строка");
	СведенияФизическогоЛица.Параметры.СНИЛС = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ФизическоеЛицо, "СНИЛС", "Строка");
	СведенияФизическогоЛица.Параметры.Должность = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ФизическоеЛицо, "Должность", "Строка");
	...
	ДокПдтвXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ФизическоеЛицо, "ДокПдтв",,,Истина);
	Если ДокПдтвXDTO <> Неопределено Тогда
		ВывестиДокументПодтверждающийПолномочия(ТабличныйДокумент, Макет, ДокПдтвXDTO);
	КонецЕсли;
	
	СведРукопXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ФизическоеЛицо, "СведРукоп",,,Истина);
...
Процедура ВывестиЛицоБезДоверенности(ТабличныйДокумент, Макет, ЛицоБезДоверенности)
	...
	СВЮЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ЛицоБезДоверенности, "СВЮЛ",,,Истина);
	Если СВЮЛXDTO <> Неопределено Тогда
		...
		СвФЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(СВЮЛXDTO, "СвФЛ", , Истина);
	...
	СвФЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ЛицоБезДоверенности, "СвФЛ",,,Истина);
	...
	СвИПXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ЛицоБезДоверенности, "СвИП",,,Истина);
...
Процедура ВывестиАдрес(ТабличныйДокумент, Макет, Адрес)
	...
	АдрРФ = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Адрес, "АдрРФ", "Строка");
	...
	ФИАСАдрРФ = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Адрес, "ФИАСАдрРФ", "Строка");
	...
	ИдФИАС = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Адрес, "ИдФИАС", "Строка");
...
Процедура ВывестиДокументПодтверждающийПолномочия(ТабличныйДокумент, Макет, ДокументПодтверждающийПолномочия)
	...
	НаимДокПдтв = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументПодтверждающийПолномочия, "НаимДокПдтв", "Строка");
	Если Не ПустаяСтрока(НаимДокПдтв) Тогда
		Текст = НСтр("ru = 'Наименование документа'") + ": " + НаимДокПдтв;
		МассивСтрок.Добавить(Текст);
	КонецЕсли;	
	
	ДатаВыд = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументПодтверждающийПолномочия, "ДатаВыд",,,Истина);
	Если ДатаВыд <> Неопределено Тогда
		Текст = НСтр("ru = 'Дата выдачи документа'") + ": " + Формат(ДатаВыд, "ДЛФ=ДД");
		МассивСтрок.Добавить(Текст);
	КонецЕсли;
	
	КемВыд = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументПодтверждающийПолномочия, "КемВыд", "Строка");
	Если Не ПустаяСтрока(КемВыд) Тогда
		Текст = НСтр("ru = 'Наименование органа, выдавшего документ'") + ": " + КемВыд;
		МассивСтрок.Добавить(Текст);
	КонецЕсли;
	
	СвУдДок = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументПодтверждающийПолномочия, "СвУдДок", "Строка");
...
Процедура ВывестиПолномочия(ТабличныйДокумент, Макет, Полномочия)
	...
	Если Полномочия.ТипПолн = ТипыПолномочийXDTO.Текстовые Тогда
		...
		ПроизвольныеСведения.Параметры.Текст =
			ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Полномочия, "ТекстПолн", "Строка");
		ТабличныйДокумент.Вывести(ПроизвольныеСведения);
		
	ИначеЕсли Полномочия.ТипПолн = ТипыПолномочийXDTO.Машиночитаемые Тогда	
		
		МашПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Полномочия, "МашПолн", , Истина, Истина);
		Если МашПолнXDTO <> Неопределено Тогда
			Для Каждого ПолномочиеXDTO Из МашПолнXDTO Цикл
				...
				ОгрСвПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПолномочиеXDTO, "ОгрСвПолн", , Истина, Истина);
				Если ОгрСвПолнXDTO <> Неопределено Тогда
					...
					Для Каждого ОграничениеXDTO Из ОгрСвПолнXDTO Цикл	
						...
						КодЗначОгр = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОграничениеXDTO, "КодЗначОгр", "Строка");
						Если Не ПустаяСтрока(КодЗначОгр) Тогда
							Текст = Текст + ", " + НСтр("ru='код значения для ограничения'") + ": " + КодЗначОгр;
						КонецЕсли;	
						
						ТексЗначОгр = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОграничениеXDTO, "ТексЗначОгр", "Строка");
...
Процедура ВывестиНотариальноеУдостоверение(ТабличныйДокумент, Макет, НотариальноеУдостоверение)
	...
	ВриоНотXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(НотариальноеУдостоверение, "ВриоНот",,,Истина);
	Если ВриоНотXDTO <> Неопределено Тогда
		ПроизвольныйЗаголовок = Макет.ПолучитьОбласть("ПроизвольныйЗаголовок");
		ПроизвольныйЗаголовок.Параметры.Заголовок = НСтр("ru='Сведения о ВРИО нотариуса'");
		ТабличныйДокумент.Вывести(ПроизвольныйЗаголовок);
	...
	ДопСвНотДовер = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(НотариальноеУдостоверение, "ДопСвНотДовер", "Строка");
	...
	ИнСвУдНадпис = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(НотариальноеУдостоверение, "ИнСвУдНадпис", "Строка");
	...
	ПодпРукописXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(НотариальноеУдостоверение, "ПодпРукопис", , Истина);
...
Функция ДанныеОбъектаXDTOПоПути(Объект, Путь)
	...
	Для Каждого ЧастьПути Из ЧастиПути Цикл
		...
		Если Не МашиночитаемыеДоверенностиКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущийОбъект, ЧастьПути) Тогда
			Возврат Неопределено;
		КонецЕсли;
...
Процедура СброситьСвойствоОбъектаXDTOРекурсивно(ОбъектXDTO, Путь)
	...
	Если Не МашиночитаемыеДоверенностиКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ОбъектXDTO, ИмяСвойства) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьСвойствоОбъектаМЧДПростогоТипа(Объект, СвойствоОбъекта, Путь, ОшибкиЗаполнения)
	...
	Значение = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Объект, СвойствоОбъекта.Имя, ИмяТипа);
...
Процедура ПрочитатьСвойствоОбъектаМЧДПростогоТипа(ДанныеОбъектаМЧД, Объект, Свойство)
	...
	Значение = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(Объект, Свойство.Имя, ИмяТипа);
...
Функция АтрибутыПроверкиПолномочий(Доверенность)
