...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Тип Из МашиночитаемыеДоверенности.Типы().Организация.Типы() Цикл
	...
	КонецЦикла;
	Для Каждого Тип Из МашиночитаемыеДоверенности.Типы().КонтрагентБЭД.Типы() Цикл
		МетаданныеОбъекта = Метаданные.НайтиПоТипу(Тип);
		Если МетаданныеОбъекта = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ИНННачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработчикКоманды = Новый ОписаниеОповещения("ВыполнитьКомандуВыбора", ЭтаФорма);
...
Функция ПолучитьИНН(Знач Ссылка)
	ИмяРеквизитаИНН = "";
	Если ЭлектронныеДокументыПереопределяемый.ЭтоОрганизация(Ссылка) Тогда
		ИмяРеквизитаИНН = ЭлектронныеДокументыПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННОрганизации");
	Иначе
		ИмяРеквизитаИНН = ЭлектронныеДокументыПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННКонтрагента");
	КонецЕсли;
...
Процедура ВыполнитьКомандуВыбора(ВыбранноеЗначение, ДополнительныеПараметры = Неопределено) Экспорт
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ЭлементСпискаЗначений") Тогда
		ОбработчикЗавершенияВыбора = Новый ОписаниеОповещения("ВыбратьУчастникаЗавершение", ЭтаФорма);
