...
Процедура ПриОткрытии(Отказ)
	...
	Если Параметры.Ключ.Пустая() Тогда
		Псевдонимы = Псевдонимы(ЭтаФорма);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("СпроситьОЗакрытииЗавершение", ЭтаФорма), ТекстВопроса,
...
Процедура ДоверенностьДокументДоверСвДоверит_ДоверительНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("ВыбратьДоверителяЗавершение", ЭтаФорма);
...
Процедура ДоверенностьДокументДоверСвУпПред_ПредставительНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("ВыбратьПредставителяЗавершение", ЭтаФорма);
...
Процедура ДоверенностьДокументДоверСвДоверитДоверитРосОргДоверЛицоБезДовСВЮЛСвФЛ_ФизическоеЛицоНачалоВыбора(Элемент,
	ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("ВыбратьФизическоеЛицоЗавершение", ЭтаФорма);
...
Процедура ПерезаполнитьСведенияОДоверителе(Доверитель)
	...
	ДоверительПоУмолчанию(ЭтаФорма);
	Если ЗначениеЗаполнено(Доверитель) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Доверитель)) Тогда
		...
		ЭлектронныеДокументыПереопределяемый.ЗаполнитьРегистрационныеДанныеОрганизации(Доверитель, СтруктураСведений);
		ДополнительныеСведения = ЭлектронныеДокументыПереопределяемый.ПолучитьДанныеЮрФизЛица(Доверитель);
		ТипыДоверителей = ТипыДоверителей();
		Если ЭлектронныеДокументыПереопределяемый.ЭтоФизЛицо(Доверитель) Тогда
			...
			СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаДоверитель_ИндивидуальныйПредприниматель,
				Истина);
		Иначе
			...
			СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаДоверитель_ЮридическоеЛицо, Истина);
		КонецЕсли;
		
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ДоверенностьДокументДоверСвДоверит_Доверитель);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПерезаполнитьСведенияОЛицеБезДоверенности(ЛицоБезДоверенности)
	...
	ЛицоБезДоверенностиПоУмолчанию(ЭтаФорма);
	...
	Если ЗначениеЗаполнено(ЛицоБезДоверенности) И ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		...
		Если МашиночитаемыеДоверенности.Типы().ФизическоеЛицо.СодержитТип(ТипЗначения) Тогда
			...
			СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаЛицоБезДоверенности_ФизическоеЛицо, Истина);
		Иначе
			...
			Иначе
				...
				Если МашиночитаемыеДоверенности.Типы().Организация.СодержитТип(ТипЗначения) Тогда
					...
					КонецЕсли;
				КонецЕсли;
				СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаЛицоБезДоверенности_ЮридическоеЛицо, Истина);
			КонецЕсли;
		КонецЕсли;
...
Процедура ПерезаполнитьСведенияОПредставителе(Представитель)
	...
	ПредставительПоУмолчанию(ЭтаФорма);
	...
	Если ЗначениеЗаполнено(Представитель) И ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		...
		Если МашиночитаемыеДоверенности.Типы().ФизическоеЛицо.СодержитТип(ТипЗначения) Тогда
			...
			СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаПредставитель_ФизическоеЛицо, Истина);
		Иначе
			...
			Иначе
				...
				СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаПредставитель_Организация, Истина);
			КонецЕсли;
		КонецЕсли;
		
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ДоверенностьДокументДоверСвУпПред_Представитель);	
	КонецЕсли;
КонецПроцедуры
...
Процедура ПерезаполнитьСведенияОФизическомЛице(ФизическоеЛицо)
	...
	Если ЗначениеЗаполнено(ФизическоеЛицо) И ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ФизическоеЛицо)) Тогда
		...
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаОдноВложенноеЛицоДействующееБезДоверенности, Истина);
	КонецЕсли;
КонецПроцедуры
...
Процедура НачатьВыборЗначенияСубъекта(ОбработчикВыбораЗначения, СписокТипов, Элемент)
	...
	ОбработчикВыбораТипа = Новый ОписаниеОповещения("ВыбратьТипСубъектаЗавершение", ЭтаФорма,
...
Процедура ВыбратьМашинноеПолномочиеЗавершение(Результат, ИдентификаторСтроки = Неопределено) Экспорт
	...
	КонецЕсли;
	СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаМашиночитаемыеПолномочия, Истина);
	ОбновитьВидимостьКомандТаблицыПолномочий(ЭтаФорма);
	ОбновитьНумерациюТаблицы(Доверенность[0].Документ[0].Довер[0].СвПолн[0].МашПолн);
КонецПроцедуры
...
Процедура ДобавитьНовоеМашинноеПолномочие()
	ОбработкаВыбора = Новый ОписаниеОповещения("ВыбратьМашинноеПолномочиеЗавершение", ЭтаФорма);
...
Процедура ДобавитьНовогоПредставителя()
	ОбработчикВводаДоверителя = Новый ОписаниеОповещения("ВвестиПредставителяЗавершение", ЭтаФорма);
