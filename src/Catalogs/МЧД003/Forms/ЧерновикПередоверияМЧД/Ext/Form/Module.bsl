...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ИсходнаяМЧД) И Не Параметры.ПередовериеРазрешено Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияДляВыбораДругойДоверенности());
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИсходнаяМЧД)
		...
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщенияТребуетсяНотариат());
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("Подключаемый_ВопросОЗакрытииЗавершение", ЭтаФорма), ТекстВопроса,
...
Процедура ДоверенностьДокументПередовСвПередПолнПередПолнИнПерПолнСвРукОП_РуководительИОПриИзменении(Элемент)
	...
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("Подключаемый_ВыборРуководителяИностраннойОрганизации", ЭтаФорма);
...
Процедура ДоверенностьДокументПередовСвПередПолнПередПолнИнПерПолнСвРукОП_РуководительИОНачалоВыбора(
	...
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("Подключаемый_ВыборРуководителяИностранногоФилиалаЗавершение", ЭтаФорма);
	НачатьВыборЗначенияСубъекта(ОбработчикВыбранногоЗначения, СписокВыбораТиповФизическогоЛица, Элемент);
...
Процедура ДоверенностьДокументПередовСвПередПолнПередПолнФилПерПолнСвРукФил_РуководительПриИзменении(Элемент)
	...
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("Подключаемый_ВыборРуководителяФилиала", ЭтаФорма);
	НачатьВыборЗначенияСубъекта(ОбработчикВыбранногоЗначения, СписокВыбораТиповФизическогоЛица, Элемент);
...
Процедура ДоверенностьДокументПередовСвПередПолнПередПолнФилПерПолнСвРукФил_РуководительНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("Подключаемый_ВыборРуководителяФилиалаЗавершение", ЭтаФорма);
	НачатьВыборЗначенияСубъекта(ОбработчикВыбранногоЗначения, СписокВыбораТиповФизическогоЛица, Элемент);
...
Процедура ДоверенностьДокументПередовСвПолучПолн_ПредставительНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("Подключаемый_ВыборПредставителяЗавершение", ЭтаФорма);
...
Процедура ДоверенностьДокументПередовСвПолучПолн_ПредставительПриИзменении(Элемент)
	...
	ОбновитьВидимостьКомандТаблицыПредставителей(ЭтаФорма);
...
Процедура ДоверенностьДокументПередовСвПередПолнПередПолнРосОргПерПолнЛицоБезДовСВЮЛСвФЛ_ФизическоеЛицоНачалоВыбора(
	...
	СтандартнаяОбработка = Ложь;
	ОбработчикВыбранногоЗначения = Новый ОписаниеОповещения("Подключаемый_ВыборФизическогоЛицаЗавершение", ЭтаФорма);
	НачатьВыборЗначенияСубъекта(ОбработчикВыбранногоЗначения, СписокВыбораТиповФизическогоЛица, Элемент);
...
Процедура ДоверенностьДокументПередовСвПолучПолнТипПредПриИзменении(Элемент)
	...
	ОбновитьВидимостьКомандТаблицыПолномочий(ЭтаФорма);
...
Процедура ЗаполнитьСпискиВыбораТипов()
	...
	ТипыСубъектов = Новый Структура("ФизическоеЛицо, Организация, Контрагент",
		МашиночитаемыеДоверенности.Типы().ФизическоеЛицо.Типы(),
		МашиночитаемыеДоверенности.Типы().Организация.Типы(),
		МашиночитаемыеДоверенности.Типы().КонтрагентБЭД.Типы());
...
Процедура ЗаполнитьСведенияПоПервоначальнойДоверенности(ОбъектXDTOМЧД)
	...
	ДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектXDTOМЧД.Документ, "Довер",,,Истина);
	Если ДоверXDTO <> Неопределено Тогда
		...
		СвДоверитXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверXDTO, "СвДоверит", , Истина)[0];
		СведенияОДоверенности.СвДоверПерв[0].ТипДоверит = СвДоверитXDTO.ТипДоверит;
		
		ДоверитXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверXDTO, "СвДоверит.Доверит", , Истина)[0];
		...
		СвНотУдXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверXDTO, "СвНотУд", , Истина);
	...
	ПередовXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектXDTOМЧД.Документ, "Передов",,,Истина);
	Если ПередовXDTO <> Неопределено Тогда
		...
		СвПереДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПередовXDTO, "СвПервДовер", , Истина)[0];
		...
		СвПередПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПередовXDTO, "СвПередПолн", , Истина)[0];
		СведенияОДоверенности.СвДоверПерв[0].ТипДоверит = СвПередПолнXDTO.ТипПерПолн;
		
		ПередПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПередовXDTO, "СвПередПолн.ПередПолн", , Истина)[0];
		...
		СвНотУдXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПередовXDTO, "СвНотУд", , Истина);
		Если СвНотУдXDTO <> Неопределено И СвНотУдXDTO.СвНотДейств <> Неопределено Тогда
			ЗаполнитьАтрибутыРекурсивно(СведенияОДоверенности.СвНотДейств[0], СвНотУдXDTO.СвНотДейств);
 		КонецЕсли;
 		Если СвНотУдXDTO <> Неопределено И СвНотУдXDTO.СвНотДейств <> Неопределено Тогда
...
Процедура СформироватьПредставлениеПоИсходнойМЧД(ОбъектXDTOМЧД)
	...
	Если ДоверXDTO <> Неопределено Тогда
		...
		ДоверитXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверXDTO, "СвДоверит.Доверит", , Истина)[0];
		Если ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ДоверитXDTO, "РосОргДовер", Истина) Тогда
			...
			Если ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ЛицоБезДоверXDTO, "СВЮЛ", Истина) Тогда
				ФИО = ЛицоБезДоверXDTO.СВЮЛ.СвФЛ[0].СведФЛ.ФИО;
			ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ЛицоБезДоверXDTO, "СвФЛ", Истина) Тогда
				ФИО = ЛицоБезДоверXDTO.СвФЛ.СведФЛ.ФИО;
			ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ЛицоБезДоверXDTO, "СвИП", Истина) Тогда
				ФИО = ЛицоБезДоверXDTO.СвИП.СведФЛ.ФИО;
			КонецЕсли;
		ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ДоверитXDTO, "ИнОргДовер", Истина) Тогда
		...
		ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ДоверитXDTO, "ИПДовер", Истина) Тогда
			ЭтоИПФЛ = Истина;
			ФИО = ДоверитXDTO.ИПДовер.СведФЛ.ФИО;
		ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ДоверитXDTO, "ФЛДовер", Истина) Тогда
			ЭтоИПФЛ = Истина;
	...
	Если ПередовXDTO <> Неопределено Тогда
		...
		ПередПолнXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПередовXDTO, "СвПередПолн.ПередПолн", , Истина)[0];
		Если ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ПередПолнXDTO, "РосОргПерПолн", Истина) Тогда
			...
			ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ЛицоБезДоверXDTO, "СвИП", Истина) Тогда
				ФИО = ЛицоБезДоверXDTO.СвИП.СведФЛ.ФИО;
			КонецЕсли;
		ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ПередПолнXDTO, "ИнПерПолн", Истина) Тогда
		...
		ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ПередПолнXDTO, "ФилПерПолн", Истина) Тогда
		...
		ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ПередПолнXDTO, "ИППерПолн", Истина) Тогда
			ЭтоИПФЛ = Истина;
			ФИО = ПередПолнXDTO.ИППерПолн.СведФЛ.ФИО;
		ИначеЕсли ЭлектронныеДокументыВнутренний.ЕстьСвойствоXDTO(ПередПолнXDTO, "ФЛПерПолн", Истина) Тогда
			ЭтоИПФЛ = Истина;
...
Процедура ПерезаполнитьСведенияОДоверителе(ОписаниеДоверителя)
	...
	СведенияОрганизации = ОбъектXDTOИзСтрокиXML(ОписаниеДоверителя.СведенияОрганизации);
	...
	Если СвПередПолн[0].ТипПерПолн = ТипыПередоверителей.ЮридическоеЛицо Тогда
		...
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаДоверительПередоверия_ЮридическоеЛицо, Истина);
	ИначеЕсли СвПередПолн[0].ТипПерПолн = ТипыПередоверителей.ФилиалИностраннойОрганизации Тогда
		...
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаДоверительПередоверия_ФилиалИностраннойОрганизации,
			Истина);
	...
	ИначеЕсли СвПередПолн[0].ТипПерПолн = ТипыПередоверителей.ФилиалЮридическогоЛица Тогда
		...
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаДоверительПередоверия_ФилиалОрганизации, Истина);

	КонецЕсли;
	
	Доверитель = ЭлектронныеДокументыПереопределяемый.НайтиСсылкуНаОбъект("Организации", , 
		Новый Структура("ИНН, КПП", СведенияОрганизации.ИННЮЛ, СведенияОрганизации.КПП));
		
	Если ЗначениеЗаполнено(Доверитель) Тогда
		РегистрационныеДанныеОрганизации = Новый Структура;
		ЭлектронныеДокументыПереопределяемый.ЗаполнитьРегистрационныеДанныеОрганизации(Доверитель, РегистрационныеДанныеОрганизации);
...
Функция СведенияДоверителей(ДокументXDTO)
	СвДоверитXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументXDTO, "Передов.СвПолучПолн", , Истина, Истина);
	Если СвДоверитXDTO = Неопределено Тогда
		СвДоверитXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДокументXDTO, "Довер.СвУпПред", , Истина, Истина);
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбораДоверителей(ОбъектXDTOМЧД)
	...
	Для Каждого ДоверительXDTO Из СвДоверитXDTO Цикл
		...
		Если ДоверительXDTO.ТипПред = ТипыПередоверителей.ЮридическоеЛицо Тогда
			...
			СведОргXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Пред.СведОрг");
		...
		ИначеЕсли ДоверительXDTO.ТипПред = ТипыПередоверителей.ФилиалИностраннойОрганизации Тогда
			...
			СведИОXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Пред.СведИО");
		...
		ИначеЕсли ДоверительXDTO.ТипПред = ТипыПередоверителей.ФилиалЮридическогоЛица Тогда
			...
			СведФилиалЛXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверительXDTO, "Пред.СведФилиал");
...
Процедура ДобавитьЗначениеВСписокВыбораДоверителей(СведенияОрганизации, ТипПредставителя)
	...
	СведенияСтрокой = ОбъектXDTOВСтрокуXML(СведенияОрганизации);
...
Процедура ЗаполнитьМашиночитаемыеПолномочияОтРодителя(ОбъектXDTOМЧД)
	...
	ДоверXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектXDTOМЧД.Документ, "Довер",,,Истина);
	Если ДоверXDTO <> Неопределено Тогда
		СвПолн = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДоверXDTO, "СвПолн",,,Истина);
	КонецЕсли;
	
	ПередовXDTO = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ОбъектXDTOМЧД.Документ, "Передов",,,Истина);
	Если ПередовXDTO <> Неопределено Тогда
		СвПолн = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ПередовXDTO, "СвПолн",,,Истина);
	КонецЕсли;	
	...
	Если Доверенность[0].Документ[0].Передов[0].СвПолн[0].ТипПолн = ТипыПолномочий().Текстовые Тогда
		...
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаТекстовыеПолномочияПередоверия, Истина);
	Иначе
		...
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаМашиночитаемыеПолномочияПередоверия, Истина);
	КонецЕсли;
...
Процедура ПерезаполнитьСведенияОЛицеБезДоверенностиПередоверия(ЛицоБезДоверенности)
	...
	Если ЗначениеЗаполнено(ЛицоБезДоверенности) И ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		...
		Если МашиночитаемыеДоверенности.Типы().ФизическоеЛицо.СодержитТип(ТипЗначения) Тогда
			...
			СброситьОформлениеЭлементовФормы(ЭтаФорма,
				Элементы.ГруппаЛицоБезДоверенностиПередоверия_ФизическоеЛицо, Истина);
			
		Иначе
			
			СтруктураСведений = ЭлектронныеДокументыПереопределяемый.ПолучитьДанныеЮрФизЛица(ЛицоБезДоверенности);

			Если ЭлектронныеДокументыПереопределяемый.ЭтоФизЛицо(ЛицоБезДоверенности) Тогда
			...
			Иначе
				...
				Если СтруктураСведений.Свойство("КодРегиона") Тогда
				...
				Если МашиночитаемыеДоверенности.Типы().Организация.СодержитТип(ТипЗначения) Тогда
					РегистрационныеДанные = Новый Структура;
					ЭлектронныеДокументыПереопределяемый.ЗаполнитьРегистрационныеДанныеОрганизации(ЛицоБезДоверенности, РегистрационныеДанные);
...
Процедура ПерезаполнитьСведенияОПредставителе(Представитель)
	...
	Если ЗначениеЗаполнено(Представитель) И ОбщегоНазначения.ЭтоСсылка(ТипЗначения) Тогда
		...
		Если МашиночитаемыеДоверенности.Типы().ФизическоеЛицо.СодержитТип(ТипЗначения) Тогда
			...
			СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаПредставительПередоверия_ФизическоеЛицо, Истина);
		Иначе
			СтруктураСведений = ЭлектронныеДокументыПереопределяемый.ПолучитьДанныеЮрФизЛица(Представитель);
			Если ЭлектронныеДокументыПереопределяемый.ЭтоФизЛицо(Представитель) Тогда
			...
			Иначе
				...
				СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаПредставительПередоверия_Организация, Истина);
			КонецЕсли;
		КонецЕсли;
		
		СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ДоверенностьДокументПередовСвПолучПолн_Представитель);
...
Процедура ЛицоБезДоверенностиПоУмолчанию()
	...
	ПоказатьГруппу(Элементы.ГруппаЛицоБезДоверенностиПередоверия,
		Элементы.ГруппаЛицоБезДоверенностиПередоверия_ФизическоеЛицо);
	СброситьОформлениеЭлементовФормы(ЭтаФорма, Элементы.ГруппаЛицоБезДоверенностиПередоверия_ФизическоеЛицо, Истина);
...
Процедура СброситьОформлениеЭлементовФормы(Форма, Элемент, Рекурсивно = Ложь)
	...
	Если МашиночитаемыеДоверенностиКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Элемент, "ЦветРамки") Тогда
...
Процедура Подключаемый_СброситьОформлениеЭлемента(Элемент)
	СброситьОформлениеЭлементовФормы(ЭтаФорма, Элемент);
КонецПроцедуры
...
Процедура НачатьВыборЗначенияСубъекта(ОбработчикВыбораЗначения, СписокТипов, Элемент)
	...
	ОбработчикВыбораТипа = Новый ОписаниеОповещения("Подключаемый_ВыборТипаСубъектаЗавершение", ЭтаФорма,
...
Процедура ДобавитьНовоеМашинноеПолномочие()
	...
	ОбработкаВыбора = Новый ОписаниеОповещения("Подключаемый_ВыборМашинногоПолномочияЗавершение", ЭтаФорма);
...
Процедура ДобавитьНовогоПредставителя()
	...
	ОбработчикВводаДоверителя = Новый ОписаниеОповещения("Подключаемый_ВводПредставителяЗавершение", ЭтаФорма);
	ОткрытьФорму("Справочник.МЧД003.Форма.ДобавлениеПредставителя", , ЭтаФорма, , , , ОбработчикВводаДоверителя);
...
Процедура ИзменитьТекущегоПредставителя()
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ОткрытьФорму("Справочник.МЧД003.Форма.ДобавлениеПредставителя", ПараметрыФормы, ЭтаФорма, , , ,
...
Функция ДоверенностьДоступнаДляПередоверия(ВыбраннаяДоверенность)
	...
	Если МашиночитаемыеДоверенности.ЭтоМЧД003(ВыбраннаяДоверенность) Тогда
...
Процедура ПолучитьСтатусВРеестреФНС()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПолучитьСтатусВРеестреФНСЗавершение", ЭтаФорма);
...
Процедура ДоверительНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОЗавершении = Новый ОписаниеОповещения("ДоверительНачалоВыбораЗавершение", ЭтаФорма);
...
Процедура ЗаполнитьКонтекстЗаписи(ПараметрыЗаписи)
	...
	ДвоичныеДанные = МашиночитаемыеДоверенности.XDTOВДвоичныеДанные(Результат.Объект, Ложь, КодировкаТекста.UTF8);
...
Процедура ПодсветитьЭлементыСОшибками(ОшибкиФормирования)
	...
	Для Каждого Элемент Из ОшибкиФормирования Цикл
		...
		Для Каждого ЭлементФормы Из ЭлементыФормы Цикл
			Если МашиночитаемыеДоверенностиКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭлементФормы, "ЦветРамки") Тогда
...
Процедура СообщитьОбОшибках(ОшибкиФормирования)
	...
	Если Не ЗначениеЗаполнено(ИсходнаяМЧД) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указана исходная МЧД.'"));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ГруппыОшибок.Общее) Тогда
		ШаблонСообщения = НСтр("ru='Не удалось заполнить общие сведения доверенности:
							   |%1';");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(ШаблонСообщения, СтрСоединить(ГруппыОшибок.Общее,
			Символы.ПС)));
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ГруппыОшибок.Доверители) Тогда
		ШаблонСообщения = НСтр("ru='Не удалось заполнить сведения о доверителе:
							   |%1';");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(ШаблонСообщения, СтрСоединить(
	...
	Если ЗначениеЗаполнено(ГруппыОшибок.Полномочия) Тогда
		ШаблонСообщения = НСтр("ru='Не удалось заполнить полномочия:
							   |%1';");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(ШаблонСообщения, СтрСоединить(ГруппыОшибок.Полномочия,
			Символы.ПС)));
	КонецЕсли;
...
Функция ЗаданВопросОбОшибкахПриРегистрацииМЧД(ПараметрыЗаписи)
	...
	ОбработчикОтвета = Новый ОписаниеОповещения("ОбработатьОтветООшибкахПриРегистрации", ЭтаФорма);
...
Процедура ЗарегистрироватьМЧДПриНеобходимости(ПараметрыЗаписи)
	...
	ОбработчикПослеПодписания = Новый ОписаниеОповещения("ОбработатьРегистрациюМЧД", ЭтаФорма);
...
Процедура СообщитьОшибкиЧтенияДанныхМЧД(ОшибкиЧтения)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрСоединить(ЧастиСообщения, Символы.ПС));
...
Функция ЗначениеВДеревеДоверенности(Путь)
	...
	Для Индекс = 1 По ЧастиПути.ВГраница() - 1 Цикл
		...
		Если Не МашиночитаемыеДоверенностиКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущаяСтрока, ДанныеЧасти.Имя) Тогда
			Возврат Неопределено;
		КонецЕсли;
