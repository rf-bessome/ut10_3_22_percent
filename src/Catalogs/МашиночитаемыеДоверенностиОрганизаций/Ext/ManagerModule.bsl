...
Функция СведенияМЧД(МЧД) Экспорт
	...
	Реквизиты = "ДатаВыдачи, ДатаОкончания, ДоверительЮЛ_ИНН, НомерДоверенности,
	...
	ЗаполнитьЗначенияСвойств(СведенияМЧД, РеквизитыМЧД,
		...
		|ИННДоверителяРодительскойДоверенности, НомерРодительскойДоверенности, НомерДоверенности");
...
Функция ЗаполнитьМЧД(НомерДоверенности, ИННДоверителя, Сведения) Экспорт
	...
	СведенияМЧД.ИННДоверителя = МашиночитаемыеДоверенности.ИННДоверителя(МЧД);	
	СведенияМЧД.Полномочия = ТаблицаЗначенийВМассив(Полномочия);
...
Функция ПолучитьИмяФайлаМЧД(Ссылка) Экспорт
	...
	ЭлементыИмениФайла = МашиночитаемыеДоверенностиКлиентСервер.ЗначениеВМассиве(МашиночитаемыеДоверенности.ПространствоИмен_МЧД());
...
Функция ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, ОбновлятьСуществующий = Ложь,
	...
	Если ЗначениеЗаполнено(ДополнительныеСведения) Тогда
		ЭлектронныеДокументыСлужебныйВызовСервера.ДополнитьСтруктуру(ДанныеДоверенности, ДополнительныеСведения, Истина);
	КонецЕсли;
...
Функция СНИЛСДоверителя(ДанныеXDTO) Экспорт
	...
	СНИЛС = ЭлектронныеДокументыВнутренний.ЗначениеСвойстваXDTO(ДанныеXDTO, ПутьКСвойству, "Строка");
...
Функция ЗаписатьЭлементСправочника(ДанныеДоверенности, ОбновлятьСуществующий, ДанныеДляЗагрузки,
	...
	ОтсутствуетВозможностьПроверитьНаСервере = Не ЭлектронныеДокументыСлужебныйВызовСервера.ВыполнятьКриптооперацииНаСервере();
	...
	Если ОбъектСправочника.ПроверитьЗаполнение() Тогда
		...
		Исключение
			ОтменитьТранзакцию();
			ИмяСобытия = НСтр("ru = 'Ошибка изменения МЧД.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция ДанныеИзИнформационнойБазы(СправочникСсылка)
	...
	Если ВыборкаИнформации.Следующий() Тогда
		...
		Если Результат.ИностраннаяОрганизация Тогда
			ТипОрганизации = "ИО";
		Иначе				
			Если ЭлектронныеДокументыПереопределяемый.ЭтоФизЛицо(ВыборкаИнформации.ОрганизацияМЧД) Тогда
...
Функция ДанныеДоверенностиВСтрокуXMLВерсия002(ДанныеДоверенности)
	...
	Доверенность = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность", ПространствоИмен);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Доверенность, "ВерсФорм", "002", Истина, Ошибки); 
	
	ИдФайл = СтрШаблон("ON_DOVBB_%1_%2",
		Формат(ТекущаяДатаСеанса(), "ДФ=yyyyMMdd"),
		ДанныеДоверенности.Номер);	
		
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Доверенность, "ИдФайл", ИдФайл, Истина, Ошибки);

	Документ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ", ПространствоИмен);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Документ, "ОКУД", ДанныеДоверенности.ОКУД, Истина, Ошибки);
	
	Довер = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер", ПространствоИмен);
		
	// формирование раздела Сведения о доверенности
	СвДов = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДов", ПространствоИмен);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "НомДовер", ДанныеДоверенности.Номер, Истина, Ошибки);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "ДатаВыдДовер", ДанныеДоверенности.ДатаВыдачи, Истина, Ошибки);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "ДатаКонДовер", ДанныеДоверенности.ДатаОкончания, Истина, Ошибки);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "ПрПередов", ?(ДанныеДоверенности.ВозможноПередоверие, "1", "2"),
		Истина, Ошибки);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "ПрСовмПолн", "1", Истина, Ошибки); // полномочия совместные	
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "ВнНомДовер", ДанныеДоверенности.Номер, Истина, Ошибки);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "ДатаВнРегДовер", ДанныеДоверенности.ДатаВыдачи, Истина, Ошибки);

	Безотзыв = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДов.Безотзыв", ПространствоИмен);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Безотзыв, "ПрБезотзыв", "1", Истина, Ошибки); // отзыв возможен
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Безотзыв, "УслОтзыва", "1", Истина, Ошибки); // по истечению срока действия
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "Безотзыв", Безотзыв, Истина, Ошибки);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДов, "СведСистОтм", ДанныеДоверенности.СведенияОбИнформационнойСистеме,
		Истина, Ошибки);
		
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Довер, "СвДов", СвДов, Ложь, Ошибки);
	
	// формирование раздела Сведения о доверителе
	СвДоверит = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит", ПространствоИмен);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДоверит, "ТипДовер", ДанныеДоверенности.ДоверительЮЛ_ТипОрганизации,
		Истина, Ошибки);
	...
	Если ДанныеДоверенности.ДоверительЮЛ_ТипОрганизации = "ИО" Тогда
		...
		ИнОргДовер = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.ИнОргДовер", 
			ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "НаимИО", ДанныеДоверенности.ДоверительЮЛ_НаимОрг, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "ИННЮЛ", ДанныеДоверенности.ДоверительЮЛ_ИНН, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "КПП", ДанныеДоверенности.ДоверительЮЛ_КПП, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "СтрРег", ДанныеДоверенности.ДоверительЮЛ_СтрРег, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "НаимРегОрг", ДанныеДоверенности.ДоверительЮЛ_НаимРегОрг,
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "РегНомер", ДанныеДоверенности.ДоверительЮЛ_РегНомер,
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "АдрСтрРег", ДанныеДоверенности.ДоверительЮЛ_Адр, 
			Истина, Ошибки);

		СвРукОП = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.ИнОргДовер.СвРукОП",
			ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРукОП, "ИННФЛ", ДанныеДоверенности.ДоверительФЛ_ИНН, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРукОП, "НаимДокПолн", "_", Истина, Ошибки);

		СведФЛ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("СведФЛТип", ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ДатаРожд", ДанныеДоверенности.ДоверительФЛ_ДатаРождения,
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "МестоРожд", ДанныеДоверенности.ДоверительФЛ_МестоРожд, 
			Истина, Ошибки);
		
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "КонтактТлф", "_", Истина, Ошибки);

		Если ДанныеДоверенности.ДоверительФЛ_Гражданство = Справочники[КлассификаторСтранМира].Россия Тогда
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ПрГражд", "1", Истина, Ошибки);
		ИначеЕсли ЗначениеЗаполнено(ДанныеДоверенности.ДоверительФЛ_Гражданство) Тогда
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ПрГражд", "2", Истина, Ошибки);
		Иначе
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ПрГражд", "3", Истина, Ошибки);
		КонецЕсли;
		...
		КонецЕсли;
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "Гражданство", ДоверительФЛ_Гражданство, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРукОП, "СведФЛ", СведФЛ, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнОргДовер, "СвРукОП", СвРукОП, Истина, Ошибки);
		СвДоверит.ИнОргДовер.Добавить(ИнОргДовер);

	ИначеЕсли ДанныеДоверенности.ДоверительЮЛ_ТипОрганизации = "ЮЛ" Тогда

		РосОргДовер = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.РосОргДовер", 
			ПространствоИмен);
			
		СвРосОрг = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.РосОргДовер.СвРосОрг", 
			ПространствоИмен);	
			
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "НаимОрг", ДанныеДоверенности.ДоверительЮЛ_НаимОрг,
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "ОГРН", ДанныеДоверенности.ДоверительЮЛ_ОГРН, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "ИННЮЛ", ДанныеДоверенности.ДоверительЮЛ_ИНН, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "КПП", ДанныеДоверенности.ДоверительЮЛ_КПП, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "АдрРФ", ДанныеДоверенности.ДоверительЮЛ_Адр, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "АдрМНФакт", "_", 
			Истина, Ошибки);	
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "НаимУчрДок", "_", 
			Истина, Ошибки);	
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвРосОрг, "Тлф", "_", 
			Истина, Ошибки);

		ЛицоБезДов = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.РосОргДовер.ЛицоБезДов",
			ПространствоИмен);

		Если Не ПустаяСтрока(ДанныеДоверенности.ЛицоБезДовЮЛ_ИНН) Тогда
			СвОрг = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("СвОргТип", ПространствоИмен);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "НаимОрг", ДанныеДоверенности.ЛицоБезДовЮЛ_НаимОрг, 
				Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "ИННЮЛ", ДанныеДоверенности.ЛицоБезДовЮЛ_ИНН, 
				Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "КПП", ДанныеДоверенности.ЛицоБезДовЮЛ_КПП, 
				Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "ОГРН", ДанныеДоверенности.ЛицоБезДовЮЛ_ОГРН, 
				Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "НаимУчрДок", "_", Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "КонтактТлф", "_", Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "РегНомер", "_", Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ЛицоБезДов, "СвОрг", СвОрг, Истина, Ошибки);
		КонецЕсли;

		СвФЛ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.РосОргДовер.ЛицоБезДов.СвФЛ",
			ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвФЛ, "ИННФЛ", ДанныеДоверенности.ЛицоБезДовФЛ_ИНН, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвФЛ, "СНИЛС", ДанныеДоверенности.ЛицоБезДовФЛ_СНИЛС, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвФЛ, "Должность", ДанныеДоверенности.ЛицоБезДовФЛ_Должность, 
			Истина, Ошибки);
		...
		КонецЕсли;
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвФЛ, "НаимДокПолн", ОснованиеПолномочий, Истина, Ошибки);

		СведФЛ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("СведФЛТип", ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ДатаРожд", ДанныеДоверенности.ЛицоБезДовФЛ_ДатаРождения,
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "МестоРожд", "_", Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "КонтактТлф", "_", Истина, Ошибки);
		
		Если ДанныеДоверенности.ЛицоБезДовФЛ_Гражданство = Справочники[КлассификаторСтранМира].Россия Тогда
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ПрГражд", "1", Истина, Ошибки);
		...
		КонецЕсли;
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "Гражданство", ЛицоБезДовФЛ_Гражданство, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвФЛ, "СведФЛ", СведФЛ, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ЛицоБезДов, "СвФЛ", СвФЛ, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(РосОргДовер, "СвРосОрг", СвРосОрг, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(РосОргДовер, "ЛицоБезДов", ЛицоБезДов, Истина, Ошибки); 
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДоверит, "РосОргДовер", РосОргДовер, Истина, Ошибки);

	ИначеЕсли ДанныеДоверенности.ДоверительЮЛ_ТипОрганизации = "ИП" Тогда

		ИПДовер = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.ИПДовер", ПространствоИмен);
		...
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИПДовер, "НаимИП", НаименованиеИП, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИПДовер, "ИННФЛ", ДанныеДоверенности.ДоверительФЛ_ИНН, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИПДовер, "ОГРНИП", ДанныеДоверенности.ДоверительФЛ_ОГРН, 
			Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИПДовер, "СНИЛС", ДанныеДоверенности.ДоверительФЛ_СНИЛС, 
			Истина, Ошибки);
		...
		КонецЕсли;
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "Гражданство", ДоверительФЛ_Гражданство, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИПДовер, "СведФЛ", СведФЛ, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДоверит, "ИПДовер", ИПДовер, Истина, Ошибки);

	Иначе

		ФЛДовер = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвДоверит.ФЛДовер", ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФЛДовер, "ПрДеесп", "1", Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФЛДовер, "ИННФЛ", ДанныеДоверенности.ДоверительФЛ_ИНН, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФЛДовер, "СНИЛС", ДанныеДоверенности.ДоверительФЛ_СНИЛС, Истина, Ошибки);
		...
		КонецЕсли;
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "Гражданство", ДоверительФЛ_Гражданство, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФЛДовер, "СведФЛ", СведФЛ, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДоверит, "ФЛДовер", ФЛДовер, Истина, Ошибки);

	КонецЕсли;

	Подписант = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("ФИОТип", ПространствоИмен);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Подписант, "Фамилия", ЛицоБезДовФЛ_Фамилия, Истина, Ошибки);
	Если НЕ ПустаяСтрока(ЛицоБезДовФЛ_Имя) Тогда
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Подписант, "Имя", ЛицоБезДовФЛ_Имя, Ложь, Ошибки);
	КонецЕсли;
	Если НЕ ПустаяСтрока(ЛицоБезДовФЛ_Отчество) Тогда
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Подписант, "Отчество", ЛицоБезДовФЛ_Отчество, Ложь, Ошибки);
	КонецЕсли; 
	
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвДоверит, "Подписант", Подписант, Истина, Ошибки);
	...
	Для Каждого Представитель Из ДанныеДоверенности.ПереченьПредставителей Цикл
		...
		СвУпПред = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвУпПред", ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвУпПред, "ТипПред", Представитель.ТипПредставителя, Истина, Ошибки);
		
		Если Не ПустаяСтрока(Представитель.ПредставительЮЛ_ИНН) Тогда
			
			Если Представитель.ТипПредставителя = "ИП" Тогда
				
				СведИП = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвУпПред.СведИП", 
					ПространствоИмен);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведИП, "НаимИП", Представитель.ПредставительЮЛ_НаимОрг,
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведИП, "ИННФЛ", Представитель.ПредставительЮЛ_ИНН, 
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведИП, "ОГРНИП", Представитель.ПредставительЮЛ_ОГРН, 
					Истина, Ошибки);
				...
				ФИО = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("ФИОТип", ПространствоИмен);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Фамилия", ПредставительФЛ_Фамилия, Истина, Ошибки);
				Если НЕ ПустаяСтрока(ПредставительФЛ_Имя) Тогда
					ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Имя", ПредставительФЛ_Имя, Ложь, Ошибки);
				КонецЕсли;
				Если НЕ ПустаяСтрока(ПредставительФЛ_Отчество) Тогда
					ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ПредставительФЛ_Отчество, Ложь, Ошибки);
				КонецЕсли;
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведИП, "ФИО", ФИО, Истина, Ошибки);
				
				СведФЛ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("СведФЛТип", ПространствоИмен);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ДатаРожд", Представитель.ПредставительФЛ_ДатаРождения,
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "МестоРожд", "_", Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "КонтактТлф", "_", Истина, Ошибки);
				
				Если Представитель.ПредставительФЛ_Гражданство = Справочники[КлассификаторСтранМира].Россия Тогда
					ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "ПрГражд", "1", Истина, Ошибки);
				...
				КонецЕсли;
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "Гражданство", ПредставительФЛ_Гражданство, Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведИП, "СведФЛ", СведФЛ, Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвУпПред, "СведИП", СведИП, Истина, Ошибки);
				
			Иначе
				
				УзелСвОрг = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвУпПред.СвОрг", ПространствоИмен);
				СвОрг = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("СвОргТип", ПространствоИмен);
				
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "НаимОрг", Представитель.ПредставительЮЛ_НаимОрг,
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "ИННЮЛ", Представитель.ПредставительЮЛ_ИНН, 
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "КПП", Представитель.ПредставительЮЛ_КПП, 
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвОрг, "ОГРН", Представитель.ПредставительЮЛ_ОГРН, 
					Истина, Ошибки);
				...
				СведФизЛ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("СведФизЛТип", ПространствоИмен);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФизЛ, "ИННФЛ", Представитель.ПредставительФЛ_ИНН, 
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФизЛ, "СНИЛС", Представитель.ПредставительФЛ_СНИЛС, 
					Истина, Ошибки);
				...
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФизЛ, "ФИО", ФИО, Истина, Ошибки);
				...
				УдЛичнФЛ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("УдЛичнФЛТип", ПространствоИмен);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(УдЛичнФЛ, "КодВидДок", Представитель.ПредставительФЛ_ВидДок,
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(УдЛичнФЛ, "СерНомДок", СокрЛП(Представитель.ПредставительФЛ_СерДок
					+ " " + Представитель.ПредставительФЛ_НомДок), Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(УдЛичнФЛ, "ДатаДок", Представитель.ПредставительФЛ_ДатаДок,
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(УдЛичнФЛ, "ВыдДок", Представитель.ПредставительФЛ_ВыдДок, 
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(УдЛичнФЛ, "КодВыдДок", Представитель.ПредставительФЛ_КодВыдДок,
					Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "УдЛичнФЛ", УдЛичнФЛ, Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФизЛ, "СведФЛ", СведФЛ, Истина, Ошибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(УзелСвОрг, "СвОрг", СвОрг, Истина, Ошибки);
				УзелСвОрг.СвФЛ.Добавить(СведФизЛ);
				
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвУпПред, "СвОрг", УзелСвОрг, Истина, Ошибки);
		...
		Если Не ПустаяСтрока(Представитель.ПредставительФЛ_ИНН) И Представитель.ТипПредставителя = "ФЛ" Тогда
			...
			СведФизЛ = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвУпПред.СведФизЛ", 
				ПространствоИмен);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФизЛ, "ИННФЛ", Представитель.ПредставительФЛ_ИНН, 
				Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФизЛ, "СНИЛС", Представитель.ПредставительФЛ_СНИЛС, 
			...
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФЛ, "УдЛичнФЛ", УдЛичнФЛ, Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведФизЛ, "СведФЛ", СведФЛ, Истина, Ошибки);
			ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвУпПред, "СведФизЛ", СведФизЛ, Истина, Ошибки);
	...
	Если ДанныеДоверенности.Полномочия.Количество() = 0 Тогда
		...
		СвПолн = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.СвПолн", ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвПолн, "ТекстПолн",
	...
	Иначе
		...
		Для Каждого Полномочие Из ДанныеДоверенности.Полномочия Цикл
			...
			СвПолн = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.Документ.Довер.СвПолн", ПространствоИмен);
			
			Если НЕ ПустаяСтрока(Полномочие.Код) Тогда
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвПолн, "КодПолн", Полномочие.Код, Истина, Ошибки);
			КонецЕсли;
			
			Если НЕ ПустаяСтрока(Полномочие.Описание) Тогда
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвПолн, "ТекстПолн", Полномочие.Описание, Истина, Ошибки);
			КонецЕсли;

			Если НЕ ПустаяСтрока(Полномочие.Содержание) Тогда
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвПолн, "СодержПолн", Полномочие.Содержание, Истина, Ошибки);
			КонецЕсли;

			Если НЕ ПустаяСтрока(Полномочие.Пояснение) Тогда
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СвПолн, "ДопТекстПолн", Полномочие.Пояснение, Истина, Ошибки);
			КонецЕсли;
	...
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Документ, "Довер", Довер, Истина, Ошибки);
	ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Доверенность, "Документ", Документ, Истина, Ошибки);
	...
	Если Не ПустаяСтрока(ДанныеДоверенности.ИныеСведения) Тогда
		ИнСвед = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML("Доверенность.ИнСвед", ПространствоИмен);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ИнСвед, "ИнСвед", ДанныеДоверенности.ИныеСведения, Истина, Ошибки);
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Доверенность, "ИнСвед", ИнСвед, Истина, Ошибки);
	КонецЕсли;
	...
	Попытка
		...
		СтрокаXML = XDTOВСтроку(Доверенность, "windows-1251");
		СтрокаXML = УдалитьПространствоИменМЧД(СтрокаXML, ПространствоИмен);
...
Функция ДанныеДоверенностиДляКонтейнера(Доверенность) Экспорт
	...
	ДанныеДоверенности = МашиночитаемыеДоверенности.НовоеОписаниеДанныхДоверенностиДляКонтейнера();
...
Функция XDTOВСтроку(ОбъектXDTO, Кодировка)
	
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку(Кодировка);
	ЗаписьXML.ЗаписатьОбъявлениеXML();
	ФабрикаXDTO.ЗаписатьXML(ЗаписьXML, ОбъектXDTO,,,,НазначениеТипаXML.Неявное);
	Возврат ЗаписьXML.Закрыть();

КонецФункции 

Функция УдалитьПространствоИменМЧД(СтрокаXML, ПространствоИмен)
	
	Возврат СтрЗаменить(СтрокаXML, "xmlns=""" + ПространствоИмен + """", "");
