...
Процедура ПередЗаписью(Отказ)
	...
	Если ДвоичныеДанные <> Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ХешФайла) Тогда
			ХешФайла = МашиночитаемыеДоверенности.КонтрольнаяСуммаСтрокой(ДвоичныеДанные, ХешФункция.SHA256);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаписатьВЖурнал(Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(СведенияФайла.ДоверительФЛ_СНИЛС) Тогда
		...
		СведенияДоверителя.ДанныеИндивидуальногоПредпринимателя.Наименование = МашиночитаемыеДоверенностиКлиентСервер.ФИОСтрокой(
			СведенияФайла.ФИО[0]);
	...
	Для Каждого ПредставительОбъекта Из СведенияФайла.Представители Цикл
		Представитель_ФИО = МашиночитаемыеДоверенностиКлиентСервер.ФИОСтрокой(ПредставительОбъекта);
