...
Функция СохранениеВозможно()
	...
	Если ЗначениеЗаполнено(ДокументВид) Тогда
		...
		Если ЭтоНеподдерживаемыйВидДокумента Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументВид",, Отказ);
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(СокрЛП(ДокументСерия) + СокрЛП(ДокументНомер)) Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументСерия", , Отказ);
	КонецЕсли;
	...
	Если СтрДлина(СокрЛП(ДокументСерия) + СокрЛП(ДокументНомер)) > МаксимальноДопустимаяДлинаНомераДокумента Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, МаксимальноДопустимаяДлинаНомераДокумента);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументСерия", , Отказ);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ДокументДатаВыдачи) Тогда
		ТекстСообщения = НСтр("ru = 'Не задана дата выдачи документа.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументДатаВыдачи", , Отказ);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(ДокументКемВыдан) И КодВидаДокумента = "21" Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументКемВыдан", , Отказ);
	КонецЕсли;
