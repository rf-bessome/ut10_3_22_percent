...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлектронныеДокументыПереопределяемый.ПолучитьТипыОрганизацииПредставителяМЧД(
	...
	Если Параметры.Свойство("СтруктураДанных") 
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, Параметры.СтруктураДанных, ,"ТипПредставителя");
		
		ФЛ_Удостоверение = ПолучитьПредставлениеУдостоверение(ЭтаФорма);
		
		СведенияФизЛица = Новый Структура;
		СведенияФизЛица.Вставить("ИНН", ФЛ_ИНН);
		ФизическоеЛицо = ЭлектронныеДокументыПереопределяемый.ПолучитьФизЛицоМЧД(
			СведенияФизЛица);
		...
		Если ЗначениеЗаполнено(Параметры.СтруктураДанных.ЮЛ_ИНН) Тогда
			...
			Организация = ЭлектронныеДокументыПереопределяемый.ПолучитьОрганизациюМЧД(
				СведенияЮрЛица);
...
Процедура ФЛ_УдостоверениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПредставительФЛ_УдостоверениеНачалоВыбораЗавершение", ЭтаФорма);
	ОткрытьФорму(
...
Функция ЗаполнитьДанныеПредставителя()
	...
	ЗаполнитьЗначенияСвойств(СведенияОПредставителеМЧД, ЭтаФорма);
...
Процедура ФизическоеЛицоПриИзмененииНаСервере()
	...
	ФЛ_Гражданство = ?(ЗначениеЗаполнено(СтруктураДанныхФизЛица),
		СтруктураДанныхФизЛица.Гражданство, ПредопределенноеЗначение("Справочник.КлассификаторСтранМира.ПустаяСсылка"));
	...
	ЗаполнитьЗначенияСвойств(ЭтаФорма, ДанныеУдостоверения);
...
Процедура ОчиститьФизическоеЛицо()
	...
	ФЛ_ИНН = "";
	ФЛ_СНИЛС = "";
	ФЛ_Гражданство = ПредопределенноеЗначение("Справочник.КлассификаторСтранМира.ПустаяСсылка");
