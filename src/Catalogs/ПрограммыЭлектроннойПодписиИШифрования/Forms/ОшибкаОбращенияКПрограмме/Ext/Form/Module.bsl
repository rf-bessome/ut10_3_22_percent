...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭлектроннаяПодписьСлужебный.УстановитьЗаголовокОшибки(ЭтаФорма,
	 ...
	 Или Ошибки.Количество() = 2
	   И Ошибки[0].ОшибкаНаСервере <> Ошибки[1].ОшибкаНаСервере Тогда
		...
		Поток.НачатьПолучениеРазмера(
			Новый ОписаниеОповещения("ПриОткрытииПродолжение", ЭтаФорма));
...
Процедура ОшибкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ОшибкиПодробнее Тогда
		...
		ОткрытьФорму("ОбщаяФорма.РасширенноеПредставлениеОшибки", ПараметрыОшибки, ЭтаФорма);
...
Процедура ПриОткрытииПродолжение(Результат, Контекст) Экспорт
	...
	ОткрытьФорму("ОбщаяФорма.РасширенноеПредставлениеОшибки", ПараметрыОшибки, ЭтаФорма,,,, ОбработкаПродолжения);
