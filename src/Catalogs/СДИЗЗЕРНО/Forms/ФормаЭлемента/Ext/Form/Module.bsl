...
Процедура Подключаемый_ГиперссылкаДанныеПеревозчикаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьТранспортныеСредства" Тогда
		...
		ИнтеграцияЗЕРНОСлужебныйКлиент.ОткрытьФормуУказанияТранспортныхСредств(ПараметрыОткрытияФормы, ЭтаФорма);
...
Процедура Аннулировать(Команда)
	...
	ИнтеграцияЗЕРНОСлужебныйКлиент.АннулироватьСДИЗ(Объект.Ссылка, ЭтаФорма, ЗапросСДИЗЗавершение);
...
Процедура Погасить(Команда)
	...
	ОткрытьФорму("Документ.ПогашениеСДИЗЗЕРНО.ФормаОбъекта",
		Новый Структура("Основание", Объект.Ссылка), ЭтаФорма);
...
Процедура НастроитьЭлементыФормыПревозчиков()
	...
	Перевозчики          = ИнтеграцияЗЕРНОКлиентСервер.УникальныеПеревозчики(ЭтаФорма);
	...
	Для Каждого СтрокаТаблицы Из РеквизитыФормыПеревозчиков Цикл
		...
		Если СтрокиТранспортныеСредства.Количество() Тогда
			...
			ЭтаФорма[СтрокаТаблицы.ИмяПоляНадписьТранспортныеСредства] = Новый ФорматированнаяСтрока(
		...
		Если СтрокиМестаПерегрузки.Количество() Тогда
			...
			ЭтаФорма[СтрокаТаблицы.ИмяПоляНадписьМестаПерегрузки] = Новый ФорматированнаяСтрока(
		...
		ЭтаФорма[СтрокаТаблицы.ИмяПоляПеревозчик] = СтрокаТаблицы.Перевозчик;
