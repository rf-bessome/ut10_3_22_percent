...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьВидимостьЭлементаВводитьПарольВПрограммеЭлектроннойПодписи(ЭтаФорма);
	...
	Если Элементы.АвтоПоляИзДанныхСертификата.Видимость Тогда
		СтандартныеПодсистемыСервер.УстановитьКлючНазначенияФормы(ЭтаФорма, "НестандартныйСертификат");
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Отказ = Истина;
		СтандартныеПодсистемыКлиент.УстановитьХранениеФормы(ЭтаФорма, Истина);
...
Процедура НапомнитьОПеревыпускеПриИзменении(Элемент)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.ИзменитьОтметкуОНапоминании(Объект.Ссылка, НапомнитьОПеревыпуске, ЭтаФорма);
...
Процедура ДекорацияПеревыпущенОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Иначе	
		ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьСертификатПослеВыбораИзСписка", ЭтаФорма, Элемент);
...
Процедура СертификатОтозван(Команда)
	...
	Если Не Объект.Отозван Тогда
		ОписаниеСледующее = Новый ОписаниеОповещения("ПослеОтветаНаВопросСертификатОтозван", ЭтаФорма);
	...
	Иначе
		...
		|Продолжить?';
		|en = 'If a certificate is revoked by a certification authority, you will not be able to sign with this certificate even if you clear the mark in the application.
		|Continue?'"), РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура СменитьПинКод(Команда)
	...
	ОписаниеСледующее = Новый ОписаниеОповещения("ПослеСменыПинКода", ЭтаФорма);
...
Процедура РедактироватьИмяИОтчество(Команда)
	...
	ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.РедактироватьИмяИОтчество",
		Новый Структура("Имя, Отчество", Объект.Имя, Объект.Отчество), ЭтаФорма,,,,
		Новый ОписаниеОповещения("РедактироватьИмяИОтчествоПродолжение", ЭтаФорма),
...
Процедура ПриСозданииНаСервереПриЧтенииНаСервере()
	...
	Если Не ОткрытьЗаявление Тогда
		...
		Если СертификатыВЛичномХранилище.НайтиПоЗначению(Объект.Отпечаток) <> Неопределено Тогда
			ВключитьВидимостьСостоянияСертификата(ЭтаФорма, ТекущаяУниверсальнаяДата());
		КонецЕсли; 
...
Процедура ОбработчикОжиданияДобавитьСертификат()
	...
	СтандартныеПодсистемыКлиент.УстановитьХранениеФормы(ЭтаФорма, Ложь);
...
Процедура ОповещениеОЗакрытииЗаявления(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено И Результат.Добавлен И Не ЭтаФорма.Открыта() Тогда
...
Процедура ОбработчикОжиданияПоказатьСостояниеСертификата()
	...
	Оповещение = Новый ОписаниеОповещения(
		"ВидимостьСостоянияСертификатаПослеПолученияСертификатовВЛичномХранилище", ЭтаФорма);
...
Процедура ВидимостьСостоянияСертификатаПослеПолученияСертификатовВЛичномХранилище(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого КлючИЗначение Из Результат.СвойстваСертификатовНаКлиенте Цикл
		Если КлючИЗначение.Ключ = Объект.Отпечаток Тогда
			ВключитьВидимостьСостоянияСертификата(ЭтаФорма, ОбщегоНазначенияКлиент.ДатаУниверсальная());
			Возврат;
		КонецЕсли;	
	КонецЦикла;
...
Процедура ОткрытьСписокПользователей(РежимПросмотра)
	...
	ОповещениеЗавершения = Новый ОписаниеОповещения("СписокПользователейЗавершение", ЭтаФорма);
	...
	ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.СписокПользователей", 
			ПараметрыФормы, ЭтаФорма, , , , ОповещениеЗавершения);
