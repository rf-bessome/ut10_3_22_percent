	...
	Если ВРег(ИмяСобытия) = ВРег("ПодтверждениеВыполнитьОсновнуюОперацию") И Источник = УникальныйИдентификатор Тогда
		Если Параметр.Выполнено Тогда
			...
			Если ОповещениеПриПодтверждении = Неопределено Тогда
				ЗашифроватьДанные(Новый ОписаниеОповещения("ЗашифроватьЗавершение", ЭтаФорма));
			Иначе
...
Процедура ЗаполнитьПрограммуШифрования(Оповещение = Неопределено)
	...
	СертификатКриптографии.НачатьИнициализацию(Новый ОписаниеОповещения(
			"ЗаполнитьПрограммуШифрованияПослеИнициализацииСертификата", ЭтаФорма, Контекст),
		ДанныеСертификата);
...
Процедура ЗаполнитьПрограммуШифрованияЦиклНачало(Контекст)
	...
	ЭлектроннаяПодписьСлужебныйКлиент.СоздатьМенеджерКриптографии(Новый ОписаниеОповещения(
			"ЗаполнитьПрограммуШифрованияПослеСозданияМенеджераКриптографии", ЭтаФорма, Контекст),
...
Процедура ЗаполнитьПрограммуШифрованияПослеСозданияМенеджераКриптографии(МенеджерКриптографии, Контекст) Экспорт
	...
	МенеджерКриптографии.НачатьШифрование(Новый ОписаниеОповещения(
			"ЗаполнитьПрограммуШифрованияПослеШифрования", ЭтаФорма, Контекст,
			"ЗаполнитьПрограммуШифрованияПослеОшибкиШифрования", ЭтаФорма),
...
Процедура ПродолжитьОткрытиеПослеНачала(Результат, Контекст) Экспорт
	...
	Если УказанНеизменяемыйНаборСертификатов Тогда
		ЗаполнитьПрограммуШифрования(Новый ОписаниеОповещения(
			"ПродолжитьОткрытиеПослеЗаполненияПрограммы", ЭтаФорма, Контекст));
	Иначе
...
Процедура ПродолжитьОткрытиеПослеЗаполненияПрограммы(Результат, Контекст) Экспорт
	...
	Если БезПодтверждения Тогда
		Если МодульСервисКриптографииDSSПодтверждениеКлиент <> Неопределено Тогда
			ТребуетПодтверждения = МодульСервисКриптографииDSSПодтверждениеКлиент.ОблачнаяПодписьТребуетПодтверждения(ЭтаФорма);
			Если НЕ ТребуетПодтверждения Тогда
				ОбработкаПослеПредупреждения = Неопределено;
				ЗашифроватьДанные(Новый ОписаниеОповещения("ПродолжитьОткрытиеПослеШифрованияДанных", ЭтаФорма, Контекст));
				Возврат;
			КонецЕсли;	
		Иначе	
			ОбработкаПослеПредупреждения = Неопределено;
	...
	Если МодульСервисКриптографииDSSПодтверждениеКлиент <> Неопределено Тогда
		Если МодульСервисКриптографииDSSПодтверждениеКлиент.ПроверкаВыполненияНачальнойОперации(ЭтаФорма, БезПодтверждения) Тогда 
			МодульСервисКриптографииDSSПодтверждениеКлиент.ВыполнитьНачальнуюОперациюСервиса(ЭтаФорма, ОписаниеДанных, "");
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьШифрование(КлиентскиеПараметры, ОбработкаЗавершения) Экспорт
	...
	ЗашифроватьДанные(Новый ОписаниеОповещения("ВыполнитьШифрованиеЗавершение", ЭтаФорма, Контекст));
	   ...
	   И ВыполнятьНаСервере <> Ложь Тогда
		...
		Иначе
			...
			ЭлектроннаяПодписьСлужебныйКлиент.ВыполнитьНаСтороне(Новый ОписаниеОповещения(
					"ЗашифроватьДанныеПослеВыполненияНаСторонеСервера", ЭтаФорма, Контекст),
...
Процедура ЗашифроватьДанныеПослеВыполненияНаСторонеСервера(Результат, Контекст) Экспорт
	...
	Иначе
		...
		ЭлектроннаяПодписьСлужебныйКлиент.ВыполнитьНаСтороне(Новый ОписаниеОповещения(
				"ЗашифроватьДанныеПослеВыполненияНаСторонеКлиента", ЭтаФорма, Контекст),
...
Процедура Подключаемый_ПодтверждениеОбработкаКоманды(Команда)
	...
	МодульСервисКриптографииDSSПодтверждениеКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииDSSПодтверждениеКлиент");
	МодульСервисКриптографииDSSПодтверждениеКлиент.ПодтверждениеОбработкаКоманды(ЭтаФорма, Команда, ОписаниеДанных, "");
...
Процедура Подключаемый_ПодтверждениеПриИзменении(Элемент)
	...
	МодульСервисКриптографииDSSПодтверждениеКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииDSSПодтверждениеКлиент");
	МодульСервисКриптографииDSSПодтверждениеКлиент.ПодтверждениеПриИзменении(ЭтаФорма, Элемент, ОписаниеДанных, "");
...
Процедура Подключаемый_ПодтверждениеОбработчикОжидания()
	...
	МодульСервисКриптографииDSSПодтверждениеКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("СервисКриптографииDSSПодтверждениеКлиент");
	МодульСервисКриптографииDSSПодтверждениеКлиент.ПодтверждениеОбработкаОжидания(ЭтаФорма, ОписаниеДанных, "");
