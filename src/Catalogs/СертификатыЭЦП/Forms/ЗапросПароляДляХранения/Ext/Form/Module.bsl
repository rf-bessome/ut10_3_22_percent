...
Процедура КомандаГотово(Команда)
	...
	НаСервере = Ложь;
	Если (ВидОперации = НСтр("ru = 'Аутентификация в сервисе 1С-ЭДО'")
		ИЛИ ВидОперации = НСтр("ru = 'Аутентификация в сервисе 1С-Такском'"))
		И ЭлектронныеДокументыСлужебныйВызовСервера.ВыполнятьАвторизациюНаСервере() Тогда
		НаСервере = Истина;
	...
	Если НаСервере Тогда
		ТестСертификатаНаСервере(ВыбранныйСертификат, ПарольПользователя, Отказ); 
	Иначе
		ТестСертификатаНаКлиенте(ВыбранныйСертификат, ПарольПользователя, Отказ);
	КонецЕсли;
...
Процедура ОбъектыДляОбработкиНажатие(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли ТаблицаЭД[0].ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД")
		И Не ЭлектронныеДокументыСлужебныйВызовСервера.ЭтоСлужебныйДокумент(ТаблицаЭД[0].ЭлектронныйДокумент) Тогда
	...
	ИначеЕсли МашиночитаемыеДоверенностиКлиентСервер.ЭтоСсылкаНаДоверенность(ТаблицаЭД[0].ВладелецЭД) Тогда
		ПоказатьЗначение(, ТаблицаЭД[0].ЭлектронныйДокумент);
	Иначе
		ПараметрыФормы = Новый Структура;
	...
	Если ТипЗнч(Параметры.ОбъектыДляОбработки) = Тип("Массив") И Параметры.ОбъектыДляОбработки.Количество() > 0 Тогда
		Если Параметры.ОбъектыДляОбработки.Количество() = 1 Тогда
			...
			Если ТипЗнч(ЭлектронныйДокумент) = Тип("ДокументСсылка.ПакетЭД") Тогда
				...
				Если ТипЗнч(НоваяСтрока.ВладелецЭД) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий")
					ИЛИ ТипЗнч(НоваяСтрока.ВладелецЭД) = Тип("ДокументСсылка.ЭлектронныйДокументИсходящий") Тогда
					
					НоваяСтрока.ВидЭД = ОбщегоНазначения.ПолучитьЗначениеРеквизита(НоваяСтрока.ВладелецЭД, "ВидЭД");
				КонецЕсли;
			ИначеЕсли ТипЗнч(ЭлектронныйДокумент) = Тип("СправочникСсылка.ЭДПрисоединенныеФайлы") Тогда
				...
				Если ОбщегоНазначения.ЭтоДокумент(НоваяСтрока.ВладелецЭД.Метаданные()) Тогда
					НоваяСтрока.ВидЭД = ОбщегоНазначения.ПолучитьЗначениеРеквизита(НоваяСтрока.ВладелецЭД, "ВидЭД");
				КонецЕсли;
			ИначеЕсли МашиночитаемыеДоверенностиКлиентСервер.ЭтоСсылкаНаДоверенность(ЭлектронныйДокумент) Тогда
				ШаблонГиперссылки = НСтр("ru = '%1'");
				ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонГиперссылки,
					ЭлектронныйДокумент);	
...
Функция ПолучитьОтпечатокСертификата(Знач СсылкаНаСертификат)
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаСертификат, "Отпечаток");
КонецФункции

&НаСервереБезКонтекста
Процедура ТестСертификатаНаСервере(Знач СсылкаНаСертификат, Знач Пароль, Отказ) Экспорт
	...
	Отпечаток = ПолучитьОтпечатокСертификата(СсылкаНаСертификат);
	Сертификат = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьСертификатПоОтпечатку(Отпечаток);
	Если Сертификат = Неопределено Тогда
		РезультатТеста = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьСообщениеОбОшибке("111");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(РезультатТеста);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Исключение
		ТекстОшибки = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьСообщениеОбОшибке("112");
		РезультатТеста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									ШаблонРезультата,
									ТекстОшибки,
									КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Исключение
		ТекстОшибки = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьСообщениеОбОшибке("113");
		РезультатТеста = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									ШаблонРезультата,
									ТекстОшибки,
									КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
...
&НаКлиенте
Процедура ТестСертификатаНаКлиенте(СсылкаНаСертификат, Пароль, Отказ) Экспорт
