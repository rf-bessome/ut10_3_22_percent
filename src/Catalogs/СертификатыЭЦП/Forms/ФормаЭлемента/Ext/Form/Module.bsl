...
Процедура УстановитьДоступностьВидимость()
	...
	Если Объект.ПрограммаБанка <> Перечисления.ПрограммыБанка.ОбменЧерезДопОбработку
		...
		Если (НовыйСертификат.Субъект.Свойство("SN") ИЛИ НовыйСертификат.Субъект.Свойство("CN"))
			...
			Если Объект.ДолжностьПоСертификату = "0" И ДолжностьДоступность Тогда
				Объект.ДолжностьПоСертификату = "";
				ЗаписатьСертификат = Ложь;
			КонецЕсли;
			
			
			Если ДолжностьДоступность и Не ЗначениеЗаполнено(Объект.ДолжностьПоСертификату) Тогда
				Модифицированность = Истина;
			КонецЕсли;
			
			Если ЗаписатьСертификат И ЗначениеЗаполнено(Объект.ДолжностьПоСертификату) Тогда
				Записать();
			КонецЕсли;
			...
			Элементы.ДолжностьПоСертификату.Доступность = ДолжностьДоступность;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДолжностьПоСертификату.ОграничениеТипа = Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(128));	
