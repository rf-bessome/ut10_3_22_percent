...
Процедура ОбновитьЗаголовокФормы(Форма)
	...
	Если Форма.Объект.УдалитьЭтоТиповое Тогда
		ТекстЗаголовкаСоздание = НСтр("ru='Типовая настройка ЭДО между организациями (создание)'");
		ТекстЗаголовка = НСтр("ru='[Наименование] (Типовая настройка ЭДО между организациями)'");
	Иначе
		ТекстЗаголовкаСоздание = НСтр("ru='Настройка ЭДО между организациями (создание)'");
		ТекстЗаголовка = НСтр("ru='[Наименование] (Настройка ЭДО между организациями)'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Форма.Объект.Ссылка) Тогда 
		Форма.Заголовок = СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ТекстЗаголовка,
			Новый Структура("Наименование", Форма.Объект.Наименование));
	Иначе
...
&НаКлиенте
Процедура ПриИзмененииОтправителя() 
...
Процедура ИсходящиеДокументыПриАктивизацииСтроки(Элемент)
	...
	ТекущаяСтрока = Элементы.ИсходящиеДокументы.ТекущаяСтрока;
	Если ТекущаяСтрокаИсходящихДокументов <> ТекущаяСтрока Тогда 
		ТекущаяСтрокаИсходящихДокументов = ТекущаяСтрока;
		ЗаполнитьТаблицуЭтапов();
	КонецЕсли;	
...
Процедура ИсходящиеДокументыИспользоватьОбменПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЭлемента Тогда
		...
		Элементы.ИсходящиеДокументы.ТекущиеДанные.ИспользоватьЭЦП           = ЗначениеЭлемента;
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда // Новый
		...
		Если НЕ ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда // Не копирование
			Если Параметры.Свойство("Организация", Объект.Организация) Тогда
				СозданиеСЗаполненнымОтправителем = Истина;
			КонецЕсли;
			Если Параметры.Свойство("Контрагент", Объект.Контрагент) Тогда
				СозданиеСЗаполненнымПолучателем = Истина;
			Иначе
	...
	ТекущаяСтрокаИсходящихДокументов = -1;
...
Процедура ПриОткрытии(Отказ)
	...
	Если СозданиеСЗаполненнымОтправителем Тогда
		УстановитьИдентификатор("Организация", Объект.Организация);
		ПриИзмененииОтправителя();
	КонецЕсли;
	
	Если СозданиеСЗаполненнымПолучателем Тогда
		УстановитьИдентификатор("Контрагент", Объект.Контрагент);
		ПриИзмененииПолучателя();
	КонецЕсли;
