...
Процедура ПередЗакрытием(Отказ,  СтандартнаяОбработка)
	...
	Если  ЕстьНераспечатанныеКоды
		...
		ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ВопросЗакрытьФормуПриЗавершении", ЭтаФорма);
...
Процедура ШтрихкодыУпаковокПередУдалением(Элемент, Отказ)
	...
	Если ПометкаВыделеныхСтрок > 0 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ВопросУдалениеСтрокЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ШтрихкодыУпаковокПометкаПриИзменении(Элемент)
	...
	Если ТекущиеДанные.Пометка Тогда
		Если Не ЕстьНераспечатанныеКоды Тогда
			...
			УстановитьДоступностьКнопкиПечати(ЭтаФорма);
		КонецЕсли;
...
Процедура Заполнить(Команда)
	...
	ОткрытьФорму(
		...
		Новый ОписаниеОповещения("ОбработкаЗакрытияФормыЗаполнения", ЭтаФорма, ПараметрыЗаполнения))
...
Процедура ОткрытьФормуГенерацииШтрихкодов()
	...
	Иначе
		ПараметрыОткрытияФормы.Вставить("ТипШтрихкода",   ПоследнийТипШтрихкода(ЭтаФорма));
		ПараметрыОткрытияФормы.Вставить("Номенклатура",   Номенклатура);
		ПараметрыОткрытияФормы.Вставить("Характеристика", Характеристика);
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриГенерацииШтрихкодаУпаковки", ЭтаФорма);
...
Процедура СформироватьШтрихкодыКлиент()
	...
	Если ШтрихкодыУпаковок.Количество() > 0 Тогда
		...
		ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ВопросОчиститьШтрихкодыУпаковокПриЗавершении", ЭтаФорма);
...
Процедура СформироватьШтрихкодыСервер()
	...
	ТипШтрихкода            = ПоследнийТипШтрихкода(ЭтаФорма);
	ТипУпаковки             = ПоследнийТипУпаковки(ЭтаФорма);
	КлючНумератора          = ПоследнийКлючНумератора(ЭтаФорма, ТипШтрихкода);
	ОбразецШтрихкода        = ПоследнийОбразецШтрихкода(ЭтаФорма);
