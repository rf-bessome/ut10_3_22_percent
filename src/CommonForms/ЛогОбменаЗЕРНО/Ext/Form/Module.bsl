...
Процедура СохранитьПротоколВФайл(Команда)
	...
	ВыборТипаСохраняемогоЛогаЗапросовЗавершение = Новый ОписаниеОповещения(
		"ВыборТипаСохраняемогоЛогаЗапросовЗавершение", ЭтаФорма);
...
Процедура ВыборТипаСохраняемогоЛогаЗапросовЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	ОповещениеВыбораФайла = Новый ОписаниеОповещения("ВыборФайлаЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ЗаполнитьПоИсточнику(Источник)
	...
	Если ТипЗнч(Источник) = Тип("Строка") Тогда
		...
		Если Выборка.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(ЭтаФорма, Выборка, "ДатаСоздания, Документ, Операция");
	...
	Иначе
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(ЭтаФорма, Выборка, "ДатаСоздания, Документ, Операция, ДополнительноеОписание, Версия");
