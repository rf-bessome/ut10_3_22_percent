...
Процедура СохранитьПротоколВФайл(Команда)
	...
	ОповещениеВыбораФайла = Новый ОписаниеОповещения("ВыборФайлаЗавершение", ЭтаФорма);
...
Процедура ЗаполнитьПоИсточнику(Источник)
	...
	Если ТипЗнч(Источник) = Тип("Строка") Тогда
		...
		Если Выборка.Следующий() Тогда
			ЗаполнитьЗначенияСвойств(ЭтаФорма, Выборка, "ДатаСоздания, Документ, Операция, ИдентификаторЗапроса");
