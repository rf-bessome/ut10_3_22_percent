...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбменСГИСЭПД.СоздатьЭлементыВводаЗначенийСписка(ЭтаФорма, "Получатели", "Адрес");
...
Процедура ПриОткрытии(Отказ)
	...
	Обработчик = Новый ОписаниеОповещения("ПослеЗагрузкиQR", ЭтаФорма);
	//ДлительныеОперацииКлиент.ОжидатьЗавершение(ФоновоеЗадание, Обработчик, НастройкиОжидания);
	
	ВыполнитьОбработкуОповещения(Обработчик, ФоновоеЗадание);
...
Функция ЗапуститьЗагрузкуQRВФоновомЗадании()
	...
	ОбменСГИСЭПД.ЗагрузитьQRВФоне(ПараметрыПроцедуры, АдресРезультата);
...
Процедура ОтправитьПоПочте(Команда)
	...
	ОкончаниеПроверки = Новый ОписаниеОповещения("ОкончаниеПроверкиНаличиеУчетнойЗаписиДляОтправкиПочты", ЭтаФорма);
	ОбменСГИСЭПДКлиент.ПроверитьНаличиеУчетнойЗаписиДляОтправкиПочты(ОкончаниеПроверки);
...
Процедура ОтправитьПисьмо(ТекстОшибки = "")
	...
	ДоступныеУчетныеЗаписи = МодульРаботаСПочтовымиСообщениями.ПолучитьДоступныеУчетныеЗаписи(Истина,,Истина);	
	...
	//МодульРаботаСПочтовымиСообщениями.ОтправитьСообщение(УчетнаяЗапись, Письма, ТекстОшибки);
