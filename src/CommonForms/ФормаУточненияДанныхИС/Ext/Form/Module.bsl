...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(
		ЭтаФорма, ПараметрыСканирования.ДопустимыеВидыПродукции, "Номенклатура");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ГосИС.ИСМП") Тогда
		...
		Модуль.УстановитьПараметрыВыбораШаблонаЭтикетки(ЭтаФорма, Элементы.ШаблонЭтикетки.Имя);
	КонецЕсли;
	...
	УстановитьФорматДатыГоденДо(ЭтаФорма);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Номенклатура)
		...
		Или ЭтоЧастичноеВыбытие(ЭтаФорма) Тогда
	...
	Если ЭтоЧастичноеВыбытие(ЭтаФорма) Тогда
	...
	Если Не ЭтоВыборКодаМаркировкиКегаНаОборудованииРозливаИСМП(ЭтаФорма) Тогда
		МассивНепроверяемыхРеквизитов.Добавить("КодМаркировки");
	КонецЕсли;
	
	ОбщегоНазначения.УдалитьНепроверяемыеРеквизитыИзМассива(ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов);
	
	СобытияФормИСПереопределяемый.ОбработкаПроверкиЗаполненияНаСервере(ЭтаФорма, Отказ, ПроверяемыеРеквизиты);
...
Процедура GTINОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение = "НастроитьГрупповыеУпаковкиИНаборы"
		...
		МодульИнтеграцияИСМПКлиент.ОткрытьФормуНастройкиВидовУпаковокПоGTIN(ПараметрыОткрытияФормы, ЭтаФорма);
...
Процедура ИдентификаторПроисхожденияВЕТИСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ПараметрыОткрытия, ЭтаФорма);
...
Процедура КоличествоВУпаковкеПриИзменении(Элемент)
	РасчетОграниченийПолейЧастичногоВыбытия(ЭтаФорма);
КонецПроцедуры
...
Процедура РежимВыбытияПриИзменении(Элемент)
	...
	НастроитьЭлементыФормыЧастичногоВыбытия(ЭтаФорма);
...
Процедура ДанныеДокументаВыбратьПриИзменении(Элемент)
	...
	Если ТекущиеДанные.Выбрать Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ТекущиеДанные,,"Количество");
...
Процедура ПоискПоШтрихкоду(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВводШтрихкодаЗавершение", ЭтаФорма);
...
Процедура ЗаписатьRFID(Команда)
	...
	ПараметрыЗаписиRFID.Вставить("Форма",                   ЭтаФорма);
	ПараметрыЗаписиRFID.Вставить("ОповещениеПриЗавершении", Новый ОписаниеОповещения("ПослеЗаписиRFID", ЭтаФорма, ДанныеRFIDДляЗаписи));
...
Процедура НоменклатураПриИзмененииСервер()
	...
	СобытияФормИСПереопределяемый.ПриИзмененииНоменклатуры(ЭтаФорма, ЭтаФорма,, ПараметрыУказанияСерий);
	...
	СобытияФормИС.ПрименитьУсловноеОформлениеКПолю(ЭтаФорма, "Характеристика");
	СобытияФормИС.ПрименитьУсловноеОформлениеКПолю(ЭтаФорма, "Серия");
...
Процедура ВывестиПоясняющийТекст()
	...
	ИначеЕсли ЭтоВыборКодаМаркировкиКегаНаОборудованииРозливаИСМП(ЭтаФорма) Тогда
...
Процедура ЗаполнитьДанныеФормыПриСозданииНаСервере()
	...
	ТипыНоменклатуры.ЗагрузитьЗначения(ОпределяемыеТипы().Номенклатура.Тип.Типы());
	...
	ИнтеграцияИСПереопределяемый.ЗаполнитьПараметрыУказанияСерий(
		ПараметрыУказанияСерий, Метаданные.ОбщиеФормы.ФормаУточненияДанныхИС, ЭтаФорма);
	...
	Если ДанныеДокумента.Количество() = 1 Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ДанныеДокумента[0],,"Количество");
...
Процедура ЗаполнитьДанныеДляПечатиЭтикеток()
	...
	Если ЗначениеЗаполнено(ВидПродукцииНоменклатуры)
		...
		Модуль.НастроитьШаблоныФормыУточненияДанных(ЭтаФорма, ВидПродукцииНоменклатуры);
	КонецЕсли;
...
Процедура ОбновитьОтображениеПоляНоменклатура()
	...
	ИначеЕсли ПараметрыСканирования.РазрешенаОбработкаКодовСПустойНоменклатурой
		...
		ДанныеДляПолученияПредставленияGTINОстатков.Колонки.Добавить("Номенклатура",             ОпределяемыеТипы().Номенклатура.Тип);
		ДанныеДляПолученияПредставленияGTINОстатков.Колонки.Добавить("GTIN",                     ОпределяемыеТипы().GTIN.Тип);
...
Процедура ВводШтрихкодаЗавершение(ДанныеШтрихкода, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ПустаяСтрока(ТекстОшибки) Тогда
		...
		ШтрихкодированиеИСКлиент.ОткрытьФормуНевозможностиДобавленияОтсканированного(ЭтаФорма, ПараметрыОткрытия);
	...
	Если Не ШтрихкодНайден Тогда
		...
		ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеСопоставленияНеизвестныхШтрихкодов", ЭтаФорма);
		ШтрихкодированиеИСКлиентПереопределяемый.ОткрытьФормуПодбораНоменклатурыПоШтрихкодам(
			Штрихкоды, ЭтаФорма, ОповещениеОЗакрытии);
...
Процедура ОбработатьШтрихкод(ДанныеШтрихкода, ШтрихкодНайден, ТекстОшибки = "")
	...
	Если ДопустимыеДанныеПоШтрихкодамEAN.Количество() = 1 Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ДопустимыеДанныеПоШтрихкодамEAN[0],,"Количество");
...
Процедура ПриИзмененииРежимаВыбытия()
	...
	Если ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИС.НастроеннаяНоменклатура Тогда
		...
		ТаблицаНоменклатуры.Колонки.Добавить("ЕдиницаИзмерения",           ОпределяемыеТипы().Упаковка.Тип);
		...
		ИнтеграцияИСПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтаФорма, ТаблицаНоменклатуры);
...
Функция ТребуетсяУточнениеФиксироватьВыбытиеБутылки()
	...
	Если ЭтоЧастичноеВыбытие(ЭтаФорма)
...
Функция ЗакрытьПослеСканированияШтрихкодаНоменклатуры()
	...
	Отказ = Ложь;
	СобытияФормИСКлиентПереопределяемый.ПроверитьЗаполнение(ЭтаФорма, Отказ);
	Возврат Не Отказ;
...
Функция МассивЗначенийGTINДляВыбора()
	...
	Возврат ИнтеграцияИСМП.МассивЗначенийGTINДляВыбора(ПараметрыВыбора, ЭтаФорма, , ДопустимыеВидыУпаковок);
