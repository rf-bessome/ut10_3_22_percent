...
Процедура Запросить(Команда)
	...
	Если ИсходящиеЗапросы.Количество() = 0 Тогда
		ПодготовитьЗапросКПередаче(
			Новый ОписаниеОповещения("ПослеПодготовкиЗапросаКПередаче", ЭтаФорма));
	ИначеЕсли МассивЗапросов.Количество() > 0 Тогда
		...
		ИнтеграцияЕГАИССлужебныйКлиент.ОбработатьОчередьПередачиДанных(
			Новый ОписаниеОповещения("ПослеПодготовкиМассиваЗапросовКПередаче", ЭтаФорма),
...
Процедура ВыполнитьОбмен()
	...
	Если ИсходящиеЗапросы.Количество() > 0 Тогда
		...
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("ПриЗавершенииОбменаПакетаЗапросов", ЭтаФорма);
	Иначе
		МассивОрганизацийЕГАИС.Добавить(ОрганизацияЕГАИС);
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("ПриЗавершенииОбмена", ЭтаФорма);
	КонецЕсли;
