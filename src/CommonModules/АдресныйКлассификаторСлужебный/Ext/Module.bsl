...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	АдресФайлаВерсий = ПолучениеФайловИзИнтернетаКлиентСервер.СтруктураURI(
	...
	АдресВебСервиса = ПолучениеФайловИзИнтернетаКлиентСервер.СтруктураURI(АдресВебСервисаКонтактнойИнформации());
...
Функция СравнитьЗначения(Значение1, Значение2)
	...
	Результат = СравнениеЗначений.Сравнить(Значение1, Значение2);
...
Функция ДанныеАутентификацииПользователяИнтернетПоддержки()
		...
		//ИнтернетПоддержкаПользователейВызовСервера.ЗагрузкаЛогинаИПароляИзПараметровИнтернетПоддержкиПользователей(
...
Функция ПараметрыАутентификацииВСервисе()
	...
	Иначе
		...
		Возврат ЭлектронныеДокументыСлужебный.ПараметрыАутентификацииНаСайте();
...
Функция ПараметрыАутентификацииНаСайте() Экспорт
 	...
 	Возврат ПараметрыАутентификацииВСервисе();
...
Функция ДоступнаЗагрузкаАдресныхСведенийИзИнтернет() Экспорт
	Возврат ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.ПолучениеФайловИзИнтернета");
КонецФункции
...
Функция СлужебныеАдресныеСведения(КаталогФайлов)
	...
	Попытка
		...
		СтрокаДаты = СтрЗаменить(ФайлЗагрузки.ДатаВерсии, "T", "");
		СтрокаДаты = СтрЗаменить(СтрокаДаты, "-", "");
		ДатаВерсии = Дата(СтрЗаменить(СтрокаДаты, ":", ""));
...
Процедура ОчиститьУровниСокращенийАдресныхСведений()
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Не удалось очистить регистр сведений Уровни сокращений адресных сведений.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Функция АдресИнтернетаВключаяПорт(Адрес)
	...
	СоставАдреса = ПолучениеФайловИзИнтернетаКлиентСервер.СтруктураURI(Адрес);
