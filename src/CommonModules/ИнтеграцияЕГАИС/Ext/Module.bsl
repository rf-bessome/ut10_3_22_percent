...
Процедура ЗаполнитьСопоставленнуюПродукцию(ТаблицаСопоставления) Экспорт
	...
	Если ТаблицаЗапроса.Колонки.Найти("Номенклатура") = Неопределено Тогда
		ТаблицаЗапроса.Колонки.Добавить("Номенклатура", ОпределяемыеТипы().Номенклатура.Тип);
	КонецЕсли;
	
	Если ТаблицаЗапроса.Колонки.Найти("Характеристика") = Неопределено Тогда
		ТаблицаЗапроса.Колонки.Добавить("Характеристика", ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
	КонецЕсли;
	
	Если ТаблицаЗапроса.Колонки.Найти("Серия") = Неопределено Тогда
		ТаблицаЗапроса.Колонки.Добавить("Серия", ОпределяемыеТипы().СерияНоменклатуры.Тип);
	КонецЕсли;
...
Функция ЗначениеОпределяемогоТипаПоНаименованию(ИмяТипа, Наименование, ТочноеСоответствие, Родитель = Неопределено, Владелец = Неопределено) Экспорт
	...
	ТипыОпределяемогоТипа = ОпределяемыеТипы()[ИмяТипа].Тип.Типы();
...
Процедура ПриОпределенииНастроекРаботыСФайлами(Настройки) Экспорт
	...
	Для Каждого Тип Из ОпределяемыеТипы().ДокументыЕГАИС.Тип.Типы() Цикл
...
Функция ИспользуетсяИнтеграцияВФормеДокументаОснования(Форма, Объект) Экспорт
	...
	Иначе
		...
		Если СтрНайти(""+ТипОбъекта, "ЕГАИС") Тогда
			Возврат Ложь;
		ИначеЕсли ОпределяемыеТипы().ОснованиеСтатусыОформленияДокументовЕГАИС.Тип.СодержитТип(ТипОбъекта) Тогда
			Возврат Истина;
		КонецЕсли;
		Возврат Ложь;
	КонецЕсли;
...
Функция ДанныеОформленияДокументовПоПриоритетам(ДокументСсылка) Экспорт
	...
	Для Каждого КлючИЗначение Из СтатусыОформления Цикл
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.АктСписанияЕГАИС.Имя Тогда
			...
			Если ОпределяемыеТипы().ОснованиеАктаСписанияЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.АктПостановкиНаБалансЕГАИС.Имя Тогда
			...
			Если ОпределяемыеТипы().ОснованиеАктаПостановкиНаБалансЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.ПередачаВРегистр2ЕГАИС.Имя Тогда
			...
			Если ОпределяемыеТипы().ОснованиеПередачиВРегистр2ЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.ТТНИсходящаяЕГАИС.Имя
			...
			Если ОпределяемыеТипы().ОснованиеТТНИсходящаяЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
			...
			КонецЕсли;
			Если ОпределяемыеТипы().ОснованиеТТНВходящаяЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.ЧекЕГАИС.Имя
			...
			Если ОпределяемыеТипы().ОснованиеЧекаЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
				Данные = ДанныеДокументаЕГАИС(Метаданные.Документы.ЧекЕГАИС, ДокументыПоОснованию, СтатусыОформления);
				ДобавитьВРезультат(ОформлениеДокументовПоПриоритетам, 1, Данные);
			КонецЕсли;
			Если ОпределяемыеТипы().ОснованиеЧекаЕГАИСВозврат.Тип.СодержитТип(ТипДокумента) Тогда
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.УведомлениеОПланируемомИмпортеЕГАИС.Имя Тогда
			...
			Если ОпределяемыеТипы().ОснованиеУведомлениеОПланируемомИмпортеЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
		...
		Если КлючИЗначение.Ключ = Метаданные.Документы.ОтчетОПроизводствеЕГАИС.Имя Тогда
			...
			Если ОпределяемыеТипы().ОснованиеОтчетОПроизводствеЕГАИС.Тип.СодержитТип(ТипДокумента) Тогда
...
Процедура СопоставитьАлкогольнуюПродукциюСНоменклатурой(Объект, НастройкиСопоставления = Неопределено) Экспорт
	...
	Если МассивКолонокДляДополнения.Найти("ИдентификаторУпаковки")<>Неопределено Тогда
		Данные.Колонки.Добавить("ИдентификаторУпаковки", ОпределяемыеТипы().СтрокаЕГАИС.Тип);
	КонецЕсли;
...
Процедура УстановитьВидимостьКомандыВыполнитьОбмен(Форма, ИмяЭлемента) Экспорт
	...
	Если НЕ ИнтеграцияИС.ПравоИзмененияДокументовИзОпределяемогоТипа(ОпределяемыеТипы().ДокументыЕГАИС) Тогда
		Кнопка = Форма.Элементы.Найти(ИмяЭлемента);
		Если Кнопка <> Неопределено Тогда
			Кнопка.Видимость = Ложь;
		КонецЕсли;
...
Функция ПодготовитьДанныеДляПодбораСправок2(Товары)
	...
	ТаблицаДляЗаполнения.Колонки.Добавить("Номенклатура",         ОпределяемыеТипы().Номенклатура.Тип);
	ТаблицаДляЗаполнения.Колонки.Добавить("Характеристика",       ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
	ТаблицаДляЗаполнения.Колонки.Добавить("Серия",                ОпределяемыеТипы().СерияНоменклатуры.Тип);
	...
	ТаблицаДляСписания.Колонки.Добавить("Номенклатура",         ОпределяемыеТипы().Номенклатура.Тип);
	ТаблицаДляСписания.Колонки.Добавить("Характеристика",       ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
	ТаблицаДляСписания.Колонки.Добавить("Серия",                ОпределяемыеТипы().СерияНоменклатуры.Тип);
...
Функция ТаблицаНеАкцизныеМарки()
	...
	ТаблицаНеАкцизныеМарки.Колонки.Добавить("Количество",                        ОпределяемыеТипы().КоличествоЕГАИСНеотрицательное.Тип);
...
Функция ОбработатьПолученныеДанныеТСД(ШтрихкодыТСД, ПараметрыПроверки) Экспорт
	...
	ТаблицаШтрихкодов.Колонки.Добавить("Количество",   ОпределяемыеТипы().КоличествоЕГАИСНеотрицательное.Тип);
