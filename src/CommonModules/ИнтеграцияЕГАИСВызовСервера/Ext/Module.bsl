...
Функция ОбработатьВходящиеДокументы(ДокументыКЗагрузке, ИдентификаторВладельца = Неопределено) Экспорт
	...
	Для Каждого КлючИЗначение Из ДокументыКЗагрузке Цикл
		...
		ВходящиеТранспортныеНакладные.Колонки.Добавить("WBRegID", ОпределяемыеТипы().СтрокаЕГАИС.Тип);
...
Функция ВозможныеТТНЕГАИС(Организация, Склад, Контрагент) Экспорт
	...
	Для Каждого ТипыДокументаПоступления Из ОпределяемыеТипы().ОснованиеТТНВходящаяЕГАИС.Тип.Типы() Цикл
	...
	Для Каждого ТипыКонтрагентов Из ОпределяемыеТипы().ОрганизацияКонтрагентГосИС.Тип.Типы() Цикл
