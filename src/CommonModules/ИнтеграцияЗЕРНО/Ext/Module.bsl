...
Процедура УстановитьВидимостьКомандыВыполнитьОбмен(Форма, ИмяЭлемента) Экспорт
	...
	Если НЕ ИнтеграцияИС.ПравоИзмененияДокументовИзОпределяемогоТипа(ОпределяемыеТипы().ДокументыЗЕРНО) Тогда
...
Процедура ПриСозданииНаСервереФормыСпискаДокументовЗЕРНО(Форма, ИменаСписков, ИменаСписковКОформлению) Экспорт
	...
	Если Метаданные.Документы.Найти(ИмяДокумента) = Неопределено
		ИЛИ НЕ ОпределяемыеТипы().ДокументыЗЕРНОПоддерживающиеСтатусыОформления.Тип.СодержитТип(Тип("ДокументСсылка." + ИмяДокумента)) Тогда
		Возврат;
	КонецЕсли;
...
Функция ИспользуетсяИнтеграцияВФормеДокументаОснования(Форма, Объект) Экспорт
	...
	ИначеЕсли НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Объект, "Ссылка") Тогда
		Возврат Ложь;
	ИначеЕсли НЕ ОпределяемыеТипы().ОснованиеСтатусыОформленияДокументовЗЕРНО.Тип.СодержитТип(ТипЗнч(Объект.Ссылка)) Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ПриОпределенииСправочниковХраненияФайлов(ТипВладелецФайла, ИменаСправочников) Экспорт
	...
	Если ОпределяемыеТипы().ДокументыЗЕРНО.Тип.СодержитТип(ТипВладелецФайла)
		Или ОпределяемыеТипы().Организация.Тип.СодержитТип(ТипВладелецФайла) Тогда
...
Процедура ПриОпределенииНастроекРаботыСФайлами(Настройки) Экспорт
	...
	Для Каждого Тип Из ОпределяемыеТипы().ДокументыЗЕРНО.Тип.Типы() Цикл
...
Функция НоваяТаблицаОрганизацияКонтрагентПодразделение() Экспорт
	...
	ТаблицаДанных.Колонки.Добавить("ОрганизацияКонтрагент", ОпределяемыеТипы().ОрганизацияКонтрагентГосИС.Тип);
	ТаблицаДанных.Колонки.Добавить("Подразделение",         ОпределяемыеТипы().Подразделение.Тип);
	...
	ТаблицаДанных.Колонки.Добавить("Организация",    ОпределяемыеТипы().Организация.Тип);
	ТаблицаДанных.Колонки.Добавить("Контрагент",     ОпределяемыеТипы().КонтрагентГосИС.Тип);
...
Процедура ЗаписатьСоответствиеНоменклатуры(Объект, ОписаниеТабличнойЧасти = "Товары") Экспорт
	...
	Если ОписаниеТабличнойЧасти = "Шапка" Тогда
		...
		Данные.Колонки.Добавить("Номенклатура", ОпределяемыеТипы().Номенклатура.Тип);
		Данные.Колонки.Добавить("Характеристика", ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
		Данные.Колонки.Добавить("Серия", ОпределяемыеТипы().СерияНоменклатуры.Тип);
	...
	Запрос.УстановитьПараметр("НезаполненныеЗначенияНоменклатура", ИнтеграцияИС.НезаполненныеЗначенияОпределяемогоТипа(ОпределяемыеТипы().Номенклатура));
	Запрос.УстановитьПараметр("ПустаяПартия", Справочники.РеестрПартийЗЕРНО.ПустаяСсылка());
	Запрос.УстановитьПараметр("ПустаяСерия", ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа(ОпределяемыеТипы().СерияНоменклатуры));
	Запрос.УстановитьПараметр("НезаполненныеЗначенияСерии", ИнтеграцияИС.НезаполненныеЗначенияОпределяемогоТипа(ОпределяемыеТипы().СерияНоменклатуры));
...
Функция ПараметрыОбмена(ЗначениеОтбора = Неопределено, УникальныйИдентификатор = Неопределено) Экспорт
	...
	ТаблицаСтопЛист.Колонки.Добавить("Организация", ОпределяемыеТипы().Организация.Тип);
...
Функция СписокАвтоподбора(ИсточникОписанияТипов, ТекстДляАвтоПодбора,
	...
	Если ТипЗнч(ИсточникОписанияТипов) = Тип("Строка") Тогда
		...
		ОпределяемыеТипы().Свойство(ИсточникОписанияТипов, МетаданныеТипа);
...
Функция ИнициализиоватьОчередьСообщений()
	...
	ОчередьСообщений.Колонки.Добавить("Подразделение",         ОпределяемыеТипы().Подразделение.Тип);
...
Функция СоответствиеКлючейОрганизацийИОрганизаций(Знач Организации)
	...
	СоответствиеКлючейОрганизацийИОрганизаций.Колонки.Добавить("Организация",   ОпределяемыеТипы().Организация.Тип);
	СоответствиеКлючейОрганизацийИОрганизаций.Колонки.Добавить("Подразделение", ОпределяемыеТипы().Подразделение.Тип);
...
Функция НоваяТаблицаОтбораСообщенийКПодписанию() Экспорт
	...
	ВозвращаемоеЗначение.Колонки.Добавить("Организация",            ОпределяемыеТипы().Организация.Тип);
	ВозвращаемоеЗначение.Колонки.Добавить("Подразделение",          ОпределяемыеТипы().Подразделение.Тип);
...
Процедура ЗаполнитьСлужебныеРеквизитыНоменклатуры(Форма, ТекущаяСтрока = Неопределено) Экспорт
	...
	ТаблицаЗаполнения.Колонки.Добавить("Номенклатура",     ОпределяемыеТипы().Номенклатура.Тип);
	ТаблицаЗаполнения.Колонки.Добавить("Характеристика",   ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
	ТаблицаЗаполнения.Колонки.Добавить("Серия",            ОпределяемыеТипы().СерияНоменклатуры.Тип);
	ТаблицаЗаполнения.Колонки.Добавить("ЕдиницаИзмерения", ОпределяемыеТипы().Упаковка.Тип);
	ТаблицаЗаполнения.Колонки.Добавить("ТипНоменклатуры",  ОпределяемыеТипы().ТипНоменклатуры.Тип);
...
Процедура НастроитьТипОрганизацияКонтрагент(Форма, ТипДанных, ИмяЭлемента, ОбновлятьПодсказкуВвода = Истина) Экспорт
	...
	Если ТипДанных = 0 Тогда
		ОписаниеТипа = ОпределяемыеТипы().Организация.Тип;
		Если ОбновлятьПодсказкуВвода Тогда
			ОбновитьПодсказкуВводаВладелецПартии(Форма, ИмяЭлемента);
		КонецЕсли;
	Иначе
		ОписаниеТипа = ОпределяемыеТипы().КонтрагентГосИС.Тип;
		Если ОбновлятьПодсказкуВвода Тогда
...
Функция ОпределитьТипОрганизацияКонтрагент(Значение) Экспорт
	...
	Если Значение = Неопределено Тогда
		Возврат 0;
	Иначе
		ОписаниеТиповОрганизация = ОпределяемыеТипы().Организация.Тип;
...
Функция ОпределитьТипСкладКонтрагент(Значение) Экспорт
	...
	Если Значение = Неопределено Тогда
		Возврат 0;
	Иначе
		ОписаниеТиповСклад = ОпределяемыеТипы().Склад.Тип;
