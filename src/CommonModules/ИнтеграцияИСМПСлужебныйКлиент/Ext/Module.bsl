...
Процедура ОбработатьРезультатОбменаСлужебный(РезультатОбмена, ДополнительныеПараметрыДлительнойОперации)
	...
	Иначе
		...
		Если НаборДанныхДляПодписанияПоОрганизациям.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение Из НаборДанныхДляПодписанияПоОрганизациям Цикл
				...
				Подписать(
					...
					Новый ОписаниеОповещения("ОбработатьСообщенияНаПолучениеВходящихДокументов_ПриЗавершенииОперацииПодписи", интеграцияисмпслужебныйклиент, Контекст));
		...
		ИначеЕсли ЗначениеЗаполнено(РезультатОбмена.КОпределениюВидаУпаковки) Тогда
			...
			ИнтеграцияИСМПКлиент.ОткрытьФормуНастройкиВидовУпаковокПоGTIN(ПараметрыОткрытия, интеграцияисмпслужебныйклиент, ВводКоэффициентаЗавершение);
...
Процедура Подписать(НаборДанныхДляПодписания, Организация, ДоступныеСертификаты, ОповещениеПриЗавершении)
	...
	ПодписатьЗавершение  = Новый ОписаниеОповещения("ПодписатьЗавершение", интеграцияисмпслужебныйклиент, ДанныеДляОповещения);
	...
	Если Контекст.НаборДанныхДляПодписанияПоОрганизациям.Количество() > 0 Тогда
		...
		Для Каждого КлючИЗначение Из Контекст.НаборДанныхДляПодписанияПоОрганизациям Цикл
			...
			Подписать(
				...
				Организация,
				ДоступныеСертификаты,
				Новый ОписаниеОповещения("ОбработатьСообщенияНаПолучениеВходящихДокументов_ПриЗавершенииОперацииПодписи", интеграцияисмпслужебныйклиент, НовыйКонтекст));
