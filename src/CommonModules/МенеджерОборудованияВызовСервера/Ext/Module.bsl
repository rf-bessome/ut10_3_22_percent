...
Функция ПолучитьПараметрыРегистрацииУстройства(Идентификатор) Экспорт
	...
	Возврат Справочники.ТорговоеОборудование.ПолучитьПараметрыРегистрацииУстройства(Идентификатор);
...
//
Функция ФискальноеУстройствоПоддерживаетПроверкуКодовМаркировки(ИдентификаторУстройства, ПараметрыККТ = Неопределено) Экспорт
	
	Результат = Ложь;                                                                                               
	
	Если ПараметрыККТ <> Неопределено Тогда
		Результат = ПараметрыККТ.Свойство("ВерсияФФДККТ") И ПараметрыККТ.ВерсияФФДККТ = "1.2"
			И ПараметрыККТ.Свойство("ВерсияФФДФН") И ПараметрыККТ.ВерсияФФДФН = "1.2";
		Возврат Результат;
	КонецЕсли;
...
Функция СохранитьПараметрыРегистрацииУстройства(Идентификатор, ПараметрыРегистрации) Экспорт
	...
	Возврат Справочники.ТорговоеОборудование.СохранитьПараметрыРегистрацииУстройства(Идентификатор, ПараметрыРегистрации);
...
Функция ДобавитьЧекВОчередьЧековККТ(ПараметрыЧекаККТ, ПараметрыПакетнойОперации = Неопределено) Экспорт
	...
	Попытка
		...
		Если ПустаяСтрока(ПараметрыЧекаККТ.ИдентификаторФискальнойЗаписи) Тогда
			ИдентификаторЗаписи = Строка(Новый УникальныйИдентификатор);
			ПараметрыЧекаККТ.ИдентификаторФискальнойЗаписи = ИдентификаторЗаписи;
		Иначе
...
Процедура ЗаписатьСтатусЧекаВОчереди(ПараметрыФискализации, СтатусЧека, ОборудованиеККТ = Неопределено, ТекстОшибки = Неопределено) Экспорт
	...
	Если СтатусЧека = Перечисления.СтатусЧекаККТВОчереди.Фискализирован Тогда
		...
		МенеджерОборудованияВызовСервераПереопределяемый.ФискализацияЧекаВОчереди(ИдентификаторФискальнойЗаписи, ПараметрыФискализации, ОборудованиеККТ, РезультатФискализации, РезультатВыполненияПакетнойОперации);
	КонецЕсли;
...
// Удаляет нефискализированные чеки из очереди чеков по документу
Процедура УдалитьНефискализированныеЧекиИзОчереди(ДокументСсылка) Экспорт
	
	НаборЗаписейОчередьЧеков = РегистрыСведений.ОчередьЧековККТ.СоздатьНаборЗаписей();
	НаборЗаписейОчередьЧеков.Отбор.ДокументОснование.Установить(ДокументСсылка);
	НаборЗаписейОчередьЧеков.Прочитать();
	
	ЗаписиКУдалению = Новый Массив;
	Для Каждого Запись Из НаборЗаписейОчередьЧеков Цикл
	...
	Для Каждого Запись Из ЗаписиКУдалению Цикл
		НаборЗаписейОчередьЧеков.Удалить(Запись);
	КонецЦикла;
	
	НаборЗаписейОчередьЧеков.Записать(Истина);
...
Функция ПодготовитьДанныеФискализацииЧека(ОбщиеПараметры, ПараметрыПодключения) Экспорт
	...
	ПараметрыФискализации.СубъектПерсональныхДанных = СтруктураОбщихПараметров.СубъектПерсональныхДанных;
	ПараметрыФискализации.ЕстьПерсональныеДанные    = СтруктураОбщихПараметров.ЕстьПерсональныеДанные;
	ПараметрыФискализации.ТипПерсональныхДанных     = СтруктураОбщихПараметров.ТипПерсональныхДанных;
...
Функция ПодготовитьДанныеИнкасации(ОбщиеПараметры, ПараметрыПодключения) Экспорт
	...
	Если ПараметрыПодключения.Свойство("ПараметрыРегистрации") Тогда
		ПараметрыРегистрации = ПараметрыПодключения.ПараметрыРегистрации;
		ПараметрыФискализации.РегистрационныйНомерККТ = ?(ПараметрыРегистрации.Свойство("РегистрационныйНомерККТ"), ПараметрыРегистрации.РегистрационныйНомерККТ, "");
		ПараметрыФискализации.ЗаводскойНомерФН  = ?(ПараметрыРегистрации.Свойство("ЗаводскойНомерФН"), ПараметрыРегистрации.ЗаводскойНомерФН, "");
		ПараметрыФискализации.ШиринаСтроки      = ?(ПараметрыПодключения.Свойство("ШиринаСтроки"), ПараметрыПодключения.ШиринаСтроки, 32);
...
//
// Возвращаемое значение: 
//  Результат - Массив - Массив Перечисления.ТипыИдентификаторовТовараККТ.
//
...
Процедура ЗаписатьДанныеКодаТоварнойНоменклатуры(ЗаписьXML, Позиция, ПараметрыФискализации)
	...
	Если ПараметрыФискализации.РевизияИнтерфейса > 3001 Тогда 
		...
		Если ПараметрыФискализации.РевизияИнтерфейса > 3003 Тогда // 3.4 и выше
			...
			КонецЕсли; 
			Если ДанныеКодаТовара.ТипИдентификатораТовара = Перечисления.ТипыИдентификаторовТовараККТ.КодТовараВФорматеDataMatrixGS1 И ДанныеКодаТовара.ТехническийКод Тогда
				ОборудованиеЧекопечатающиеУстройстваВызовСервера.ЗаполнитьОтраслевойРеквизитМолочнаяПродукцияСНечитаемымиКМ(Позиция);
			КонецЕсли;
		Иначе
			// ФФД 1.1
			ТипыТоваров = ОтраслевойРеквизитЗаполняетсяДляТиповТоваров(); 
...
Процедура ЗаписатьСведенияОПокупателе(ЗаписьXML, ОбщиеПараметры, ПерсональныеДанные = Неопределено, ТипПерсональныхДанных = Неопределено);
	...
	Если Не ПустаяСтрока(Покупатель) И Не ПустаяСтрока(ПокупательИНН) Тогда
...
Процедура ПодготовитьДанныеДляФискализацииЧека(Ссылка, ОбщиеПараметры, ВерсияФормата, ТО, КомуОтКого, Основание) Экспорт
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ВозвратТоваровОтПокупателя") ИЛИ 
		...
		ВидОперации = Перечисления.ВидыОперацийОплатаОтПокупателяПлатежнойКартой.ВозвратДенежныхСредствПокупателю ИЛИ 
		ВидОперации = Перечисления.ВидыОперацийППИсходящее.ВозвратДенежныхСредствПокупателю Тогда
		ИмяТипаРасчета = "ВозвратДенежныхСредств";
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.РасходныйКассовыйОрдер")
		ИЛИ ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее") Тогда
	...
	Иначе
		...
		Если Ссылка.ВалютаДокумента = глЗначениеПеременной("ВалютаРегламентированногоУчета") Тогда
			КурсОплаты = 1;
		Иначе
			СтруктураКурса = МодульВалютногоУчета.ПолучитьКурсВалюты(Ссылка.ВалютаДокумента, Ссылка.Дата);
			КурсОплаты     = СтруктураКурса.Курс / СтруктураКурса.Кратность;
		КонецЕсли;

		ЗаполнитьОплатыДляПлатежногоДокумента(Ссылка, ОбщиеПараметры, КурсОплаты);
...
Процедура ЗаполнитьОплатыДляТоварногоДокумента(Ссылка, ОбщиеПараметры, КурсОплаты)
	...
	Если ДокументыРасчетов.Количество() = 0 Тогда
		...
		СуммаОплаты = Ссылка.СуммаДокумента * КурсОплаты;
	...
	Иначе
		...
		Если СуммаОплаты >= Ссылка.СуммаДокумента * КурсОплаты Тогда
			...
			СтрокаОплаты.Вставить("Сумма", Ссылка.СуммаДокумента * КурсОплаты);
			ОбщиеПараметры.ТаблицаОплат.Добавить(СтрокаОплаты);
		...
		Иначе
			...
			СтрокаОплаты.Вставить("ТипОплаты", Перечисления.ТипыОплатыККТ.Постоплата);
			СтрокаОплаты.Вставить("Наименование", НСтр("ru = 'Постоплата'"));
			СтрокаОплаты.Вставить("Сумма", Ссылка.СуммаДокумента * КурсОплаты - СуммаОплаты);
			ОбщиеПараметры.ТаблицаОплат.Добавить(СтрокаОплаты);
...
Процедура ЗаполнитьТоварныеПозицииДляТоварногоДокумента(Ссылка, ОбщиеПараметры, КурсОплаты)
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаЧека.НомерТаможеннойДекларации = ?(ЗначениеЗаполнено(Выборка.НомерГТД), Выборка.НомерГТД.РегистрационныйНомер, "");
		СтрокаЧека.КодСтраныПроисхожденияТовара = ?(ЗначениеЗаполнено(Выборка.СтранаПроисхождения), СокрЛП(Выборка.СтранаПроисхождения.Код), "");
...
Процедура ЗаполнитьОплатыДляПлатежногоДокумента(Ссылка, ОбщиеПараметры, КурсОплаты)
	
	СуммаОплаты = Ссылка.СуммаДокумента * КурсОплаты;
	Если СуммаОплаты <> 0 И СуммаОплаты <> Неопределено Тогда
		СтрокаОплаты = Новый Структура();
		Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ОплатаОтПокупателяПлатежнойКартой") Или ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее") Или 
			ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПлатежноеПоручениеИсходящее") Тогда
			СтрокаОплаты.Вставить("ТипОплаты", ПредопределенноеЗначение("Перечисление.ТипыОплатыККТ.Электронно"));
			СтрокаОплаты.Вставить("Наименование", ?(ТипЗнч(Ссылка) = Тип("ДокументСсылка.ОплатаОтПокупателяПлатежнойКартой"), "Платежная карта", "Безналичная оплата"));
		Иначе
			...
			Иначе
				...
				Если СуммаТовара <> 0 Тогда
					Если СтрокаСтавки.СуммаПлатежа > СуммаТовара Тогда
						...
						ЗаполнитьФиктивнуюСтрокуЧека(ОбщиеПараметры, СтрокаПозиции, НаименованиеПредметаРасчета, 
							СтрокаСтавки.СуммаПлатежа - СуммаТовара, СтрокаСтавки.СуммаНДС - СтрокаСтавки.СуммаНДСТовара);
...
Процедура ЗаполнитьТоварнуюСтрокуЧека(ОбщиеПараметры, СтрокаПозиции, КурсОплаты)
	...
	Если ЗначениеЗаполнено(СтрокаПозиции.Серия) Тогда
		СтрокаЧека.НомерТаможеннойДекларации = ?(ЗначениеЗаполнено(СтрокаПозиции.Серия.НомерГТД), СтрокаПозиции.Серия.НомерГТД.РегистрационныйНомер, "");
		СтрокаЧека.КодСтраныПроисхожденияТовара = ?(ЗначениеЗаполнено(СтрокаПозиции.Серия.СтранаПроисхождения), СокрЛП(СтрокаПозиции.Серия.СтранаПроисхождения.Код), "");
	КонецЕсли;
...
Процедура ЗаполнитьФиктивнуюСтрокуЧека(ОбщиеПараметры, СтрокаРасшифровки, НаименованиеПредметаРасчета, Сумма = Неопределено, СуммаНДС = Неопределено)
...
// Получить таблицу параметров описания из Таблицы XML.
//
// Возвращаемое значение
//  Структура
//
Функция ПараметрыИзXMLПакетаОписанияДрайвера(Данные) Экспорт
	...
	Если ЧтениеXML.Имя = "DriverDescription" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
		Параметры.Наименование  					= СокрЛП(ЧтениеXML.ЗначениеАтрибута("Name"));
		Параметры.Описание      					= СокрЛП(ЧтениеXML.ЗначениеАтрибута("Description"));
		Параметры.ИнтеграционнаяКомпонента			= ЧтениеXML.ЗначениеАтрибута("IntegrationComponent");
		Параметры.ДрайверУстановлен         		= ЧтениеXML.ЗначениеАтрибута("MainDriverInstalled");
		Параметры.ВерсияДрайвера            		= ЧтениеXML.ЗначениеАтрибута("DriverVersion");
		Параметры.ВерсияИнтеграционнойКомпоненты	= ЧтениеXML.ЗначениеАтрибута("IntegrationComponentVersion");
		Параметры.АдресЗагрузки				       	= СокрЛП(ЧтениеXML.ЗначениеАтрибута("DownloadURL"));
		Параметры.ЛогированиеВключено			    = ЧтениеXML.ЗначениеАтрибута("LogIsEnabled");
		Параметры.ПутьКФайламЛогирования 			= СокрЛП(ЧтениеXML.ЗначениеАтрибута("LogPath"));
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ПараметрыФискализации.РезультатОперацииXML) Тогда
		...
		Если ЧтениеXML.Имя = "DocumentOutputParameters" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			Если ЧтениеXML.Прочитать() И ЧтениеXML.Имя = "Parameters" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
				...
				КонецЕсли;                                                                                       
				ПараметрыФискализации.НомерУведомленияРеализацииМТ = ЧтениеXML.ЗначениеАтрибута("MTNumber"); 
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция МакетыДляПереустановкиДрайверов(ИдентификаторКлиента)
	...
	МассивРабочихМест = НайтиРабочиеМестаПоИД(ИдентификаторКлиента);
...
Функция ОпределитьПризнакПредметаРасчета(Номенклатура) Экспорт
	...
	Если ТипЗнч(Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "АлкогольнаяПродукция, Услуга, ВидПродукцииИС, ПодакцизныйТовар");
	Иначе
		РеквизитыНоменклатуры = Новый Структура("АлкогольнаяПродукция, Услуга, ВидПродукцииИС, ПодакцизныйТовар", Ложь, Ложь, ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.ПустаяСсылка"), Ложь);
	КонецЕсли;
	
	Если РеквизитыНоменклатуры.АлкогольнаяПродукция Или РеквизитыНоменклатуры.ВидПродукцииИС = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.Табак")
		Или РеквизитыНоменклатуры.ПодакцизныйТовар Тогда
...
Функция ЗапросДанныеЧека(Ссылка, ВалютаРеглУчета)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |				ТОГДА 20
				   |			КОГДА ПДРП.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
	               |				ТОГДА 22
				   |			КОГДА ПДРП.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
	               |				ТОГДА 5
	               |			КОГДА ПДРП.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	               |				ТОГДА 7
	               |			ИНАЧЕ 0
...
Функция ПолучитьXMLПакетДляФискализацияЧека_ТО(ОбщиеПараметры, ВыходныеПараметры, СуммаЧека, ШиринаСтроки = 32, НовыйФормат = Ложь, ВерсияФФД = 0, ПараметрыФискализации) Экспорт
				...
				Если ТекущаяПозиция.Свойство("ФискальнаяСтрока") Тогда
					...
					Если НЕ ПустаяСтрока(ТекущаяПозиция.СтавкаНДС) Тогда 
						...
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 120 Тогда
							...
							//ТЕАЛ НДС ++
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 105 Тогда
							ЗначениеСтавкаНДС = "5/105";
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 107 Тогда
							ЗначениеСтавкаНДС = "7/107";
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 122 Тогда
							ЗначениеСтавкаНДС = "22/122";	
							//ТЕАЛ НДС --
						Иначе
...
//
Функция ПолучитьXMLПакетДляФискализацияЧека(ОбщиеПараметры, ВыходныеПараметры, СуммаЧека, ШиринаСтроки = 32, НовыйФормат = Ложь, ПараметрыФискализации) Экспорт
		...
		Если ОбщиеПараметры.ПозицииЧека <> Неопределено Тогда
			...
			Для ИндексМассива = 0 По ОбщиеПараметры.ПозицииЧека.Количество() - 1 Цикл
				...
				Если ТекущаяПозиция.Свойство("ФискальнаяСтрока") Тогда
						...
						ЗаписатьДанныеАгента(ЗаписьXML, ТекущаяПозиция.ДанныеАгента);
						ЗаписатьДанныеПоставщика(ЗаписьXML, ТекущаяПозиция.ДанныеПоставщика);
						ЗаписатьДанныеКодаТоварнойНоменклатуры(ЗаписьXML, ТекущаяПозиция.ДанныеКодаТоварнойНоменклатуры, ПараметрыФискализации);
...
Функция ПолучитьXMLПакетДляЧекаКоррекции_ТО(ОбщиеПараметры, ВыходныеПараметры, НовыйФормат = Ложь, ВидовОплаты = 0, ВерсияФФД = 0) Экспорт
	...
	Если НовыйФормат Тогда
		...
		//ТЕАЛ НДС ++
		ЗаписьXML.ЗаписатьАтрибут("SumTAX5"  , XMLСтрока(ОбщиеПараметры.СуммаНДС5)); 
		ЗаписьXML.ЗаписатьАтрибут("SumTAX7"  , XMLСтрока(ОбщиеПараметры.СуммаНДС7));
		ЗаписьXML.ЗаписатьАтрибут("SumTAX105" , XMLСтрока(ОбщиеПараметры.СуммаНДС105));
		ЗаписьXML.ЗаписатьАтрибут("SumTAX107" , XMLСтрока(ОбщиеПараметры.СуммаНДС107));
		ЗаписьXML.ЗаписатьАтрибут("SumTAX22"  , XMLСтрока(ОбщиеПараметры.СуммаНДС22)); 
		ЗаписьXML.ЗаписатьАтрибут("SumTAX122" , XMLСтрока(ОбщиеПараметры.СуммаНДС122));
		//ТЕАЛ НДС --
				...
				Если ТекущаяПозиция.Свойство("ФискальнаяСтрока") Тогда // Фискальная строка
					...
					Если НЕ ПустаяСтрока(ТекущаяПозиция.СтавкаНДС) Тогда 
						...
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 120 Тогда
							ЗначениеСтавкаНДС = "20/120"; 
							//ТЕАЛ НДС ++
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 0 Тогда
							ЗначениеСтавкаНДС = "none";
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 105 Тогда
							ЗначениеСтавкаНДС = "5/105";
						ИначеЕсли ТекущаяПозиция.СтавкаНДС = 107 Тогда
					...
					Если ТекущаяПозиция.Свойство("СуммаАкциза") И ТекущаяПозиция.СуммаАкциза <> Неопределено Тогда
						Если Число(ТекущаяПозиция.СуммаАкциза) > 0 Или ЗначениеЗаполнено(ОбщиеПараметры.ПолучательИНН) 
							Или ТекущаяПозиция.ПризнакПредметаРасчета = Перечисления.ПризнакиПредметаРасчета.ПодакцизныйТовар Тогда 
...
Функция ЗагрузитьДанныеФискализацииИзXML(ДанныеXML) Экспорт
	...
	СтавкиНДС.Вставить("5/105", 105);
	СтавкиНДС.Вставить("7/107", 107);
	СтавкиНДС.Вставить("22", 22);
	СтавкиНДС.Вставить("22/122", 122);
	//ТЕАЛ НДС --
		...
		Если ЧтениеXML.Имя = "CheckPackage" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Пока ЧтениеXML.Прочитать() Цикл
				Если ЧтениеXML.Имя = "Parameters" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					...
					АтрибутTaxVariant = ЧтениеXML.ЗначениеАтрибута("TaxVariant");
					СистемаНалогообложения = ?(АтрибутTaxVariant <> Неопределено, ?(АтрибутTaxVariant = "", 0, АтрибутTaxVariant), ЧтениеXML.ЗначениеАтрибута("TaxationSystem"));
					...
					Пока ЧтениеXML.Прочитать() И НЕ (ЧтениеXML.Имя = "Positions" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента) Цикл
						...
						Если ЧтениеXML.Имя = "FiscalString" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
							...
							Если НЕ ПустаяСтрока(СуммаАкциза) Тогда
								Если Число(СуммаАкциза) > 0
									Или (ЗначениеЗаполнено(ФискальнаяСтрока.ПризнакПредметаРасчета) 
									И ФискальнаяСтрока.ПризнакПредметаРасчета = Перечисления.ПризнакиПредметаРасчета.ПодакцизныйТовар) Тогда 
		...
		ИначеЕсли ЧтениеXML.Имя = "CheckCorrectionPackage" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			Пока ЧтениеXML.Прочитать() Цикл
				Если ЧтениеXML.Имя = "Parameters" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда  
					...
					//ТЕАЛ НДС ++
					СуммаНДС5  = Число(ЗначениеXMLАтрибута(ЧтениеXML, "SumTAX5"  , , 0));
					СуммаНДС7  = Число(ЗначениеXMLАтрибута(ЧтениеXML, "SumTAX7"  , , 0));
					СуммаНДС105 = Число(ЗначениеXMLАтрибута(ЧтениеXML, "SumTAX105" , , 0));
					СуммаНДС107 = Число(ЗначениеXMLАтрибута(ЧтениеXML, "SumTAX107" , , 0));
					СуммаНДС22  = Число(ЗначениеXMLАтрибута(ЧтениеXML, "SumTAX22"  , , 0));
					СуммаНДС122 = Число(ЗначениеXMLАтрибута(ЧтениеXML, "SumTAX122" , , 0)); 
					
					Если СуммаНДС5 > 0 Тогда
						ФискальнаяСтрока = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека();
						ФискальнаяСтрока.Наименование = НСтр("ru='Фискальная позиция НДС 5%'");
						ФискальнаяСтрока.Количество   = 1;
						ФискальнаяСтрока.ЦенаСоСкидками = Мин(СуммаНДС5 * 105 / 5, СуммаДокумента);
						ФискальнаяСтрока.Сумма          = ФискальнаяСтрока.ЦенаСоСкидками;
						ФискальнаяСтрока.СтавкаНДС      = 5;
						ОбщиеПараметры.ПозицииЧека.Добавить(ФискальнаяСтрока);
					КонецЕсли;  
					
					Если СуммаНДС7 > 0 Тогда
						ФискальнаяСтрока = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека();
						ФискальнаяСтрока.Наименование = НСтр("ru='Фискальная позиция НДС 7%'");
						ФискальнаяСтрока.Количество   = 1;
						ФискальнаяСтрока.ЦенаСоСкидками = Мин(СуммаНДС7 * 107 / 7, СуммаДокумента);
						ФискальнаяСтрока.Сумма          = ФискальнаяСтрока.ЦенаСоСкидками;
						ФискальнаяСтрока.СтавкаНДС      = 7;
						ОбщиеПараметры.ПозицииЧека.Добавить(ФискальнаяСтрока);
					КонецЕсли;
					
					Если СуммаНДС105 > 0 Тогда
						ФискальнаяСтрока = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека();
						ФискальнаяСтрока.Наименование = НСтр("ru='Фискальная позиция НДС 5/105'");
						ФискальнаяСтрока.Количество   = 1;
						ФискальнаяСтрока.ЦенаСоСкидками = Мин(СуммаНДС105 * 105 / 5, СуммаДокумента);
						ФискальнаяСтрока.Сумма          = ФискальнаяСтрока.ЦенаСоСкидками;
						ФискальнаяСтрока.СтавкаНДС      = 105;
						ОбщиеПараметры.ПозицииЧека.Добавить(ФискальнаяСтрока);
					КонецЕсли;

					Если СуммаНДС107 > 0 Тогда
						ФискальнаяСтрока = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека();
						ФискальнаяСтрока.Наименование = НСтр("ru='Фискальная позиция НДС 7/107'");
						ФискальнаяСтрока.Количество   = 1;
						ФискальнаяСтрока.ЦенаСоСкидками = Мин(СуммаНДС107 * 107 / 7, СуммаДокумента);
						ФискальнаяСтрока.Сумма          = ФискальнаяСтрока.ЦенаСоСкидками;
						ФискальнаяСтрока.СтавкаНДС      = 107;
						ОбщиеПараметры.ПозицииЧека.Добавить(ФискальнаяСтрока);
					КонецЕсли;	
					
					Если СуммаНДС22 > 0 Тогда
						ФискальнаяСтрока = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека();
