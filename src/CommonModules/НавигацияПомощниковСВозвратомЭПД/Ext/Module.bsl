...
Процедура РазместитьКартинки(Форма, СтруктураНавигацииПомощника, Сдвиг = 0, ПостфиксЭлемента = "", ОтмечатьТекущий = Ложь)
	...
	Для НомерШага = 1 По СтруктураНавигацииПомощника.Количество() Цикл
		...
		Если Форма.ИмяФормы = "Документ.ЭлектроннаяСопроводительнаяВедомость.Форма.ОсновнаяФорма"
			ИЛИ Форма.ИмяФормы = "Документ.ЭлектронныйЗаказНаряд.Форма.ОсновнаяФорма"
			ИЛИ Форма.ИмяФормы = "Документ.ЭлектронныйЗаказЗаявка.Форма.ОсновнаяФорма" Тогда
			
			ИмяЛинии = ИмяЛинии + "Широкий";
		КонецЕсли;
		...
		КонецЕсли;
		МассивФорматированныхСтрок.Добавить(ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(СтруктураКартинокНавигации[ИмяКартинкиШага],,,,ИмяСсылки));
		
	КонецЦикла;
	
	ЭлементНавигацииКартинка = Форма.Элементы.Найти("НавигацияКартинка" + ПостфиксЭлемента);
	
	ЭлементНавигацииКартинка.Заголовок = ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(МассивФорматированныхСтрок);
...
Процедура РазместитьТексты(Форма, СтруктураНавигацииПомощника, Сдвиг = 0, ПостфиксЭлемента = "", ОтмечатьТекущий = Истина)
	...
	Для НомерШага = 1 По КоличествоШагов Цикл
		...
		ШрифтТекста = ?(ОтмечатьТекущий = Истина И НомерШагаСоСдвигом = ТекущийНомерШага, ШрифтыСтиля.Шрифт12EDI, Неопределено);
		//ШрифтТекста = ?(ОтмечатьТекущий = Истина И НомерШагаСоСдвигом = ТекущийНомерШага, ШрифтыСтиля.Шрифт12EDI, ШрифтыСтиля.Шрифт12EDI);
		Если Форма.ИмяФормы = "Документ.ЭлектроннаяСопроводительнаяВедомость.Форма.ОсновнаяФорма"
			ИЛИ Форма.ИмяФормы = "Документ.ЭлектронныйЗаказНаряд.Форма.ОсновнаяФорма"
			ИЛИ Форма.ИмяФормы = "Документ.ЭлектронныйЗаказЗаявка.Форма.ОсновнаяФорма"
			ИЛИ Форма.ИмяФормы = "Документ.ЭлектроннаяТранспортнаяНакладная.Форма.ОсновнаяФорма" Тогда
			
			ШрифтТекста = Неопределено;
		КонецЕсли;
		...
		МассивФорматированныхСтрок = Новый Массив;
		МассивФорматированныхСтрок.Добавить(ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(СтруктураШага.ТекстНавигации, ШрифтТекста, ЦветТекста, , ИмяСсылки));
		...
		Если СтруктураШага.КоличествоВерсий > 1 Тогда
			...
			Если ОтмечатьТекущий = Истина И НомерШагаСоСдвигом = ТекущийНомерШага Тогда
				...
				Иначе
					МассивФорматированныхСтрок.Добавить(ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(БиблиотекаКартинок.НавигацияСтрелкаВлевоАктивнаяБЭД,,,,"Влево"));
				КонецЕсли;
				МассивФорматированныхСтрок.Добавить(ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(
				...
				Иначе
					МассивФорматированныхСтрок.Добавить(ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(БиблиотекаКартинок.НавигацияСтрелкаВправоАктивнаяБЭД,,,,"Вправо"));
				КонецЕсли;
			Иначе
				МассивФорматированныхСтрок.Добавить(ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока("(" + Строка(СтруктураШага.КоличествоВерсий) + ")", ШрифтТекста, ЦветТекста));
			КонецЕсли;         
			ЭлементФормы.Высота = Мин(4, ЭлементФормы.Высота + 2);
		КонецЕсли;
			
		ЭлементФормы.Заголовок = ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(МассивФорматированныхСтрок);
...
Функция СтруктураКартинокНавигации()
	...
	СтруктураКартинокНавигации.Вставить("ЛинияШагВыполненныйШирокий",          БиблиотекаКартинок.НавигацияЛинияШагВыполненныйБЭДШирокий);
	СтруктураКартинокНавигации.Вставить("ЛинияШагНеВыполненныйШирокий",        БиблиотекаКартинок.НавигацияЛинияШагНеВыполненныйБЭДШирокий);
	СтруктураКартинокНавигации.Вставить("ЛинияШагВыполненныйТекущийШирокий",   БиблиотекаКартинок.НавигацияЛинияШагВыполненныйТекущийБЭДШирокий);
	СтруктураКартинокНавигации.Вставить("ЛинияШагНеВыполненныйТекущийШирокий", БиблиотекаКартинок.НавигацияЛинияШагНеВыполненныйТекущийБЭДШирокий);
