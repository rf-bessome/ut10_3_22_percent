...
Процедура ОповеститьОбИзмененииСпискаВыбораЕдиницИзмерения(КоличествоНовыхЭлементов, МассивЭлементов) Экспорт
	...
	Иначе
		...
		СклоняемоеВыражение = ОбщегоНазначения.ФормаМножественногоЧисла(НСтр("ru='новая единица измерения'"),
...
// Конец области интерактивных действий
...
Функция ДляУказанияСерийНуженСерверныйВызов(Форма, ПараметрыУказанияСерий, Текст, ТекущиеДанные = Неопределено, ОпределятьРаспоряжение = Истина, ВыдаватьСообщения = Истина) Экспорт
	...
	Иначе
		...
		ЭтоОперацияИдентификации = Ложь;
		
		Если ТекущиеДанные.ХарактеристикиИспользуются
		...
		ИначеЕсли НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(ТекущиеДанные.СтатусУказанияСерий, ПараметрыУказанияСерий)
			И ТекущиеДанные.Свойство("Серия") Тогда
...
Функция НеобходимоОбновитьСтатусыСерий(ТаблицаФормы,КэшированныеЗначения,ПараметрыУказанияСерий, Удаление = Ложь) Экспорт
	...
	Если Не НоменклатураСервер.СтруктурыРавны(КэшированныеЗначения,ТекущиеДанные,
...
//
// Возвращаемое значение:
//  Булево - подтвердил или нет пользователь.
//
Функция ЗадатьВопросОПерезаполненииСерийПоFEFO() Экспорт 
	...
	Возврат ЗадатьВопросОПерезаполненииСерийПоFEFOЗавершение(Вопрос(ТекстВопроса, РежимДиалогаВопрос.ОКОтмена, , КодВозвратаДиалога.ОК));
		
КонецФункции

// Функция выполняет обработку оповещания после вопроса о перезаполнении серий по FEFO.
//
// Параметры:
...
//  Булево - подтвердил или нет пользователь.
//
Функция ЗадатьВопросОПерезаполненииСерийПоFEFOЗавершение(РезультатВопроса) Экспорт
	...
	Если ТекущиеДанные = Неопределено Тогда 
		Если Не ПараметрыУказанияСерий.ТоварВШапке Тогда
			...
			Если ТекущиеДанные = Неопределено Тогда
				Возврат;
			КонецЕсли;
		ИначеЕсли Форма.ИмяФормы = "Справочник.РеестрПартийЗЕРНО.Форма.ФормаЭлемента" Тогда
			ТекущиеДанные = Форма;
		Иначе
...
Функция ПараметрыРасшифровкиОтчетВедомостьПоСериямНоменклатуры() Экспорт
	...
	// Движение серии номенклатуры контекст
	...
	// Конец области движения серии номенклатуры контекст
...
Функция ЭтоУказаниеСерий(ИсточникВыбора) Экспорт
	...
	Если ТипЗнч(ИсточникВыбора) = Тип("Форма")
		...
		И ТипЗнч(ИсточникВыбора.СправочникСписок) = Тип("СправочникСписок.СерииНоменклатуры") Тогда
		ЭтоВыборСерии = Истина;
	ИначеЕсли ТипЗнч(ИсточникВыбора) = Тип("ФормаКлиентскогоПриложения")
		И (СтрНачинаетсяС(ИсточникВыбора.ИмяФормы, "Справочник.СерииНоменклатуры")
		ИЛИ ИсточникВыбора.ИмяФормы = "Обработка.ПодборСерийВДокументы.Форма.УказаниеСерииВСтрокеТоваров") Тогда
		ЭтоВыборСерии = Истина;
	Иначе
		ЭтоВыборСерии = Ложь;
	КонецЕсли;
	
	Возврат ЭтоВыборСерии;
