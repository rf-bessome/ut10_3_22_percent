		...
		Если ПравилоКонвертации.ЕстьОбработчикПриОтправкеДанных Тогда
			...
			Для Каждого ПКС Из ПравилоКонвертации.Свойства Цикл
				...
				Иначе
					...
					ВложенныеСвойства = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СвойствоФормата_Имя,".",Ложь);
		...
		Если ЭтоПолнаяЗагрузкаОбъекта
			...
			Если ДанныеИБ <> Неопределено Тогда
				...
				Если СтрокаОбъекта <> Неопределено Тогда
					УстановитьОбменДаннымиЗагрузка(ДанныеИБ, Истина, Ложь, КомпонентыОбмена.УзелКорреспондента);
			...
			Иначе
				...
				Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
					...
					Если ТекущееПКО.Значение Тогда
						...
						СтрокаТаблицы.ПоляТаблицы = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(ПравилоКонвертации.РеквизитыШапкиПолученныхДанных, ",");
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Для Каждого ВерсияФормата Из ВерсииФорматаОбменаМассив() Цикл
...
Процедура ОткрытьФайлЗагрузки(КомпонентыОбмена, ИмяФайлаОбмена) Экспорт
	...
	Пока НомерИтерации = 0 Цикл
		...
		КомпонентыОбмена.МенеджерОбмена      = ОбменДаннымиПереопределяемый.МенеджерУниверсальногоФорматаОбмена();
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			ФорматПланаОбмена = ОбменДаннымиПереопределяемый.ПространствоИменУниверсальногоФорматаОбмена();
			...
			Если ВерсииФорматаОбменаМассив().Найти(ФорматОбмена.Версия) = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ДоступныеВерсииПолучателя  = ВерсииФорматаОбменаМассив();
			...
			// Проверяем, чтобы максимальная доступная версия не превышала версии корреспондента.
			Если СравнитьВерсии(МаксимальнаяОбщаяВерсия, НомерВерсииКорреспондента) > 0 Тогда
...
Процедура ВыполнитьПроведениеДокументаПриЗагрузке(
	...
	Если Не ДокументПроведенУспешно Тогда
		...
		ОбменДаннымиСервер.ЗарегистрироватьОшибкуПроведенияДокумента(
			Объект, УзелКорреспондента, ОписаниеОшибки);
...
////////////////////////////////////////////////////////////////////////////////
// Процедуры и функции версионирования формата обмена
...
Функция ФорматОбмена(Знач ИмяПланаОбмена, Знач ВерсияФормата) Экспорт
	ЭлементыФормата = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(
					ОбменДаннымиПереопределяемый.ПространствоИменУниверсальногоФорматаОбмена(), "/");
	ЭлементыФормата.Добавить(ВерсияФормата);
	
	Возврат СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(ЭлементыФормата, "/");
КонецФункции
...
//
Функция ВерсииФорматаОбменаМассив() Экспорт
	
	Возврат СортироватьВерсииФормата(ВерсииФорматаОбмена());
...
Функция СоздатьТЧПриемникаПоПКС(ПКСДляТЧ)
	...
	Для Каждого ПКС Из ПКСДляТЧ Цикл
		...
		Если Найти(ИмяКолонки, ".") > 0 Тогда
			ВложенныеСвойства = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИмяКолонки,".",Ложь);
...
////////////////////////////////////////////////////////////////////////////////
// Конвертация структуры в данные ИБ
	...
	Попытка
		...
		Для Каждого ТЧ Из ПравилоКонвертации.СвойстваТабличныхЧастей Цикл
			...
			Если НомерЭтапа = 1 И ЗначениеЗаполнено(ТЧ.ТЧКонфигурации) И ЗначениеЗаполнено(ТЧ.ТЧФормата) Тогда
				...
				ИменаКолонокСтрокой = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(МассивКолонокТЧ, ",");
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Попытка
		Если НомерЭтапа = 1 Тогда
			...
			СвойствоФормата_Имя = СокрЛП(ПКС.СвойствоФормата);
			ПозицияТочки = Найти(СвойствоФормата_Имя, ".");
			// Указано полное имя свойства, входящего в группу общих свойств.
...
Функция СсылкаОбъектаПоСвойствамОбъектаXDTO(ПравилоКонвертации, ПолученныеДанные, ДанныеXDTOСодержатСсылку, УзелОбмена)
	...
	Для Каждого ПопыткаПоиска Из ПравилоКонвертации.ПоляПоиска Цикл
		...
		Если ПоискЗапросом Тогда
			...
			УсловиеОтбора = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(Отбор, " И ");
...
Процедура ЗаполнитьДанныеИБПоПолученнымДанным(ДанныеИБ, ПолученныеДанные, ПравилоКонвертации)
	...
	Если ПоляКопии.Количество() > 0 Тогда
		...
		ПоляКопии = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(ПоляКопии, ",");
...
Процедура УдалитьВременныеОбъектыСозданныеПоСсылкам(КомпонентыОбмена) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОбъектовСозданныхПоСсылкам Цикл
		...
		Если ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
			...
			УстановитьОбменДаннымиЗагрузка(ОбъектСозданныйПоСсылке, Истина, Ложь, КомпонентыОбмена.УзелКорреспондента);
...
Функция РазложитьФорматОбмена(Знач ФорматОбмена)
	...
	Результат.БазовыйФормат = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(ЭлементыФормата, "/");
	...
	Попытка
		...
		Для Каждого Движение Из Объект.Движения Цикл
			...
			Если ЕстьИзменения Тогда
				...
				УстановитьОбменДаннымиЗагрузка(Движение, Истина, Ложь, Отправитель);
				Движение.Записать();
			КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// Обработчики событий правил обработки данных
...
////////////////////////////////////////////////////////////////////////////////
// Обработчики событий правил конвертации
...
Функция МинимальнаяВерсияФорматаОбмена(Знач УзелИнформационнойБазы)
	...
	ВерсииФормата = ВерсииФорматаОбменаМассив();
	
	Для Каждого ВерсияФормата Из ВерсииФормата Цикл
		
		Если Результат = Неопределено Тогда
			Результат = ВерсияФормата;
			Продолжить;
		КонецЕсли;
		Если СравнитьВерсии(СокрЛП(Результат), СокрЛП(ВерсияФормата)) > 0 Тогда
			Результат = ВерсияФормата;
		КонецЕсли;
...
//
Процедура УстановитьОбменДаннымиЗагрузка(Объект, Значение = Истина, ОтправкаНазад = Ложь, УзелОбмена = Неопределено)
