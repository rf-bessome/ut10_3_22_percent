...
Функция ТекстВопросаОПоддержкеОператоромЭПД(ТипДокумента, ПриСозданииДокумента = Ложь) Экспорт
	...
	МассивФорматированныхСтрок.Добавить(" ");
	МассивФорматированныхСтрок.Добавить(
			ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока("Постановлением", , , , 
	...
	Если ПриСозданииДокумента = Истина Тогда
		...
		МассивФорматированныхСтрок.Добавить(" ");
		Если ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыЭД.ЭТрН") Тогда
			МассивФорматированныхСтрок.Добавить(НСтр("ru='электронной транспортной накладной'"));	
		ИначеЕсли ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыЭД.ЭЗН") Тогда
			МассивФорматированныхСтрок.Добавить(НСтр("ru='электронного заказа-наряда'"));	
		ИначеЕсли ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыЭД.ЭСВ") Тогда
	...
	Возврат ОбменСГИСЭПДСовместимость.ФорматированнаяСтрока(МассивФорматированныхСтрок);
...
Процедура ПолучитьНастройкиОтправкиЭДО(Организация, Контрагент, ВидДокумента, ОбработкаЗавершения) Экспорт
	...
	Если НастройкиОтправки = Неопределено Тогда	
		ПараметрыНастройкиОбменаСКонтрагентом = НовыеПараметрыНастройкиОбменаСКонтрагентом();
		ПараметрыНастройкиОбменаСКонтрагентом.НастройкаОдногоДокумента = Истина;
		НастроитьОбменСКонтрагентом(КлючНастроекОтправки, ОбработкаЗавершения, ПараметрыНастройкиОбменаСКонтрагентом);
	Иначе
		...
		Для Каждого ИмяТаблицы Из МассивИменТаблиц Цикл
			...
			Для Каждого СтрокаТаблицыФормы Из Форма[ИмяТаблицы] Цикл
				...
				Для Каждого КиЗ Из СтруктураСтроки Цикл
					Если КиЗ.Ключ <> "ИдентификаторСтроки" 
...
Процедура СохранитьАдресДляПодбора(ЗначениеОтбора, Результат) Экспорт
	...
	Если Результат.Свойство("Значение") И АдресДоставкиКорректен(Результат.Значение) = Ложь Тогда
		Возврат;
	КонецЕсли;
	
	Если Результат.Свойство("Представление") И ЗначениеЗаполнено(Результат.Индекс) = Ложь Тогда
		Возврат;
	КонецЕсли;
	...
	Если СохраненныеАдреса.Получить(Результат.Представление) = Неопределено Тогда
		...
		СохранитьАдресДляПодбора_Завершение = Новый ОписаниеОповещения("СохранитьАдресДляПодбора_Завершение", ОбменСГИСЭПДКлиент, ПараметрыФормы);
...
Процедура СохранитьАдресДляПодбора_Завершение(Результат, ПараметрыФормы) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		Результат = ОткрытьФормуМодально("РегистрСведений.АдресаДоставкиЭПД.Форма.ФормаЗаписи", ПараметрыФормы, ОбменСГИСЭПДКлиент);	
	КонецЕсли;
...
Функция ПолучитьОтборХранимыхДанных(КонтейнерРеквизитов, Форма, ГруппаДанных) Экспорт
	...
	ИначеЕсли ТипЗнч(КонтейнерРеквизитов.Ссылка) = Тип("ДокументСсылка.ЭлектроннаяТранспортнаяНакладная") Тогда
		Если КонтейнерРеквизитов.ЭтоВходящий = Ложь
			И КонтейнерРеквизитов.ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовВерсииЭД.ЭТрН_Титул1") Тогда
	...
	Результат.Вставить("ИмяОбъекта", ОписаниеГруппыДанных.ИмяОбъекта);
...
Процедура ВывестиФормуВводаДанных(Форма, Знач ИмяКоманды, ТекущиеДанные = Неопределено, ЭтоВыбор = Истина, ЕстьХранимыеДанные = Ложь, ОповещениеОЗавершении = Неопределено) Экспорт
	...
	Если Отказ = Ложь Тогда
		...
		ОписаниеРеквизитовФормы.Вставить("ПараметрыИзОбъекта", ПараметрыИзОбъекта);
		ОписаниеРеквизитовФормы.Вставить("ПараметрыФормыИзФормы", ПараметрыИзФормы);
		
		ОписаниеРеквизитовФормы.Вставить("ФормаРодительСОбъектом", ОписаниеРеквизитовФормыРодителя.ЕстьОбъект);
		ПараметрыОткрытияФормы.Вставить("ОписаниеРеквизитовФормы", ОписаниеРеквизитовФормы);
...
Процедура ОткрытиеФормыПоГиперссылке_Завершение(РезультатВыбора, ВходящийКонтекст) Экспорт
	...
	Если Модифицированность = Истина Тогда
		...
		ЗаполнитьТаблицы(КонтейнерТекущейФормы, Форма, Результат.ДанныеЗаполненияТаблицФормы, ИдентификаторСтрокиРодителя, ЭтоДопТитул);
		...
		Для Каждого КиЗ Из КонтейнерРеквизитов Цикл
			...
			Если МассивЧастей.Количество() = 3
				...
				И (МассивИдентификаторов.Найти(КиЗ.Значение) <> Неопределено Или ЗначениеЗаполнено(КиЗ.Значение) = Ложь) Тогда
					ПодчиненныеСтроки.Добавить(МассивЧастей[0] + "__" + МассивЧастей[1] + "__");
...
Процедура ЗаполнитьТаблицы(КонтейнерРеквизитов, Форма, Знач ТаблицыФормы, Знач ИдентификаторСтрокиРодителя = Неопределено, Знач ЭтоДопТитул = Ложь)
	...
	Для Каждого ТаблицаКиЗ Из ТаблицыФормы Цикл  
		ИмяТаблицы = ?(ЗначениеЗаполнено(ПрефиксТитула) = Ложь 
						Или СтрНачинаетсяС(ТаблицаКиЗ.Ключ, ПрефиксТитула), "", ПрефиксТитула) + ТаблицаКиЗ.Ключ;
		Если ЭтоДопТитул = Истина Тогда
...
Процедура ВводАдреса(ИмяЭлемента, Форма, ВладелецАдреса = Неопределено, ТекущиеДанные = Неопределено) Экспорт
	...
	ПараметрыФормаВыбораАдреса = ПараметрыФормаВыбораАдреса(ВладелецАдреса,
		ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Адрес"));
		
	Если ЗначениеЗаполнено(ВладелецАдреса) Тогда
		ЗначениеАдреса = ОткрытьФормуКонтактнойИнформации(ПараметрыФормаВыбораАдреса, Форма);
	Иначе
		Возврат;
	КонецЕсли;
	...
	ВводАдреса_Завершение(ЗначениеАдреса, ДополнительныеПараметры);
	
КонецПроцедуры


Процедура ВводАдреса_Завершение(РезультатВыбораАдреса, ДополнительныеПараметры) Экспорт
	
	ТекущиеДанные = Неопределено;
	ДополнительныеПараметры.Свойство("ТекущиеДанные", ТекущиеДанные);
	Если РезультатВыбораАдреса <> Неопределено И НЕ РезультатВыбораАдреса.Представление = "" Тогда
		Результат = ПолучитьЗначениеАдреса(РезультатВыбораАдреса);
...
Процедура ПолучитьУИДМинтранса(ОписаниеОповещения, ИдентификаторАбонента, ЭтоВторойТитул = Ложь, ДокументСсылка = Неопределено) Экспорт
	...
	УчетныеЗаписи.Добавить(ИдентификаторАбонента);
	...
	ОбработкаОповещения = Новый ОписаниеОповещения("ПолучитьУИДМинтранса_ПослеАвторизации", ОбменСГИСЭПДКлиент, ДополнительныеПараметры);
	
	АвторизоватьсяВСервисеЭлектронногоДокументооборота(
		ОбработкаОповещения, УчетныеЗаписи);
...
Процедура ПоказатьХранимыеДанныеЭПД(ГруппаДанных, Отбор, ФормаВладелец, ОписаниеОповещения, Отказ, ИдентификаторСтрокиРодителя = Неопределено) Экспорт
	...
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИмяОбъекта");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.ПравоеЗначение = ИмяОбъекта;
...
Процедура ОткрытьОписаниеОшибки(ОбъектСсылка, ФормаВладелец) Экспорт
	...
	Результат = ОткрытьФормуМодально("ОбщаяФорма.ОшибкиОбменаСГИСЭПД", ОписаниеОшибки, ФормаВладелец);
	
КонецПроцедуры

Процедура АвторизоватьсяВСервисеЭлектронногоДокументооборота(Оповещение, МассивИдентификаторов) Экспорт
	...
	Для Каждого КиЗ Из СоответствиеПрофилейИД Цикл
		МассивПрофилей.Добавить(КиЗ.Значение);	 
	КонецЦикла;

	СоотвСоглашенийИСтруктурСертификатов = ЭлектронныеДокументыСлужебныйКлиент.СоотвСоглашенийИСтруктурСертификатовДляАвторизации(МассивПрофилей);
	
	КлючиСинхронизации = Новый Соответствие;
	Для Каждого ИдентификаторЭДО Из МассивИдентификаторов Цикл 
		ПрофильНастроекЭДО = СоответствиеПрофилейИД.Получить(ИдентификаторЭДО);        
		Если ПрофильНастроекЭДО <> Неопределено Тогда
			СтруктураСертификата = СоотвСоглашенийИСтруктурСертификатов.Получить(ПрофильНастроекЭДО); 
			КлючиСинхронизации.Вставить(ИдентификаторЭДО, СтруктураСертификата);
		КонецЕсли;
	КонецЦикла;    
	
	Результат.Вставить("КлючиСинхронизации", КлючиСинхронизации); 
...
Процедура ИсходящиеДокументыФормироватьПриИзменении(ТекущиеДанные) Экспорт
	...
	Если РеквизитыВида.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыЭД.ЭТрН")
		Или РеквизитыВида.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыЭД.ЭЗН")
		Или РеквизитыВида.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыЭД.ЭСВ")
		Или РеквизитыВида.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыЭД.ЭПЛ") Тогда
		Если ТекущиеДанные.Формировать = Истина
			...
			ВопросЗавершение = Новый ОписаниеОповещения("ВопросОФормированииЭПД_Завершение", ОбменСГИСЭПДКлиент, ТекущиеДанные);
...
// Подключает обработчик ожидания начального оформления.
// 
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
Процедура ПодключитьОбработчикОжиданияНачальногоОформления(Форма) Экспорт
	
	ДополнительныеПараметрыЗавершенияОжидания = Новый Структура("УникальныйИдентификатор", Форма.УникальныйИдентификатор);
	
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ФормированияДереваСоответствийЗавершение",
		 ОбменСГИСЭПДКлиент,
		 ДополнительныеПараметрыЗавершенияОжидания);						
	...
	ПараметрыЗаданияНачальногоОформления = Новый Структура(Форма.ПараметрыЗаданияНачальногоОформления);
														   
	ДлительныеОперацииКлиент.ОжидатьЗавершение(ПараметрыЗаданияНачальногоОформления,
...
Функция ОткрытьФормуКонтактнойИнформации(Параметры, ФормаВладелец) Экспорт
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		ФормаВыбора.РегистрСведенийСписок.Отбор.Объект.Значение = Параметры.Владелец;
	...
	#Иначе
		Возврат ОткрытьФормуМодально("Обработка.ЭлектронныеПеревозочныеДокументы.Форма.ФормаВыбораКонтактнойИнформации", Параметры, ФормаВладелец);
	#КонецЕсли
	
КонецФункции

// Конструктор для структуры параметров открытия формы выбора адреса.
Функция ПараметрыФормаВыбораАдреса(Владелец, ТипКонтактнойИнформации) Экспорт
...
Функция ПолучитьЗначениеАдреса(Результат)
	
	СтруктураРезультата = Новый Структура;
	СтруктураРезультата.Вставить("ВВидеГиперссылки",		Ложь);
	СтруктураРезультата.Вставить("ВведеноВСвободнойФорме",	Ложь);
	СтруктураРезультата.Вставить("Вид",						Результат.Вид);
	СтруктураРезультата.Вставить("Комментарий",				Результат.Комментарий);
	СтруктураРезультата.Вставить("Представление",			Результат.Представление);
	СтруктураРезультата.Вставить("Тип",						Результат.Тип);
	СтруктураРезультата.Вставить("Адрес",					"");
	
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Результат, "Значение") И ЗначениеЗаполнено(Результат.Значение) Тогда
		СтруктураРезультата.Вставить("Значение",				Результат.Значение);
	Иначе
		СтруктураРезультата.Адрес = УправлениеКонтактнойИнформацией.ПолучитьПолныйАдрес(Результат);
		СтруктураРезультата.Вставить("ТипАдреса", Результат.Тип);
		СтруктураРезультата.Вставить("Индекс",    Результат.Поле1);
		
		АдресСтруктурой = УправлениеКонтактнойИнформацией.ПолучитьСтруктуруАдресаИзСтроки(СтруктураРезультата.Адрес);
		СтруктураКлассификатора = УправлениеКонтактнойИнформацией.ВернутьСтрокуАдресногоКлассификатораПоАдреснымЭлементам(АдресСтруктурой.Регион, "", "", "", "");
		АдресСтруктурой.Вставить("КодРегиона", Формат(СтруктураКлассификатора.КодРегионаВКоде, "ЧЦ=2; ЧВН=; ЧГ="));
		
		СтруктураРезультата.Вставить("АдресСтруктурой", АдресСтруктурой);
...
Процедура ПроверитьНаличиеУчетнойЗаписиДляОтправкиПочты(ОповещениеЗавершения) Экспорт
	
	Результат = Ложь;
	
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаСПочтовымиСообщениями") Тогда
		МодульЭлектроннаяПочтаВызовСервера = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ЭлектроннаяПочтаВызовСервера");	
		Результат = МодульЭлектроннаяПочтаВызовСервера.ЕстьДоступныеУчетныеЗаписиДляОтправки();
	КонецЕсли;
...
#Область МетодыДляБЭД1_1

Процедура ОткрытьЭлектронныйДокументОбъектаУчета(ОбъектСсылка) Экспорт
	...
	Для Каждого ТекЭл Из МассивСсылок Цикл
		...
		Если ЗначениеЗаполнено(СсылкаНаЭД) Тогда
			ЭлектронныеДокументыСлужебныйКлиент.ОткрытьЭДДляПросмотра(СсылкаНаЭД, , ОбъектСсылка, Истина);
		Иначе
...
Процедура СостояниеЭДОНажатие_ФормаСписка(ВыбраннаяСтрока, СтандартнаяОбработка) Экспорт
	
	СтандартнаяОбработка = Ложь;
	ОбменСГИСЭПДКлиент.ОткрытьЭлектронныйДокументОбъектаУчета(ВыбраннаяСтрока.Ссылка);

КонецПроцедуры

// НастроитьОбменСКонтрагентом(КлючНастроекОтправки, ОповещениеОкончаниеНастройки, ПараметрыНастройкиОбменаСКонтрагентом)
Процедура НастроитьОбменСКонтрагентом(КлючНастроекОтправки, ОповещениеОкончаниеНастройки, ПараметрыНастройкиОбменаСКонтрагентом) Экспорт

	ЭлектронныеДокументыКлиент.ОбработкаНавигационнойСсылкиВФормеОбъектаИБ(КлючНастроекОтправки.СсылкаНаДокумент);
	НастройкиОтправки = ОбменСГИСЭПДВызовСервера.НастройкиОтправки(КлючНастроекОтправки);
...
Процедура АвтоПодборАдреса(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка) Экспорт
	...
	ДанныеВыбора = ОбменСГИСЭПДВызовСервера.ВариантыАвтоподбораАдреса(Текст);
