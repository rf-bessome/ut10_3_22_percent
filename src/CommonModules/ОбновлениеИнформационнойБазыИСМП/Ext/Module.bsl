...
Функция ВерсияБиблиотеки()
	Возврат ИнтеграцияИС.ВерсияПодсистемы("10");
КонецФункции
...
Процедура РегистрыСведенийСтатусыПроверкиИПодбораДокументовИСМПОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	
	Сообщить(НСтр("ru = 'Заполнение нового измерения регистра ""Вид маркируемой продукции"" ссылками на перечисление ""ВидыПродукцииИС.Табак"".'"));
	ДанныеКОбработке = РегистрыСведений.СтатусыПроверкиИПодбораДокументовИСМП.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию();
	РегистрыСведений.СтатусыПроверкиИПодбораДокументовИСМП.ОбработатьДанныеДляПереходаНаНовуюВерсию(ДанныеКОбработке);
	
КонецПроцедуры

Процедура РегистрыСведенийНастройкиУчетаМаркируемойПродукцииИСМПОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	
	Сообщить(НСтр("ru = 'Заполнение даты начала обязательной маркировки товаров'"));
	ДанныеКОбработке = РегистрыСведений.НастройкиУчетаМаркируемойПродукцииИСМП.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюДатыОбязательнойМаркировки();
	РегистрыСведений.НастройкиУчетаМаркируемойПродукцииИСМП.ОбработатьДанныеДляПереходаНаНовуюВерсиюДатыОбязательнойМаркировки(ДанныеКОбработке);
	
КонецПроцедуры

Процедура РегистрыСведенийПулКодовМаркировкиСУЗОбработатьДанныеДляПереходаНаНовуюВерсиюНормализацияБезМРЦ() Экспорт
	
	Сообщить(НСтр("ru = 'Обработка кодов маркировки табачной продукции (нормализация без МРЦ)'"));
	ДанныеКОбработке = РегистрыСведений.ПулКодовМаркировкиСУЗ.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюНормализацияБезМРЦ();
	РегистрыСведений.ПулКодовМаркировкиСУЗ.ОбработатьДанныеДляПереходаНаНовуюВерсиюНормализацияБезМРЦ(ДанныеКОбработке);
	
КонецПроцедуры

Процедура СправочникиШтрихкодыУпаковокТоваровОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнениеХешСуммаЗначенияШтрихкодаGS1() Экспорт
	
	Сообщить(НСтр("ru = 'Заполнение реквизита ХешСуммаЗначенияШтрихкодаGS1'"));
	ДанныеКОбработке = Справочники.ШтрихкодыУпаковокТоваров.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюЗаполнениеХешСуммаЗначенияШтрихкодаGS1();
	Справочники.ШтрихкодыУпаковокТоваров.ОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнениеХешСуммаЗначенияШтрихкодаGS1(ДанныеКОбработке);
	
КонецПроцедуры

Процедура ОбработкиПанельАдминистрированияИСМПОбработатьДанныеДляПереходаНаНовуюВерсиюРаздельныйКонтрольСтатусовВладельцевКМ() Экспорт
	
	Сообщить(НСтр("ru = 'Переход на настройки раздельного контроля статусов и владельцев кодов маркировки'"));
	ДанныеКОбработке = Обработки.ПанельАдминистрированияИСМП.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюРаздельныйКонтрольСтатусовВладельцевКМ();
	Обработки.ПанельАдминистрированияИСМП.ОбработатьДанныеДляПереходаНаНовуюВерсиюРаздельныйКонтрольСтатусовВладельцевКМ(ДанныеКОбработке);
