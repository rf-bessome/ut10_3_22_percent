...
Процедура ОткрытьФормуСправочникаНоменклатурыДляВыбора(СтруктураВыборСХарактеристикой, ФормаВыбора = Неопределено) Экспорт
	...
	Если ФормаВыбора = Неопределено Тогда
		...
		ФормаСправочника.ВладелецФормы = ВладельцыФормы["Элемент"];
...
Процедура ЗаполнитьКодТНВЭД(СтрокаТабличнойЧасти) Экспорт
	...
	Если ТипЗнч(СтрокаТабличнойЧасти.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("КодТНВЭД", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
			СтрокаТабличнойЧасти.КодТНВЭД = СтрокаТабличнойЧасти.Номенклатура.КодТНВЭД;
		КонецЕсли;
		
		Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("КодТНВЭДДоИзменения", МетаданныеДокумента, ИмяТабличнойЧасти) Тогда
			СтрокаТабличнойЧасти.КодТНВЭД = СтрокаТабличнойЧасти.Номенклатура.КодТНВЭД;
		КонецЕсли;
...
Процедура ЗаполнитьЕдиницуЦенуПродажиТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета,
	...
	Если ЭтоРозница И ТекСклад.ВидСклада = Перечисления.ВидыСкладов.Розничный Тогда
		...
		Иначе
			...
			// для частичного выбытия используется другая единица измерения
			Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ЕдиницаИзмерения)
				И СтрокаТабличнойЧасти.ЕдиницаИзмерения <> СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков Тогда
					Цена = Ценообразование.ПолучитьЦенуНоменклатуры(СтрокаТабличнойЧасти.Номенклатура, ХарактеристикаНоменклатуры,
...
//
Процедура ЗаполнитьНеобходимостьВводаАкцизнойМарки(СтрокаТабличнойЧасти, СтруктураОпределенияНеобходимостиВводаАкцизнойМарки = Неопределено) Экспорт
	
	СтрокаТабличнойЧасти.НеобходимостьВводаАкцизнойМарки = Ложь;
	
	ПроверяемаяДата = ТекущаяДатаСеанса();
	
	Если НЕ СтруктураОпределенияНеобходимостиВводаАкцизнойМарки = Неопределено Тогда
		Если СтруктураОпределенияНеобходимостиВводаАкцизнойМарки.Свойство("Дата") Тогда
			ПроверяемаяДата = СтруктураОпределенияНеобходимостиВводаАкцизнойМарки.Дата;
		КонецЕсли;
	КонецЕсли;
	
	ДатаНачалаРегистрацииРозничныхПродажВЕГАИС = Константы.ДатаНачалаРегистрацииРозничныхПродажВЕГАИС.Получить();
	
	Если ДатаНачалаРегистрацииРозничныхПродажВЕГАИС > ПроверяемаяДата
		ИЛИ НЕ ЗначениеЗаполнено(ДатаНачалаРегистрацииРозничныхПродажВЕГАИС) Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Номенклатура.ВидАлкогольнойПродукцииЕГАИС.Маркируемый КАК Маркируемый
	|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		СтрокаТабличнойЧасти.НеобходимостьВводаАкцизнойМарки = Выборка.Маркируемый = Истина;
	КонецЕсли;
