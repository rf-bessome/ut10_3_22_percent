...
//
Функция ПровестиДокументы(Документы, ТипПроведенныхДокументов = Неопределено) Экспорт
...
Функция ПолучитьЗначениеПеременной(ИмяПараметра, Кэш = Неопределено, КэшИзменен = Ложь) Экспорт
	...
	ВРегИмяПараметра = ВРег(ИмяПараметра);
	
	Если ВРегИмяПараметра = ВРег("глТекущийПользователь") Тогда
		НайденноеЗначение = ПараметрыСеанса.ТекущийПользователь;
		
	ИначеЕсли ВРегИмяПараметра = ВРег("глСоответствиеТекстовЭлектронныхПисем") Тогда
		НайденноеЗначение = Новый Соответствие;				
		
	ИначеЕсли ВРегИмяПараметра = ВРег("УчетнаяПолитикаПоУправленческомуУчету") Или ВРегИмяПараметра = ВРег("УчетнаяПолитикаУпр")Тогда
		НайденноеЗначение = СоздатьКЭШУчетнойПолитики("");
		
	ИначеЕсли ВРегИмяПараметра = ВРег("УчетнаяПолитикаПоНалоговомуУчету") Тогда
		НайденноеЗначение = СоздатьКЭШУчетнойПолитики("НалоговыйУчет");
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ВалютаРегламентированногоУчета") Тогда
		НайденноеЗначение = Константы.ВалютаРегламентированногоУчета.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ВалютаУправленческогоУчета") Тогда
		НайденноеЗначение = Константы.ВалютаУправленческогоУчета.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ИспользоватьСерииНоменклатуры") Тогда
		НайденноеЗначение = Константы.ИспользоватьСерииНоменклатуры.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ИспользоватьХарактеристикиНоменклатуры") Тогда
		НайденноеЗначение = Константы.ИспользоватьХарактеристикиНоменклатуры.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ИспользоватьВозвратнуюТару") Тогда
		НайденноеЗначение = Константы.ИспользоватьВозвратнуюТару.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("УказаниеСкладовВТабличнойЧастиДокументов") Тогда
		НайденноеЗначение = Константы.УказаниеСкладовВТабличнойЧастиДокументов.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ИспользоватьОплатуБанковскимиКредитами") Тогда
		НайденноеЗначение = Константы.ИспользоватьОплатуБанковскимиКредитами.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ИспользоватьОплатуПлатежнымиКартами") Тогда
		НайденноеЗначение = Константы.ИспользоватьОплатуПлатежнымиКартами.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ПечатьПрефиксаРИБ") Тогда
		НайденноеЗначение = Константы.ПечатьПрефиксаРИБ.Получить();
		
	ИначеЕсли ВРегИмяПараметра = ВРег("глОбработкаАвтоОбменДанными") Тогда
		НайденноеЗначение = ПроцедурыОбменаДанными.ВыполнитьИнициализациюАвтообменаПриНачалеРаботы();
		
	#Если Клиент Тогда

	ИначеЕсли ВРегИмяПараметра = ВРег("глОбработкаАвтоПолученияОтправкиЭлектронныхПисем") Тогда	
	...
	ИначеЕсли ВРегИмяПараметра = ВРег("КэшСтратегииАвтонумерации") Тогда	
		НайденноеЗначение = Новый Соответствие;		
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ФормаОповещенияОЗадачах") Тогда	
		НайденноеЗначение = Задачи.ЗадачиПользователя.ПолучитьФорму("ФормаОповещенияОЗадачах", ,"Оригинал");
		
	#КонецЕсли

	ИначеЕсли ВРегИмяПараметра = ВРег("глКоличествоСекундОпросаОбмена") Тогда
	...
	ИначеЕсли ВРегИмяПараметра = ВРег("ЗначенияНастроекПользователей") Тогда
		НайденноеЗначение = Новый Соответствие;
		
	ИначеЕсли ВРегИмяПараметра = ВРег("ЗначенияДополнительныхПравПользователя") Тогда
		НайденноеЗначение = Новый Соответствие;
	
	ИначеЕсли ВРегИмяПараметра = ВРег("НомерВерсииКонфигурации") Тогда
		НайденноеЗначение = ОбновлениеИнформационнойБазы.ВерсияИБ(Метаданные.Имя);
		
	ИначеЕсли ВРегИмяПараметра = ВРег("глКомпонентаСклоненияФИО") Тогда
		#Если Клиент Тогда
		Попытка
...
Функция ФамилияИнициалыФизЛица(Объект = "", Фамилия = " ", Имя = " ", Отчество = " ") Экспорт
	...
	Если ТипОбъекта = Тип("Строка") И ЗначениеЗаполнено(Объект) Тогда
	...
	Если ФИО.ВГраница() >=3 
		И СокрЛП(ФИО[0]) = "ИП" Тогда
		ИндексПервогоЭлемента = 1;
	КонецЕсли;
	
	КоличествоПодстрок = ФИО.Количество();
	Фамилия = ?(КоличествоПодстрок > 0,ФИО[ИндексПервогоЭлемента],"");
	Имя		= ?(КоличествоПодстрок > 1,ФИО[ИндексПервогоЭлемента + 1],"");
	Отчество= ?(КоличествоПодстрок > 2,ФИО[ИндексПервогоЭлемента + 2],"");
	
	Если КоличествоПодстрок > 3 И ИндексПервогоЭлемента = 0 Тогда
		...
		Если ДополнительныеЧастиОтчества.Найти(НРег(ФИО[ИндексПервогоЭлемента + 3])) <> Неопределено Тогда
			Отчество = Отчество + " " + ФИО[ИндексПервогоЭлемента + 3];
		КонецЕсли;
...
Функция ПолучитьПараметрыУчетнойПолитики(КонДата, Отказ, Организация = Неопределено, Учет = "Упр", СообщатьОбОшибке = Истина) Экспорт
	...
	Если Учет = "Нал" ИЛИ Учет = "регл" Тогда		
...
Процедура МониторингЗаконодательства() Экспорт
	ПоказатьВебСтраницу("http://v8.1c.ru/lawmonitor/lawchanges.jsp");
КонецПроцедуры
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	ОбщиеПараметры.Вставить("ИспользуетсяНедопустимаяВерсияПлатформы", Ложь);
...
Процедура ПередУдалениемДокумента(Источник, Отказ) Экспорт
	ПараметрыСеанса.УдаляемыйДокумент = Источник.Ссылка;
КонецПроцедуры
...
Функция WSОпределения(Знач АдресWSDL, Знач ИмяПользователя, Знач Пароль, Знач Таймаут = 10) Экспорт
	...
	Если ДоступноПолучениеФайловИзИнтернета = Истина Тогда
		...
		Определения = Новый WSОпределения(ИмяФайлаWSDL, , , , , Новый ЗащищенноеСоединениеOpenSSL);
...
Функция ВнутренняяWSПрокси(Знач АдресWSDL, Знач URIПространстваИмен, Знач ИмяСервиса,
	...
	Параметры.Добавить(Таймаут);
	Параметры.Добавить(Новый ЗащищенноеСоединениеOpenSSL);
...
Функция ПолучитьWSDL(Знач Адрес, Знач ИмяПользователя, Знач Пароль, Знач Таймаут)
	...
	Определения = Новый WSОпределения(ОписаниеФайла.Путь, , , , , Новый ЗащищенноеСоединениеOpenSSL);
