...
Функция ОбработатьСобытие(ОбъектДрайвера, Параметры, ПараметрыПодключения, Событие, Данные, ВыходныеПараметры) Экспорт
	...
	Если Событие = "Штрихкод" Или Событие = "Barcode" Или Событие = "ПолученШтрихкод" Тогда
	...
	ИначеЕсли Событие = "Взвешивание" Или Событие = "Weighting" Тогда
		...
		Исключение
			Результат = Ложь;    
		КонецПопытки;
		ДанныеСобытия = Новый Структура("Вес,Стабилизация", Вес, Стабилизация);
...
Функция ЗакрытьСмену(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ПараметрыПодключения.Свойство("ТипОборудования") Тогда
		...
		Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
			...
			ПараметрыОперации = МенеджерОборудованияВызовСервера.ПолучитьXMLПакетДляОперации(ВходныеПараметры, ПараметрыПодключения.РевизияИнтерфейса);
			Попытка
				Если РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.1)
	...
	Иначе
		...
		Если ПараметрыПодключения.РевизияИнтерфейса > 2001 Тогда // Версия стандарта 2.2 (ОФД 1.05/1.1)
			Попытка
				...
				Если РевизияИнтерфейса >= 3000 Тогда // Версия стандарта 3.0 (ОФД 1.05/1.1)
					Результат = ОбъектДрайвера.ЗакрытьСмену(ПараметрыПодключения.ИДУстройства, ПараметрыОперации, ПараметрыСостояния);
					Если Результат Тогда
						ФискальныеДанныеСтруктура = МенеджерОборудованияВызовСервера.ПолучитьПараметрыСменыИзXMLПакета(ПараметрыСостояния, РевизияИнтерфейса);
						НомерСмены     = ФискальныеДанныеСтруктура.НомерСменыККТ;
...
Функция Инкассация(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Иначе	
		...
		ИначеЕсли Найти(Параметры.Идентификатор, "ККТ") > 0 Тогда
...
Функция ФискализацияЧека(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры);
	...
	Если ПараметрыПодключения.Свойство("ТипОборудования") Тогда
		...
		ИначеЕсли ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
			...
			Если ПараметрыФискализации.РевизияИнтерфейса >= 3004  Тогда
				Если ПараметрыПодключения.Свойство("ИдентификаторУстройства") Тогда
			...
			Иначе
				Попытка
					...
					ИначеЕсли ПараметрыФискализации.РевизияИнтерфейса > 2001 Тогда // Версия стандарта 2.2 и выше (ОФД 1.05)
			...
			Если Результат Тогда  
				...
				ОбщиеПараметры.Вставить("АдресСайтаПроверки"     , ПараметрыФискализации.АдресСайтаПроверки);
	...
	Иначе
		...
		Если Результат Тогда  
			...
			ФискальныйПризнак = ПараметрыФискализации.ФискальныйПризнак;

			Если (ПараметрыФискализации.Отправляет1СSMS Или ПараметрыФискализации.Отправляет1СEmail) Тогда
				ПокупательEmail = ?(ВходныеПараметры[0].Отправляет1СEmail, ВходныеПараметры[0].ПокупательEmail, Неопределено);
				ПокупательНомер = ?(ВходныеПараметры[0].Отправляет1СSMS  , ВходныеПараметры[0].ПокупательНомер, Неопределено);
				МенеджерОборудованияКлиентПереопределяемый.НачатьОтправкуЭлектронногоЧека(ВходныеПараметры, ПараметрыФискализации.ТекстСообщения, ПокупательEmail, ПокупательНомер);
			КонецЕсли;
			МенеджерОборудованияКлиент.УстановитьСессиюПроверкиКодовМаркировки(ПараметрыПодключения.ИДУстройства);
...
Функция ПечатьЧекаКоррекции(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	Если ПараметрыПодключения.Свойство("ТипОборудования") Тогда
		...
		ОбщиеПараметры = ВходныеПараметры[0];
		Кассир = ОбщиеПараметры.Кассир;
		ПараметрыФискализации = МенеджерОборудованияВызовСервера.ПодготовитьДанныеЧекаКоррекции(ОбщиеПараметры, ПараметрыПодключения);
	Иначе
		ОбщиеПараметры = ВходныеПараметры[0];
		Кассир = ОбщиеПараметры.Кассир;
		ПараметрыФискализации = МенеджерОборудованияВызовСервера.ПодготовитьДанныеЧекаКоррекции(ОбщиеПараметры, ПараметрыПодключения);
			...
			Иначе
				// Шаг 1 - Формируем чек ККТ с обратным признаком расчета.
					Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда
						ВыходнаяТаблицаXML = "";
						Результат = ОбъектДрайвера.СформироватьЧек(ПараметрыПодключения.ИДУстройства, ПараметрыФискализацииСторно.Электронно, 
							ПараметрыФискализацииСторно.ДанныеЧекаXML, ВыходнаяТаблицаXML); 
						ПараметрыФискализации.РезультатОперацииXML = ВыходнаяТаблицаXML;
					Иначе
						Результат = ОбъектДрайвера.СформироватьЧек(ПараметрыПодключения.ИДУстройства, ПараметрыФискализацииСторно.Электронно, 
							ПараметрыФискализацииСторно.ДанныеЧекаXML, ПараметрыФискализацииСторно.НомерЧекаККТ, ПараметрыФискализацииСторно.НомерСменыККТ, ПараметрыФискализацииСторно.ФискальныйПризнак, 
							ПараметрыФискализацииСторно.АдресСайтаПроверки);
					КонецЕсли;
				...
				Если Результат Тогда
					Если ПараметрыПодключения.РевизияИнтерфейса >= 3000 Тогда
						ВыходнаяТаблицаXML = "";
						Результат = ОбъектДрайвера.СформироватьЧек(ПараметрыПодключения.ИДУстройства, ПараметрыФискализацииСторно.Электронно, 
							ПараметрыФискализации.ДанныеЧекаXML, ВыходнаяТаблицаXML); 
						ПараметрыФискализации.РезультатОперацииXML = ВыходнаяТаблицаXML;
					Иначе
						Результат = ОбъектДрайвера.СформироватьЧек(ПараметрыПодключения.ИДУстройства, ПараметрыФискализации.Электронно, 
							ПараметрыФискализации.ДанныеЧекаXML, ПараметрыФискализации.НомерЧекаККТ, ПараметрыФискализации.НомерСменыККТ, ПараметрыФискализации.ФискальныйПризнак, ПараметрыФискализации.АдресСайтаПроверки); 
	...
	Попытка
		...
		Иначе
			...
			Иначе
				...
				Если ПараметрыПодключения.РевизияИнтерфейса > 2999 Тогда
					ИдентификаторТО = ?(ПараметрыПодключения.Свойство("ИдентификаторТО"), ПараметрыПодключения.ИдентификаторТО, "");
					
					ВыходныеПараметры = МенеджерОборудованияВызовСервера.ПараметрыИзXMLПакетаККТ(ПараметрыККТ, Параметры.Идентификатор, РевизияИнтерфейса, ИдентификаторТО);
					ВыходныеПараметры.Вставить("РевизияИнтерфейса", ПараметрыПодключения.РевизияИнтерфейса);
				Иначе					
...
Функция ЗапросКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ПустаяСтрока(ИдентификаторСессии) Тогда  
		ВремВыходныеПараметры = Новый Массив();
		Результат = ОткрытьСессиюРегистрацииКМ(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВремВыходныеПараметры);
		Если Не Результат Тогда
	...
	Попытка
		...
		Иначе
			Для Каждого СтрокаДанных Из МассивДанных Цикл
				Если ПустаяСтрока(СтрокаДанных.ШтрихкодBase64) Тогда
				...
				КонецЕсли;
				Если ПустаяСтрока(СтрокаДанных.ШтрихкодУпаковкиBase64) Тогда
