...
Процедура ТоварыНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, ДанныеОбъекта) Экспорт
	...
	СведенияОНоменклатуреПрослеживаемогоТовара = ПолучитьСведенияПрослеживаемогоТовара(
...
Функция ПараметрыНоменклатуры(Номенклатура) Экспорт
	...
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "КодТНВЭД, ЕдиницаХраненияОстатков");
...
Функция ТекстДополнительныеСведения(ДополнительныеРеквизиты) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеРеквизиты.КодФормыРеорганизации) Тогда
		...
		Если УниверсальныеМеханизмы.Просклонять(глЗначениеПеременной("глКомпонентаСклоненияФИО"),
				ОписаниеКодаРеорганизации(ДополнительныеРеквизиты.КодФормыРеорганизации), 2,, РезультатСклонения) Тогда
				
			ТекстКодаРеорганизации = РезультатСклонения;
...
Функция ЕдиницыИзмеренийНоменклатуры(Номеклатура) Экспорт
	
	ПараметрыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				Номеклатура, "ЕдиницаХраненияОстатков,ЕдиницаИзмеренияПрослеживаемости,КодТНВЭД,ЕдиницаХраненияОстатков.Коэффициент");
				
	Возврат Новый Структура("ЕдиницаИзмерения,ЕдиницаПрослеживаемости,КодТНВЭД,Коэффициент",
							ПараметрыНоменклатуры.ЕдиницаХраненияОстатков,
							ПараметрыНоменклатуры.ЕдиницаИзмеренияПрослеживаемости,
							ПараметрыНоменклатуры.КодТНВЭД,
							ПараметрыНоменклатуры.ЕдиницаХраненияОстатковКоэффициент,
							);
...
Функция ПолучитьСведенияПрослеживаемогоТовара(Номенклатура, ДанныеОбъекта)
	
	СведенияОНоменклатуре = Новый Структура("ЕдиницаИзмерения,ЕдиницаПрослеживаемости,КодОКПД2");
