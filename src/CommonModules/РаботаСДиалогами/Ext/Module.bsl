			...
			Если ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем
			 Или ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером Тогда
				...
				Если ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СКомиссионером 
					Или (ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПлатежноеПоручениеВходящее") И ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийППИсходящее.ВозвратДенежныхСредствПокупателю) Тогда
...
Процедура УдалитьКнопки(КнопкиПанели, ИменаУдаляемыхКнопок, ИРазделителиСлева = Истина) Экспорт
...
Функция ПоказатьПредупреждениеБезопасности(Режим) Экспорт
	ПараметрыОткрытия = Новый Структура("Режим", Режим);
	Результат = ОткрытьФормуМодально("ОбщаяФорма.ПредупреждениеБезопасности", ПараметрыОткрытия);
	Возврат Результат;
КонецФункции

Процедура УстановитьСвойстваЭлементовОтправкиЭлектронногоЧека(ЭлементыФормы, ДополнительноеУсловие) Экспорт
	
	ВидимостьЭлементовОтправки = МенеджерОборудованияВызовСервера.ВидимостьЭлементовОтправкиЧека();
	
	ЭлементыФормы.НадписьАдресЭП.Видимость = ВидимостьЭлементовОтправки И ДополнительноеУсловие;
	ЭлементыФормы.АдресЭП.Видимость = ВидимостьЭлементовОтправки И ДополнительноеУсловие;
	Если ЭлементыФормы.Найти("ОтправитьПоЭП") <> Неопределено Тогда
		ЭлементыФормы.ОтправитьПоЭП.Видимость = ВидимостьЭлементовОтправки И ДополнительноеУсловие;
    	ЭлементыФормы.ОтправитьПоЭП.Доступность = Не глЗначениеПеременной("ОтправлятьЭлектронныеЧекиПоEmailЧерезОФД");
	КонецЕсли;
	
КонецПроцедуры

Процедура ВыполнитьОтправкуЭлектронногоЧека(ЭтотОбъект) Экспорт
	
	МассивОшибок = Новый Массив;
	МенеджерОборудованияКлиентСервер.ПроверитьВозможностьОтправкиЭлектронногоЧека(ЭтотОбъект, МассивОшибок);
	Если МассивОшибок.Количество() > 0 Тогда
		Для Каждого Ошибка Из МассивОшибок Цикл
			ОбщегоНазначения.СообщитьОбОшибке(Ошибка);
		КонецЦикла;
		Возврат;
	КонецЕсли;
	
	ПараметрыЧека = Новый Структура("ДокументОснование, ТекстЭлектронногоЧека, ПокупательEmail",
		ЭтотОбъект.Ссылка, ЭтотОбъект.ТекстЭлектронногоЧека, ЭтотОбъект.АдресЭП);
	
	ПодключаемоеОборудованиеУниверсальныйДрайверКлиент.ОтправитьЭлектронныйЧек(ПараметрыЧека);
...
Процедура УстановитьАдресЭПДляОтправкиЧека(Объект, СтарыйКонтрагент, НовыйКонтрагент, НужноУстанавливать) Экспорт
	...
	Если НужноУстанавливать И 
		...
		Если СокрЛП(Объект.АдресЭП) = МенеджерОборудованияВызовСервера.АдресЭПКонтрагентаПоУмолчанию(СтарыйКонтрагент) 
			ИЛИ ПустаяСтрока(Объект.АдресЭП) Тогда
			Объект.АдресЭП = МенеджерОборудованияВызовСервера.АдресЭПКонтрагентаПоУмолчанию(НовыйКонтрагент);
		КонецЕсли;
		
	КонецЕсли;
КонецПроцедуры

Процедура ВыбратьАдресЭПКонтрагента(Объект, ЭтаФорма, СтандартнаяОбработка) Экспорт
	
	Если Не ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Не заполнен контрагент.");
		Возврат;
	КонецЕсли;
	...
	ФормаВыбора.Заголовок = "Выбор адреса электронной почты";

	ФормаВыбора.РегистрСведенийСписок.Отбор.Объект.Значение = Объект.Контрагент;
	ФормаВыбора.РегистрСведенийСписок.Отбор.Объект.Использование = Истина;

	СтруктураОтбораАдресов = Новый СписокЗначений;
	СтруктураОтбораАдресов.Добавить(Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты);
	...
	КолонкиСписка = ФормаВыбора.ЭлементыФормы.РегистрСведенийСписок.Колонки;
	КолонкиСписка.Объект.Видимость = Ложь;
	КолонкиСписка.Тип.Видимость = Ложь;
	Если КолонкиСписка.Найти("КонтрагентДляОграниченияПравДоступа") <> Неопределено Тогда
		КолонкиСписка.КонтрагентДляОграниченияПравДоступа.Видимость = Ложь;
	КонецЕсли;
	Если КолонкиСписка.Найти("КонтактноеЛицоДляОграниченияПравДоступа") <> Неопределено Тогда
		КолонкиСписка.КонтактноеЛицоДляОграниченияПравДоступа.Видимость = Ложь;
	КонецЕсли;

	ВыбАдресЭП = ФормаВыбора.ОткрытьМодально();

	Если ЗначениеЗаполнено(ВыбАдресЭП) Тогда
		Объект.АдресЭП = ВыбАдресЭП;
	КонецЕсли;
