...
//  <Булево> - Результат проверки.
//
Функция ТипШтрихкодаPDF417(Штрихкод) Экспорт
...
//
Функция ПолучитьПроверитьПараметрыДляОплатыКартой(ЭС, ФУ) Экспорт

	МассивВидовТО = Новый Массив();
	МассивВидовТО.Добавить(Перечисления.ВидыТорговогоОборудования.ФискальныйРегистратор);
	МассивВидовТО.Добавить(Перечисления.ВидыТорговогоОборудования.ККТ);
	
	ФУ = ПолучитьЭлементТО(МассивВидовТО,
		"Необходимо выбрать фискальное устройство", "Фискальное устройство не подключено!");
	...
	Возврат ЗначениеЗаполнено(ФУ) И ЗначениеЗаполнено(ЭС);

КонецФункции // ПолучитьПроверитьПараметрыДляОплатыКартой()

// Функция определяет и проверяет фискальное устройство.
//
// Параметры:
//  ФУ - В эту переменную будет возвращено выбранное фискальное устройство.
//
// Возвращаемое значение:
//  Булево - Истина, если фискальное устройство выбрано.
//
Функция ПолучитьПроверитьПараметрыДляПробитияЧека(ФУ, УчитыватьККТ = Ложь, Организация = Неопределено, ТолькоККТ = Ложь) Экспорт
	
	Если УчитыватьККТ = Истина Тогда
		Если ТолькоККТ Тогда
			ФУ = ПолучитьЭлементТО(Перечисления.ВидыТорговогоОборудования.ККТ,
	   			"Необходимо выбрать фискальное устройство", "Фискальное устройство не подключено!", Организация);
		Иначе
			МассивВидовТО = Новый Массив();
			МассивВидовТО.Добавить(Перечисления.ВидыТорговогоОборудования.ФискальныйРегистратор);
			МассивВидовТО.Добавить(Перечисления.ВидыТорговогоОборудования.ККТ);
		...
		КонецЕсли;
	Иначе
		ФУ = ПолучитьЭлементТО(Перечисления.ВидыТорговогоОборудования.ФискальныйРегистратор,
	   		"Необходимо выбрать фискальный регистратор", "Фискальный регистратор не подключен!", Организация);
	КонецЕсли;

	Возврат ЗначениеЗаполнено(ФУ);
...
//
// Параметры:
//  ВидТО - список видов или вид торгового оборудования, подключенный элемент которого требуется получить.
...
//
Функция ПолучитьЭлементТО(ВидТО, ТекстЗаголовкаВыбора, СообщениеНеПодключен, Отбор = Неопределено)
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.ТорговоеОборудование) Тогда
		ЭлементТО = Неопределено;
		Предупреждение("Нарушение прав доступа!");
	ИначеЕсли МассивТО.Количество() = 0 Тогда
		ЭлементТО = Неопределено;
