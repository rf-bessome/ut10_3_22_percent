...
Функция ПараметрыРаботыКлиента2() Экспорт
	...
	Возврат СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента2();
...
Функция ПроверитьВерсиюПлатформыПриЗапуске() Экспорт
	...
	Возврат ПроверитьВерсиюПлатформы(ПараметрыРаботы.МинимальноНеобходимаяВерсияПлатформы, ПараметрыРаботы.РаботаВПрограммеЗапрещена, ПараметрыРаботы.ИспользуетсяНедопустимаяВерсияПлатформы);
...
//
Функция ПроверитьВерсиюПлатформы(знач РекомендуемаяВерсияПлатформы, знач РаботаВПрограммеЗапрещена = Ложь, знач ИспользуетсяНедопустимаяВерсияПлатформы = Ложь) Экспорт
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Если ИспользуетсяНедопустимаяВерсияПлатформы Тогда
		
		ТекстСообщения = 
		НСтр("ru='Используемая версия платформы может работать некорректно.
		|Удалите текущую платформу и установите более новую версию или предыдущую версию выше минимально допустимой.'");
