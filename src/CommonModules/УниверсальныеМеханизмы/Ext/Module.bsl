...
Процедура ПодготовитьТаблицуДокументаДляПечати(ТаблицаДокумента, МассивТЧ) Экспорт
	
	// Исключим строки, которые не удовлетворяют условиям печати. При выборке запросом их необходимо получать, 
	// чтобы распределять сумму взаиморасчетов в рублях на все строки документа.
	Если МассивТЧ <> Неопределено И МассивТЧ.Количество() <> 0 Тогда
		Инд = ТаблицаДокумента.Количество() - 1;
		Пока Инд >= 0 Цикл
			СтрокаТовар = ТаблицаДокумента[Инд];
			
			Если МассивТЧ.Найти(СтрокаТовар.Метка) = Неопределено Тогда
				ТаблицаДокумента.Удалить(Инд);
			КонецЕсли;
...
// Определяет курс документа для печати с учетом курса аванса
// и заполняет параметры запроса к табличным частям документа
//
Процедура ОпределитьКурсыДокументаДляПечати(ДокументОбъект, Запрос, ВВалютеДокумента = Ложь) Экспорт
	...
	Если ВВалютеДокумента = Истина Тогда
		...
		Запрос.УстановитьПараметр("КурсДокумента",      1);
		Запрос.УстановитьПараметр("КратностьДокумента", 1);
	...
	ИначеЕсли ДокументОбъект.ВалютаДокумента <> мВалютаРегламентированногоУчета
		...
		СуммаДокумента = УчетНДС.ПолучитьСуммуДокументаСНДС(ДокументОбъект);
		...
		СуммаВзаиморасчетов = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаДокумента,
			ДокументОбъект.ВалютаДокумента, ДокументОбъект.ДоговорКонтрагента.ВалютаВзаиморасчетов,
			КурсДокумента, ДокументОбъект.КурсВзаиморасчетов,
			КратностьДокумента, ДокументОбъект.КратностьВзаиморасчетов);
		
		НеоплаченнаяСумма = СуммаВзаиморасчетов - ВыборкаСуммаВзаиморасчетов;
		Если НеоплаченнаяСумма > 0 Тогда
			КурсДляПечати = (НеоплаченнаяСумма * ДокументОбъект.КурсВзаиморасчетов + ВыборкаСуммаРегл)/(НеоплаченнаяСумма + ВыборкаСуммаВзаиморасчетов);
		Иначе
			КурсДляПечати = КурсОплаты;
		КонецЕсли;
		
		Если КурсДляПечати = 0 Тогда
			Запрос.УстановитьПараметр("Курс",      ДокументОбъект.КурсВзаиморасчетов);
			Запрос.УстановитьПараметр("Кратность", ДокументОбъект.КратностьВзаиморасчетов);
		Иначе
	...
	// Для возвратной тары
	Запрос.УстановитьПараметр("КурсДокумента", ДокументОбъект.КурсВзаиморасчетов);
	Запрос.УстановитьПараметр("КратностьДокумента", ДокументОбъект.КратностьВзаиморасчетов);

	// Передача параметров в точку вызова
	СуммаАвансаВал = ?(ЗначениеЗаполнено(ВыборкаСуммаВзаиморасчетов), ВыборкаСуммаВзаиморасчетов, 0);
	СуммаАвансаРуб = ?(ЗначениеЗаполнено(ВыборкаСуммаРегл), ВыборкаСуммаРегл, 0);
	Запрос.УстановитьПараметр("СуммаАвансаВал", СуммаАвансаВал);
	Запрос.УстановитьПараметр("СуммаАвансаРуб", СуммаАвансаРуб);
