...
Процедура ЗаполнитьПоляЗаписиСписанияУпр(Движение, СтрокаПартии, СтрокаДокумента, СтруктураПараметров, РегистрУчета, КоэффСписания, КоэффПоступления)
	...
	Если СтрокаДокумента.ОтражатьВУправленческомУчете Тогда
		...
		Если РегистрУчета="НаСкладах" Тогда
			...
			Если СтруктураПараметров.ТипЗначенияРегистратора = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
...
Процедура СписаниеПартий(СтрокаДокумента, СтруктураПараметров, РегистрУчета)
	...
	НачалоГашения = Истина;
	
	Если СтрокаДереваПартий <> Неопределено Тогда
		
		Для Каждого СтрокаПартииРаспределения ИЗ СтрокаДереваПартий.Строки Цикл
			
			Если (КоличествоОсталосьПогасить <= 0 И (Не НачалоГашения)) Или КоличествоОсталосьПогасить = 0 Тогда
				Прервать;
			КонецЕсли;
			
			Если НачалоГашения Тогда 
				НачалоГашения = Ложь;
			КонецЕсли;
	...
	Если КоличествоОсталосьПогасить < 0 И ТипЗнч(СтрокаДокумента.Регистратор) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах") Тогда
		ВыполнитьДвиженияВозвратВРознице(СтрокаДокумента, СтруктураПараметров, РегистрУчета, ИмяРегистра, КоличествоОсталосьПогасить);
	КонецЕсли;
...
Процедура ВыполнитьДвиженияВозвратВРознице(СтрокаДокумента, СтруктураПараметров, РегистрУчета, ИмяРегистра, КоличествоОсталосьПогасить)
	
	Если НЕ СтруктураПараметров.Свойство("ИспользоватьСебестоимостьДляВозврата") ИЛИ НЕ СтруктураПараметров.ИспользоватьСебестоимостьДляВозврата Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(СтрокаДокумента.СебестоимостьУпр) Тогда
		Возврат;
	КонецЕсли;
	...
	Движение.Стоимость  = СтрокаДокумента.СебестоимостьУпр;
	
	// Вспомогательное поле, не являющееся ресурсом, но используемое
	// для формирования записей по переоценке принятого на комиссию.
	Движение.СтоимостьПоступление = СтоимостьПоступлениеОсталосьПогасить;
	...
	Если СтруктураПараметров.ВестиПартионныйУчетПоСкладамУпр Тогда
	...
	СпособОценкиМПЗ = ВРег(СтруктураПараметров.СпособОценкиМПЗУпр);
	Если СпособОценкиМПЗ <> "ПО СРЕДНЕЙ" тогда
		Движение.ДокументОприходования = СтрокаДокумента.Регистратор;
	Иначе
	...
	// Сумма выручки и сумма НДС  - для учета НДС
	Движение.СуммаВыручкиБУ     = СтрокаДокумента.СуммаЗадолженностиБУ;
	Движение.СуммаНДСВыручкиБУ  = СтрокаДокумента.СуммаНДС;
	...
	// Сумма выручки в валюте документа - для отчета комитенту
	Движение.СуммаВыручки = СтрокаДокумента.СуммаЗадолженности;
	// Валюты и курсы - для пересчета
	Движение.ВалютаДокумента = СтрокаДокумента.ВалютаДокумента;
	Движение.КурсДокумента = СтрокаДокумента.КурсДокумента;
	Движение.КратностьДокумента = СтрокаДокумента.КратностьДокумента;
	...
	// Количество поступления - для комплектации
	Движение.КоличествоПоступление    = СтрокаДокумента.КоличествоПоступление;
	СтрокаДокумента.КоличествоПоступление = СтрокаДокумента.КоличествоПоступление - Движение.КоличествоПоступление;
...
Процедура ВыполнитьСписание(СтруктураПараметров, ТаблицаСписания, МоментКон, Останавливаться = Ложь, ПроведениеОстановлено = Ложь )
	...
	ДатаНачалаДвиженийПартийПоВозврату = Константы.ДатаНачалаДвиженийПартийТоваровПоВозвратуВРознице.Получить();
	ИспользоватьСебестоимостьДляВозврата = Ложь;
	Если ЗначениеЗаполнено(ДатаНачалаДвиженийПартийПоВозврату) Тогда
		ИспользоватьСебестоимостьДляВозврата = ДатаНачалаДвиженийПартийПоВозврату <= ТаблицаСписания[0].Период;
	КонецЕсли;
	СтруктураПараметров.Вставить("ИспользоватьСебестоимостьДляВозврата", ИспользоватьСебестоимостьДляВозврата);
...
Процедура ДвижениеПартийТоваров(ОбрабатываемыйДокумент, ТаблицаСписания = Неопределено,
	...
	ДополнитьСписанныеТоварыДаннымиПоСебестоимости(ОбрабатываемыйДокумент, ТаблицаСписания);
...
Процедура ДополнитьСписанныеТоварыДаннымиПоСебестоимости(ОтчетОРозничныхПродажах, ТаблицаСписанныхТоваров)
	
	Если ТипЗнч(ОтчетОРозничныхПродажах) <> Тип("ДокументСсылка.ОтчетОРозничныхПродажах")
		ИЛИ ТипЗнч(ТаблицаСписанныхТоваров) <> Тип("ТаблицаЗначений")
		ИЛИ ТаблицаСписанныхТоваров.Колонки.Найти("СебестоимостьУпр") <> Неопределено
		ИЛИ ТаблицаСписанныхТоваров.Колонки.Найти("НомерСтроки") = Неопределено Тогда
		Возврат;
	КонецЕсли;
		
	Если ОтчетОРозничныхПродажах.СоставНабора.Количество() > 0 Тогда
		ТаблицаСписанныхТоваров.Колонки.Добавить("СебестоимостьУпр", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
		Возврат;
	КонецЕсли;
	...
	СтруктураКурса = МодульВалютногоУчета.ПолучитьКурсВалюты(ВалютаУпрУчета, ОтчетОРозничныхПродажах.Дата);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетОРозничныхПродажахТовары.КлючСтроки КАК КлючСтроки,
	|	ОтчетОРозничныхПродажахТовары.Себестоимость КАК СебестоимостьУпр" + ТекстСписанныеТовары + "
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.Товары КАК ОтчетОРозничныхПродажахТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_СписанныеТовары КАК СписанныеТовары
	|		ПО ОтчетОРозничныхПродажахТовары.НомерСтроки = СписанныеТовары.НомерСтроки
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаТаблицы.СебестоимостьУпр =  МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(Выборка.СебестоимостьУпр, ВалютаРеглУчета,
		                                   ВалютаУпрУчета,
		                                   1, СтруктураКурса.Курс,
		                                   1, СтруктураКурса.Кратность);
										   
		СтрокаТаблицы.СебестоимостьУпр = ?(СтрокаТаблицы.Количество < 0, - СтрокаТаблицы.СебестоимостьУпр, СтрокаТаблицы.СебестоимостьУпр);
