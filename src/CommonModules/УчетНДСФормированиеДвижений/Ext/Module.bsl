...
Процедура СформироватьДвиженияПоРегиструНДСНачисленный_ОтражениеРеализации(СтруктураШапкиДокумента, ТабЧасть, Движения, Отказ, ЭтоВозврат = ложь,УчетнаяПолитикаНУ = Неопределено) Экспорт
	...
	Для Каждого СтрокаВыбытия Из ТабЧасть Цикл
		...
		Если СтрокаВыбытия.СтавкаНДС = Ставка0 
			...
			Если ТаблицаДвижений_НДСРеализация0 = Неопределено Тогда
				...
				ТаблицаДвижений_НДСРеализация0.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
				
			КонецЕсли;
			
			СтрокаДвижения0 = ТаблицаДвижений_НДСРеализация0.Добавить();
			
			ЗаполнитьЗначенияСвойств(СтрокаДвижения0, СтрокаДвижения);
	...
	Если ТаблицаДвижений_НДСНачисленный.Количество() > 0 Тогда
		ПереопределитьВидЦенности(ТаблицаДвижений_НДСНачисленный, СтруктураШапкиДокумента, Истина);
	...
	Если не ТаблицаДвижений_НДСРеализация0 = Неопределено Тогда
		ПереопределитьВидЦенности(ТаблицаДвижений_НДСРеализация0, СтруктураШапкиДокумента);
		Если НЕ ТипЗнч(Движения) = Тип("Структура") тогда
...
// Переопределяет вид ценности для несырьевых товаров и услуг не на территории РФ
//
Процедура ПереопределитьВидЦенности(ТаблицаРеализацийПоСтавке0, СтруктураШапкиДокумента, НДСНачисленный=Ложь)
	
	Если СтруктураШапкиДокумента.Дата < '20160701' Тогда
		Если НЕ НДСНачисленный Тогда
			КолонкиГруппировок = "Активность,МоментВремени,НомерСтроки,ВидДвижения,ВидЦенности,ДатаСобытия,
				|Организация,Период,Покупатель,Регистратор,Событие,Состояние,СтавкаНДС,СчетФактура,СписаниеПартий";
			КолонкиСуммирования = "СуммаБезНДС,НДС";
			ТаблицаРеализацийПоСтавке0.Свернуть(КолонкиГруппировок, КолонкиСуммирования);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	
	ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	КодОперацииПоСделке = Справочники.КодыОперацийРаздела7ДекларацииПоНДС.ПустаяСсылка();
	...
	Если СтруктураШапкиДокумента.Свойство("ДоговорКонтрагента") Тогда
		...
		Если СтруктураШапкиДокумента.ДоговорКонтрагента <> Неопределено Тогда
			...
			КодОперацииПоСделке = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДоговорКонтрагента, "КодРаздел7ДекларацииНДС");
		КонецЕсли;
		
	КонецЕсли;
	
	КодОперацииПоСделкеСоответствуетСт149НКРФ = Ложь;
	Если ЗначениеЗаполнено(КодОперацииПоСделке) Тогда 
		КодОперацииПоСделкеСоответствуетСт149НКРФ = 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(КодОперацииПоСделке, "ОперацияНеПодлежитНалогообложению");
	КонецЕсли;
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("ТаблицаРеализацийПоСтавке0", ТаблицаРеализацийПоСтавке0);
	Запрос.УстановитьПараметр("КодОперацииПоСделке", КодОперацииПоСделке);
	Запрос.УстановитьПараметр("КодОперацииПоСделкеСоответствуетСт149НКРФ", КодОперацииПоСделкеСоответствуетСт149НКРФ);
	...
	Если НЕ НДСНачисленный Тогда
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ТаблицаРеализацийПоСтавке0.ВидДвижения,
		|	ТаблицаРеализацийПоСтавке0.ВидЦенности,
		|	ТаблицаРеализацийПоСтавке0.ДатаСобытия,
		|	ТаблицаРеализацийПоСтавке0.НДС,
		|	ТаблицаРеализацийПоСтавке0.Номенклатура,
		|	ТаблицаРеализацийПоСтавке0.Организация,
		|	ТаблицаРеализацийПоСтавке0.Период,
		|	ТаблицаРеализацийПоСтавке0.Покупатель,
		|	ТаблицаРеализацийПоСтавке0.Регистратор,
		|	ТаблицаРеализацийПоСтавке0.Событие,
		|	ТаблицаРеализацийПоСтавке0.Состояние,
		|	ТаблицаРеализацийПоСтавке0.СписаниеПартий,
		|	ТаблицаРеализацийПоСтавке0.СтавкаНДС,
		|	ТаблицаРеализацийПоСтавке0.СуммаБезНДС,
		|	ТаблицаРеализацийПоСтавке0.СчетФактура
		|ПОМЕСТИТЬ ТаблицаРеализацийПоСтавке0
		|ИЗ
		|	&ТаблицаРеализацийПоСтавке0 КАК ТаблицаРеализацийПоСтавке0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТаблицаРеализацийПоСтавке0.ВидДвижения,
		|	ВЫБОР
		|		КОГДА СвойстваНоменклатуры.Услуга ЕСТЬ NULL 
		|				ИЛИ СвойстваНоменклатуры.Услуга = ИСТИНА
		|			ТОГДА ТаблицаРеализацийПоСтавке0.ВидЦенности
		|		КОГДА КлассификаторТНВЭД.СырьевойТовар = ИСТИНА
		|			ТОГДА ТаблицаРеализацийПоСтавке0.ВидЦенности
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ТоварыНесырьевые)
		...
		|	ТаблицаРеализацийПоСтавке0.СчетФактура,
		|	ТаблицаРеализацийПоСтавке0.СписаниеПартий,
		|	СУММА(ТаблицаРеализацийПоСтавке0.СуммаБезНДС) КАК СуммаБезНДС,
		|	СУММА(ТаблицаРеализацийПоСтавке0.НДС) КАК НДС
		|ИЗ
		|	ТаблицаРеализацийПоСтавке0 КАК ТаблицаРеализацийПоСтавке0
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СвойстваНоменклатуры
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторТНВЭД КАК КлассификаторТНВЭД
		|			ПО СвойстваНоменклатуры.КодТНВЭД = КлассификаторТНВЭД.Ссылка
		|		ПО ТаблицаРеализацийПоСтавке0.Номенклатура = СвойстваНоменклатуры.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаРеализацийПоСтавке0.Организация,
		|	ТаблицаРеализацийПоСтавке0.Событие,
	...
	Иначе
		...
		|	ТаблицаРеализацийПоСтавке0.ВидДвижения,
		|	ТаблицаРеализацийПоСтавке0.ВидЦенности,
		|	ТаблицаРеализацийПоСтавке0.ВидНачисления,
		|	ТаблицаРеализацийПоСтавке0.ДатаСобытия,
		|	ТаблицаРеализацийПоСтавке0.ДоговорАванса,
		|	ТаблицаРеализацийПоСтавке0.ИсправленныйСчетФактура,
		|	ТаблицаРеализацийПоСтавке0.НДС,
		|	ТаблицаРеализацийПоСтавке0.Номенклатура,
		|	ТаблицаРеализацийПоСтавке0.Организация,
		|	ТаблицаРеализацийПоСтавке0.Период,
		...
		|	ТаблицаРеализацийПоСтавке0.ВидДвижения,
		|	ВЫБОР
		|		КОГДА &ДатаДокумента >= ДАТАВРЕМЯ(2019, 7, 1)
		|				И ТаблицаРеализацийПоСтавке0.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС)
		|				И ТаблицаРеализацийПоСтавке0.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги)
		...
		|		КОГДА ТаблицаРеализацийПоСтавке0.СтавкаНДС <> ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС0)
		|			ТОГДА ТаблицаРеализацийПоСтавке0.ВидЦенности
		|		КОГДА СвойстваНоменклатуры.Услуга ЕСТЬ NULL 
		|				ИЛИ СвойстваНоменклатуры.Услуга = ИСТИНА
		|			ТОГДА ТаблицаРеализацийПоСтавке0.ВидЦенности
		...
		|			ПО СвойстваНоменклатуры.КодРаздел7ДекларацииНДС = КодыОперацийРаздела7ДекларацииПоНДС.Ссылка
		|		ПО ТаблицаРеализацийПоСтавке0.Номенклатура = СвойстваНоменклатуры.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаРеализацийПоСтавке0.Организация,
	...
	ТаблицаРеализацийПоСтавке0Переопределенная = Запрос.Выполнить().Выгрузить();
	
	ТаблицаРеализацийПоСтавке0.Очистить();
	
	ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(ТаблицаРеализацийПоСтавке0Переопределенная, ТаблицаРеализацийПоСтавке0);
	
	ИндексНоменклатуры = ТаблицаРеализацийПоСтавке0.Колонки.Найти("Номенклатура");
	Если ИндексНоменклатуры <> Неопределено Тогда
		ТаблицаРеализацийПоСтавке0.Колонки.Удалить(ИндексНоменклатуры);
	КонецЕсли;
...
Процедура СформироватьДвиженияЖурналУчетаСчетовФактурРегистрация(ЗаписиЖурнала, Движения, Отказ) Экспорт
...
Функция КорректировкаПоступленияПодготовитьТаблицуВычетНДСНаОснованииДокумента(СтруктураШапкиДокумента, СтруктураТаблицДокумента)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл 
		СтрокаТаблицы.ВидЦенности = УчетНДС.ОпределитьВидЦенности(
			СтрокаТаблицы.Номенклатура, , , , СтрокаТаблицы.Услуга, СтруктураШапкиДокумента);
	КонецЦикла;	
...
Функция КорректировкаПоступленияПодготовитьТаблицуКорректировкаНДСПоПартиямЗапасов(СтруктураШапкиДокумента, СтруктураТаблицДокумента)
	...
	|			ТОГДА 20 / 120
	|		КОГДА КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22)
	|				ИЛИ КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС22_122)
	|			ТОГДА 22 / 122
	...
	|			ТОГДА 10 / 110 
	|		КОГДА КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
	|				ИЛИ КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
	|			ТОГДА 5 / 105 
	|		КОГДА КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
	|				ИЛИ КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7_107)
	|			ТОГДА 7 / 107
	|		ИНАЧЕ 0
...
//ТЕАЛ НДС ++
////ДОбавил в запросе в двух местах
//|		КОГДА КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5)
//	|				ИЛИ КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС5_105)
//	|			ТОГДА 5 / 105 
//	|		КОГДА КорректировкаПоступленияТовары.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС7)
...
//ТЕАЛ НДС --
	...
	Для Каждого СтрокаТаблицы Из ТаблицаКорректировки Цикл 
		СтрокаТаблицы.ВидЦенности = УчетНДС.ОпределитьВидЦенности(
			СтрокаТаблицы.Номенклатура, , , , Ложь, СтруктураШапкиДокумента);
	КонецЦикла;
...
Процедура КорректировкаРеализацииДвиженияПоРегистрамНДС(СтруктураШапкиДокумента, СтруктураТаблицДокумента, СтруктураТаблицСторноНДС, Движения)
	...
	Если СтруктураШапкиДокумента.СложныйУчетНДС И СтруктураШапкиДокумента.ОтражатьВУправленческомУчете Тогда
...
Функция КорректировкаРеализацииПодготовитьТаблицуКорректировочныеДвиженияНДСПартииЗапасов(СтруктураШапкиДокумента, СтруктураТаблицДокумента)
	...
	|	РегистрНакопления.ПартииТоваровНаСкладах КАК ПартииТоваровНаСкладах
	|ГДЕ
	|	ПартииТоваровНаСкладах.Регистратор = &ДокументРеализацииСсылка
	...
	|	&ИсправленныйСчетФактура КАК ИсправленныйСчетФактура,
	|	ВЫБОР
	|		КОГДА (КорректировкаРеализацииТовары.Номенклатура, КорректировкаРеализацииТовары.ХарактеристикаНоменклатуры, КорректировкаРеализацииТовары.СерияНоменклатуры) В
	|				(ВЫБРАТЬ
	|					ВТ.Номенклатура,
	...
	ТаблицаУменьшение = Результат[2].Выгрузить();
	ТаблицаУвеличение = Результат[3].Выгрузить();
	
	Для Каждого СтрокаТаблицы Из ТаблицаУменьшение Цикл
		СтрокаТаблицы.ВидЦенности = УчетНДС.ОпределитьВидЦенности(
			СтрокаТаблицы.Номенклатура, , , , СтрокаТаблицы.ЭтоУслуга, СтруктураШапкиДокумента);
	КонецЦикла;
...
Процедура ЗарегистрироватьВЖурналеУчетаСчетовФактур(ТаблицаЗаписейЖурнала, Знач СуммаДокументаДляРегистрацииВЖурнале = 0, Знач НДСДокументаДляРегистрацииВЖурнале = 0, Отказ, Заголовок) Экспорт
	...
	Для Каждого СтруктураШапкиДокумента ИЗ ТаблицаЗаписейЖурнала Цикл
		...
		ВерсияКодовВидовОпераций = УчетНДС.ВерсияКодовВидовОпераций(СтруктураШапкиДокумента.Дата);
		ВозможенСоставнойКод     = (ВерсияКодовВидовОпераций < 3);
		
		Если СтруктураШапкиДокумента.СуммаДокументаКомиссия <> 0
			...
			Если СтрДлина(СтруктураШапкиДокумента.КодВидаОперации) >= 4
		...
		Если СтруктураШапкиДокумента.ВидДокумента = "СчетФактураПолученный" Тогда
			...
			РегистрироватьСФДатойСоставления = УчетНДС.ВерсияПостановленияНДС1137(СтруктураШапкиДокумента.Дата) >= 4;
			
			СтрокаДвижения.ЧастьЖурнала = Перечисления.ЧастиЖурналаУчетаСчетовФактур.ПолученныеСчетаФактуры;
			
			Если РегистрироватьСФДатойСоставления Тогда
				Если СтруктураШапкиДокумента.Исправление Тогда
					СтрокаДвижения.ДатаВыставленияПолучения = СтруктураШапкиДокумента.ДатаИсправления;
				Иначе
			...
			Если РасчетыВУсловныхЕдиницах И НЕ СтруктураШапкиДокумента.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыПолученного.Корректировочный Тогда
				...
				КурсДляКомиссииНДС = ?(НДСДокументаДляРегистрацииВЖурнале = 0, 0, ПараметрыСФ.СуммаНДСДокумента/НДСДокументаДляРегистрацииВЖурнале);
				
				СтрокаДвижения.СуммаПоСчетуФактуреКомиссия = СтруктураШапкиДокумента.СуммаДокументаКомиссия * КурсДляКомиссии;
				СтрокаДвижения.СуммаНДСКомиссия            = СтруктураШапкиДокумента.СуммаНДСДокументаКомиссия * КурсДляКомиссииНДС;
		...
		ИначеЕсли СтруктураШапкиДокумента.ВидДокумента = "СчетФактураВыданный" Тогда
			...
			РегистрироватьСФДатойСоставления = УчетНДС.ВерсияПостановленияНДС1137(СтруктураШапкиДокумента.ДатаВыставления) >= 4;
...
Процедура ПересчитатьСчетФактуруВРубли(СчетФактура, СтруктураШапкиДокумента, Результат)
	...
	Для Каждого СтрокаОснования ИЗ СчетФактура.ДокументыОснования Цикл
		...
		Если ТипЗнч(ТекущееОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") 
		...
		УчетНДС.ПерезаполнитьСуммыПоУЕвРублях(ТаблицаДокумента, ПараметрыПересчета, Новый Структура("СуммаНДС", "НДС"));
...
Функция ПолучитьСуммуВзаиморасчетов(СчетФактура, ТекущееОснование, СтруктураШапкиДокумента) Экспорт
	...
	Запрос = Новый Запрос();
	УниверсальныеМеханизмы.ОпределитьКурсыДокументаДляПечати(ИсходныйДокумент, Запрос);
	Курс      = Запрос.Параметры.Курс;
	Кратность = Запрос.Параметры.Кратность;
