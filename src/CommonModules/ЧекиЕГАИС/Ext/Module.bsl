...
Функция ЧекЕГАИСXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц, Операция = Неопределено) Экспорт
	...
	АкцизныеМарки.Колонки.Добавить("ИдентификаторСтроки",  ОпределяемыеТипы().УникальныйИдентификаторИС.Тип);
	...
	Если ИспользоватьФорматОбменаV4 Тогда
		...
		ПустойИдентификаторСтроки = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа(ОпределяемыеТипы().УникальныйИдентификаторИС);
		...
		ТаблицаТовары.Колонки.Добавить("КоличествоЕГАИС",  ОпределяемыеТипы().КоличествоЕГАИСНеотрицательное.Тип);
		ТаблицаТовары.Колонки.Добавить("ЦенаЕГАИС",        ОпределяемыеТипы().ДенежнаяСуммаГосИСНеотрицательная.Тип);
...
Функция ЧекXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц, Операция = Неопределено) Экспорт
	...
	АкцизныеМарки.Колонки.Добавить("Номенклатура",         ОпределяемыеТипы().Номенклатура.Тип);
	АкцизныеМарки.Колонки.Добавить("Характеристика",       ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
	АкцизныеМарки.Колонки.Добавить("Серия",                ОпределяемыеТипы().СерияНоменклатуры.Тип);
	АкцизныеМарки.Колонки.Добавить("КоличествоВПотребительскойУпаковке", ОбщегоНазначения.ОписаниеТипаЧисло(18, 8));
