...
Процедура ЗаполнитьБазовыеПараметрыСканирования(ПараметрыСканирования, Контекст) Экспорт
	...
	Если ТипЗнч(Контекст) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Контекст, "ПараметрыУказанияСерий") Тогда
			...
			ИнтеграцияИСКлиентСерверПереопределяемый.ПриОпределенииПараметровУказанияСерий(
...
Функция GCPизGTIN(Знач Штрихкод) Экспорт
	...
	ШтрихкодEAN = ШтрихкодEANИзGTIN(Штрихкод);
...
Функция ПараметрыОткрытияФормыУточненияДанных() Экспорт
	...
	// Реквизиты продукции ЕГАИС
	ПараметрыОткрытия.Вставить("АлкогольнаяПродукция", Неопределено);
	ПараметрыОткрытия.Вставить("Статус",               Неопределено);	// Статус акцизной марки
...
Функция ВозможнаГрупповаяУпаковкаИлиНабор(ВидУпаковки, ДанныеРазбора) Экспорт
	...
	Для Каждого ВидПродукции Из ДанныеРазбора.ВидыПродукции Цикл
		...
		Если Не ИнтеграцияИСКлиентСерверПовтИсп.ВидПродукцииИспользуетНаборыИлиГрупповыеУпаковки(ВидПродукции) Тогда
			Продолжить;
		КонецЕсли;
...
Функция ЭтоСтрокаДанныхЧастичногоВыбытия(СтрокаДанных, ПараметрыСканирования, ВидПродукции = Неопределено, ПроверятьФлагВыбытия = Ложь) Экспорт
	...
	Если (ИнтеграцияИСКлиентСервер.ЭтоПродукцияИСМП(ВидПродукции, Истина)
...
Функция ИнициализироватьДанныеRFID(СырыеДанные = Неопределено) Экспорт
	...
	Если СырыеДанные <> Неопределено Тогда
		...
		РезультатПроверки = ТребуетсяИндивидуализацияRFID(СырыеДанные, СырыеДанные.GTIN);
...
Функция СтатусRFID(ДанныеRFID, GTIN) Экспорт
	...
	Иначе
		...
		ТребуетсяИндивидуализацияRFID = ТребуетсяИндивидуализацияRFID(ДанныеRFID, GTIN).НужноЗаписатьМетку;
...
Функция ТребуетсяСброситьСохраненныйВыбор(ДанныеВыбора, ДанныеШтрихкода) Экспорт
	...
	ИначеЕсли ЗначениеЗаполнено(ДанныеВыбора.GTIN)
			И (Не ЭтоВнутреннийШтрихкод(ДанныеШтрихкода.GTIN) Или Не ЗначениеЗаполнено(ДанныеШтрихкода.Номенклатура))
