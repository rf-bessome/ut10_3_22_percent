...
Функция РаспределениеШтрихкодовУпаковокПоТоварам(ПараметрыРаспределения, Товары, МассивУпаковок) Экспорт
	...
	ТаблицаТовары.Колонки.Добавить("Упаковка",               ОпределяемыеТипы().Упаковка.Тип);
	...
	ДобавляемыеКолонки.Вставить("Склад",             ОпределяемыеТипы().Склад.Тип);
...
Функция НоваяТаблицаПоискаКодаМаркировкиВПуле(КолонкиВЕТИС = Ложь) Экспорт
	...
	Если КолонкиВЕТИС Тогда
		КодыМаркировки.Колонки.Добавить(
			...
			ОпределяемыеТипы().ИдентификаторПроисхожденияВЕТИС.Тип);
		КодыМаркировки.Колонки.Добавить(
...
Процедура ДополнитьРаспределениеПоТоварамБезШтрихкодов(ВозвращаемоеЗначение, ПараметрыРаспределения, ПараметрыСканирования, Дата, ТаблицаТовары, Товары)
	...
	Если ПараметрыСканирования.Свойство("ПоддерживаетсяОбъемноСортовойУчет") И ПараметрыСканирования.ПоддерживаетсяОбъемноСортовойУчет Тогда
		...
		ТаблицаДанныхОСУ.Колонки.Добавить("ЗначениеШтрихкода",                  ОпределяемыеТипы().GTIN.Тип);
		...
		ТаблицаТовары.Колонки.Добавить("Ссылка",                                ОпределяемыеТипы().ДокументыИСМППоддерживающиеСтатусыОформления.Тип);
