		...
		Если ЗначениеЗаполнено(СообщениеЖурналаРегистрации) Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Ошибка добавления подписи';
					|en = 'Electronic signature. An error occurred while adding a signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		Если ЗначениеЗаполнено(СообщениеЖурналаРегистрации) Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Ошибка удаления подписи';
					|en = 'Electronic signature.Signature deletion error'", ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				Объект.Метаданные(),
				Объект.Ссылка,
				СообщениеЖурналаРегистрации + "
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось прочитать данные подписи: %1';
				|en = 'Cannot read the signature data: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;
			...
			Исключение
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось усовершенствовать подпись: %1';
							|en = 'Cannot enhance the signature: %1'"), КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
				Возврат Результат;
			КонецПопытки;
			...
			Исключение
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Не удалось добавить архивную метку времени в подпись: %1';
							|en = 'Cannot add an archive timestamp to the signature: %1'"), КраткоеПредставлениеОшибки(
					ИнформацияОбОшибке()));
				Возврат Результат;
			КонецПопытки;
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось прочитать данные усовершенствованной подписи: %1';
					|en = 'Cannot read the enhanced signature data: %1'"), КраткоеПредставлениеОшибки(
			ИнформацияОбОшибке()));
		Возврат Результат;
	КонецПопытки;
			...
			Исключение
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При чтении данных подписи: %1';
						|en = 'When reading the signature data: %1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
				Если ВызыватьИсключение Тогда
					ВызватьИсключение;
				КонецЕсли;
				Возврат Ложь;
...
Функция ДобавлениеИзменениеЭлектронныхПодписей() Экспорт
	...
	Возврат ИспользоватьЭлектронныеПодписи() И Пользователи.РолиДоступны("ДобавлениеИзменениеЭлектронныхПодписейИШифрование");
...
Функция ПрочитатьДанныеПодписи(Подпись, ОписаниеОшибки = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(СвойстваПодписиИзДвоичныхДанных.ТипПодписи) Тогда
		...
		ОписаниеОшибки = НСтр("ru = 'Файл не является подписью.';
								|en = 'This is not a signature file.'");
...
Процедура ДобавитьСтрокуПодписи(ОбъектДанных, СвойстваПодписи, СообщениеЖурналаРегистрации)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru = 'Электронная подпись.Добавление подписи';
			|en = 'Electronic signature.Add signature'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
