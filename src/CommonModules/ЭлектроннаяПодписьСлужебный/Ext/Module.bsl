...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("ru = 'Обновление справочника Программы электронной подписи и шифрования.';
			|en = 'Update the ""Digital signature and encryption applications"" catalog.'");
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("8e76369a-e16c-415d-bfeb-95e7e5f07a00");
	Обработчик.Процедура = "Справочники.ПрограммыЭлектроннойПодписиИШифрования.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Справочники.ПрограммыЭлектроннойПодписиИШифрования.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	...
	Обработчик.Комментарий = НСтр("ru = 'Удаление путей из заполненных имен файлов подписей.';
									|en = 'Delete paths from filled signature file names.'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВнешниеКомпоненты")
		...
		Обработчик.Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Добавление внешней компоненты %1 в справочник Внешние компоненты.';
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение настроек для усовершенствования подписей.';
									|en = 'Fill in settings to improve signatures.'");
...
Функция КлассификаторОшибокКриптографииПродолжение() Экспорт
	...
	Возврат ПредставлениеКлассификатораОшибок(
...
Функция АккредитованныеУдостоверяющиеЦентры() Экспорт
	...
	Для Каждого ТекущееЗначениеПоиска Из МассивЗначенийПоиска Цикл
		ЗначениеПоиска = ЭлектроннаяПодписьКлиентСервер.ПодготовитьЗначениеПоиска(ТекущееЗначениеПоиска);
	...
	Если Операция <> "Усовершенствование"
		...
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.БазоваяCAdESBES, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Базовая подпись (%1)
					 ...
					 |becomes invalid after a signer certificate expires (usually within a year).'"),
			"CAdES-BES"));
	КонецЕсли;
		...
		СписокВыбора.Добавить(Перечисления.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Подпись с меткой доверенного времени (%1)
					 ...
					 |is valid after a signer certificate expires.'"), "CAdES-T"));
	КонецЕсли;
...
Функция СохраненныеСвойстваСертификата(Отпечаток, Адрес, ПараметрыРеквизитов, ДляШифрования = Ложь) Экспорт
	...
	Иначе
		...
		Иначе
			ИмяСправочникаОрганизации = "Справочники.Организации";
...
Процедура НастроитьПояснениеВводаПароля(Форма, ИмяЭлементаВводитьПарольВПрограммеЭлектроннойПодписи = "", ИмяЭлементаПояснениеУсиленногоПароля = "") Экспорт
	...
	Если ЗначениеЗаполнено(ИмяЭлементаВводитьПарольВПрограммеЭлектроннойПодписи) Тогда
		...
		Элемент.Заголовок = НСтр("ru = 'Вводить пароль в программе электронной подписи';
								|en = 'Enter password in the electronic signature application'");
		Элемент.Подсказка =
			НСтр("ru = '- Включается интерактивный режим работы программы электронной подписи,
			           ...
			           |en = '- Interactive mode of the electronic signature application is enabled,
...
Процедура ЗарегистрироватьУсовершенствованиеПодписиВЖурнале(СвойстваПодписи) Экспорт
	...
	ИмяСобытия = НСтр("ru = 'Электронная подпись.УсовершенствованиеПодписи';
						|en = 'Digital signature. ImproveSignatures'",
		ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьОшибкуУсовершенствованияПодписиВЖурнале(ОписаниеОшибки, СвойстваПодписи = Неопределено) 
	...
	ИмяСобытия = НСтр("ru = 'Электронная подпись.Ошибка усовершенствования подписи';
						|en = 'Digital signature. Signature improving error'",
			ОбщегоНазначения.КодОсновногоЯзыка());
	ЗаписьЖурналаРегистрации(ИмяСобытия,
		УровеньЖурналаРегистрации.Информация,
		ПодписанныйОбъектМетаданные,
...
Функция ОбновитьУсовершенствованнуюПодпись(СвойстваПодписи) Экспорт
	...
	Исключение
		...
		ПредставлениеОшибки = НСтр("ru = 'При записи подписи возникла ошибка:';
									|en = 'An error occurred when saving the signature:'")
...
Функция УсовершенствоватьПодписьВСервисе(Подпись, ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ТипПодписи <> Перечисления.ТипыПодписиКриптографии.СМеткойДоверенногоВремениCAdEST Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Невозможно усовершенствовать подпись в приложении в Интернете до типа: %1.';
				|en = 'Cannot enhance the signature in the web application to type: %1.'"), 
	...
	Исключение
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|%1';
			|en = 'An error occurred when reading data of the signature to be enhanced in the web application:
	...
	Если РезультатПроверкиСертификата.Выполнено = Ложь Или РезультатПроверкиСертификата.Результат <> Истина Тогда
		...
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при проверке сертификата в приложении в Интернете, подробности см. в журнале регистрации:
				|%1';
				|en = 'An error occurred when checking the certificate in the web application. See details in the event log:
				|%1'"), СведенияОСертификате);
...
Процедура ПеренестиНастройкиМенеджераКриптографии() Экспорт
	...
	Попытка
		...
		Если Константы.ИспользоватьЭлектронныеЦифровыеПодписи.Получить()
		...
		// 2 механизма ЭП: ОчиститьКонстанту(Константы.УдалитьПровайдерЭП);
		// 2 механизма ЭП: ОчиститьКонстанту(Константы.УдалитьТипПровайдераЭП);
		// 2 механизма ЭП: ОчиститьКонстанту(Константы.УдалитьАлгоритмПодписи);
		// 2 механизма ЭП: ОчиститьКонстанту(Константы.УдалитьАлгоритмХеширования);
		// 2 механизма ЭП: ОчиститьКонстанту(Константы.УдалитьАлгоритмШифрования);
...
Функция Подписать(Знач КонвертXML, ПараметрыXMLDSig, СертификатКриптографии, МенеджерКриптографии) Экспорт
	...
	СертификатКриптографииBase64 = СертификатКриптографииBase64(
...
Функция ОбъектВнешнейКомпонентыXMLDSig()
	...
	ОписаниеКомпоненты = ЭлектроннаяПодписьСлужебныйКлиентСервер.ОписаниеКомпоненты();
	ОбъектКомпоненты = ОбщегоНазначения.ПодключитьКомпонентуИзМакета(ОписаниеКомпоненты,
		ОписаниеКомпоненты.ПолноеИмяМакета);
...
Функция СтараяПрограмма()
	...
	ИмяПрограммы = СокрЛП(Константы.ПровайдерЭЦП.Получить());
	ТипПрограммы = Константы.ТипПровайдераЭЦП.Получить();
	...
	ПрограммаОбъект.АлгоритмПодписи     = Константы.АлгоритмПодписи.Получить();
	ПрограммаОбъект.АлгоритмХеширования = Константы.АлгоритмХеширования.Получить();
	ПрограммаОбъект.АлгоритмШифрования  = Константы.АлгоритмШифрования.Получить();
...
Процедура ЗаполнитьСуществующиеСертификатыПользователя(СписокВыбора, ОтпечаткиСертификатовНаКлиенте,
	...
	Если ТипЗнч(ОтборСертификатов) = Тип("СписокЗначений") Тогда
		Если ОтборСертификатов.Количество() > 0 Тогда
			...
			Возврат;
		КонецЕсли;
	ИначеЕсли ОпределяемыеТипы().Организация.Тип.СодержитТип(ТипЗнч(ОтборСертификатов)) Тогда
...
Функция КлассификаторОшибокКриптографии() Экспорт
		...
		Если ЗначениеЗаполнено(ТекстОшибки) Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Электронная подпись.Обновление классификатора ошибок';
					|en = 'Electronic signature.Error classifier update'",
				ОбщегоНазначения.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,,,
				Комментарий);
		КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ДанныеКлассификатора)
		...
		ДанныеКлассификатора = КлассификаторОшибокКриптографииПродолжение();
...
Функция ПрочитатьАрхивКлассификатора(Знач Архив, Версия = 0)
	...
	Если ИмяФайлаПериодыДействия <> Неопределено Тогда
		...
		Для Каждого УЦ Из СписокУЦ Цикл
			...
			Наименование = ЭлектроннаяПодписьКлиентСервер.ПодготовитьЗначениеПоиска(УЦ.Наименование);
			КраткоеНаименование = ЭлектроннаяПодписьКлиентСервер.ПодготовитьЗначениеПоиска(УЦ.КраткоеНаименование);
	...
	Если Не ЭлектроннаяПодпись.СоздаватьЭлектронныеПодписиНаСервере()
		...
		ТехническаяИнформация = ТехническаяИнформация + Символы.ПС
			+ НСтр("ru = 'Электронная подпись на сервере не используется.';
					|en = 'Electronic signature is not used on the server.'") + Символы.ПС;
	Иначе
...
Функция ТехническаяИнформацияОПрограммах()
	...
	ДиагностическаяИнформация = Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Программы на сервере ""%1"":';
			|en = 'Applications on server ""%1"":'"), ИмяКомпьютера()) + Символы.ПС;
	...
	Если ИспользуемыеПрограммы.Количество() = 0 Тогда
		ДиагностическаяИнформация = ДиагностическаяИнформация
			+ НСтр("ru = 'В справочник не добавлено ни одной программы.';
					|en = 'No applications were added to the catalog.'");
		Возврат ДиагностическаяИнформация;
	КонецЕсли;
			...
			Если БлокировкаВыполняется Тогда // Список отзыва находится в процессе обновления.
				Если Не ЗначениеЗаполнено(ДанныеИзКэша) Тогда
					РезультатЗагрузки.СообщениеОбОшибке = НСтр("ru = 'Список отзыва находится в процессе обновления.';
																|en = 'Revocation list is updating.'");
				КонецЕсли;
			Иначе
				ИнформацияОбОшибке = ИнформацияОбОшибке();
...
Процедура УстановитьСписокОтзываСертификата(Сертификат, Адреса, АдресВнутренний)
	...
	Если Не ЗначениеЗаполнено(Адреса) Тогда
		ВызватьИсключение НСтр("ru = 'Не указаны адреса списков отзыва.';
								|en = 'Revocation list addresses are not specified.'");
	Иначе
