...
Функция АдресСпискаОтзываВнутренний(ИмяИздателя, Сертификат) Экспорт
	...
	Возврат ЭлектроннаяПодписьКлиентСервер.АдресСпискаОтзываВнутренний(ИмяИздателя, Сертификат);
...
Функция ЗаписатьСертификатВСправочник(Контекст, Ошибка) Экспорт
	...
	Исключение
		...
		Ошибка.ЗаголовокОшибки = НСтр("ru = 'Не удалось записать сертификат по причине:';
										|en = 'Cannot save the certificate due to:'");
...
Функция ПредставлениеСертификата(Сертификат, ДобавкаВремени) Экспорт
	...
	Представление = "";
	ЭлектроннаяПодписьКлиентСервер.ПриПолученииПредставленияСертификата(Сертификат, ДобавкаВремени, Представление);
...
Функция ПредставлениеСубъекта(Сертификат) Экспорт 
	...
	Представление = "";
	ЭлектроннаяПодписьКлиентСервер.ПриПолученииПредставленияСубъекта(Сертификат, Представление);
	Если ПустаяСтрока(Представление) Тогда
...
Функция СвойстваСубъектаСертификата(Сертификат) Экспорт
	...
	ЭлектроннаяПодписьКлиентСервер.ПриПолученииРасширенныхСвойствСубъектаСертификата(Субъект, РасширенныеСвойства);
...
Функция СвойстваИздателяСертификата(Сертификат) Экспорт
	...
	РасширенныеСвойства = Неопределено;
	ЭлектроннаяПодписьКлиентСервер.ПриПолученииРасширенныхСвойствИздателяСертификата(Издатель, РасширенныеСвойства);
	Если ТипЗнч(РасширенныеСвойства) = Тип("Структура") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСтруктуру(Свойства, РасширенныеСвойства, Истина);
	КонецЕсли;
...
Функция ОписаниеКомпоненты() Экспорт
	...
	Параметры.Вставить("ПолноеИмяМакета",
		"Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Макет.КомпонентаExtraCryptoAPI_3_0_2_24");
	Возврат Параметры;
...
Функция ПобитовыйСдвигВправо(СдвигаемоеЧисло, Смещение)
	
	СдвинутоеЧисло = СдвигаемоеЧисло;
	
	Для Сч = 1 По Смещение Цикл
		СдвинутоеЧисло = Цел(СдвинутоеЧисло / 2);
	КонецЦикла;
	
	Возврат СдвинутоеЧисло;
КонецФункции
