...
Функция ПолучитьАктуальныеВидыЭД() Экспорт
	...
	МассивЭД.Добавить(Перечисления.ВидыЭД.МашиночитаемаяДоверенность);
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		МодульОбменСГИСЭПД.ПолучитьАктуальныеВидыЭД(МассивЭД);
	КонецЕсли;
...
// См. ЭлектронныеДокументыВнутренний.ПолучитьСоединение
//
Функция ПолучитьСоединение(СпособОбменаЭД, Таймаут = 30) Экспорт
	...
	Если СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезОператораЭДОТакском Тогда
		НастройкиСервисаЭДО = ЭлектронныеДокументыВнутренний.СтруктураНастроекТакском();
	
	ИначеЕсли СпособОбменаЭД = Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО Тогда
		НастройкиСервисаЭДО = ЭлектронныеДокументыВнутренний.СтруктураНастроек1СЭДО();
		
	Иначе
		НастройкиСервисаЭДО = ЭлектронныеДокументыВнутренний.СтруктураНастроекСервисаНастроек1С();
	
	КонецЕсли;
	
	ЭлектронныеДокументыВнутренний.ОпределитьПараметрыСайта(
...
Функция ПреобразованиеXSL_ПараметрыПроизвольногоДокумента() Экспорт
	...
	ТекстПравила = ЭлектронныеДокументыСлужебный.ТекстПравилаПреобразованияФормата(Параметры);
...
Функция ФабрикаXDTOЭлектронногоДокумента(ФорматЭД) Экспорт
	...
	ТекстСхемыXML = ЭлектронныеДокументыСлужебный.ТекстПравилаПреобразованияФормата(ПараметрыСхемыXML);
...
Функция СтавкаНДСИзПредставления(ПредставлениеСтавкиНДС) Экспорт
	...
	Если СтрСтавкаНДС = Неопределено Тогда
		Возврат Неопределено;
	ИначеЕсли Найти("БЕЗ НДС", ВРег(СтрСтавкаНДС)) > 0 Тогда
		ЗначениеНДС = "без НДС";
	Иначе
...
Функция КНДПоВидуЭД(СвойстваДокумента) Экспорт
	...
	Если ВидЭД = Перечисления.ВидыЭД.СчетФактура Тогда
		Если ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ЭСФ Тогда
			КНД = "1115101";
		ИначеЕсли Найти(ИмяФайла, "ON_NSCHFDOPPR") > 0 Тогда
			КНД = "1115131";
	...
	Если ВидЭД = Перечисления.ВидыЭД.КорректировочныйСчетФактура Тогда
		Если ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ЭСФ Тогда
			КНД = "1115108";
		ИначеЕсли Найти(ИмяФайла, "ON_NKORSCHFDOPPR") > 0 Тогда
			КНД = "1115133";
			Если Подтверждение Тогда
				КНД = "1115134";
			КонецЕсли;
		ИначеЕсли Найти(ИмяФайла, "ON_NKORSCHFDOPPOK") > 0 Тогда
			КНД = "1115134";
		Иначе
			КНД = "1115127";
			Если Подтверждение Тогда
	...
	ИначеЕсли ВидЭД = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель 
		...
		Если Найти(ИмяФайла, "ON_NKORSCHFDOPPR") > 0 Тогда
			КНД = "1115133";
			Если Подтверждение Тогда
				КНД = "1115134";
			КонецЕсли;
...
Функция ПредставлениеОснованияДляВидаЭД(ВидЭД) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		МодульОбменСГИСЭПД = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ОбменСГИСЭПД");
		МодульОбменСГИСЭПД.СоответствиеИсходящихВидовЭДДокументамИБ(СоответствиеВидовЭДДокументамИБ);	
	КонецЕсли;
	
	// Электронное взаимодействие
	СоответствиеВидовЭДДокументамИБ.Вставить(Перечисления.ВидыЭД.ПроизвольныйЭД, НСтр("ru = 'Исходящий произвольный документ'"));
	
	СоответствиеВидовЭДДокументамИБ.Вставить(Перечисления.ВидыЭД.МашиночитаемаяДоверенность, НСтр("ru = 'Машиночитаемая доверенность'"));
...
Функция ДанныеФайлаОблачногоХранилищаНастроек(ПутьКФайлу, ВидОперации = "") Экспорт
	...
	Возврат ЭлектронныеДокументыСлужебный.ДанныеФайлаОблачногоХранилищаНастроек(ПутьКФайлу, ВидОперации);
