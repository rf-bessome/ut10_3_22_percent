...
Функция ПроверитьИспользованиеИнтернетПоддержкаПользователей() Экспорт
	...
	Если Использование = Неопределено Тогда
		...
		Использование = Ложь;
		Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
			Попытка
				МодульИнтернетПоддержки = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("ИнтернетПоддержкаПользователейКлиент");
...
Процедура ОткрытьЭДДляПросмотра(СсылкаНаЭД, ПараметрыОткрытия = Неопределено, ВладелецФормы = Неопределено, ОткрытьАктуальныйЭД = Ложь) Экспорт
	...
	ИмяФормыЭлектронногоДокумента = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьИмяФормыЭД(СсылкаНаЭД);
	...
	Если ЗначениеЗаполнено(СсылкаНаЭД) Тогда
		Если ТипЗнч(СсылкаНаЭД) = Тип("СправочникСсылка.ЭДПрисоединенныеФайлы") Тогда
			СтруктураЭД = ЭлектронныеДокументыСлужебныйВызовСервера.СтруктураСодержимогоЭД(СсылкаНаЭД);
	...
	ПараметрыФормы = Новый Структура("Ключ, МассивОтпечатков", ВладелецЭД, МассивСтруктурСертификатов);
...
Функция ТестСвязиСОператоромЭДО(СтруктураПараметровЗапросаМаркера, ВыводитьСообщения = Истина, ПараметрыАутентификации = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Маркер) Тогда
		...
		СтруктураПараметровЗапросаМаркера.Вставить("МаркерРасшифрованный", РасшифрованныеДвоичныеДанные);
...
Функция ПроверитьДоступностьКаталогаДляПрямогоОбмена(ПутьККаталогу) Экспорт
	...
	Если ЗначениеЗаполнено(ПутьККаталогу) Тогда
		...
		Если НЕ КаталогиДоступны Тогда
			ШаблонСообщения = НСтр("ru = 'Указанный каталог ""%1"" не может использоваться для обмена, так как он недоступен с сервера.
...
Процедура ОткрытьИнструкциюПо1СБухфон() Экспорт
	...
	ПопытатьсяПерейтиПоНавигационнойСсылке("https://1c-connect.com/ru/forcustomers/");
...
Процедура ОткрытьДеревоЭД(СсылкаНаОбъект, ПараметрыОткрытия = Неопределено, ЭД = Неопределено) Экспорт
	...
	Иначе
		...
		ФормаДерева = ПолучитьФорму("Обработка.ЭлектронныеДокументы.Форма.ДеревоЭД", ПараметрыФормы,
...
// Возвращает сылку для обращения в службу технической поддержки по ЭДО.
// 
// Возвращаемое значение:
// Строка - ссылка для обращения в службу технической поддержки.
//
Функция СсылкаДляОбращенияВСлужбуПоддержки() Экспорт
	
	Возврат "http://1c-edo.ru/handbook";
	
КонецФункции

Функция ДоступныеВерсииФорматаПоПараметрамЭД(ИсходящийДокумент, СпособОбменаЭД, ИспользоватьУПД, ИспользоватьУКД) Экспорт 
	ДействующиеФорматы = ЭлектронныеДокументыСлужебныйВызовСервера.ДействующиеФорматы(ИсходящийДокумент);
	
	СписокВыбора = Новый СписокЗначений;
	Для Каждого ДанныеФормата Из ДействующиеФорматы Цикл
		СписокВыбора.Добавить(ДанныеФормата.Идентификатор, ДанныеФормата.Представление);
	КонецЦикла;
	...
	ИначеЕсли ИсходящийДокумент = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		Если ЭлектронныеДокументыСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
			...
			ФорматыДляУдаления.Добавить("ДИСУКД2020");
		КонецЕсли;
	КонецЕсли;
...
Процедура ТестНастроекСертификата(
	...
	Если ПараметрыСертификата.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru = 'Сертификат помечен на удаление.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Функция ОбработатьОтклонениеАннулированиеЭД(СсылкаНаЭД, ПараметрыЭД, ОтклонитьАннулирование = Ложь) Экспорт
	...
	Если ПараметрыЭД.Отклонить Тогда
		ФормироватьЭД = Ложь;
		ПараметрыОбработкиЭД = ЭлектронныеДокументыСлужебныйВызовСервера.ПараметрыОтклоненияЭД(МассивЭД, ФормироватьЭД);
	...
	Иначе
		...
		ПараметрыОбработкиЭД = ЭлектронныеДокументыСлужебныйВызовСервера.ПараметрыАннулированияЭД(МассивЭД);
	...
	Если ПараметрыОбработкиЭД.Количество() > 0 Тогда
		...
		Если УточнениеВведено Тогда
			Если ЗначениеЗаполнено(ТекстУточнения) Тогда
				...
				Если Не ПараметрыОбработкиЭД[0].ФормироватьУОУ Тогда
		...
		Для Каждого ТекДокумент Из ДобавленныеФайлы Цикл
			Если ЭлектронныеДокументыКлиентПереопределяемый.ЭлектронныйДокументГотовКПодписи(ТекДокумент) Тогда
				...
				Попытка     
					...
					ДвоичныеДанныеФайла = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьДвоичныеДанныеФайла(ТекДокумент, СертификатПодписи, Доверенность);
					...
					РезультатПроверкиМЧД =  ЭлектронныеДокументыСлужебныйВызовСервера.ОпределитьСтатусыПодписейПоМЧДПриПодписании(ТекДокумент, ДанныеПодписи, Доверенность);
					...
					Если Не РезультатПроверкиМЧД.ТребуетсяДоверенность Или РезультатПроверкиМЧД.ПодписьВерна Тогда
						...
						ЭлектронныеДокументыСлужебныйВызовСервера.ДобавитьПодпись(ТекДокумент, ДанныеПодписи, Доверенность);
						КоличествоПодписанныхЭД = КоличествоПодписанныхЭД + 1;
						
						ХешПодписи = МашиночитаемыеДоверенностиВызовСервера.ХешПодписи(ДанныеПодписи.НоваяПодписьДвоичныеДанные);
...
Функция ПодготовитьИОтправитьПЭД(ДобавленныеФайлы,
	ПризнакПодписи, СоотвСоглашенийИПараметровСертификатов = Неопределено) Экспорт
	...
	ПакетыКПовторнойОтправке = Неопределено;
	
	СтруктураРезультата = ЭлектронныеДокументыСлужебныйВызовСервера.СоздатьИОтправитьДокументыПЭД(
		ДобавленныеФайлы, ПризнакПодписи, СоотвСоглашенийИПараметровСертификатов, ПакетыКПовторнойОтправке);
	
	Если ПакетыКПовторнойОтправке <> Неопределено И ПакетыКПовторнойОтправке.Количество() Тогда
		
		// Маркер истек, требуется отправить документы заново.
		НовСоотвСоглашенийИСтруктурСертификатов = Новый Соответствие;
		Для Каждого Элемент ИЗ СоотвСоглашенийИПараметровСертификатов Цикл
		...
		СтруктураРезультата = ЭлектронныеДокументыСлужебныйВызовСервера.СоздатьИОтправитьДокументыПЭД(
			ДобавленныеФайлы, ПризнакПодписи, НовСоотвСоглашенийИСтруктурСертификатов, ПакетыКПовторнойОтправке);
	...
	Если СтруктураРезультата.МассивПакетовДляОбработкиНаКлиенте.Количество() > 0 Тогда
		...
		ЭлектронныеДокументыСлужебныйВызовСервера.СохранитьИОтправитьЗашифрованныеДанные(
			СоответствиеДанныхПакетов, СоотвСоглашенийИПараметровСертификатов, КолОтправленных);
	...
	Если СтруктураРезультата.Свойство("ДанныеДляОтправкиСКлиента")
		...
		ОтправитьПакетыЧерезДопОбработку(СтруктураРезультата.ДанныеДляОтправкиСКлиента, КоличествоОтправленныхПакетовЧерезДопОбработку);
		СтруктураРезультата.КолОтправленных = СтруктураРезультата.КолОтправленных + КоличествоОтправленныхПакетовЧерезДопОбработку;
...
//
Процедура ОпределитьСтатусыПодписей(ЭД, ХешПодписи = Неопределено) Экспорт
	...
	Если ВыполнятьКриптооперацииНаСервере Тогда
		ЭлектронныеДокументыСлужебныйВызовСервера.ОпределитьСтатусыПодписей(ЭД, Истина, ХешПодписи);
	...
	Для Каждого СтрокаЭЦП Из СтруктураСодержимогоЭД.Подписи Цикл
		СтруктураЗаписи = Новый Структура("НомерСтроки", СтрокаЭЦП.ПорядковыйНомер);
		Попытка
			ДвоичныеДанныеЭЦП = СтрокаЭЦП.Подпись;
			Сертификат = Неопределено;
			
			Если ЗначениеЗаполнено(ХешПодписи) 
				И ХешПодписи <> МашиночитаемыеДоверенностиВызовСервера.ХешПодписи(ДвоичныеДанныеЭЦП) Тогда
				Продолжить;
			КонецЕсли;
			
			МенеджерКриптографии.ПроверитьПодпись(СтруктураСодержимогоЭД.ДанныеЭД, СтрокаЭЦП.Подпись, Сертификат);
		...
		Если Сертификат = Неопределено Тогда
			Сертификаты = МенеджерКриптографии.ПолучитьСертификатыИзПодписи(СтрокаЭЦП.Подпись);
			Сертификат = ЭлектронныеДокументыКлиентСервер.СертификатПодписавшейСтороны(Сертификаты);
		КонецЕсли;
		
		Если Сертификат <> Неопределено Тогда
			КомуВыданСертификат = ЭлектроннаяЦифроваяПодписьКлиентСервер.ПолучитьПредставлениеПользователя(Сертификат.Субъект);
			СтруктураЗаписи.Вставить("КомуВыданСертификат", КомуВыданСертификат);
			СтруктураЗаписи.Вставить("Отпечаток", Base64Строка(Сертификат.Отпечаток));
			СтруктураЗаписи.Вставить("ДвоичныеДанныеСертификата", Сертификат.Выгрузить());
		КонецЕсли;
	...
	ЭлектронныеДокументыСлужебныйВызовСервера.ОпределитьСтатусыПодписейПоМЧД(ЭД, ХешПодписи);
...
Процедура ОбработатьЭД(Знач МассивСсылокНаОбъект, Действие, ДопПараметры = "", ЭД = Неопределено) Экспорт
	...
	Если ЭлектронныеДокументыКлиентСервер.ЕстьДействие(Действие, "Утвердить") Тогда
		ОбрабатываемыйЭД = Неопределено;
		Если ЗначениеЗаполнено(ЭД)
			И ТипЗнч(ЭД) <> Тип("Массив") Тогда 
			ОбрабатываемыйЭД = ЭД;
		ИначеЕсли ЗначениеЗаполнено(ЭД)
			И ЭД.Количество() = 1 Тогда
			ОбрабатываемыйЭД = ЭД[0];
		ИначеЕсли МассивСсылокНаОбъект.Количество() = 1 Тогда
			Если ТипЗнч(МассивСсылокНаОбъект[0]) = Тип("СправочникСсылка.ЭДПрисоединенныеФайлы") Тогда
				ОбрабатываемыйЭД = МассивСсылокНаОбъект[0];
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ОбрабатываемыйЭД) Тогда
			
			//Уточним нужно ли проверять МЧД при утверждении Входящих ЭД, но только если обрабатывается один документ
			ПодписиСОшибкамиМЧД = ЭлектронныеДокументыСлужебныйВызовСервера.ПодписиСОшибкамиМЧД(ОбрабатываемыйЭД);			
			НеУдалосьИсправитьОшибкиМЧД = Ложь;
			Для Каждого ПодписьСОшибкамиМЧД Из ПодписиСОшибкамиМЧД Цикл				
				Если ПодписьСОшибкамиМЧД.ОшибкаПроверкиПолномочий Тогда
					Если НЕ ВыполнитьПроверкуПолномочийПриУтверждении(ПодписьСОшибкамиМЧД.ПодписанныйОбъект, ПодписьСОшибкамиМЧД.ХешПодписи) Тогда
						НеУдалосьИсправитьОшибкиМЧД = Истина;
						Прервать;
					КонецЕсли;
				Иначе
					Если ВыполнятьПроверкуМЧДПриУтверждении(ОбрабатываемыйЭД) Тогда
						НеУдалосьИсправитьОшибкиМЧД = Истина;
						Прервать;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;			
			Если НеУдалосьИсправитьОшибкиМЧД Тогда
				ЭлектронныеДокументыСлужебныйВызовСервера.УстановленныеПодписиВалидны(ОбрабатываемыйЭД);
				Возврат;				
			КонецЕсли;
			
			
			//Проверим требуется ли от пользователя ручной ввод данных перед утверждением и созданием ответного титула, например для акта о расхождениях
			Сообщение = "";
			ТребуетсяВводДопДанных = ЭлектронныеДокументыСлужебныйВызовСервера.ПоДокументуТребуетсяВводДопДанных(ОбрабатываемыйЭД, Сообщение);
			Если ТребуетсяВводДопДанных Тогда
				ДопДанные = ПоказатьФормуВводаДопДанных(ОбрабатываемыйЭД);
				
				Если ЗначениеЗаполнено(ДопДанные) Тогда 
					ДопПараметры.Вставить("ДопДанные", ДопДанные);
				Иначе
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		СоотвСертификатовИИхСтруктур = Неопределено;
		ОписаниеНабораДанныхМЧД = Неопределено;
		Если Результат.Свойство("СоотвСертификатовИИхСтруктур", СоотвСертификатовИИхСтруктур)
			...
			Результат.Свойство("ОписаниеНабораДанныхМЧД", ОписаниеНабораДанныхМЧД);
			...
			Для Каждого Элемент Из СтруктСтруктурМассивовЭДиСертификатов Цикл
				...
				ПараметрыПодписания.Вставить("ВыбранныйСертификат");
				
				РезультатыПодписанияПоДоверенности = Новый Структура;
				РезультатыПодписанияПоДоверенности.Вставить("ПеревыборСертификата", Ложь);
				РезультатыПодписанияПоДоверенности.Вставить("ОшибкиПроверкиПолномочий", Новый Массив);
				...
				МЧДПоЭДИСертификату = ОписаниеНабораДанныхМЧД.ПодобранныеДоверенности[СертификатПодписи];
				ПараметрыПодписания.ВыбранныйСертификат = СертификатПодписи;
				// удалим те документы по которым требуется доверенность, но подобрать её не удалось
				ЭДСОшибкойПодбораДоверенности = ОписаниеНабораДанныхМЧД.ОшибкиПодбораДоверенности[СертификатПодписи];
				Если ЭДСОшибкойПодбораДоверенности <> Неопределено Тогда
					Для Каждого КлючЗначение Из ЭДСОшибкойПодбораДоверенности Цикл
						ЭДСОшибкойПодбораМЧД = КлючЗначение.Ключ;
						ОшибкиПодбораМЧД = КлючЗначение.Значение;
						ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(МассивЭД, ЭДСОшибкойПодбораМЧД);
						МассивЭДКУдалениюИзОтправки.Добавить(ЭДСОшибкойПодбораМЧД);
						
						РезультатыПодписанияПоДоверенности.ПеревыборСертификата = Истина;
						Для Каждого ОшибкаПодбораМЧД Из ОшибкиПодбораМЧД Цикл
							Если ТипЗнч(ОшибкаПодбораМЧД) = Тип("Структура") Тогда
								РезультатыПодписанияПоДоверенности.ОшибкиПроверкиПолномочий.Добавить(ОшибкаПодбораМЧД);
							КонецЕсли;
						КонецЦикла;
											
					КонецЦикла;
					
					ОбработатьРезультатПодписания(ПараметрыПодписания, РезультатыПодписанияПоДоверенности);
				КонецЕсли;
				...
				Если МассивЭД.Количество() > 0 Тогда
					Если НЕ ВыполнятьКриптооперацииНаСервере Тогда
						КолПодписанных = ПодписатьЭДОпределеннымСертификатом(МассивЭД,
							СертификатПодписи, СтруктураСертификата, МассивЭДКУдалениюИзОтправки, МЧДПоЭДИСертификату);
					Иначе
			...
			Если ВыполнятьКриптооперацииНаСервере И СоотвСертификатовИМассивовЭДКПодписи.Количество() > 0 Тогда
				РезультатПодписания = ЭлектронныеДокументыСлужебныйВызовСервера.ПодписатьЭДНаСервере(СоотвСертификатовИМассивовЭДКПодписи,
					СоотвСертификатовИИхСтруктур, МассивЭДКУдалениюИзОтправки, ОписаниеНабораДанныхМЧД.ПодобранныеДоверенности);
	...
	Если Результат <> Неопределено И Результат.Свойство("СообщенияПользователю") Тогда
		Для Каждого СообщениеПользователю Из Результат.СообщенияПользователю Цикл
			Сообщение = Новый СообщениеПользователю;
			ЗаполнитьЗначенияСвойств(Сообщение, СообщениеПользователю);
			Сообщение.Сообщить();
		КонецЦикла;
	КонецЕсли;
...
Процедура ОтправитьПолучитьЭлектронныеДокументы() Экспорт
	...
	ПараметрыОтправкиПолученияЭД = ЭлектронныеДокументыСлужебныйВызовСервера.ПараметрыОтправкиПолученияЭД();
	...
	Если ПараметрыОтправкиПолученияЭД.Свойство("ПрофилиЭДОБезСертификатов") Тогда
		...
		Для Каждого ПрофильЭДО Из ПараметрыОтправкиПолученияЭД.ПрофилиЭДОБезСертификатов Цикл
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, ПрофильЭДО);
	...
	Для каждого КлючЗначение Из СоотвСоглашенийИСтруктурСертификатов Цикл
		НаборПрофилей.Добавить(КлючЗначение.Ключ);
	КонецЦикла;
	Если Не ПроверитьПринятиеУсловийИспользования(НаборПрофилей) Тогда
		Возврат;
	КонецЕсли;
	
	// Блок обновления статусов настроек ЭДО и получения новых приглашений.
	ОбновитьСтатусыНастроекЭД(СоотвСоглашенийИСтруктурСертификатов);
	...
	ЭдДляФормированияИзвещений = Новый Массив; // здесь будут ЭД, по котрым нужно сформировать извещения
	Для Каждого СтруктураРаспаковки Из НовыеДокументы.ПараметрыРаспаковки Цикл
		...
		РаспаковатьПакетЭДНаКлиенте(СтруктураРаспаковки.ПакетЭД, СтруктураШифрования, ДанныеРаспаковки, ЭдДляФормированияИзвещений);
	КонецЦикла;
	
	// сформируем и отправим извещения
	Если ЭдДляФормированияИзвещений.Количество() Тогда
		ЭлектронныеДокументыКлиентСервер.СформироватьПодписатьИОтправитьСлужебныеЭД(
			ЭдДляФормированияИзвещений, ПредопределенноеЗначение("Перечисление.ВидыЭД.ИзвещениеОПолучении"));
	КонецЕсли;
...
// Обновляет статусы настроек ЭДО и получает новые приглашения.
//
Процедура ОбновитьСтатусыНастроекЭД(СоотвСоглашенийИСтруктурСертификатов = Неопределено) Экспорт
	
	Оповестить = Ложь;
	Если СоотвСоглашенийИСтруктурСертификатов = Неопределено Тогда
		Оповестить = Истина;
		СоотвСоглашенийИСтруктурСертификатов = СоотвСоглашенийИСтруктурСертификатовДляАвторизации();
	КонецЕсли;
...
// Получает от сервера 1С-ЭДО настройки уведомлений
//
Процедура ЗаполнитьСвойстваУведомленийЭДО(СвойстваУведомлений, СертификатыПрофиля, СвойстваПрофиля) Экспорт
	...
	Если Не ЭлектронныеДокументыСлужебныйКлиент.ПарольКСертификатуПолучен(СертификатыПрофиля, ВидОперации) Тогда
		Возврат;
	КонецЕсли;
	
	СвойстваСертификата = Неопределено;
	Для Каждого КлючЗначение Из СертификатыПрофиля Цикл
		СвойстваСертификата = КлючЗначение.Значение;
		Если СвойстваСертификата.ПарольПолучен Тогда
			Пароль = СвойстваСертификата.ПарольПользователя;
			СертификатПодписи = КлючЗначение.Ключ;
		КонецЕсли;
		
	КонецЦикла;
	
	ПараметрыЗапросаМаркера = СвойстваСертификата;
	
	ПараметрыЗапросаМаркера.Вставить("СертификатПодписи", СертификатПодписи);
	ПараметрыЗапросаМаркера.Вставить("СпособОбменаЭД", СвойстваПрофиля.СпособОбменаЭД);
	ПараметрыЗапросаМаркера.Вставить("ИдентификаторОрганизации", СвойстваПрофиля.ИдентификаторОрганизации);
	ПараметрыЗапросаМаркера.Вставить("ПараметрыАутентификации", ПараметрыАутентификации);
	ПараметрыЗапросаМаркера.Вставить("Пароль", Пароль);
	
	АдресРесурса = "GetSubscriptions/?query=" + СвойстваПрофиля.ИдентификаторОрганизации;
	ВидОперации = НСтр("ru = 'Получение информации о свойствах подписки ЭДО'");
	
	Контекст = Новый Структура;
	Контекст.Вставить("КриптографияНаСервере", Ложь);
	Контекст.Вставить("АвторизацияНаСервере", Ложь);
	
	ЭлектронныеДокументыСлужебныйВызовСервера.ЗаполнитьКонтестЭДО(Контекст);
	
	Если Контекст.КриптографияНаСервере Тогда
		
		ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьСвойстваУведомленийЭДО(АдресРесурса, ВидОперации, ПараметрыЗапросаМаркера,
			СвойстваУведомлений);
		
	Иначе
		
		// Получает маркер от сервера ЭДО
		ЗашифрованныйМаркер = ЭлектронныеДокументыСлужебныйВызовСервера.ЗашифрованныйМаркер(ПараметрыЗапросаМаркера, ПараметрыАутентификации);
		
		// Расшифровывает маркер полученный от сервера
		РасшифроватьМаркерИзСтруктурыСертификата(ПараметрыЗапросаМаркера, Контекст.АвторизацияНаСервере);
		
		// Выполняет метод сервера ЭДО
		ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьСвойстваУведомленийЭДО(АдресРесурса, ВидОперации, ПараметрыЗапросаМаркера,
			СвойстваУведомлений);
...
Процедура ЗаписатьСвойстваУведомленийЭДО(АдресРесурса, СвойстваПрофиля) Экспорт
	...
	СертификатыПрофиля = ЭлектронныеДокументыСлужебныйВызовСервера.СвойстваСертификатовПрофиляЭДО(СвойстваПрофиля.Профиль);
	...
	Если Контекст.КриптографияНаСервере Тогда
		ЭлектронныеДокументыСлужебныйВызовСервера.ОбновитьСвойстваУведомленийЭДО(АдресРесурса, ПараметрыЗапросаМаркера);
...
Процедура ПоказатьПроблемыПриФормированииДокументов(СсылкиНаОбъекты, АдресСведенийОбОшибках, Действие)
	...
	Если Не ПараметрыФормы.ФормаОткрыта Тогда
		Если СсылкиНаОбъекты.Количество() = 1 Тогда
			Результат = ОткрытьФормуМодально("Обработка.ЭлектронныеДокументы.Форма.ПомощникУстраненияОшибок", ПараметрыФормы);
			ОбработатьДокументПослеУстраненияОшибок(Результат);
		Иначе
			ОткрытьФорму("Обработка.ЭлектронныеДокументы.Форма.ПроблемыПриФормированииДокументов", ПараметрыФормы);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьДокументПослеУстраненияОшибок(Результат)
	...
	МассивСсылокНаОбъекты.Добавить(Результат.Документ);
...
//
Функция АутентификацияНаСайте(ПроверятьКорректность = Ложь) Экспорт
	
	РезультатАутентификации = ЭлектронныеДокументыСлужебныйВызовСервера.ПроверитьПараметрыАутентификацииНаСайте(ПроверятьКорректность);
	ОткрыватьФорму = РезультатАутентификации <> Неопределено И ПроверятьКорректность И Не РезультатАутентификации.ПараметрыКорректны = Истина;
...
Процедура РаспаковатьМассивПакетовЭД(МассивПЭД) Экспорт
	...
	ЭДДляФормированияИзвещений = Новый Массив;
	// Распакуем пакеты
	Для Каждого СтруктураРаспаковки Из ПараметрыРаспаковки Цикл
		...
		ДанныеРаспаковки = Неопределено;
		СтруктураРаспаковки.Свойство("ДанныеРаспаковки", ДанныеРаспаковки);
		КоличествоРаспакованныхДокументов = РаспаковатьПакетЭДНаКлиенте(СтруктураРаспаковки.ПакетЭД, СтруктураШифрования, ДанныеРаспаковки, ЭДДляФормированияИзвещений);
...
// Подключает обрабочик ожидания клиентского сеанса
//
Процедура ПодключитьОповещенияЭДО() Экспорт
	
	ОповещатьОНовыхЭД = ЭлектронныеДокументыСлужебныйВызовСервера.ОповещатьОНовыхЭД();
	
	Если Не ОповещатьОНовыхЭД Тогда
		Возврат;
	КонецЕсли;
	
	ПодключитьОбработчикОжидания("ОповеститьОНовыхЭД", 120, Истина);
	...
	Если ТипЗнч(Параметр) = Тип("Структура") Тогда
		...
		Если Параметр.Свойство("ЭлектронныеДокументы") Тогда
			Если ТипЗнч(Параметр.ЭлектронныеДокументы) = Тип("Массив")
				И Параметр.ЭлектронныеДокументы.Найти(Форма.Объект.Ссылка) = Неопределено Тогда
...
//  ЭДДляФормированияИзвещений - Массив, куда добавляются ЭД, для которых нужно сформировать извещение о получении
//
Функция РаспаковатьПакетЭДНаКлиенте(ПакетЭД, СтруктураШифрования, ДанныеРаспаковки, ЭДДляФормированияИзвещений)
	...
	Попытка
		...
		Если ИспользуютсяЭП И Не ВыполнятьКриптооперацииНаСервере Тогда
			Попытка
				МенеджерКриптографии = ЭлектроннаяЦифроваяПодписьКлиент.ПолучитьМенеджерКриптографии();
		...
		// выполняется получение всех доступных данных по подписям и сертификатам, но если криптосредства
		// установлены на клиенте, то данные по сертификатам необходимо получить на клиенте.
		...
		Для Каждого ЭлементСоответствия Из ДанныеРаспаковки.СоответствиеФайлПараметры Цикл
			...
			Иначе
				...
				Если СоответствиеФайлПараметры.ДокументСодержитПодписи
					...
					Если НЕ ВыполнятьКриптооперацииНаСервере И НЕ ЕстьКриптосредствоНаКлиенте Тогда
						...
						ТекстОшибки = НСтр("ru = 'Пакет электронных документов %1 содержит электронные подписи,
							|в программе указана настройка ""Выполнять криптографические операции: На клиенте"",
							|при работе с криптосредством на клиенте возникли ошибки.
							|Проверьте корректность настроек программы и криптосредства на клиенте.'");
						ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", ПакетЭД);
						ТекстОшибки = СтрЗаменить(ТекстОшибки, "%1", ПакетЭД);
						ЭлектронныеДокументыСлужебныйВызовСервера.ОбработатьИсключениеПоЭДНаСервере(НСтр("ru = 'распаковка пакета ЭД'"), ТекстОшибки, ТекстСообщения);
						Продолжить;
					КонецЕсли;
					
					// Получаем структуры сертификатов только для незашифрованных данных (для зашифрованных этого сделать не получится).
					Если Не СоответствиеФайлПараметры.Зашифрован Тогда
						Для Каждого ДанныеПодписи Из МассивДанныхПодписей Цикл
							Если НЕ ДанныеПодписи.ДанныеСертификатаПолучены Тогда
								СтруктураСертификата = ЭлектронныеДокументыКлиентСервер.СтруктураДанныхСертификата(
									ДанныеПодписи.ДвоичныеДанныеФайла, ДанныеПодписи.ДвоичныеДанные, МенеджерКриптографии);
								Если СтруктураСертификата.ДанныеСертификатаПолучены Тогда
									ЗаполнитьЗначенияСвойств(ДанныеПодписи, СтруктураСертификата);
									ДанныеПодписи.ДанныеСертификатаПолучены = Истина;
								КонецЕсли;
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
		...
		Для Каждого Элемент Из МассивКУдалениюИзСоответствия Цикл
			ДанныеРаспаковки.СоответствиеФайлПараметры.Удалить(Элемент);
		КонецЦикла;
		
		Если ДанныеРаспаковки.СоответствиеФайлПараметры.Количество() = 0 Тогда
			Возврат Неопределено;
		КонецЕсли;
		...
		Если ЕстьТипДанных И ТипДанных <> "ЭД" И ТипДанных <> "Подпись" Тогда
			// Получен служебный документ (Подтверждение, Извещение) в пакете формата Оператора ЭДО
			// (с карточкой и транспортной информацией)
			...
			Если ТипЗнч(МассивДобавленныхФайлов) = Тип("Массив") И МассивДобавленныхФайлов.Количество() > 0 Тогда
				...
				Для Каждого ИмяФайла Из ДанныеРаспаковки.СоответствиеФайлПараметры Цикл
					СтруктураСлужебныхДанных = ИмяФайла.Значение;
					Если СтруктураСлужебныхДанных.Свойство("ДопДанные")
						И СтруктураСлужебныхДанных.ДопДанные.Свойство("ДопПараметры")
						И СтруктураСлужебныхДанных.ДопДанные.ДопПараметры.Свойство("IsWarrantActive", РезультатПроверкиМЧДОператором) Тогда
						Прервать;
					КонецЕсли;
				КонецЦикла;
				...
				Для Каждого ЭД ИЗ МассивДобавленныхФайлов Цикл
					ЭлектронныеДокументыСлужебныйВызовСервера.ЗаписатьРезультатПроверкиМЧДОператором(ЭД, РезультатПроверкиМЧДОператором);
					
					МашиночитаемыеДоверенностиВызовСервера.ДобавитьДоверенностиСообщений(ЭД, Доверенности);
				КонецЦикла;				
				
				ДоверенностиДляПроверки = ЭлектронныеДокументыСлужебныйВызовСервера.ДоверенностиДляПроверкиИзРаспаковкиПакетаЭД(МассивДобавленныхФайлов, Доверенности);
				МашиночитаемыеДоверенностиКлиент.ПослеОтраженияВУчете(Неопределено, ПредопределенноеЗначение("Перечисление.ВидыЭД.МашиночитаемаяДоверенность"), ДоверенностиДляПроверки, Неопределено);
			...
			Если МассивДобавленныхФайловДляИзвещений.Количество() > 0 Тогда
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЭДДляФормированияИзвещений, МассивДобавленныхФайловДляИзвещений);
			КонецЕсли;
		...
		Иначе
			...
			Если МассивДвоичныхДанныхИзвещений.Количество() > 0 Тогда
				Для Каждого СтруктураДанных Из МассивДвоичныхДанныхИзвещений Цикл
					...
					СтруктураСлужебныхДанных = ДанныеРаспаковки.СоответствиеФайлПараметры.Получить(СтруктураДанных.ИмяФайла);
					
					Если СтруктураСлужебныхДанных.ДокументСодержитПодписи Тогда
						ФлагОшибки = Ложь;
						Для Каждого ДанныеПодписи Из СтруктураСлужебныхДанных.МассивДанныхПодписей Цикл
							Если ДанныеПодписи.ДвоичныеДанные <> Неопределено Тогда
								Попытка
									ЭлектроннаяЦифроваяПодписьКлиент.ПроверитьПодпись(МенеджерКриптографии, СтруктураДанных.ДвоичныеДанные,
			...
			Если МассивДвоичныхДанных.Количество() = 0 Тогда
				...
				ДоверенностиДляПроверки = ЭлектронныеДокументыСлужебныйВызовСервера.ДоверенностиДляПроверкиИзРаспаковкиПакетаЭД(Новый Массив, ДанныеРаспаковки.Доверенности);
				МашиночитаемыеДоверенностиКлиент.ПослеОтраженияВУчете(Неопределено, ПредопределенноеЗначение("Перечисление.ВидыЭД.МашиночитаемаяДоверенность"), ДоверенностиДляПроверки, Неопределено);
			...
			Иначе
				...
				Для Каждого СтруктураДанных Из МассивДвоичныхДанных Цикл
					...
					Если Не МассивДобавленныхФайлов = Неопределено И МассивДобавленныхФайлов.Количество() > 0 Тогда
						...
						РезультатПроверкиМЧДОператором = Неопределено;
						СтруктураСлужебныхДанных = ДанныеРаспаковки.СоответствиеФайлПараметры[СтруктураДанных.ИмяФайла];
						Если СтруктураСлужебныхДанных.Свойство("ДопДанные")
							И СтруктураСлужебныхДанных.ДопДанные.Свойство("ДопПараметры") Тогда
							СтруктураСлужебныхДанных.ДопДанные.ДопПараметры.Свойство("IsWarrantActive", РезультатПроверкиМЧДОператором);
						КонецЕсли;						
						
						Доверенности = Неопределено;
						ДанныеРаспаковки.Свойство("Доверенности", Доверенности);
						
						//Данные доверенностей нужно проверить заранее, до проверки подписей документов
						Для Каждого ЭД ИЗ МассивДобавленныхФайлов Цикл
							ЭлектронныеДокументыСлужебныйВызовСервера.ЗаписатьРезультатПроверкиМЧДОператором(ЭД, РезультатПроверкиМЧДОператором);
					...
					Если Не ЕстьОшибкаРаспаковки Тогда
						...
						ЭтоПроизвольныйЭД = ?(СоответствиеФайлПараметры.Свойство("ПроизвольныйЭД"),
							СоответствиеФайлПараметры.ПроизвольныйЭД = Истина, Ложь);
						Если ЗначениеЗаполнено(ТипДанных) И ЭтоПроизвольныйЭД
							...
							ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЭДДляФормированияИзвещений, МассивДобавленныхФайлов);
						КонецЕсли;
...
// Отображет форму запроса доп данных 
// Только для внутреннего использования
//
// Параметры: 
//  ЭД - СправочникСсылка.ЭДПрисоединенныеФайлы - ссылка на электронный документ, для которого нужно получить данные
// Возвращаемое значение:
//   Структура, строка - значение доп данных введенных пользователем
//
Функция ПоказатьФормуВводаДопДанных(ЭД)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ЭД", ЭД);
	
	Возврат ОткрытьФормуМодально("Обработка.ЭлектронныеДокументы.Форма.ВводДополнительныхСведений", ПараметрыФормы);
...
Процедура ОтправитьПакетыЧерезДопОбработку(ДанныеОтправки, КоличествоОтправленных)
	...
	Для Каждого КлючЗначение Из ДанныеОтправки Цикл
		...
		СтруктураОтправки.Вставить("ВерсияСхемыДанных", ЭлектронныеДокументыКлиентСервер.ВерсияФорматаСинхронногоОбмена());
	...
	Для Каждого Элемент ИЗ ДоступныеСертификаты Цикл
		...
		Если ТребуетсяПИН Тогда
			...
			Если ПинКод = Неопределено Тогда
				Возврат;
			КонецЕсли;
			Если Не УстановитьPINКодХранилища(ВнешнийПодключаемыйМодуль, ДанныеСертификата.ИдентификаторХранилища, ПинКод) Тогда
				Возврат;
			КонецЕсли;
			СообщитьПользователю(РезультатТеста, ИдентификаторНазначения);
		КонецЕсли;
...
Функция НастройкиРегламентаЭДО(ВидЭД, ВерсияФормата, СпособОбменаЭДО) Экспорт
	...
	Если ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктИсполнитель")
		И (ВРег(ВерсияФормата) = ВРег("ФНС 5.01 (С 2016Г.)")
			Или ВРег(ВерсияФормата) = ВРег("ФНС 5.01 (УПД:Первичный документ)")
			ИЛИ ВРег(ВерсияФормата) = ВРег("ФНС 2019 (УПД:Первичный документ)")) Тогда
	...
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктНаПередачуПрав")
		И (ВРег(ВерсияФормата) = ВРег("ФНС 5.01 (УПД:Первичный документ)")
		ИЛИ ВРег(ВерсияФормата) = ВРег("ФНС 2019 (УПД:Первичный документ)")) Тогда
	...
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетФактура") Тогда
		...
		Если ВРег(ВерсияФормата) = ВРег("ФНС 5.01 (УПД:Счет-фактура и первичный документ)")
			ИЛИ ВРег(ВерсияФормата) = ВРег("ФНС 2019 (УПД:Счет-фактура и первичный документ)") Тогда
			Настройки.РедактироватьОтветнуюПодпись = Истина;
		КонецЕсли;
		
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.КорректировочныйСчетФактура") Тогда
		Настройки.РедактироватьИзвещение = Истина;
		Если ВРег(ВерсияФормата) = ВРег("ФНС 5.01 (УКД:Корректировочный счет-фактура и первичный документ)")
			Или ВРег(ВерсияФормата) = "КСЧФДИСУКД2020" Тогда
			Настройки.РедактироватьОтветнуюПодпись = Истина;
		КонецЕсли;
		
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.СоглашениеОбИзмененииСтоимостиОтправитель") Тогда
		Если ВРег(ВерсияФормата) = ВРег("ФНС 5.01 (УКД:Первичный документ)")
			Или ВРег(ВерсияФормата) = "ДИСУКД2020" Тогда
			Настройки.РедактироватьОтветнуюПодпись = Истина;
		КонецЕсли;
	...
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.СчетНаОплату") Тогда
		Настройки.РедактироватьОтветнуюПодпись = Истина;
		
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПрайсЛист") Тогда
		Настройки.РедактироватьОтветнуюПодпись = Истина;
		
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ЗаказТовара") Тогда
		Настройки.РедактироватьОтветнуюПодпись = Истина;
		
	ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ОтветНаЗаказ") Тогда
		Настройки.РедактироватьОтветнуюПодпись = Истина;
		
	КонецЕсли;
	
	Если ЭлектронныеДокументыСлужебныйВызовСервера.ЭтоПрямойОбмен(СпособОбменаЭДО) Тогда
		Если ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД") Тогда
			Настройки.РедактироватьПодпись = Ложь;
		ИначеЕсли ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.АктОРасхождениях") Тогда
			Настройки.РедактироватьПодпись = Ложь;
...
Функция ПризнакПринятияУсловийИспользования(Знач НаборПрофилейНастроекЭДО)
	...
	ВсеПризнаки = ЭлектронныеДокументыСлужебныйВызовСервера.ПризнакПринятияУсловийИспользования();
...
Функция ПроверитьПринятиеУсловийИспользования(Знач НаборПрофилейНастроекЭДО)
	...
	Результат = ОткрытьФормуПринятияУсловийИспользования(НеПринято);
	Если Результат = Неопределено Тогда
		Результат = Ложь;
	КонецЕсли;
...
Функция ОткрытьФормуПринятияУсловийИспользования(Знач НаборПрофилейНастроекЭДО = Неопределено) Экспорт
	...
	Результат = ОткрытьФормуМодально("Обработка.ЭлектронныеДокументы.Форма.УсловияИспользования", ПараметрыФормы);
	
	Возврат Результат;
	
КонецФункции

Функция ТребуетсяУказаниеПричиныОказаОтПриглашения(СпособОбмена) Экспорт
...
//
// Параметры:
//  Параметры - см. ЭлектронныеДокументыСлужебныйКлиент.ДополнительныеПараметрыВводаСтроки - параметры ввода строки.
// Возвращаемое значение:
//  Строка, Неопределено - Строка - если пользователь ввел строку, если отказался от ввода - Неопределено.
//
Функция ВвестиСтрокуБЭД(Параметры) Экспорт
	
	Возврат ОткрытьФормуМодально("Обработка.ЭлектронныеДокументы.Форма.ВводСтроки", Параметры);
...
Функция ВыполнитьПроверкуПолномочийПриУтверждении(ЭД, ХешПодписи)
	
	ПараметрыПроверки = МашиночитаемыеДоверенностиКлиентСервер.НовыеПараметрыПроверкиПолномочий();
	ПараметрыПроверки.ПодписанныйОбъект = ЭД;
	ПараметрыПроверки.ХешПодписи        = ХешПодписи;
	
	Результаты = ОткрытьФормуМодально("Обработка.РезультатыПроверкиПодписи.Форма.ПроверкаПолномочийДоверенности", 
		ПараметрыПроверки);
		
	Если ЗначениеЗаполнено(Результаты) Тогда
		Возврат МашиночитаемыеДоверенностиКлиент.ОбработатьРезультатПроверкиПолномочий(Результаты);
	КонецЕсли;
...
Функция ВыполнятьПроверкуМЧДПриУтверждении(ЭД)
	
	Результат = Истина;
	
	ДействияПриОтсутствииВхМЧД = ЭлектронныеДокументыКлиентСервер.ДействияПриОтсутствииВхМЧД();
	Действие = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьДействиеПриОтсутствииВхМЧД(ЭД);
	
	Если Действие = ДействияПриОтсутствииВхМЧД.Спрашивать Тогда
		
		Если Не МашиночитаемыеДоверенностиВызовСервера.ИмеетсяПравоИзмененияМЧД() Тогда 
			Возврат Истина;
		КонецЕсли;		
		
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 %2" + Символы.ПС +  "%3" + Символы.ПС + "%4",
		...
		Кнопки.Добавить("КодВозвратаДиалога.Да", "Да, доверяю");
		Кнопки.Добавить("КодВозвратаДиалога.Нет", "Нет, не доверяю");
		...
		Результат = СтандартныеПодсистемыКлиент.ВопросПользователю(
			...
			НСтр("ru = 'Подпись не проверена'"), 
			КодВозвратаДиалога.Нет,
			БольшеНеЗадаватьЭтотВопрос);
		
		Если Результат = КодВозвратаДиалога.Да Тогда
			Результат = Ложь;
			Действие = ДействияПриОтсутствииВхМЧД.Доверять;
			
			ЭлектронныеДокументыСлужебныйВызовСервера.ЗаписатьРезультатыРучнойПроверкиПодписейПоДокументу(ЭД);
			Если Не БольшеНеЗадаватьЭтотВопрос Тогда
				ЭлектронныеДокументыСлужебныйВызовСервера.УстановитьИгнорироватьОтсутствиеМЧД(ЭД);
			КонецЕсли;
			
			ПараметрыОповещения = Новый Структура("ЭлектронныеДокументы", МашиночитаемыеДоверенностиКлиентСервер.ЗначениеВМассиве(ЭД));
			Оповестить("ОбновитьСостояниеЭД", ПараметрыОповещения);			
		Иначе
			Результат = Истина;
			Действие = ДействияПриОтсутствииВхМЧД.Требовать;
		КонецЕсли;			
		
		Если БольшеНеЗадаватьЭтотВопрос Тогда
			ЭлектронныеДокументыСлужебныйВызовСервера.УстановитьДействиеПриОтсутствииВхМЧД(ЭД, Действие);
		КонецЕсли;			
		
	ИначеЕсли Действие = ДействияПриОтсутствииВхМЧД.Доверять Тогда
		Результат = Ложь;
	ИначеЕсли Действие = ДействияПриОтсутствииВхМЧД.Требовать Тогда
		Результат = Истина;			
	КонецЕсли;
...
Функция ПодписатьДвоичныеДанныеОпределеннымСертификатом(ДвоичныеДанные, ПараметрыСертификата) Экспорт
	...
	СертификатПодписи = ЭлектроннаяЦифроваяПодписьКлиент.ПолучитьСертификатПоОтпечатку(ПараметрыСертификата.Отпечаток);
	...
	СтруктураДанныхПодписи = Новый Структура("Сертификат, ПарольПользователя, Пароль, Комментарий", 
		СертификатПодписи, ПараметрыСертификата.ПарольПользователя, ПараметрыСертификата.ПарольПользователя, ПараметрыСертификата.Комментарий);
	...
	Если ДанныеПодписи <> Неопределено и ДанныеПодписи.ДатаПодписи = Дата('00010101') Тогда
		ДанныеПодписи.ДатаПодписи = ЭлектронныеДокументыСлужебныйВызовСервера.ДатаУстановкиПодписи(ДанныеПодписи.НоваяПодписьДвоичныеДанные);
...
Процедура ОбработатьРезультатПодписания(РезультатПодписания, ПараметрыПодписания)
	...
	Если ПараметрыПодписания.ПеревыборСертификата Тогда
		...
		Сертификат = РезультатПодписания.ВыбранныйСертификат;
		
		Оповещение = Новый ОписаниеОповещения(
			"ОбработатьРезультатВопросаПовторитьПодписание", ЭлектронныеДокументыСлужебныйКлиент, ПараметрыОповещения);
		...
		Если ПараметрыПодписания.ОшибкиПроверкиПолномочий.Количество() = 0 Тогда
			...
			ТекстВопроса = МашиночитаемыеДоверенностиВызовСервера.ФорматированнаяСтрока(
