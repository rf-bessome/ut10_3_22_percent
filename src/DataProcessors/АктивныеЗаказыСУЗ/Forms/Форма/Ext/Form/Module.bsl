...
Процедура УстановитьУсловноеОформление()
	...
	СтандартныеПодсистемыСервер.УстановитьУсловноеОформлениеПоляДата(ЭтаФорма, "СписокЗаказов.ДатаЗаказа", Элементы.СписокЗаказовДатаЗаказа.Имя);
...
Процедура ЗаполнитьСписокОрганизаций()
	...
	Если Элементы.Организация.СписокВыбора.Количество() = 1 Тогда
		ЭтаФорма.Организация = Элементы.Организация.СписокВыбора[0].Значение;
...
Функция ЗакрытьЗаказыВСУЗ(ВызовПослеАвторизации = Ложь)
	...
	Иначе
		АдресХранилищаЗакрытыеЗаказы = ПоместитьВоВременноеХранилище(ЗакрытыеЗаказы, ЭтаФорма.УникальныйИдентификатор);
	КонецЕсли;
