...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыУказанияСерий = ИнтеграцияИС.ПараметрыУказанияСерий(
		Метаданные.Обработки.ВиртуальнаяАгрегацияУпаковокИСМП, ЭтаФорма);
...
Процедура ПередЗакрытием(Отказ,  СтандартнаяОбработка)
	...
	Если НЕ ВыполняетсяЗакрытие И Модифицированность Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриОтветеНаВопросПередЗакрытиемФормы", ЭтаФорма);
...
Процедура РежимФормированияМонотоварнойУпаковкиПриИзменении(Элемент)
	...
	Если СоставУпаковки.Количество() > 0 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриОтветеНаВопросПриИзмененииРежимаФормирования", ЭтаФорма);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура ПроизвольныйСоставУпаковкиПриИзменении(Элемент)
	...
	УправлениеЗависимымиОтИсторииЭлементами(ЭтаФорма);
...
Процедура СоставУпаковкиСерияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ПолныйСоставУпаковки.Количество() > 1 Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриОкончанииУказанияСерий", ЭтаФорма, ДополнительныеПараметры);
		
		ОткрытьФорму("Обработка.ВиртуальнаяАгрегацияУпаковокИСМП.Форма.ФормаУказанияСерий",
			ПараметрыОткрытия, ЭтаФорма,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СформироватьУпаковки(Команда)
	...
	Если Не Элементы.ГруппаСписокУпаковок.Видимость Тогда
		ПоказатьОповещениеПользователя(
			...
			Новый ОписаниеОповещения("ПриОповещенииОФормированииУпаковок", ЭтаФорма),
			НСтр("ru = 'Нажмите для просмотра'"),,
			СтатусОповещенияПользователя.Информация, ЭтаФорма.УникальныйИдентификатор);
	КонецЕсли;
...
Процедура Печать(Команда)
	...
	Если Модифицированность Тогда
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ЗаписьУпаковокЗавершение", ЭтаФорма);
...
Процедура УправлениеЭлементамиФормыПриСоздании()
	...
	УправлениеЗависимымиОтИсторииЭлементами(ЭтаФорма);
	УправлениеЗависимымиОтРежимаФормированияЭлементами(ЭтаФорма);
...
Процедура ПриВыбореСтрокиСоставаУпаковки(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриОкончанииРедактированияСоставаУпаковок", ЭтаФорма, СтруктураПоиска);
...
Процедура ЗаполнитьСоставУпаковки(МассивСтрок, ЗаполнятьИзИстории)
	...
	ТаблицаТоваров.Колонки.Добавить("GTIN",                 ОпределяемыеТипы().GTIN.Тип);
...
Процедура ПолучитьДанныеЗаполнения()
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("ПолучениеДанныхЗаполненияЗавершение", ЭтаФорма);
...
Процедура ОткрытьФормуГенерацииШтрихкодов(СтрокиАгрегации)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПриГенерацииШтрихкодаУпаковки", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ПроверитьВозможностьУдаленияЗаписаннойУпаковки(УдаляемыеДанные)
	...
	Иначе
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриОтветеНаВопросПередУдалениемУпаковок", ЭтаФорма, УдаляемыеДанные);
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
...
Процедура ПриОтветеНаВопросПередУдалениемУпаковок(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ОповещениеОЗавершении = Новый ОписаниеОповещения("УдалениеУпаковокЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ОткрытьФормуПечати()
	...
	ОповещениеПриЗакрытии = Новый ОписаниеОповещения("ПриЗакрытииФормыПечати", ЭтаФорма);
	
	ОткрытьФорму("РегистрСведений.ПулКодовМаркировкиСУЗ.Форма.ФормаПечати",
		ПараметрыФормыПечати, ЭтаФорма,,,, ОповещениеПриЗакрытии, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
