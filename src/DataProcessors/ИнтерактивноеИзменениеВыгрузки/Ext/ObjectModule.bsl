...
Функция ИнициализироватьЭтотОбъект(АдресОбъекта="") Экспорт
	...
	Если Не ПустаяСтрока(АдресОбъекта) Тогда
		...
		Источник = ПолучитьИзВременногоХранилища(АдресОбъекта);
...
Функция КомпоновщикНастроекОбщегоОтбора(АдресСохраненияСхемы=Неопределено) Экспорт
	...
	Результат.Вставить("АдресКомпоновщикаВсехДокументов", ПоместитьВоВременноеХранилище(Данные, АдресСохранения));
...
Функция ИнициализироватьКомпоновщик(СписокИменМетаданных=Неопределено, ОграничиватьИспользованиеОтбором=Ложь, АдресСохраненияСхемы=Неопределено)
	...
	Возврат Новый Структура("ТаблицаМетаданныхСоставаУзла, СхемаКомпоновки, КомпоновщикНастроек, Настройки", 
		ТаблицаМетаданныхСоставаУзла, СхемаКомпоновки, КомпоновщикНастроек, КомпоновщикНастроек.Настройки
	);
КонецФункции
