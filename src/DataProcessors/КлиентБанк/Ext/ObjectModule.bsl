...
Перем РеквизитыПлДок_КодНазПлатежа;
...
Процедура ЗаполнитьСтрокиРеквизитовПлДокументовДляИмпорта()
	...
	РеквизитыПлДок_КодНазПлатежа = "КодНазПлатежа";
...
Процедура СформироватьСтруктуруТаблицыИмпорта(Таб, ТабПоле) Экспорт
	...
	Т_Строка01  = Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(1));
	...
	СоздатьКолонкуВТаблице(Таб, ТабПоле, "КодНазПлатежа",        Т_Строка01,  "Код вида дохода",            НеВидимая);
...
Функция ПолучитьТекстЗапросаПоПлатежноеПоручениеИсходящее()
	...
	Возврат "ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			|	ПлатежноеПоручениеИсходящее.КодВыплат КАК КодВыплат,
			|	ПлатежноеПоручениеИсходящее.КодВидаДохода КАК КодВидаДохода,
	        |	"""" КАК ВидАккредитива,
	        |	"""" КАК СрокАкцепта,
	        |	"""" КАК СрокДействия,
	        |	"""" КАК УсловиеОплаты1,
...
Процедура ПроверитьЗаполнениеНалоговыхРеквизитов(СтрокаЭкспорта)
	...
	Если Лев(СтрокаЭкспорта.ИННПлательщика, 2) = "00" Тогда
		ДобавитьЗамечание(СтрокаЭкспорта, 3, "Первые две цифры ИНН плательщика не могут быть ""00""");
	КонецЕсли;
	Если Лев(СтрокаЭкспорта.ИННПолучателя, 2) = "00" Тогда
		ДобавитьЗамечание(СтрокаЭкспорта, 3, "Первые две цифры ИНН получателя не могут быть ""00""");
	КонецЕсли;
	Если Лев(СтрокаЭкспорта.КПППлательщика, 2) = "00" Тогда
		ДобавитьЗамечание(СтрокаЭкспорта, 3, "КПП плательщика не должен начинаться на 00");
	КонецЕсли;
	Если Лев(СтрокаЭкспорта.КПППолучателя, 2) = "00" Тогда
		ДобавитьЗамечание(СтрокаЭкспорта, 3, "КПП получателя не должен начинаться на 00");
	КонецЕсли;
...
Процедура ВыборФайлаДляВыгрузки(Элемент) Экспорт
	...
	ДиалогФыбораФайла.ПолноеИмяФайла              = ?(ПустаяСтрока(Элемент.Значение),
		ИмяФайлаВыгрузки(), Элемент.Значение);
...
Процедура ЗаполнитьЗначениеЭкспорта(СтрокаЭкспорта, ВыборкаДляЭкспорта)
	...
	Если ВыборкаДляЭкспорта.Дата >= УправлениеДенежнымиСредствами.НачалоДействияУказанияБанкаРоссии5286У() Тогда
...
//СтатусСоставителя, ПлательщикКПП ПолучательКПП ПоказательКБК ОКАТО ПоказательОснования ПоказательПериода ПоказательНомера ПоказательДаты ПоказательТипа
	Если ВыборкаДляЭкспорта.ПеречислениеВБюджет Тогда
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаДляЭкспорта.ПоказательДаты) Тогда
			Если ВыборкаДляЭкспорта.ПоказательОснования = "00" Тогда
				СтрокаЭкспорта.ПоказательДаты = "00";
			Иначе
		...
		Если ПоказательТипаПредусмотренСтандартамиОбмена Тогда
			Если СтрокаЭкспорта.Дата >= ПлатежиВБюджетКлиентСервер.НачалоДействияПриказа126н() Тогда
...
Функция ПолучитьСекциюДокумент(СтруктураДокумента, КоллекцияРеквизитов, ВерсияФормата, ЗаполнятьПоказательТипа)
...
Функция Выгрузить(ДокументыНаЭкспорт, ВерсияФормата = "") Экспорт
	
	ПотокВыгрузки = Новый ТекстовыйДокумент();
	
	Если Не ЗначениеЗаполнено(ВерсияФормата) Тогда
		Если КонПериода >= УправлениеДенежнымиСредствами.НачалоДействияУказанияБанкаРоссии5286У() Тогда
			ВерсияФормата = "1.03";
		Иначе
			ВерсияФормата = "1.02";
		КонецЕсли;
	...
	ПотокВыгрузки.ДобавитьСтроку("ВерсияФормата=" + ВерсияФормата); // Поддерживаются версии "1.01" - "1.03"
	...
	Для каждого СтрокаСекции из ДокументыНаЭкспорт.Значение Цикл
		...
		Буфер = ПолучитьСекциюДокумент(СтрокаСекции, ДокументыНаЭкспорт.Колонки, ВерсияФормата, ЗаполнятьПоказательТипа);
	...
	Попытка
		...
		ВозвращаемыеДанные.Вставить("ИмяФайла", ФайлВыгрузки);
		ВозвращаемыеДанные.Вставить("Выгрузка", ПотокВыгрузки);
...
Процедура РаспознатьДанныеВСтрокеДокумента(СтрокаДокумента)
	...
	Исходяший = СтрокаДокумента.ПлательщикСчет = БанковскийСчет.НомерСчета И СтрокаДокумента.ПлательщикБИК = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БанковскийСчет.Банк, "Код");
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		Если СтрокаДокумента.ВидДокумента = "ПлатежноеПоручениеИсходящее" ИЛИ СтрокаДокумента.ВидДокумента = "ПлатежноеТребованиеВыставленное"  
			...
			Если Не ОбщегоНазначения.ЕстьНеЦифры(СтрокаДокумента.Номер) Тогда
				НомерДок = Число(Прав(СтрокаДокумента.Номер, 6));
				НомерДляПоискаДок = Формат(НомерДок, "ЧЦ=6; ЧН=; ЧВН=; ЧГ=0");
	...
	Если ЗначениеЗаполнено(СтрокаДокумента.ПлательщикБИК) Тогда
		...
		Если    ((Длина = 9 И Лев(СтрокаДокумента.ПлательщикБИК,2)<>"04")
			...
			ИЛИ (Длина < 8))
			И Не ЭтоБИКТОФК(СтрокаДокумента.ПлательщикБИК) Тогда
	...
	Если ЗначениеЗаполнено(СтрокаДокумента.ПолучательБИК) Тогда
		...
		Если    ((Длина = 9 И Лев(СтрокаДокумента.ПолучательБИК,2)<>"04")
			...
			ИЛИ (Длина < 8))
			И Не ЭтоБИКТОФК(СтрокаДокумента.ПолучательБИК) Тогда
...
Процедура ЗадатьПометкуНаУдаление(ОбъектДляПометки, Пометка)
	...
	Исключение
		...
		Если ТипЗнч(Инфо.Причина) = Тип("ИнформацияОбОшибке") Тогда
			ПричинаОшибки = Инфо.Причина.Описание;
		Иначе
			ПричинаОшибки = Инфо.Описание;
		КонецЕсли;
		Сообщить("Причина: " + ПричинаОшибки);
...
//СтатусСоставителя и реквизиты налогового платежа: КодБК,КодОКАТО,ПоказательОснования,ПоказательПериода,ПоказательНомера,ПоказательДаты,ПоказательТипа 
	...
	ИначеЕсли ЗначениеЗаполнено(ИсходныеДанные.ПоказательТипа) Тогда
		УстановитьСвойство(ОбъектДокумента, "КодВыплат",           ИсходныеДанные.ПоказательТипа);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ИсходныеДанные.КодНазПлатежа) Тогда
		УстановитьСвойство(ОбъектДокумента, "КодВидаДохода",       ИсходныеДанные.КодНазПлатежа);
	КонецЕсли;
