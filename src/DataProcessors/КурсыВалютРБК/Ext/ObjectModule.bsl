...
//													   загружаемой валюты
//
Процедура ЗагрузитьКурсы(ИндикаторФормы ,НадписьВалютыФормы ) Экспорт
	...
	Иначе
		...
		НастройкиСоединения = ОбработкаПолученияФайлов.ПараметрыАутентификацииИнтернетПоддержки();
		
		Если НастройкиСоединения = Неопределено Тогда
			ИнтернетПоддержкаПользователейКлиент.СтартоватьМеханизм("handStart");
			Сообщить("После ввода логина и пароля следует повторить операцию загрузки данных.");
			Возврат;
		КонецЕсли;
	...
	Для каждого СтрокаСпВалют из СписокВалют Цикл
		...
		ИмяВходящегоФайла = "" + ВремКаталог + "\" + ИмяФайла;
		СтрокаПараметраПолучения = Адрес + Прав(ТекВалюта.Код,3) + ТМП + ".tsv";
		Если ОбработкаПолученияФайлов.ЗапроситьФайлыССервера(СерверИсточник, СтрокаПараметраПолучения, ИмяВходящегоФайла, HTTP, , , , , , , , , ВариантЗагрузки) <> Истина Тогда
			Сообщить("Не удалось получить ресурс для валюты " + СокрЛП(ТекВалюта.Наименование) + " (код " + ТекВалюта.Код + "). Курс для валюты не загружен."); 
			Продолжить;
		КонецЕсли; 
...
Процедура ЗаполнитьВалюты(ЗаполнятьТолькоВалютамиБезАктуальныхКурсов = Ложь) Экспорт
	...
	Пока ВыборкаВалют.Следующий() Цикл
		...
		СтрокаСпискаВалют.Загрузка = Истина;
	КонецЦикла;	
