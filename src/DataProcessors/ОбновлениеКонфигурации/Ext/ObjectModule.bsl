...
Процедура ВыполнитьОбновлениеКонфигурации() Экспорт
	...
	ОбщегоНазначения.ЗапуститьПрограмму("""" + ПапкаСистемныхПриложений() + "mshta.exe"" """ + ИмяГлавногоФайлаСкрипта + """", ПолучитьКаталогФайла(ИмяГлавногоФайлаСкрипта));
...
Функция ЗапланироватьОбновлениеКонфигурации() Экспорт
	...
	КодЗадачиПланировщика = СоздатьЗадачуПланировщика(
		"""" + ПапкаСистемныхПриложений() + "wscript.exe"" //B //nologo """ + ИмяГлавногоФайлаСкрипта + """", ДатаВремяОбновления);
