...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьЗначенияСвойств(ЭтаФорма, СодержимоеКонстанты);
...
Процедура СохранитьЗначениеРеквизита(РеквизитПутьКДанным, НастройкиСканирования)
	...
	Если КонстантаИмя <> "" Тогда
		...
		Если Не НаборКонстант.ЗапрашиватьДанныеСервисаИСМП Тогда
			...
			Для Каждого КлючИЗначение Из ЭлементыНастроек Цикл
				ЭтаФорма[КлючИЗначение.Значение] = Ложь;
			КонецЦикла;
			КонстантаИмя = "";
		КонецЕсли;
	...
	Если КонстантаИмя = "" Тогда
		...
		ЗаполнитьЗначенияСвойств(НастройкиСканирования, ЭтаФорма);
		...
		Для Каждого КлючИЗначение Из ЭлементыНастроек Цикл
			ГруппаПараметров = НастройкаПараметровСканированияСлужебныйКлиентСервер.ЗначениеГруппыНастроек(
				...
				КлючИЗначение.Ключ);
			ГруппаПараметров.Включено = ЭтаФорма[КлючИЗначение.Значение];
		КонецЦикла;
		...
		СобытияФормИСПереопределяемый.ОбновитьФормуНастройкиПриЗаписиПодчиненныхКонстант(ЭтаФорма, "НастройкиСканированияКодовМаркировкиИСМП", Неопределено);
...
Процедура НастроитьФормуПриИзменииЭлементовУправления(НастройкиСканирования = Неопределено)
	...
	Для Каждого КлючИЗначение Из ПараметрыКонтроля Цикл
		...
		ЭтаФорма[ИмяРеквизитаФормы] = ГруппаНастроек.Включено;
