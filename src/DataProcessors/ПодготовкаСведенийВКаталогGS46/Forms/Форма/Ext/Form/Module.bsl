...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтранаМираРоссия = Справочники.КлассификаторСтранМира.Россия;
...
#Если Не МобильныйКлиент Тогда
	...
	Оповещение = Новый ОписаниеОповещения("ПослеВыбораФайлаДляЗагрузки", ЭтаФорма, ДополнительныеПараметры);
...
#Если Не МобильныйКлиент Тогда
	...
	ФайловаяСистемаКлиент.СохранитьФайл(
		Новый ОписаниеОповещения("ПослеВыбораФайлаДляВыгрузки", ЭтаФорма, ДополнительныеПараметры),
		АдресМакета,,
		ПараметрыСохранения);
#КонецЕсли
...
Функция МассивЗначенийGTINДляВыбора(ТекущаяСтрока)
	...
	Возврат ИнтеграцияИСМП.МассивЗначенийGTINДляВыбора(ТекущиеДанные, ЭтаФорма);
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормИСПереопределяемый.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(
		ЭтаФорма, "ТоварыХарактеристика", "Объект.Товары.ХарактеристикиИспользуются");
...
Процедура ОбработатьПереданныеПараметры()
	...
	Если ТипЗнч(Параметры.Товары) = Тип("Массив") Тогда
		Для Каждого ЭлементМассива Из Параметры.Товары Цикл
			...
			СобытияФормИСМППереопределяемый.ПриИзмененииНоменклатуры(ЭтаФорма,
...
Функция ДополнительныеСведенияПродукции()
	...
	КлассификаторСтранМира = Новый Массив;
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.СтранаПроизводства)
			И КлассификаторСтранМира.Найти(СтрокаТовары.СтранаПроизводства) = Неопределено Тогда
			КлассификаторСтранМира.Добавить(СтрокаТовары.СтранаПроизводства);
		КонецЕсли;
	...
	Если КлассификаторСтранМира.Количество() > 0 Тогда
		Сведения.КодыАльфа2 = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(КлассификаторСтранМира, "КодАльфа2");
	КонецЕсли;
...
Процедура ЗаполнитьКолонку(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнитьКолонкуЗавершение", ЭтаФорма, Новый Структура("ИмяКолонки", ИмяКолонки));
	...
	Если ИмяКолонки = "СтранаПроизводства" Тогда
		ТипКолонки = Новый ОписаниеТипов("СправочникСсылка.КлассификаторСтранМира");
	КонецЕсли;
...
Функция ПроверитьЗагрузитьТовары(РезультатЗагрузки)
	...
	ТипНоменклатура = ОпределяемыеТипы().Номенклатура.Тип;
	...
	ТипХарактеристика = ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип;
...
Процедура ИнициализироватьКлассификаторы()
	...
	ФайловаяСистемаКлиент.ПодключитьРасширениеДляРаботыСФайлами(
		Новый ОписаниеОповещения("ИнициализироватьКлассификаторыПослеПодключенияРасширения", ЭтаФорма));
...
Процедура ИнициализироватьКлассификаторыПослеПодключенияРасширения(РаботаСФайламиДоступна, ДополнительныеПараметры) Экспорт
	...
	Если РаботаСФайламиДоступна Тогда
		...
		НачатьПолучениеКаталогаВременныхФайлов(
			Новый ОписаниеОповещения("ИнициализироватьКлассификаторыПослеПолученияКаталога", ЭтаФорма));
...
Процедура ИнициализироватьКлассификаторыПослеПолученияКаталога(ИмяКаталогаВременныхФайлов, ДополнительныеПараметры) Экспорт
	...
	ФайловаяСистемаКлиент.СохранитьФайл(
		Новый ОписаниеОповещения("ИнициализироватьКлассификаторыЗавершение",ЭтаФорма),
