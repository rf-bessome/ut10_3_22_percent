...
Процедура ПередЗакрытием(Отказ,  СтандартнаяОбработка)
	...
	Если Элементы.ПанельНавигации.ТекущаяСтраница = Элементы.СтраницаНавигацииОкончание Тогда
		...
		Если ЕстьМаркиВПромежуточномСтатусе(Объект.ОрганизацияЕГАИС) Тогда
			...
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ЗакрытиеФормы_ЕстьНеобработанныеРасхожденияМарок_Подтверждение", ЭтаФорма),
				ТекстВопроса,
				Кнопки);
	...
	Если Объект.СозданныеДокументы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗакрытиеФормы_Подтверждение", ЭтаФорма),
			ТекстВопроса,
			Кнопки);
	...
	ИначеЕсли Модифицированность И  Элементы.ПанельНавигации.ТекущаяСтраница = Элементы.СтраницаНавигацииПроверкаМарок Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗакрытиеФормы_Марки_Подтверждение", ЭтаФорма),
			ТекстВопроса,
			Кнопки);
...
//#Область Шаг2_ПроверкаДанных
	...
	ИначеЕсли СтрНайти(Действие, ВРег("НеподтвержденныеДокументы")) <> 0
		...
		ОткрытьФорму("Документ." + ИмяДокумента + ".Форма.ФормаСпискаДокументов", ПараметрыФормы, ЭтаФорма);
	...
	ИначеЕсли СтрНачинаетсяС(Действие, ВРег("СоздатьПриказ")) Тогда
		...
		СтандартнаяОбработка = Ложь;
		КорректировкаОстатковЕГАИСКлиентПереопределяемый.СоздатьПриказНаПроведениеИнвентаризации(ЭтаФорма, ТорговыйОбъект);
		
	ИначеЕсли Действие = ВРег("ОткрытьСписокИнвентаризаций") Тогда
		
		СтандартнаяОбработка = Ложь;
		КорректировкаОстатковЕГАИСКлиентПереопределяемый.ОткрытьСписокИнвентаризаций(ЭтаФорма, ТорговыйОбъект);
	...
	ИначеЕсли Действие = ВРег("ВыполнитьОбмен") Тогда
		...
		ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(
			Новый ОписаниеОповещения("ПриЗавершенииВыполненияОбмена", ЭтаФорма),
...
//#Область Шаг3_Марки
	...
	ИначеЕсли СтрНачинаетсяС(Действие, ВРег("МаркиФильтр")) Тогда
		...
		МаркиУстановитьФильтр(ЭтаФорма);
...
//#Область Шаг4_Остатки
	...
	ИначеЕсли Действие = ВРег("ОстаткиПодтвердитьРасхождения") Тогда
		...
		ОстаткиПроверитьТекущийФильтр(ЭтаФорма, ОстаткиТекущийФильтр);
		ОстаткиОбновитьФильтр(ЭтаФорма);
...
Процедура ВариантКорректировкиПриИзменении(Элемент)
	...
	УстановитьТекущуюСтраницуОписания(ЭтаФорма);
...
Процедура СозданныеДокументыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		ПоказатьЗначение(
			Новый ОписаниеОповещения("ПросмотрДокумента_Завершение", ЭтаФорма),
...
Процедура СопоставитьКлассификаторы(Команда)
	...
	СобытияФормЕГАИСКлиент.ОткрытьФормуСопоставленияКлассификаторовЕГАИС(
		ЭтаФорма,
		Новый ОписаниеОповещения("ПриЗавершенииСопоставленияКлассификаторов", ЭтаФорма),
...
Процедура ЗаполнитьПересорт(Команда)
	ЗаполнитьКоличествоПоОперацииПересорт(ЭтаФорма);
КонецПроцедуры
...
Процедура ВыгрузитьВЕГАИС(Команда)
	...
	ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(Объект.ОрганизацияЕГАИС,
		Новый ОписаниеОповещения("ВыгрузкаВЕГАИС_ЗавершениеОбмена", ЭтаФорма),
...
Процедура СтраницаОкончание_ПриПереходеНазад(Отказ)
	...
	Если Объект.СозданныеДокументы.Количество() > 0 Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("УдалениеДокументов_Подтверждение", ЭтаФорма),
...
Процедура ЗапроситьОстаткиЕГАИС()
	...
	Если Элементы.ПанельНеподтвержденныеДокументы.ТекущаяСтраница <> Элементы.СтраницаНеподтвержденныеДокументы_НетДокументов
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗапросОстатковЕГАИС_Подтверждение", ЭтаФорма),
...
Процедура Подключаемый_ОбменПроверкаПолученияОстатковЕГАИС()
	...
	ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(
		Объект.ОрганизацияЕГАИС,
		Новый ОписаниеОповещения("ПроверкаПолученияОстатковЕГАИС",ЭтаФорма),
		УникальныйИдентификатор);
...
Процедура ВыполнитьПредварительнуюПроверкуДанных()
	...
	Иначе
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("ПроверкаДанных_ПослеПроверкиНезагруженныхДокументов", ЭтаФорма);
...
Процедура ПроверитьСтатусУТМ(МассивМарок, ПервоначальнаяПроверка)
	...
	ИнтеграцияЕГАИСКлиент.ПодготовитьЗапросПроверкиАкцизныхМарокНаОстаткахОрганизацииКПередаче(
		Новый ОписаниеОповещения("ПослеПроверкиАкцизныхМарокУТМ", ЭтаФорма, ДополнительныеПараметры),
			Объект.ОрганизацияЕГАИС,
...
Процедура ЗавершитьПроверкуСтрокиМарок(ИдентификаторСтроки)
	...
	Если УдалитьМарок Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ЗавершитьПроверкуУточнитьУдалениеМарок", ЭтаФорма, ДополнительныеПараметры),
...
Процедура ЗапросОстатковПоСправкамОбработчикОжидания()
	...
	Если Не АвтоматическийОбменНастроен Тогда
		ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(
			Объект.ОрганизацияЕГАИС,
			Новый ОписаниеОповещения("ПроверкаПолученияОтчетаЕГАИС", ЭтаФорма),
			УникальныйИдентификатор);
...
Функция ПоместитьДляСопоставления(Номенклатура, Характеристика, АлкогольнаяПродукция, УникальныйИдентификатор)
	...
	Таблица.Колонки.Добавить("Серия", ОпределяемыеТипы().СерияНоменклатуры.Тип);
	Таблица.Колонки.Добавить("ИдентификаторУпаковки",ОпределяемыеТипы().СтрокаЕГАИС.Тип);
...
Функция ПоместитьНесопоставленныеТоварыВоВремененноеХранилище()
	...
	Товары.Колонки.Добавить("ИдентификаторУпаковки",ОпределяемыеТипы().СтрокаЕГАИС.Тип);
...
Процедура ОстаткиЗаполнитьОписаниеИФильтры(ТекущийФильтр = Неопределено)
	...
	ОстаткиПроверитьТекущийФильтр(ЭтаФорма, ТекущийФильтр);
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормЕГАИСПереопределяемый.ДобавитьСлужебныеРеквизиты(ЭтаФорма, "ПомарочныеОстатки");
	...
	СтандартныеПодсистемыСервер.УстановитьУсловноеОформлениеПоляДата(ЭтаФорма, "СозданныеДокументы", "СозданныеДокументыДата");
...
Процедура ЗаполнитьСлужебныеРеквизитыТабличныхЧастей()
	...
	ИнтеграцияИСПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтаФорма, Объект.Корректировки);
	ИнтеграцияИСПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтаФорма, Объект.Остатки);
