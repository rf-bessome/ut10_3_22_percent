...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнтеграцияИС.НастроитьПодключаемоеОборудование(ЭтаФорма, ПоддерживаемыеТипыПодключаемогоОборудования, "Форма");
...
Процедура ПриОткрытии(Отказ)
	...
	НастроитьЭлементыФормыНаСервере(ЭтаФорма);
...
Процедура ДеревоКодовМаркировкиПриАктивизацииСтроки(Элемент)
	...
	УстановитьПоляПредставленияСтроки(ЭтаФорма, ТекущиеДанныеСтроки);
...
Процедура ДекорацияКоэффициентРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ИзменитьПлановоеКоличествоПотребительскихУпаковокПоGTIN" Тогда
		...
		ИнтеграцияИСМПКлиент.ОткрытьФормуНастройкиВидовУпаковокПоGTIN(ПараметрыОткрытия, ЭтаФорма, ВводКоэффициентаЗавершение);
...
Процедура ОбработатьПолученныеДанныеТСДНаСервере(ДанныеДляТСД)
	...
	РезультатОбработкиШтрихкодов = ШтрихкодированиеИС.ОбработатьШтрихкоды(
		...
		Неопределено, ЭтаФорма, КешДанныхРазбора);
...
Функция ОбработатьШтрихкодыНаСервере(ПараметрыСканирования, СоответствиеШтрихкодовСтрокДерева)
	...
	Если РежимРаботыОдинКод И Не ЭтоОтображениеОдногоКода(ЭтаФорма) Тогда
		РежимРаботыОдинКод = Ложь;
		НастроитьЭлементыФормыНаСервере(ЭтаФорма);
	КонецЕсли;
	Если ИдентификаторСтрокиДерева <> Неопределено Тогда
		СпозиционироватьсяНаСтрокеДерева(ЭтаФорма, ИдентификаторСтрокиДерева);
	КонецЕсли;
...
Функция ОбработатьОчереднойШтрихкодНаСервере(ПараметрыСканирования, СоответствиеШтрихкодовСтрокДерева, КешСтрокДереваУпаковок, ИдентификаторСтрокиДерева, ПараметрыОбработки)
	...
	ИдентификаторСтрокиДерева = НайтиДобавитьСтрокуДерева(ЭтаФорма, ДанныеШтрихкода, СоответствиеШтрихкодовСтрокДерева);
...
Процедура Подключаемый_ПолученыДанныеИзТСД(Штрихкоды, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ЗагрузкаДанныхТСД.ТребуетсяАвторизация Тогда
		...
		ИнтерфейсАвторизацииИСМПКлиент.ЗапроситьКлючСессии(
			...
			Новый ОписаниеОповещения("Подключаемый_ЗагрузитьИзТСДПослеАвторизации", ЭтаФорма, ДополнительныеПараметры));
	...
	Если ЗагрузкаДанныхТСД.ОтсутствуютCDNПлощадки Тогда
		...
		ПараметрыЗапроса.ОповещениеОЗавершении   = Новый ОписаниеОповещения("Подключаемый_ЗагрузитьИзТСДПослеАвторизации", ЭтаФорма, ДополнительныеПараметры);
...
Процедура РучнойВводШтрихкодаЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ШтрихкодированиеИСКлиент.ОбработатьДанныеШтрихкода(
		"ПоискПоШтрихкодуЗавершение", ЭтаФорма, ДанныеШтрихкода, ПараметрыСканированияКодовМаркировки());
...
Процедура ЗаполнитьНачальныеПараметрыПриСоздании()
	...
	РежимРаботыОдинКод     = ЭтоОтображениеОдногоКода(ЭтаФорма);
...
Процедура ДобавитьШтрихкодДляПроверки(Штрихкод, СоответствствиеШтрихкодов)
	...
	ИдентификаторНовойСтроки = НайтиДобавитьСтрокуДерева(ЭтаФорма, ДанныеШтрихкода, СоответствствиеШтрихкодов);
