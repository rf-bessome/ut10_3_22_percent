...
Процедура ПересчетНДСиВалютнойСуммыПоСтроке(ТД, ПересчетВалютнойСуммы = Ложь, СуммаСтарая = 0) Экспорт
	...
	ИначеЕсли ТД.СтавкаНДС = Перечисления.СтавкиНДС.НДС18_118 Тогда
		...
		//ТЕАЛ НДС ++
	ИначеЕсли ТД.СтавкаНДС = Перечисления.СтавкиНДС.НДС5 Тогда
		ТД.СуммаНДС = ТД.Сумма * 5 / 100;
	ИначеЕсли ТД.СтавкаНДС = Перечисления.СтавкиНДС.НДС7 Тогда
		ТД.СуммаНДС = ТД.Сумма * 7 / 100;
	ИначеЕсли ТД.СтавкаНДС = Перечисления.СтавкиНДС.НДС5_105 Тогда
		ТД.СуммаНДС = ТД.Сумма * 5 / 105;
	ИначеЕсли ТД.СтавкаНДС = Перечисления.СтавкиНДС.НДС7_107 Тогда
		ТД.СуммаНДС = ТД.Сумма * 7 / 107; 
	ИначеЕсли ТД.СтавкаНДС = Перечисления.СтавкиНДС.НДС22 Тогда
		ТД.СуммаНДС = ТД.Сумма * 22 / 100;	
	ИначеЕсли ТД.СтавкаНДС = Перечисления.СтавкиНДС.НДС22_122 Тогда
		ТД.СуммаНДС = ТД.Сумма * 22 / 122;
	...
	Если ТаблицаАвансов <> Неопределено 
		...
		Для Каждого Выборка Из ТаблицаАвансов Цикл
			...
			Иначе // Если ДокАванса.Метаданные().ТабличныеЧасти.Найти("РасшифровкаПлатежа") <> Неопределено Тогда
				...
				СтрокаТП.СтавкаНДС			= УчетНДС.ОбщаяРасчетнаяСтавкаНДС(Выборка.ДокументАвансаДата);
...
Процедура ОпределитьРанееВыписанныеСчетаФактуры(ТаблицаАвансов)
	...
	Если ТаблицаАвансов.Количество() > 0 Тогда
		...
		|	И СчетФактураВыданный.Ссылка.ВидСчетаФактуры В (ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаАванс), ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.КорректировочныйНаАванс), ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаАвансКомитента))
...
Процедура СформироватьСчетаФактуры(НеиспользуемыеСчетаФактуры, ЕстьОшибки = ложь) Экспорт
	...
	ТаблицаОснований = Список.Выгрузить(, "Дата, ДокументОснование");
	...
	Для каждого СтрокаТаблицы Из ТаблицаОснований Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ОтчетыКомиссионера = ОбщегоНазначения.УдалитьПовторяющиесяЭлементыМассива(ОтчетыКомиссионера, Истина);
	...
	Для Каждого СтрокаДокумента Из ТаблицаОснований Цикл
		...
		ТаблицаКлючевыхПолей = Список.Выгрузить(Новый Структура("Дата, ДокументОснование", СтрокаДокумента.Дата, СтрокаДокумента.ДокументОснование), 
			...
			Для Каждого СтрокаСФ Из СтрокиСФ Цикл
				...
				Если СчФ = Неопределено Тогда
					...
					Если СчФ.ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.НаАванс
					   И СчФ.ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.НаАвансКомитента
					   И СчФ.ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.КорректировочныйНаАванс Тогда
					...
					Если УчетНДС.ПолучитьВерсиюПостановления(СчФ.Дата) = 2 Тогда
						...
						ВерсияКодовВидовОпераций = УчетНДС.ВерсияКодовВидовОпераций(СчФ.Дата);
						...
						Если ЗначениеЗаполнено(СчФ.КодВидаОперации) Тогда
							СчФ.КодВидаОперации = УчетНДС.АктуальныйКодВидаОперации(СчФ.КодВидаОперации, ВерсияКодовВидовОпераций);
						КонецЕсли;
