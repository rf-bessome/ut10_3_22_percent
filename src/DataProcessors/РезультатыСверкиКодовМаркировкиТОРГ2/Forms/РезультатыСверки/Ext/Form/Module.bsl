...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не СохраненВыборПоМаркируемойПродукции Тогда
		...
		СверкаКодовМаркировкиИСМПКлиентСервер.ОтобразитьФильтрПоМаркируемойПродукции(ЭтаФорма, ДанныеВыбораПоМаркируемойПродукции);
	КонецЕсли;
...
Процедура ПредставлениеУстановитьОтборОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "СохранитьДанныеВыбораПоМаркируемойПродукции" Тогда
		...
		СкрытьСтрокиДереваСОтборомПоНоменклатуре(ЭтаФорма);
		
		ШтрихкодированиеИСКлиентСервер.ОтобразитьСохраненныйВыборПоМаркируемойПродукции(ЭтаФорма);
		СверкаКодовМаркировкиИСМПКлиентСервер.ОтобразитьФильтрПоМаркируемойПродукции(ЭтаФорма);
...
Процедура ДеревоРезультатовЭДОВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ДеревоРезультатовЭДОПредставление
		...
		ОбновитьТекущуюСтрокуДереваПоОтборам(ЭтаФорма);
...
Процедура ОтображатьРезультатыФизПроверкиПриИзменении(Элемент)
	...
	СверкаКодовМаркировкиИСМПКлиентСервер.ПересчитатьИтогиПоПроверкеСодержимогоУпаковок(ЭтаФорма.ДеревоМаркированнойПродукции,
		ЭтаФорма.ОтображатьРезультатыФизПроверки,
		ЭтаФорма.ЭтоПродукцияМОТП);
...
Процедура ДобавитьКолонкиПриВосстановленииДерева(ДеревоМаркированнойПродукцииОбъект)
	...
	Если ДеревоМаркированнойПродукцииОбъект.Колонки.Найти(ИмяКолонки) = Неопределено Тогда
		ДеревоМаркированнойПродукцииОбъект.Колонки.Добавить(ИмяКолонки, ОпределяемыеТипы().ВариантДействийПоРасхождениямКодовМаркировкиИСМП.Тип);
	КонецЕсли;
...
Функция ДеревоРезультатовЭДО()
	...
	ДеревоЗначенийРезультатовЭДО.Колонки.Добавить("Номенклатура",                        ОпределяемыеТипы().Номенклатура.Тип);
	ДеревоЗначенийРезультатовЭДО.Колонки.Добавить("Характеристика",                      ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
...
Процедура УстановитьДействиеДляВыделенныхСтрок(ВыделенныеСтроки, НовоеДействие)
	...
	Для Каждого ИдентификаторВыделеннойСтроки Из ВыделенныеСтроки Цикл
		...
		УстановитьДействиеДляПодчиненных(ЭтаФорма, ДанныеСтроки, НовоеДействие);
