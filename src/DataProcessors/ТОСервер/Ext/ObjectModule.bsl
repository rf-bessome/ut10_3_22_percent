...
Функция ПодключитьКлиента(Клиент) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") Тогда
		Если ТипЗнч(Клиент) = Тип("Форма") Или ТипЗнч(Клиент) = Тип("УправляемаяФорма") Тогда
			...
			ВидТО = Перечисления.ВидыТорговогоОборудования.ТерминалСбораДанных;
			Если Клиент.ПоддерживаетсяВидТО(ВидТО) Тогда
					МенеджерОборудованияКлиентПереопределяемый.НачатьПодключениеОборудованиеПриОткрытииФормы(
						Клиент,
						МенеджерОборудованияУТ.ВидПодключаемогоОборудованияПоВидуТорговогоОборудования(ВидТО));
...
Процедура ОтключитьКлиента(Клиент) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") Тогда
		Если ТипЗнч(Клиент) = Тип("Форма") Или ТипЗнч(Клиент) = Тип("УправляемаяФорма") Тогда
			МенеджерОборудованияКлиентПереопределяемый.НачатьОтключениеОборудованиеПриЗакрытииФормы(Клиент);
			Возврат;
		КонецЕсли;
	КонецЕсли;
	// Конец ПодключаемоеОборудование
...
Функция ОбработатьВнешнееСобытие(Источник, Событие, Данные, Клиент) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование")
		И (Источник = "СканерШтрихкода" Или СтрНайти(Источник, "InputDevice") > 0)
		И (ТипЗнч(Клиент) = Тип("Форма") Или ТипЗнч(Клиент) = Тип("УправляемаяФорма")) Тогда
		...
		Если Результат <> Неопределено
			...
			И ТипЗнч(Результат.Параметр) = Тип("Массив")
			И Результат.Параметр.Количество() > 0 Тогда
			
			ОбработатьВведенныйШтрихкод(Результат.Параметр[0], Клиент);
...
Функция ПолучитьСписокУстройств(Вид, Отбор = Неопределено) Экспорт
	...
	// ПодключаемоеОборудование
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование")
		И (Вид = Перечисления.ВидыТорговогоОборудования.СканерШтрихКода
		Или Вид = Перечисления.ВидыТорговогоОборудования.ТерминалСбораДанных) Тогда
		
		ВидОборудования = МенеджерОборудованияУТ.ВидПодключаемогоОборудованияПоВидуТорговогоОборудования(Вид);
		Возврат Справочники.ПодключаемоеОборудование.ОборудованиеПоПараметрам(ВидОборудования);
...
Функция ПолучитьПредставлениеУстройства(Идентификатор, Вид, Представление) Экспорт
	...
	// ПодключаемоеОборудование
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") И ТипЗнч(Идентификатор) = Тип("Структура") Тогда
		
		Вид           = МенеджерОборудованияУТ.ВидТорговогоОборудованияПоВидуПодключаемогоОборудования(Идентификатор.ТипОборудования);
...
Функция ВыгрузитьТаблицуТСД(Идентификатор, ТаблицаВыгрузки) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") И ТипЗнч(Идентификатор) = Тип("Структура") Тогда
		...
		ОповещениеПриЗавершении = ОписаниеОповещенияИС(
			"ЗавершениеВыполненияКоманды",
			"МенеджерОборудованияУТ",
			РезультатВыполнения);
			
		МенеджерОборудованияКлиент.НачатьВыгрузкуДанныеВТСД(
			ОповещениеПриЗавершении,
			Идентификатор,
			МенеджерОборудованияУТ.ТаблицуЗначенийВМассив(ТаблицаВыгрузки));
		
		Если РезультатВыполнения.Результат.Результат Тогда
			Сообщить(НСтр("ru='Выгрузка на терминал произведена успешно'"));
		КонецЕсли;
...
Функция ПечатьЧека(Идентификатор, Пароль, НомерЧека, НомерСмены,
                ОписаниеЧека, СуммаНал, СуммаБезнал, ПризнакВозврата, ДопДанные = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			...
			ЗакрытьЧек = Истина;
			Если НЕ ЗначениеЗаполнено(Результат) И ДопДанные <> Неопределено И ТипЗнч(ДопДанные) = Тип("Структура") Тогда
				...
				Если ДопДанные.Свойство("ПараметрыПередачиВЕГАИС", ПараметрыПередачи) Тогда
					...
					ОписаниеОповещения = ОписаниеОповещенияИС("ПробитьЧекЗавершение", Чек);
					
					ЕстьАлкогольнаяПродукция = Ложь;
					Для Каждого СтрокаТЧ Из Чек.Товары Цикл
						Если СтрокаТЧ.Номенклатура.АлкогольнаяПродукция
							И СтрокаТЧ.Номенклатура.ВидАлкогольнойПродукцииЕГАИС.Маркируемый Тогда
					...
					Если ЕстьАлкогольнаяПродукция Тогда
						...
						ПараметрыОперации.Вставить("Обработка", Обработка);
						ПараметрыОперации.Вставить("Объект", Объект);
						ПараметрыОперации.Вставить("СуммаНал", СуммаНал);
						ПараметрыОперации.Вставить("СуммаБезнал", СуммаБезнал);
						ПараметрыОперации.Вставить("НомерЧека", НомерЧека);
						ПараметрыОперации.Вставить("Чек", Чек);
						ПараметрыОперации.Вставить("мНетОшибки", мНетОшибки);
						...
						Если ПараметрыВыполнения.ПродолжитьПечать Тогда
							...
							ПараметрыВыполнения.Вставить("ПараметрыВозвратаИзЕГАИС", ПараметрыВыполнения);
						...
						Иначе
							...
							Обработка.ОтменитьЧек(Объект);
							Результат = Перечисления.ТООшибкиОбщие.ОшибкаНеизвестно;
...
//  ПараметрыККТ				   - <Структура>
//                                 - Структура параметров ККТ полученная из драйвера.
//
// Возвращаемое значение:
//  <ПеречислениеСсылка.ТООшибки*> - Результат работы функции
//
Функция ВнестиСумму(Идентификатор, Пароль, Сумма, ПараметрыККТ = Неопределено ) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		Результат = Обработка.Сумма(Объект, Пароль, Сумма, ПараметрыККТ);
		мОписаниеОшибки = Объект.ОписаниеОшибки;
	Иначе
		мОписаниеОшибки = ПолучитьТекстОшибкиПодключенияТО("ФР");
	КонецЕсли;
...
Функция ВыгрузитьТоварыККМ(Идентификатор, Товары) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		НомерЧека  = Неопределено;
		НомерСмены = Неопределено;
		КолонкаСерий = Товары.Колонки.Найти("СерияНоменклатуры");
		
		Если КолонкаСерий <> Неопределено Тогда
			Попытка
				КолонкаСерий.Имя = "Серия";
			Исключение
				мОписаниеОшибки = "Недопустимое новое имя колонки или такая колонка существует...";
				Возврат Перечисления.ТООшибкиККМOffline.ОшибкаВыгрузкиТоваров;
			КонецПопытки;
		КонецЕсли;
...
// Функция осуществляет получение модели устройства.
//
// Параметры:
//  Идентификатор               - <Строка>
...
//
// Возвращаемое значение:
//  <СправочникСсылка.ТорговоеОборудование> - Модель устройства.
//
Функция ПолучитьМодель(Идентификатор) Экспорт

	Результат = мПустаяМодель;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		Результат = Выборка.Модель;
	КонецЕсли;

	Возврат Результат;

КонецФункции // ПолучитьМодель()

// Функция осуществляет получение вида устройства.
//
// Параметры:
//  Идентификатор               - <Строка>
//                              - Идентификатор устройства.
//
// Возвращаемое значение:
//  <ПеречислениеСсылка.ВидыТорговогоОборудования> - Вид устройства.
//
Функция ПолучитьВид(Идентификатор) Экспорт

	// ПодключаемоеОборудование
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") И ТипЗнч(Идентификатор) = Тип("Структура") Тогда
		
		Возврат МенеджерОборудованияУТ.ВидТорговогоОборудованияПоВидуПодключаемогоОборудования(Идентификатор.ТипОборудования);
		
	КонецЕсли;
	// Конец ПодключаемоеОборудование
	
	Результат = Перечисления.ВидыТорговогоОборудования.ПустаяСсылка();
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
		Результат = Выборка.Вид;
	КонецЕсли;

	Возврат Результат;

КонецФункции // ПолучитьВид()

// Функция осуществляет получение параметров устройства.
//
// Параметры:
//  Идентификатор               - <Строка>
//                              - Идентификатор устройства.
...
//
Функция ПолучитьПараметры(Идентификатор) Экспорт
...
Функция ОбработатьСобытиеСШК(Обработка, Объект, Клиент, Событие, Данные, СШК)
	...
	Если НЕ ЗначениеЗаполнено(ТипШК) И ЭтоITF14 Тогда
		...
		|            РегШК.Качество
		|    КОНЕЦ                            КАК Качество,
		|    РегШК.ЕдиницаИзмерения.Коэффициент          КАК Количество
		|ИЗ
		|    РегистрСведений.Штрихкоды        КАК РегШК
		|ГДЕ
		|    РегШК.ТипШтрихкода      = &ТипEAN13
...
Функция ПолучитьОбъектДрайвера(Идентификатор, Обработка, Объект) Экспорт
	...
	// ПодключаемоеОборудование
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") И ТипЗнч(Идентификатор) = Тип("Структура") Тогда
		СписокОборудования = Справочники.ПодключаемоеОборудование.ОборудованиеПоПараметрам(
			Идентификатор.ТипОборудования,
			Идентификатор);
			
		Если СписокОборудования.Количество() > 0 Тогда
			Обработка = СписокОборудования[0].ОбработчикДрайвера;
			Объект    = СписокОборудования[0].ДрайверОборудования;
...
Процедура ЗагрузитьИзТерминалаСбораДанных(Терминалы, ФормаКлиент) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПодключаемоеОборудование") Тогда
		...
		ДополнительныеПараметры.Вставить("Клиент", ФормаКлиент);
		ДополнительныеПараметры.Вставить("СерверТО", ПолучитьСерверТО());
		МенеджерОборудованияКлиент.НачатьЗагрузкуДанныеИзТСД(
			ОписаниеОповещенияИС("НачатьЗагрузкуДанныеИзТСД", ИнтеграцияЕГАИСУТКлиентСервер, ДополнительныеПараметры),
			ФормаКлиент.УникальныйИдентификатор);
	...
	Если Терминалы = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
мПустаяМодель               = Справочники.ТорговоеОборудование.ПустаяСсылка();
