...
Процедура ПодготовитьДанныеДляЗаполненияДокументов(МассивСсылокНаОбъект, АдресХранилища) Экспорт
	...
	Для Каждого СсылкаНаОбъект Из МассивСсылокНаОбъект Цикл
		...
		ВерсияФормата = ЭлектронныеДокументыСлужебный.АктуальнаяВерсияФорматаЭД(НастройкиОбмена.ВидЭД);
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		Если НЕ ЗначениеЗаполнено(ПолноеИмяФайла) Тогда
			Продолжить;
		КонецЕсли;
		Если ВРег(СтруктураОбмена.СтруктураЭД.ВидЭД) <> ВРег("РеквизитыОрганизации") Тогда
			СтруктураОбмена.СтруктураЭД.Вставить("ВладелецЭД", СтруктураОбмена.СтруктураЭД.ДокументыОснования[0]);
		КонецЕсли;
...
Функция СформироватьЭДПрисоединенныйФайлПакетаТакском(СтруктураОбмена) Экспорт
	...
	// Формируем meta.xml.
	ЭлектронныеДокументыВнутренний.СформироватьЭДТранспортнойИнформации(СтруктураОбмена.СтруктураЭД,
	СтруктураФайловЭД, АдресКаталога, ТекстОшибки);
	
	// Формируем card.xml.
	ЭлектронныеДокументыВнутренний.СформироватьЭДКарточки(СтруктураОбмена.СтруктураЭД, АдресКаталога, ТекстОшибки);
...
Функция БыстрыйОбменИмяСохраняемогоФайла(ВладелецЭД)
	...
	НаименованиеФайла = ЭлектронныеДокументыСлужебный.СтрокаЛатиницей(НаименованиеФайла);
