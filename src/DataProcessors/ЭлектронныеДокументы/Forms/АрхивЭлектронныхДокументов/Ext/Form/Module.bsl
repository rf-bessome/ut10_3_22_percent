	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ВидПараметра = "Ответственный" Тогда
				...
				ЭДВходящий.Ответственный = ЗначениеПараметра;
...
Процедура СравнитьДанныеЭД(ТекущийСписок)
	...
	#Если НЕ ТолстыйКлиентУправляемоеПриложение И НЕ ТолстыйКлиентОбычноеПриложение Тогда
		Сообщить(НСтр("ru='Сравнение электронных документов можно сделать только в режиме толстого клиента (обычного приложения).'"));
		Возврат;
	#Иначе
...
Функция ПолучитьДанныеПрисоединенныхФайловПакетовЭДНаСервере(Знач ПакетыЭД, УникальныйИдентификатор)
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеФайла = ПрисоединенныеФайлы.ПолучитьДанныеФайла(Выборка.Ссылка, УникальныйИдентификатор);
...
Процедура ВыполнитьСравнениеЭД(СтруктураПараметров)
	...
	#Иначе
		...
		МассивЭД = Новый Массив;
		МассивЭД.Добавить(ЭлектронныеДокументыСлужебный.ПрисоединенныйФайл(ЭДПервый));
		МассивЭД.Добавить(ЭлектронныеДокументыСлужебный.ПрисоединенныйФайл(ЭДВторой));
...
Процедура СохранитьПакетыЭДНаДиск(Команда)
	...
	НаборДанныхФайлов = ПолучитьДанныеПрисоединенныхФайловПакетовЭДНаСервере(Элементы.ВсеПакеты.ВыделенныеСтроки, УникальныйИдентификатор);
	
	Для каждого ДанныеФайла Из НаборДанныхФайлов Цикл
...
&НаКлиенте
Процедура СостояниеЭДОИсхПриИзменении(Элемент)
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ВидЭД.СписокВыбора.СортироватьПоЗначению();
	...
	Элементы.ВидЭДИсх.СписокВыбора.СортироватьПоЗначению();
	
	// установим выделение красным неверных подписей по доверенности
	ЭлементыФормыСКонтролемПодписи = Новый Массив;
	ЭлементыФормыСКонтролемПодписи.Добавить(ЭтаФорма.Список);
	ЭлементыФормыСКонтролемПодписи.Добавить(ЭтаФорма.СписокИсх);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ОтборПоЭлементуПриЗагрузкеИзНастроек(ЭтаФорма, Список, "СостояниеЭДО", Настройки);
	
	ОтборПоЭлементуПриЗагрузкеИзНастроек(ЭтаФорма, СписокИсх, "ВидЭДИсх",        Настройки);
	ОтборПоЭлементуПриЗагрузкеИзНастроек(ЭтаФорма, СписокИсх, "СостояниеЭДОИсх", Настройки);
