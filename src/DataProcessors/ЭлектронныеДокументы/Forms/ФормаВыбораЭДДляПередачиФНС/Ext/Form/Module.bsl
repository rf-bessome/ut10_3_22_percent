...
Процедура ДоступныеДокументыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Не ТипЗнч(ВыбраннаяСтрока) = Тип("Массив") Тогда
...
Процедура БыстрыеОтборыЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли ВРег(Элементы.БыстрыеОтборы.ТекущиеДанные.Параметр) = ВРег("СостояниеВерсииЭД") Тогда
		...
		Если СостоянияЭД.ОтметитьЭлементы("Состояния ЭД") Тогда
			ИзменитьСостоянияЭД(СостоянияЭД);
		КонецЕсли;
...
&НаКлиенте
Процедура ИзменитьСостоянияЭД(Результат) Экспорт
...
Функция СформироватьФайлВыгрузки(Знач ТаблицаДокументов)
	...
	Пока Результат.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Пока Результат.СледующийПоЗначениюПоля("ЭлектронныйДокумент") Цикл
			...
			СтрокаТЗОписи.КНД = ЭлектронныеДокументыПовтИсп.КНДпоВидуЭД(СвойстваДокумента);
			...
			Если ЗначениеЗаполнено(Результат.ПодтверждениеСсылка) Тогда
				...
				СтрокаТЗОписи.КНДПодтверждения = ЭлектронныеДокументыПовтИсп.КНДпоВидуЭД(СвойстваДокумента);
...
Функция ФайлОписанияВыгрузки(Организация, ТЗОписи, АдресКаталога)
	...
	Попытка
		...
		ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(Файл, "ВерсФорм", "1.03", Истина, ТекстОшибки);
