...
Процедура ЗаполнитьПоДаннымОрганизации() Экспорт
	...
	Для каждого СтрокаДвижений Из ТабЗнач Цикл
		...
		Поз = Найти(Представление, "входящее");
		Если Поз > 0 Тогда
			Представление = Лев(Представление, Поз - 1) + "исходящее" + Сред(Представление, Поз + 10);
			
		Иначе	
			Поз = Найти(Представление, "исходящее");
			Если Поз > 0 Тогда
				Представление = Лев(Представление, Поз - 1) + "входящее" + Сред(Представление, Поз + 9);
			КонецЕсли;
