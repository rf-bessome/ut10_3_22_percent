...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДокументОснование.ДоступныеТипы = ОпределяемыеТипы().ОснованиеВнесениеСведенийОСобранномУрожаеЗЕРНО.Тип;
...
Процедура ТоварыОКПД2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияЗЕРНОКлиент.ОткрытьФормуПодбораОКПД2(ЭтаФорма, ТекущиеДанные, Элемент, ДополнительныеПараметры);
...
Процедура Аннулировать(Команда)
	...
	Иначе
		...
		КонецЕсли;
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ВопросАннулированиеЗавершение", ЭтаФорма, Данные),
			ТекстВопроса,
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ИнтеграцияЗЕРНОКлиентСервер.УстановитьПараметрыВыбораНоменклатуры(ЭтаФорма, Перечисления.ВидыПродукцииИС.Зерно);
...
Процедура ВыполнитьАннулирование(Данные)
	...
	ПоказатьВопрос(
		Новый ОписаниеОповещения("ПодтверждениеАннулированияЗавершение", ЭтаФорма, Данные),
		ТекстВопроса,
		РежимДиалогаВопрос.ДаНет);
