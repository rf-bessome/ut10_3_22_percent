...
Процедура ПервичныйДокументПриИзменении(Элемент)
	...
	Если ДанныеПервичногоДокумента <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ЭтаФорма, ДанныеПервичногоДокумента);
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормы(СписокРеквизитов = "")
	...
	Если Инициализация Или СтруктураРеквизитов.Свойство("ВидПервичногоДокумента") Тогда
		СобытияФормИСМПКлиентСервер.УправлениеДоступностьюЭлементовФормы(ЭтаФорма);
	...
	Для Каждого ДоступныйТип Из ОпределяемыеТипы().ВозвратВОборотПервичныйДокументИСМП.Тип.Типы() Цикл
		
		ЭтоОптовыйДокумент = ОпределяемыеТипы().ОснованиеОтгрузкаТоваровИСМП.Тип.СодержитТип(ДоступныйТип);
