...
Процедура ЗаполнитьТоварыПоОснованию(ДокументОснование) Экспорт
	...
	ЕстьПрослеживаемыйТовар      = ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ПрослеживаемыйТовар",МетаданныеДокументаОснования, "Товары");
	...
	|"
		+ ?(ЕстьПрослеживаемыйТовар,"
	|	Док.ПрослеживаемыйТовар 				КАК ПрослеживаемыйТовар,
	|	Док.СтранаПроисхождения 				КАК СтранаПроисхождения,
	|	Док.ИдентификаторСтроки					КАК ИдентификаторСтроки,
	|","
...
Процедура ЗаполнитьРНПТПоОснованию(ДокументОснование) Экспорт
	ТипДокументаОснования = ТипЗнч(ДокументОснование);
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ВидРегистрацииВКнигеПокупок = 3 Тогда
		...
		КонецЕсли;	
	Иначе
		Если (ЗаполненНомерРКО И НЕ ЗаполненаДатаРКО) ИЛИ (НЕ ЗаполненНомерРКО И ЗаполненаДатаРКО) Тогда
			ОбщегоНазначения.СообщитьОбОшибке("При регистрации в книге покупок даты и номера расходно-кассового ордера должны быть заполнены и дата и номер.",
			Отказ, Заголовок);
		КонецЕсли;	
	КонецЕсли;
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(УсловияПрослеживаемости, СтруктураШапкиДокумента)
	...
	Запрос.Текст = Документы.ВозвратТоваровОтПокупателя.ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, УсловияПрослеживаемости);
	...
	Если УсловияПрослеживаемости.ВедетсяУчетПрослеживаемыхТоваров Тогда
		...
		Если ЭтоКомиссия Тогда
			ТаблицаПрослеживаемыеТоварыВозвращаемыеОтКомиссионера = ПараметрыПроведения.ПрослеживаемыеТовары.Скопировать();
			ТаблицаПрослеживаемыеТоварыВозвращаемыеОтКомиссионера.ЗаполнитьЗначения(
				СтруктураШапкиДокумента.Контрагент, "Комиссионер");
			ПрослеживаемостьБП.СформироватьДвиженияРеализацияТоваров(
				ТаблицаПрослеживаемыеТоварыВозвращаемыеОтКомиссионера,
				Неопределено,
				ТаблицаРеквизиты,
				Движения);
		КонецЕсли;
		...
		ПрослеживаемостьБП.СформироватьДвиженияПоступлениеТоваров(
			...
			?(ЭтоКомиссия, Неопределено, ПараметрыПроведения.ПрослеживаемыеОперации),
			Неопределено,
			ТаблицаРеквизиты,
			Движения);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Если Основание.Проведен Тогда
			...
			ЗаполнитьРНПТПоОснованию(Основание);
