...
Функция АннулироватьВыводИзОборотаJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	//@skip-warning
	ПрисоединенныеФайлыСписок = РезультатЗапроса["ПрисоединенныеФайлы"].Выгрузить();
	Если ПрисоединенныеФайлыСписок.Количество() = 0 Тогда
...
Функция ПолучитьЕдиницуИзмеренияПоКлассификатору(Номенклатура, ЕдиницаПоКлассификатору)
