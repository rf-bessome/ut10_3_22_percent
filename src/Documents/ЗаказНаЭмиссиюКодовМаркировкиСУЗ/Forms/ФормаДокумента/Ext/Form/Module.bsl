...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДокументОснование.ДоступныеТипы = ОпределяемыеТипы().ОснованиеЗаказНаЭмиссиюКодовМаркировкиИСМП.Тип;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ОбновитьВидимостьДоступностьЭлементовФормы(ЭтаФорма);
	
	ЗаполнитьТаблицуПредставленийНоменклатуры();
	
	РазблокироватьДанныеФормыДляРедактирования();

	УстановитьПризнакНабораИГрупповойУпаковки(ЭтаФорма, Объект.Товары);

	НастроитьЗависимыеЭлементыФормы(ЭтаФорма);
	
	// РаботаСПолямимСоставногоТипа
	СобытияФормИС.ПоляСоставногоТипаОбновитьПредставление(ЭтаФорма);
	// Конец РаботаСПолямимСоставногоТипа
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЭтоМаркировкаОстатков Тогда
		...
		Если ЭтоРасширеннаяМаркировкаОстатковЛегкойПромышленности(ЭтаФорма) Тогда
		...
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			...
			ПроверитьОчиститьРеквизитыУпрощенногоОписанияОстатков(ЭтаФорма, СтрокаТовары);
...
Процедура ВидПродукцииПриИзменении(Элемент)
	...
	ЗаполнитьФлагЭтоМаркировкаОстатков(ЭтаФорма);
	
	ВидПродукцииПриИзмененииНаСервере(Ложь);
	
	ПроверитьОчиститьВладельцаGTIN(ЭтаФорма);
	ПроверитьОчиститьСервисПровайдера(ЭтаФорма);
	ОбновитьВидимостьДоступностьЭлементовФормы(ЭтаФорма);
...
Процедура ВидПродукцииОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Объект.Товары.Количество() > 0
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ВопросПриИзмененииВидаПродукцииПриЗавершении", ЭтаФорма, ВыбранноеЗначение);
...
Процедура ТоварыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НоваяСтрока и ОтменаРедактирования Тогда
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтрокиВладельца) Тогда
			...
			ДобавитьСтрокиПоВладельцу(ЭтаФорма, МассивСтрок, ТекущиеДанные.ИдентификаторСтрокиВладельца);
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	Иначе
		...
		ИначеЕсли ЗначениеЗаполнено(ДанныеСтроки.ИдентификаторСтрокиВладельца) Тогда
			...
			СтрокаВладелец = ПолучитьСтрокуВладельца(ЭтаФорма, ДанныеСтроки.ИдентификаторСтрокиВладельца);
	...
	Если Отказ Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ВопросУдалениеСтрокТоваровЗавершение", ЭтаФорма, ДополнительныеПараметры);
	...
	Иначе
		Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			ДанныеСтроки = Объект.Товары.НайтиПоИдентификатору(ВыделеннаяСтрока);
			ОчиститьСерийныеНомераПоИдентификатору(ЭтаФорма, ДанныеСтроки.ИдентификаторСтроки);
		КонецЦикла;
	КонецЕсли;
...
Процедура ТоварыНоменклатураХарактеристикаПриИзмененииСервер(
	...
	Если НоменклатураХарактеристикаУказана Тогда
		...
		Если ВыполнятьЗаполнение Тогда
			...
			Если ИзмененаНоменклатураХарактеристика Тогда
				Для Каждого ДобавленнаяСтрока Из СтрокиДляОбработки Цикл
					ОбновитьСтатусУказанияСерийныхНомеровПоСтроке(ЭтаФорма, ДобавленнаяСтрока);
				КонецЦикла;
			КонецЕсли;
			ПолноеОписание = Неопределено;
		КонецЕсли;
		...
		ПроверитьОчиститьРеквизитыУпрощенногоОписанияОстатков(ЭтаФорма, ДанныеСтроки);
...
Процедура ТоварыТоварныйЗнакПриИзменении(Элемент)
	...
	Если ИспользуютсяГрупповыеУпаковки
		...
		ДобавитьСтрокиПоВладельцу(ЭтаФорма, СтрокиГрупповойУпаковки, ТекущиеДанные.ИдентификаторСтрокиВладельца);
...
Процедура ТоварыПолноеОписаниеОстатковПриИзменении(Элемент)
	...
	ПроверитьОчиститьРеквизитыУпрощенногоОписанияОстатков(ЭтаФорма, ТекущиеДанные);
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	СобытияФормИСМПКлиентПереопределяемый.ПриИзмененииКоличества(ЭтаФорма, ТекущиеДанные, КэшированныеЗначения);
	
	ОбновитьСтатусУказанияСерийныхНомеровПоСтроке(ЭтаФорма, ТекущиеДанные);
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтрокиВладельца) Тогда
		...
		ИначеЕсли ИспользуютсяГрупповыеУпаковки
			...
			МассивСтрокБезПотребительскихУпаковок = ПересчитатьКоличествоСоставляющихГрупповойУпаковки(ЭтаФорма, ТекущиеДанные);
...
Процедура ТоварыСпособФормированияСерийногоНомераПриИзменении(Элемент)
	...
	Если ТекущиеДанные.СпособФормированияСерийногоНомера = АвтоматическоеФормирование Тогда
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ВопросОбУдаленииСвязанныхСерийныхНомеровЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ТоварыGTINПриИзменении(Элемент)
	...
	ПроверитьОчиститьРеквизитыУпрощенногоОписанияОстатков(ЭтаФорма, ТекущиеДанные);
	ОбновитьДанныеФормыМаркировкаОстатков();
	ПроверитьОчиститьВладельцаGTIN(ЭтаФорма, ТекущиеДанные);
...
Процедура ТоварыВладелецGTINСтрокойПриИзменении(Элемент)
	...
	ВладелецGTINПриИзменении(ЭтаФорма, Элементы.Товары.ТекущиеДанные);
...
Процедура Включить1СНоменклатуру(Команда)
	...
	ОткрытьФорму(
		...
		ЭтаФорма,,,,
		Новый ОписаниеОповещения("Включить1СНоменклатуруЗавершение", ЭтаФорма));
...
Процедура ПодготовитьПередатьСведенияОТоварахВНациональныйКаталог(Команда)
	...
	РаботаСНоменклатуройКлиент.ОткрытьФормуПомощникВыгрузки(
		ПараметрыФормы,
		ЭтаФорма,
		Новый ОписаниеОповещения("ПодготовитьПередатьСведенияОТоварахВНациональныйКаталогЗавершение", ЭтаФорма, ПараметрыОповещения),
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура Печать(Команда)
	...
	СобытияФормИСМПКлиент.ОтрытьФормуПечатиКодовМаркировок(ЭтаФорма, Объект);
...
Процедура ЗаполнитьПараметрыОписанияОстатков(Команда)
	...
	ОповещениеЗакрытияФормыЗаполненияОстатков = Новый ОписаниеОповещения(
		"Подключаемый_ЗаполнениеПараметровОстатковЗавершение", ЭтаФорма);
...
Процедура СброситьПризнакПечати(Команда)
	...
	ОписаниеОповещенияОЗавершении = Новый ОписаниеОповещения("ВопросСбросаПризнакаПечатиПриЗавершении", ЭтаФорма);
...
Процедура УстановитьОтборПоНаборуГрупповойУпаковке(Команда)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ИдентификаторСтрокиВладельца) Тогда
		...
		Иначе
			СтруктураОтбора.Вставить("ИдентификаторСтрокиВладельца", ТекущиеДанные.ИдентификаторСтрокиВладельца);
			СтрокаВладелец = ПолучитьСтрокуВладельца(ЭтаФорма, ТекущиеДанные.ИдентификаторСтрокиВладельца);
			Если СтрокаВладелец = Неопределено Тогда
				НаименованиеТовараВладельца = "";
			Иначе
				НаименованиеТовараВладельца = Строка(СтрокаВладелец.Номенклатура);
...
Процедура ЗагрузитьИзТСДЗавершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	ОбновитьСтатусУказанияСерийныхНомеров(ЭтаФорма);
...
Процедура РасчетПризнакаПолногоОписанияОстатков(СтрокиДляОбработки, СброситьПолноеОписание = Истина, НовыеСтроки = Ложь)
	...
	Если Не НовыеСтроки
		И ЭтоРасширеннаяМаркировкаОстатковЛегкойПромышленности(ЭтаФорма) Тогда
		ПолноеОписание = Неопределено;
	Иначе
		ПолноеОписание = ИнтеграцияИСКлиентСервер.ИспользованиеСпособаОписанияОстатков(Объект.ВидПродукции);
	КонецЕсли;
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если Не ПодключенаОбработкаКодовМаркировки Тогда
		ПроверкаИПодборПродукцииИС.ПодключитьОбработкуКодовМаркировки(ЭтаФорма, Ложь, КлючСвязиСтатусаСтрок(Объект.ВидПродукции));
...
Процедура ОбработкаРезультатаПодбораНоменклатуры(ВыбранноеЗначение, КэшированныеЗначения)
	...
	Если ДобавленныеСтроки.Количество() > 0 Тогда
		УстановитьПризнакНабораИГрупповойУпаковки(ЭтаФорма, ДобавленныеСтроки);
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, КэшированныеЗначения, ТекстОшибки = "")
	...
	ШтрихкодированиеИСПереопределяемый.ПодготовитьДанныеДляОбработкиШтрихкодов(
		ЭтаФорма, ДанныеПоШтрихкодам, ПараметрыЗаполнения, ДанныеДляОбработкиШтрихкодов);
	
	ШтрихкодированиеИСПереопределяемый.ОбработатьШтрихкоды(ЭтаФорма, ДанныеДляОбработкиШтрихкодов, КэшированныеЗначения);
...
Процедура ОткрытьФормуВыбораСерий()
	...
	ОткрытьФорму("Документ.ЗаказНаЭмиссиюКодовМаркировкиСУЗ.Форма.ФормаВыбораСерий", 
		...
		Новый ОписаниеОповещения("ФормаВыбораСерийЗавершение", ЭтаФорма, ДополнительныеПараметры),
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВопросОбУдаленииСвязанныхСерийныхНомеровЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ОчиститьСерийныеНомераПоИдентификатору(ЭтаФорма, ДополнительныеПараметры.ТекущиеДанные.ИдентификаторСтроки);
		ОбновитьСтатусУказанияСерийныхНомеровПоСтроке(ЭтаФорма, ДополнительныеПараметры.ТекущиеДанные);
	КонецЕсли;
...
Процедура ЗаполнитьДоступныеGTINНаКлиенте(РезультатАвторизации = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ТребуетсяОбновитьКлючСессии
		...
		ОповещениеПриАвторизации = Новый ОписаниеОповещения("ЗаполнитьДоступныеGTINНаКлиенте", ЭтаФорма);
		...
		Для Каждого СтрокаДокумента Из Объект.Товары Цикл
			...
			Для Каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
				...
				ИначеЕсли СтрокаТаблицы.ДоступныеGTIN.Количество() И Не ЗначениеЗаполнено(СтрокаДокумента.GTIN) Тогда
					...
					ПроверитьОчиститьРеквизитыУпрощенногоОписанияОстатков(ЭтаФорма, СтрокаДокумента);
				КонецЕсли;
...
Функция ПоместитьДанныеДляПередачиСведенийОТоварахВНациональныйКаталог(Данные, УникальныйИдентификатор)
	...
	Результат.Колонки.Добавить("Характеристика", ОпределяемыеТипы().ХарактеристикаНоменклатуры.Тип);
...
Функция ОбработатьПолученныеДанныеТСДНаСервере(ШтрихкодыТСД, ПараметрыСканирования, КэшированныеЗначения)
	...
	Результат = ГрупповаяОбработкаШтрихкодовИС.ОбработатьПолученныеДанныеТСДВДокументе(
		ЭтаФорма, ШтрихкодыТСД, ПараметрыСканирования, Ложь);
...
Процедура ПослеОбработкиШтрихкодовСервер(РезультатОбработкиШтрихкода, КэшированныеЗначения)
	...
	Для Каждого ДобавленнаяСтрока Из ДобавленныеСтроки Цикл
		...
		СобытияФормИСМППереопределяемый.ПриИзмененииНоменклатуры(
			ЭтаФорма, ДобавленнаяСтрока, Неопределено, Неопределено);
	...
	Для Каждого ИзмененнаяСтрока Из ИзмененныеСтроки Цикл
		...
		СобытияФормИСМППереопределяемый.ПриИзмененииКоличества(
			ЭтаФорма, ИзмененнаяСтрока, Неопределено, Неопределено);
	...
	Если ИзмененныеСтрокиГрупповыхУпаковок.Количество() > 0 Тогда
		ПересчитатьКоличествоСоставляющихГрупповойУпаковки(ЭтаФорма, ИзмененныеСтрокиГрупповыхУпаковок);
	КонецЕсли;
...
Функция ПроверкаВыбранногоСервисПровайдера(СервисПровайдер, ВидПродукции, СпособВводаВОборот, ТекстОшибки = Неопределено)
	...
	|	ВЫБОР
	|		КОГДА СервисПровайдерыСУЗ.Страна = ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.Россия)
...
//    ** Ключ - СправочникСсылка.КлассификаторСтранМира.Россия
//            - СправочникСсылка.КлассификаторСтранМира.ПустаяСсылка
...
Функция ДоступныеСпособыВводаВОборотПоВидуСервисПровайдера()
	...
	Россия = ПредопределенноеЗначение("Справочник.КлассификаторСтранМира.Россия");
	Прочее = ПредопределенноеЗначение("Справочник.КлассификаторСтранМира.ПустаяСсылка");
...
Процедура ЗаполнитьСоставляющиеНаборов(МассивСтрок, КэшированныеЗначения, ДобавленныеСтроки)
	...
	Для Каждого КомплектующаяНабора Из КомплектующиеНаборов Цикл
		...
		Если КомплектующаяНабора.НомерСтрокиНабора <> НомерСтрокиНабора Тогда
			...
			ТекущаяСтрока = ЭтаФорма.Объект.Товары[НомерСтроки - 1];
		...
		ТекущаяСтрока = ЭтаФорма.Объект.Товары.Вставить(НомерСтроки);
		...
		СобытияФормИСМППереопределяемый.ПриИзмененииНоменклатуры(ЭтаФорма, ТекущаяСтрока, КэшированныеЗначения, ПараметрыУказанияСерий);
...
Процедура ОбработатьДобавленныеСтрокиТоваров(ДобавленныеСтроки, ДополнительныеПоляПоиска = "")
	...
	ЗаполнениеОбъектовПоСтатистикеИСМП.ЗаполнитьДанныеПоТоварамЗаказаНаЭмиссиюКодовМаркировкиСУЗ(
		ЭтаФорма, ДобавленныеСтроки, ДополнительныеПоляПоиска);

	ЗаполнитьПустойGTINВСтроках(ДобавленныеСтроки);
	ПроверитьОчиститьВладельцаGTIN(ЭтаФорма, ДобавленныеСтроки);
...
Процедура ВыполнитьПерезаполнениеНаборовИГрупповыхУпаковок(МассивСтрок)
	...
	Для Каждого Строка Из МассивСтрок Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца)
			...
			Если ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца) Тогда
				...
				ДобавитьСтрокиПоВладельцу(ЭтаФорма, МассивСтрокДляУдаления, СтрокаТовары.ИдентификаторСтрокиВладельца);
			КонецЕсли;
			...
			ИначеЕсли ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца) Тогда
				СтрокаВладелец = ПолучитьСтрокуВладельца(ЭтаФорма, СтрокаТовары.ИдентификаторСтрокиВладельца);
				Если СтрокаВладелец <> Неопределено Тогда
	...
	Для Каждого Строка Из МассивСтрокДляУдаления Цикл
		ОчиститьСерийныеНомераПоИдентификатору(ЭтаФорма, Строка.ИдентификаторСтроки);
		Объект.Товары.Удалить(Строка);
	КонецЦикла;
	...
	Если СтрокиДляОбработкиГрупповыеУпаковки.Количество() > 0 Тогда
		...
		МассивСтрокБезПотребительскихУпаковок = ПересчитатьКоличествоСоставляющихГрупповойУпаковки(ЭтаФорма,
...
Процедура ПересчитатьКоличествоСоставляющихНабора(ВходящиеДанные, КэшированныеЗначения)
	...
	ДобавитьСтрокиПоВладельцу(ЭтаФорма, СтрокиНабора, ТекущиеДанные.ИдентификаторСтрокиВладельца);
	...
	Для Каждого СтрокаНабора Из СтрокиНабора Цикл
		...
		СобытияФормИСМППереопределяемый.ПриИзмененииКоличества(ЭтаФорма, СтрокаНабора, КэшированныеЗначения);

		ОбновитьСтатусУказанияСерийныхНомеровПоСтроке(ЭтаФорма, СтрокаНабора);
...
Процедура ОбработатьИзменениеШаблонаНабораИГрупповойУпаковки(ДанныеСтроки)
	...
	Если ДанныеСтроки.Шаблон <> ШаблонДоИзменения
		...
		Если ШаблонЕстьВСписке(ДанныеСтроки.Шаблон, ШаблоныГрупповойУпаковки) И ШаблонЕстьВСписке(ШаблонДоИзменения, ШаблоныНабора)
			...
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ВопросДобавлениеСоставляющихНабораИГрупповойУпаковкиЗавершение", ЭтаФорма, ДополнительныеПараметры);
		...
		Иначе
			...
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ВопросИзменениеГрупповойУпаковкиНаПотребительскуюЗавершение", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ВопросИзменениеГрупповойУпаковкиНаПотребительскуюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	МассивСтрок      = Новый Массив;
	ДобавитьСтрокиПоВладельцу(ЭтаФорма, МассивСтрок, ДанныеСтроки.ИдентификаторСтрокиВладельца);
	Для Каждого Строка Из МассивСтрок Цикл
...
Процедура ВопросУдалениеСтрокТоваровЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ВыделеннаяСтрока Из ДополнительныеПараметры.ВыделенныеСтроки Цикл
		...
		Если Результат = "УдалитьСтроку" Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца) Тогда
				МассивСтрок = Новый Массив;
				ДобавитьСтрокиПоВладельцу(ЭтаФорма, МассивСтрок, СтрокаТовары.ИдентификаторСтрокиВладельца, Истина);
				Для Каждого Строка Из МассивСтрок Цикл
					Строка.ИдентификаторСтрокиВладельца = Неопределено;
					Строка.ЭтоГрупповаяУпаковка         = Ложь;
					Строка.ЭтоНабор                     = Ложь;
		...
		ИначеЕсли Результат = "УдалитьВсе" Тогда
				...
				Если ЗначениеЗаполнено(СтрокаТовары.ИдентификаторСтрокиВладельца) Тогда
					Если МассивСтрокДляПоиска.Найти(СтрокаТовары.ИдентификаторСтрокиВладельца) = Неопределено Тогда
						МассивСтрокДляПоиска.Добавить(СтрокаТовары.ИдентификаторСтрокиВладельца);
						ДобавитьСтрокиПоВладельцу(ЭтаФорма, МассивСтрокДляУдаления, СтрокаТовары.ИдентификаторСтрокиВладельца, Истина);
					КонецЕсли;
				Иначе
					МассивСтрокДляУдаления.Добавить(СтрокаТовары);
				КонецЕсли;
...
Процедура ЗапроситьУточнениеКоэффициентовУпаковок(МассивСтрок)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗапросКоличестваПотребительскихУпаковокВГрупповойУпаковкеЗавершение", ЭтаФорма, ДополнительныеПараметры);
	...
	ИнтеграцияИСМПКлиент.ОткрытьФормуНастройкиВидовУпаковокПоGTIN(ПараметрыОткрытияФормы, ЭтаФорма, ОписаниеОповещения);
...
Процедура ЗапросКоличестваПотребительскихУпаковокВГрупповойУпаковкеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПересчитатьКоличествоСоставляющихГрупповойУпаковки(ЭтаФорма, ДополнительныеПараметры.МассивСтрок);
