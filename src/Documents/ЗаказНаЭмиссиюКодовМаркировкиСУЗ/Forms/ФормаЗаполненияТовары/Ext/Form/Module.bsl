...
Процедура Заполнить(Команда)
	...
	Для Каждого ИмяПоля Из ИменаПолейУпрощенногоОписанияОстатков Цикл
		Если ЭтаФорма["Заполнять" + ИмяПоля] Тогда
			СтруктураРезультата.Вставить(ИмяПоля, ЭтаФорма[ИмяПоля]);
		КонецЕсли;
	КонецЦикла;
...
Процедура ПолноеОписаниеОстатковПриИзменении(Элемент)
	...
	Для Каждого ИмяПоля Из ИменаПолейУпрощенногоОписанияОстатков Цикл
		...
		Если ПолноеОписаниеОстатков Тогда
			ЭтаФорма["Заполнять" + ИмяПоля] = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура КодТНВЭДНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ТекущаяСтрока",                           ЭтаФорма.КодТНВЭД);
	ПараметрыФормы.Вставить("ВидПродукции",                            ЭтаФорма.ВидПродукции);
...
Процедура КодТНВЭДПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(КодТНВЭД) Тогда
		...
		ЭтаФорма[ЗапрещенныйЭлемент.Имя] = Ложь;
...
Процедура УправлениеДоступностьюСвязанныхЭлементов(Элемент)
	...
	Элементы[СтрЗаменить(Элемент.Имя, "Заполнять", "")].ТолькоПросмотр = Не ЭтаФорма[Элемент.Имя];
