...
Функция ПечатьИНВ19()
	...
	ИтогоРезультатПоСтраницеИзлишекКолво   = 0;
	ИтогоРезультатПоСтраницеИзлишекСумма   = 0;
	ИтогоРезультатПоСтраницеНедостачаКолво = 0;
	ИтогоРезультатПоСтраницеНедостачаСумма = 0;
	...
	ОбластьИтоговПоДокументу = Макет.ПолучитьОбласть("ИтогоТаблицы2");
	...
	Пока ВыборкаСтрокТовары.Следующий() Цикл
		...
		Если Ном = КоличествоСтрок Тогда
			МассивВыводимыхОбластей.Добавить(ОбластьИтоговПоДокументу);
			МассивВыводимыхОбластей.Добавить(ОбластьПодвала);
		КонецЕсли;		
		
		Если НЕ ТабДокумент.ПроверитьВывод(МассивВыводимыхОбластей) Тогда

			ОбластьИтоговПоСтранице.Параметры.ИтогоРезультатИзлишекКолво   = ИтогоРезультатПоСтраницеИзлишекКолво;
			ОбластьИтоговПоСтранице.Параметры.ИтогоРезультатИзлишекСумма   = ИтогоРезультатПоСтраницеИзлишекСумма;
			ОбластьИтоговПоСтранице.Параметры.ИтогоРезультатНедостачаКолво = ИтогоРезультатПоСтраницеНедостачаКолво;
			ОбластьИтоговПоСтранице.Параметры.ИтогоРезультатНедостачаСумма = ИтогоРезультатПоСтраницеНедостачаСумма;
		...
		ИтогоРезультатНедостачаКолво = ИтогоРезультатНедостачаКолво + ОбластьМакета.Параметры.РезультатНедостачаКолво;
		ИтогоРезультатНедостачаСумма = ИтогоРезультатНедостачаСумма + ОбластьМакета.Параметры.РезультатНедостачаСумма;
		ИтогоРезультатИзлишекКолво   = ИтогоРезультатИзлишекКолво   + ОбластьМакета.Параметры.РезультатИзлишекКолво;
		ИтогоРезультатИзлишекСумма   = ИтогоРезультатИзлишекСумма   + ОбластьМакета.Параметры.РезультатИзлишекСумма;

		ИтогоРезультатПоСтраницеНедостачаКолво = ИтогоРезультатПоСтраницеНедостачаКолво + ОбластьМакета.Параметры.РезультатНедостачаКолво;
		ИтогоРезультатПоСтраницеНедостачаСумма = ИтогоРезультатПоСтраницеНедостачаСумма + ОбластьМакета.Параметры.РезультатНедостачаСумма;
		ИтогоРезультатПоСтраницеИзлишекКолво   = ИтогоРезультатПоСтраницеИзлишекКолво   + ОбластьМакета.Параметры.РезультатИзлишекКолво;
		ИтогоРезультатПоСтраницеИзлишекСумма   = ИтогоРезультатПоСтраницеИзлишекСумма   + ОбластьМакета.Параметры.РезультатИзлишекСумма;
	...
	ОбластьИтоговПоДокументу.Параметры.ИтогоРезультатИзлишекКолво   = ИтогоРезультатИзлишекКолво;
	ОбластьИтоговПоДокументу.Параметры.ИтогоРезультатИзлишекСумма   = ИтогоРезультатИзлишекСумма;
	ОбластьИтоговПоДокументу.Параметры.ИтогоРезультатНедостачаКолво = ИтогоРезультатНедостачаКолво;
	ОбластьИтоговПоДокументу.Параметры.ИтогоРезультатНедостачаСумма = ИтогоРезультатНедостачаСумма;
	ТабДокумент.Вывести(ОбластьИтоговПоДокументу);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ЗарегистрироватьПрослеживаемыйТовар(ПараметрыПроведения.Реквизиты);
...
Функция ПодготовитьПараметрыРегистрацииПрослеживаемогоТовара(ТаблицаРеквизиты)
	...
	Параметры.Вставить("Реквизиты", 
		ПрослеживаемостьБП.ПолучитьТаблицуПараметровПроведения(ТаблицаРеквизиты, СписокОбязательныхКолонок));
...
Процедура ЗаполнитьИнвентаризациюСОтборомПоНоменклатуре(ДанныеЗаполнения)
	...
	ЗаполнитьПоОстаткамНаСкладе(ПостроительОтчета,Ложь, ДанныеЗаполнения.СписокНоменклатуры);
...
// 
Процедура ЗаполнитьПоОстаткамНаСкладе(ПостроительОтчета, ТолькоУчетные = Ложь, МассивНоменклатуры = Неопределено) Экспорт
	...
	Если МассивНоменклатуры <> Неопределено Тогда
		...
		ПостроительОтчета.Отбор.Номенклатура.Значение = МассивНоменклатуры;
