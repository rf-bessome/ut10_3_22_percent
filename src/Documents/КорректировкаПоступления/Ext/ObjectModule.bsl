...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, 
		ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект), Ссылка);
...
Функция ПечатьТОРГ12(БезУслуг = Ложь)
	...
	ТекстЗапросаШапка = "ВЫБРАТЬ
	                    ...
	                    |	КорректировкаПоступления.Организация КАК ЮрФизЛицо,
	                    |	КорректировкаПоступления.Организация КАК Поставщик,
	                    |	КорректировкаПоступления.Организация КАК Контрагент,
	                    |	КорректировкаПоступления.Организация КАК Руководители,
	                    |	ВЫБОР
	                    |		КОГДА КорректировкаПоступления.Грузополучатель = &ПустойКонтрагент
	                    |			ТОГДА КорректировкаПоступления.Контрагент
	                    |		ИНАЧЕ КорректировкаПоступления.Грузополучатель
	                    |	КОНЕЦ КАК Грузополучатель,
	                    |	ВЫБОР
	                    |		КОГДА КорректировкаПоступления.Грузоотправитель = &ПустойКонтрагент
	                    ...
	                    |	КорректировкаПоступления.Контрагент КАК Покупатель,
	                    |	КорректировкаПоступления.Контрагент КАК Плательщик,
	                    |	КорректировкаПоступления.Сделка,
	                    |	КорректировкаПоступления.ДоговорКонтрагента.Представление КАК Основание,
	                    |	КорректировкаПоступления.ДоговорКонтрагента.ВедениеВзаиморасчетов КАК ВедениеВзаиморасчетов,
	                    |	ОтветственныеЛица.ФизическоеЛицо КАК ОтветственноеЛицо,
	                    |	КорректировкаПоступления.Подразделение КАК Подразделение,
	                    |	КорректировкаПоступления.ВалютаДокумента,
	                    |	КорректировкаПоступления.КурсВзаиморасчетов КАК Курс,
	                    |	КорректировкаПоступления.КратностьВзаиморасчетов КАК Кратность,
	                    |	КорректировкаПоступления.УчитыватьНДС,
	                    ...
	                    |		ПО (ОтветственныеЛица.СтруктурнаяЕдиница = КорректировкаПоступления.СкладОрдер)
	                    |ГДЕ
	                    |	КорректировкаПоступления.Ссылка = &ТекущийДокумент";
	...
	|		КорректировкаПоступления.Коэффициент,
	|		КорректировкаПоступления.ЕдиницаИзмерения,
	|		КорректировкаПоступления.ЕдиницаИзмеренияМест,
	|		КорректировкаПоступления.ЕдиницаИзмеренияМест.Коэффициент     КАК КоэффициентМест,
	|		КорректировкаПоступления.ХарактеристикаНоменклатуры           КАК Характеристика,
	|		КорректировкаПоступления.СерияНоменклатуры                    КАК Серия,
	|		КорректировкаПоступления.СтавкаНДС,
	|		КорректировкаПоступления.Цена * &Курс / &Кратность            КАК Цена,
	|		СУММА(КорректировкаПоступления.КоличествоДоКорректировки - КорректировкаПоступления.Количество) КАК Количество,
	|		СУММА(КорректировкаПоступления.КоличествоДоКорректировки - КорректировкаПоступления.Количество) КАК КоличествоМест,
	|		СУММА((КорректировкаПоступления.СуммаДоКорректировки - КорректировкаПоступления.Сумма)    * &Курс / &Кратность) КАК Сумма,
	|		СУММА((КорректировкаПоступления.СуммаНДСДоКорректировки - КорректировкаПоступления.СуммаНДС) * &Курс / &Кратность) КАК СуммаНДС,
	|		МИНИМУМ(КорректировкаПоступления.НомерСтроки)                 КАК НомерСтроки,
	|		0.00                                                        КАК Метка
	|	ИЗ
	|		Документ.КорректировкаПоступления.Товары КАК КорректировкаПоступления
	|	ГДЕ
	|		КорректировкаПоступления.Ссылка = &ТекущийДокумент
	|		И КорректировкаПоступления.КоличествоДоКорректировки > КорректировкаПоступления.Количество
	|
	|СГРУППИРОВАТЬ ПО
	|		КорректировкаПоступления.Номенклатура,
	|		КорректировкаПоступления.Коэффициент,
	|		КорректировкаПоступления.ЕдиницаИзмерения,
	|		КорректировкаПоступления.ЕдиницаИзмеренияМест,
	|		КорректировкаПоступления.ХарактеристикаНоменклатуры,
	|		КорректировкаПоступления.СерияНоменклатуры,
	|		КорректировкаПоступления.СтавкаНДС,
	...
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КорректировкаПоступления_ТОРГ12";
	...
	Запрос.УстановитьПараметр("СтруктурнаяЕдиница", ЭтотОбъект.СкладОрдер);
	...
	Запрос.УстановитьПараметр("Курс",            ЗаполнениеДокументов.КурсДокумента(ЭтотОбъект.Ссылка, мВалютаРегламентированногоУчета));
	Запрос.УстановитьПараметр("Кратность",       ЗаполнениеДокументов.КратностьДокумента(ЭтотОбъект.Ссылка, мВалютаРегламентированногоУчета));
...
Процедура ЗаполнитьПоПоступлению()
	...
	УчетРНПТ.ЗаполнитьРНПТПоОснованию(ДокументПоступления, СведенияПрослеживаемости);
...
Процедура ЗаполнитьСвойстваШапки(ЗаполнятьРедактируемыеРеквизиты = Истина) Экспорт
	...
	Если КорректироватьБУиНУ Тогда 
		...
		Если ТипЗнч(СтруктураНезаполняемыеРеквизиты.ДокументПоступления) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") 
			...
			СкладОрдер = ДокументПоступления.СкладОрдер.Склад;
...
Функция ПолучитьПервичныйДокументПоступления(ТекущийДокументПоступления)
	...
	Если ТипЗнч(ПервичныйДокументПоступления) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
		ПервичныйДокументПоступления = УчетНДС.ПолучитьИсправляемыйДокументПоступления(
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	СтруктураПечатныхФорм.Вставить("ТОРГ12", "ТОРГ-12 (Товарная накладная на возврат)");
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(УсловияПрослеживаемости, СтруктураШапкиДокумента)
	...
	ДокументПоступленияСсылка = СтруктураШапкиДокумента.ДокументПоступленияСсылка;
	
	Если ТипЗнч(СтруктураШапкиДокумента.ИсправляемыйДокументПоступления) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
		ПервичныйДокументПоступления = ПолучитьПервичныйДокументПоступления(СтруктураШапкиДокумента.ИсправляемыйДокументПоступления);
	Иначе
		ПервичныйДокументПоступления = СтруктураШапкиДокумента.ИсправляемыйДокументПоступления
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ДокументПоступления", СтруктураШапкиДокумента.ДокументПоступления);
	...
	Запрос.УстановитьПараметр("ПервичныйДокументПоступления", ПервичныйДокументПоступления);
	Запрос.УстановитьПараметр("ПериодИсходнойОперации", НачалоКвартала(СтруктураШапкиДокумента.ДокументПоступленияДата));
	Запрос.УстановитьПараметр("ДатаИсходногоДокумента", ?(ЕстьДатаВходящегоДокумента, ДокументПоступленияСсылка.ДатаВходящегоДокумента, ""));
	Запрос.УстановитьПараметр("НомерИсходногоДокумента", ?(ЕстьНомерВходящегоДокумента, ДокументПоступленияСсылка.НомерВходящегоДокумента, ""));
	Запрос.УстановитьПараметр("ЭтоИсправлениеКорректировки", 
		СтруктураШапкиДокумента.ВидОперации <> Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение
		И ТипЗнч(ИсправляемыйДокумент) = Тип("ДокументСсылка.КорректировкаПоступления"));
	
	НомераТаблиц = Новый Структура;
	ПараметрыПроведения = Новый Структура;
	
	Запрос.Текст = Документы.КорректировкаПоступления.ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, УсловияПрослеживаемости);
...
Процедура ДвиженияПоРегистрам(
	...
	УсловияПрослеживаемости.Вставить("ЕстьТовары",          (ТаблицаПоТоварамПоступление.Количество() > 0) Или (ТаблицаПоТоварамВозврат.Количество() > 0));
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам) Экспорт
	...
	СтруктураПолей = Новый Структура;
	СтруктураПолей.Вставить("Регистратор"                       , "Ссылка");
	СтруктураПолей.Вставить("Ссылка"                            , "Ссылка"); // TODO
	СтруктураПолей.Вставить("Номенклатура"                      , "Номенклатура");
	СтруктураПолей.Вставить("СтавкаНДС"                         , "СтавкаНДС");
	...
	Если СтруктураШапкиДокумента.КорректироватьБУиНУ И НЕ Отказ Тогда
		...
		Для итератор = 0 По Макс(ТаблицаПоТоварам.Количество(), ТаблицаПоУслугам.Количество()) Цикл
			Если итератор <= ТаблицаПоТоварам.Количество() - 1 Тогда
				...
				Если (СтрокаТаблицы.Количество > 0.00 И СтрокаТаблицы.ОтражатьТоварныйУчет) 
					Или (СтрокаТаблицы.Количество = 0 И СтрокаТаблицы.Сумма <> 0) Тогда
