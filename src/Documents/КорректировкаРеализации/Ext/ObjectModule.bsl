...
Процедура ЗаполнитьПоРеализации()
	...
	|	КорректировкаРеализацииТовары.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
	|	""Товары"" КАК ТЧ,";
	...
	ТекстАвтоматическиеСкидки = "
		|	КорректировкаРеализацииТовары.ПроцентАвтоматическихСкидок КАК ПроцентАвтоматическихСкидок,
		|	КорректировкаРеализацииТовары.УсловиеАвтоматическойСкидки КАК УсловиеАвтоматическойСкидки,
		|	КорректировкаРеализацииТовары.ЗначениеУсловияАвтоматическойСкидки КАК ЗначениеУсловияАвтоматическойСкидки,
		|";
	...
	Текст = Текст + ?(ЕстьАвтоматическиеСкидки, ТекстАвтоматическиеСкидки, "");
	...
	|	NULL,
	|	NULL,
	|	КорректировкаРеализацииУслуги.ПроцентСкидкиНаценки,
	|	""Услуги"",";
	...
	ЕстьАвтоматическиеСкидки = ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ПроцентАвтоматическихСкидок", МетаданныеДокумента, "Услуги");
	ТекстАвтоматическиеСкидки = "
		|	КорректировкаРеализацииУслуги.ПроцентАвтоматическихСкидок,
		|	КорректировкаРеализацииУслуги.УсловиеАвтоматическойСкидки,
		|	КорректировкаРеализацииУслуги.ЗначениеУсловияАвтоматическойСкидки,
		|";
	Текст = Текст + ?(ЕстьАвтоматическиеСкидки, ТекстАвтоматическиеСкидки, "");
	...
	// Заполнение ТЧ СоставНабора
	УправлениеЗапасами.ЗаполнитьСоставНабораПоОснованию(ЭтотОбъект, ДокументРеализации, Истина);
	
	УчетРНПТ.ЗаполнитьРНПТПоОснованию(ДокументРеализации, СведенияПрослеживаемости);
...
Процедура ЗаполнитьСвойстваШапки(ЗаполнятьРедактируемыеРеквизиты = Истина) Экспорт
	...
	НезаполняемыеРеквизиты =
		...
		|ДокументРеализации, КорректироватьБУиНУ, Ответственный, Комментарий, ПризнаватьЗачитыватьАванс, НомерЧекаККМ, ККМ, ТекстЭлектронногоЧека, ВидПередачи, Грузоотправитель, Грузополучатель";
...
Функция ПодготовитьТаблицуТоваров(РезультатЗапросаПоТоварам, СтруктураШапкиДокумента, ТаблицаПоТоварамВозврат, ТаблицаПоТоварамВзаиморасчеты)
	...
	Если ТипЗнч(ИсправляемыйДокументРеализации) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		ТаблицаТоваров.ЗаполнитьЗначения(мДокументРеализацииСсылка, "ДокументПартии");
		ТаблицаПоТоварамВозврат.ЗаполнитьЗначения(мДокументРеализацииСсылка, "ДокументПартии");
	КонецЕсли;
...
Процедура ПодготовитьТаблицуТоваровУпр(ТаблицаТоваров, СтруктураШапкиДокумента)
	...
	ТаблицаТоваров.Колонки.Добавить("СтоимостьФлагОперации"          , ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(15,2));			
	...
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		СтрокаТаблицы.НДСУпрФлагОперации = ?(СтрокаТаблицы.НДСВзаиморасчеты < 0.00, - СтрокаТаблицы.НДСУпр, СтрокаТаблицы.НДСУпр); 
		СтрокаТаблицы.СтоимостьФлагОперации = ?(СтрокаТаблицы.СуммаВзаиморасчеты < 0.00, - СтрокаТаблицы.Стоимость, СтрокаТаблицы.Стоимость);
...
Процедура ПодготовитьТаблицуКомплектов(ТаблицаПоКомплектам)
	
	ОтборКоличество = Новый Структура("Количество", 0);
	
	УдаляемыеСтроки = ТаблицаПоКомплектам.НайтиСтроки(ОтборКоличество);
	Для Каждого СтрокаТЧ Из УдаляемыеСтроки Цикл
		ТаблицаПоКомплектам.Удалить(СтрокаТЧ);
	КонецЦикла;
...
Процедура ДвиженияПоРегистрам(
	...
	Тогда
		...
		ПодготовитьТаблицуКомплектов(ТаблицаПоКомплектам);		
	...
	УсловияПрослеживаемости.Вставить("ЕстьТовары", (ТаблицаПоТоварамРеализация.Количество() > 0) Или (ТаблицаПоТоварамВозврат.Количество() > 0));
	УсловияПрослеживаемости.Вставить("КорректироватьБУиНУ", СтруктураШапкиДокумента.КорректироватьБУиНУ);
	УсловияПрослеживаемости.Вставить("КорректироватьНДС", СтруктураШапкиДокумента.КорректироватьНДС);
	...
	Если СтруктураШапкиДокумента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем Тогда
		Если СуммаДокумента <> 0 Тогда
			...
			Если ЗначениеЗаполнено(СтруктураШапкиДокумента.ДисконтнаяКарта) Тогда
				...
				СуммаДоИзменения = Товары.Итог("СуммаДоИзменения") + Услуги.Итог("СуммаДоИзменения");
				СуммаДвижения = СуммаДокумента - СуммаДоИзменения;
				...
				СтрокаДвижений.ДисконтнаяКарта = СтруктураШапкиДокумента.ДисконтнаяКарта;
				СтрокаДвижений.ВладелецДисконтнойКарты = СтруктураШапкиДокумента.Контрагент;
				СтрокаДвижений.Сумма = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаДвижения, ВалютаДокумента,
				СтруктураШапкиДокумента.ВалютаУправленческогоУчета,
				СтруктураШапкиДокумента.КурсДокумента,
				СтруктураШапкиДокумента.КурсВалютыУправленческогоУчета,
				СтруктураШапкиДокумента.КратностьДокумента,
		...
		СтруктураИменКолонок = Новый Структура("НДС", "НДСУпрФлагОперации");
		ПереименоватьКолонку(ТаблицаПоТоварам, СтруктураИменКолонок);		
		СтруктураИменКолонок = Новый Структура("Стоимость", "СтоимостьФлагОперации");		
		ПереименоватьКолонку(ТаблицаПоТоварам, СтруктураИменКолонок);
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(УсловияПрослеживаемости, СтруктураШапкиДокумента)
	...
	ДокументРеализацииСсылка = СтруктураШапкиДокумента.ДокументРеализацииСсылка;
	Отказ = Ложь;
	...
	Если ЗначениеЗаполнено(ДокументРеализацииСсылка) Тогда
		...
		ЕстьНомерВходящегоДокумента = ОбщегоНазначения.ЕстьРеквизитОбъекта("НомерВходящегоДокумента", МетаданныеДокументаРеализации);
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС",
		ЕстьНДС ИЛИ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС);
	
	Запрос.УстановитьПараметр("ДокументРеализации", СтруктураШапкиДокумента.ДокументРеализации);
	Запрос.УстановитьПараметр("ДокументРеализацииСсылка", ДокументРеализацииСсылка);
	Запрос.УстановитьПараметр("ДатаДокументаРеализации", ДокументРеализацииСсылка.Дата);
	Запрос.УстановитьПараметр("ПериодИсходнойОперации", НачалоКвартала(СтруктураШапкиДокумента.ДокументРеализацииДата));
	Запрос.УстановитьПараметр("НомерИсходногоДокумента", ?(ЕстьНомерВходящегоДокумента, ДокументРеализацииСсылка.НомерВходящегоДокумента, ""));
	...
	Запрос.Текст = Документы.КорректировкаРеализации.ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, УсловияПрослеживаемости);
	...
	Если ПараметрыПроведения.ПрослеживаемыеОперации <> Неопределено Тогда
		Если НЕ ПараметрыПроведения.ПрослеживаемыеОперации.Колонки.Найти("НомерДокумента") = Неопределено 
			И СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение Тогда
...
Процедура ДвиженияПоРегиструСписанныеТоварыРеализация(
	...
	Для каждого Строка Из ТаблицаДвижений Цикл
		...
		НомСтр = НомСтр + 1;
		Строка.НомерСтрокиДокумента = НомСтр;
...
Процедура ДвиженияПоРегиструСписанныеТоварыВозврат(
	...
	КоличествоСтрокВНабореДвижений = НаборДвижений.Количество();
	...
	// Недостающие поля.
	НомСтр = НаборДвижений.Количество();
	Для каждого Строка Из ТаблицаДвижений Цикл
	...
	НомерПервойСтроки = КоличествоСтрокВНабореДвижений + 1;
	ОбщегоНазначения.ПронумероватьСтрокиТаблицыЗначений(ТаблицаДвижений, "НомерСтрокиДокумента", НомерПервойСтроки);
...
Процедура ПодготовитьТаблицыДокумента(
	...
	|		КОГДА (Количество - КоличествоДоИзменения) * Коэффициент / Номенклатура.ЕдиницаХраненияОстатков.Коэффициент < 0.00
	|		ТОГДА ЛОЖЬ
	|		КОГДА ((Сумма - СуммаДоИзменения <> 0) 
	|				И ((Количество - КоличествоДоИзменения) * Коэффициент / Номенклатура.ЕдиницаХраненияОстатков.Коэффициент) = 0)
	|		ТОГДА ИСТИНА	
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|КОНЕЦ");
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ОперацияВыбытияМаркированныхТоваров", ДокументРеализации.Метаданные()) Тогда
		ОперацияВыбытияМаркированныхТоваров = ДокументРеализации.ОперацияВыбытияМаркированныхТоваров;
	КонецЕсли;
	...
	Если СтруктураШапкиДокумента.КорректироватьБУиНУ И НЕ Отказ Тогда
		...
		Для итератор = 0 По Макс(ТаблицаПоТоварам.Количество(), ТаблицаПоУслугам.Количество()) Цикл
			Если итератор <= ТаблицаПоТоварам.Количество() - 1 Тогда
				...
				Если СтрокаТаблицы.ФлагОперации = ИСТИНА Тогда
