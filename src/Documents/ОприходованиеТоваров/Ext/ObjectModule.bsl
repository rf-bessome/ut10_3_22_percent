...
Процедура ЗаполнитьТоварыПоИнвентаризацииТоваровУпр(ДокументОснование, ПодтверждатьОчисткуСтрок) Экспорт
	...
	|	СУММА(Док.Количество) - МАКСИМУМ(Док.КоличествоУчет) КАК КоличествоОтклонениеИнвентаризации,
	|	Док.Цена,
	|	ВЫБОР
	|		КОГДА &НТТ
	...
	|ПОМЕСТИТЬ ВТ_ИнвентаризацияТовары
	|ИЗ
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(Док.Количество) - МАКСИМУМ(Док.КоличествоУчет) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ОприходованиеТоваровТовары.СерияНоменклатуры,
	|	ОприходованиеТоваровТовары.ЦенаВРознице,
	|	СУММА(ОприходованиеТоваровТовары.Количество) КАК Количество
	|ПОМЕСТИТЬ ВТ_ОприходованияТовары
	|ИЗ
	|	Документ.ОприходованиеТоваров.Товары КАК ОприходованиеТоваровТовары
	|ГДЕ
	|	ОприходованиеТоваровТовары.Ссылка.ИнвентаризацияТоваровНаСкладе = &ДокументОснованиеСсылка
	|	И ОприходованиеТоваровТовары.Ссылка.Проведен
	|	И ОприходованиеТоваровТовары.Ссылка <> &ДокументСсылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ОприходованиеТоваровТовары.Ссылка.Склад,
	|	ОприходованиеТоваровТовары.ХарактеристикаНоменклатуры,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ИнвентаризацияТовары.Номенклатура,
	|	ВТ_ИнвентаризацияТовары.ЕдиницаИзмерения,
	|	ВТ_ИнвентаризацияТовары.ЕдиницаИзмеренияМест,
	|	ВТ_ИнвентаризацияТовары.Качество,
	|	ВТ_ИнвентаризацияТовары.КоэффициентЕдиницыИнвентаризации,
	|	ВТ_ИнвентаризацияТовары.КоличествоОтклонениеИнвентаризации,
	|	ЕСТЬNULL(ВТ_ОприходованияТовары.Количество, 0) КАК КоличествоОприходованное,
	|	ВТ_ИнвентаризацияТовары.Цена,
	|	ВТ_ИнвентаризацияТовары.ЦенаВРознице,
	|	ВТ_ИнвентаризацияТовары.ХарактеристикаНоменклатуры,
	|	ВТ_ИнвентаризацияТовары.СерияНоменклатуры,
	|	ВТ_ИнвентаризацияТовары.НомерСтроки КАК НомерСтроки
	|ИЗ
	|	ВТ_ИнвентаризацияТовары КАК ВТ_ИнвентаризацияТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОприходованияТовары КАК ВТ_ОприходованияТовары
	|		ПО ВТ_ИнвентаризацияТовары.Склад = ВТ_ОприходованияТовары.Склад
	|			И ВТ_ИнвентаризацияТовары.Номенклатура = ВТ_ОприходованияТовары.Номенклатура
	|			И ВТ_ИнвентаризацияТовары.ХарактеристикаНоменклатуры = ВТ_ОприходованияТовары.ХарактеристикаНоменклатуры
	|			И ВТ_ИнвентаризацияТовары.СерияНоменклатуры = ВТ_ОприходованияТовары.СерияНоменклатуры
	|			И ВТ_ИнвентаризацияТовары.ЦенаВРознице = ВТ_ОприходованияТовары.ЦенаВРознице
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(СтруктураШапкиДокумента)
	...
	|	Реквизиты.ОтражатьВПрослеживаемости
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		И Выборка.ОтражатьВПрослеживаемости;
		
		НомераТаблиц = Новый Структура;
		ПараметрыПроведения = Новый Структура;
		
		Запрос.Текст = Документы.ОприходованиеТоваров.ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, ВедетсяУчетПрослеживаемыхТоваров);
