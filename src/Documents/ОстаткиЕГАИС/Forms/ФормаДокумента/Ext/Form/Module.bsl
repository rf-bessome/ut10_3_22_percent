...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(
		ЭтаФорма, Перечисления.ВидыПродукцииИС.Алкогольная, "ОстаткиПоДаннымЕГАИСНоменклатура");
	
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(ЭтаФорма,   "ОстаткиПоДаннымЕГАИСХарактеристика", "Элементы.ОстаткиПоДаннымЕГАИС.ТекущиеДанные.Номенклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(ЭтаФорма,   "ОстаткиПоДаннымЕГАИССерия", "Элементы.ОстаткиПоДаннымЕГАИС.ТекущиеДанные.Номенклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСХарактеристикой(ЭтаФорма, "ОстаткиПоДаннымЕГАИССерия", "Элементы.ОстаткиПоДаннымЕГАИС.ТекущиеДанные.Характеристика");
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ИнтеграцияИСПереопределяемый.ЗаполнитьСлужебныеРеквизитыВКоллекции(ЭтаФорма, Объект.ОстаткиПоДаннымЕГАИС);
	ЗаполнитьСлужебныеРеквизиты();
	
	ИнтеграцияИСПереопределяемый.ЗаполнитьПараметрыУказанияСерийФормы(Документы.ОстаткиЕГАИС, ЭтаФорма);
	ИнтеграцияИСПереопределяемый.ЗаполнитьСтатусыУказанияСерийФормы(ЭтаФорма);
...
Процедура ПередЗакрытием(Отказ,  СтандартнаяОбработка)
	...
	Если Не ПринудительноЗакрытьФорму И ДанныеТекущегоЗапроса <> Неопределено Тогда
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ОбработатьОтветНаВопросПередЗакрытием", ЭтаФорма),
...
Процедура ОстаткиПоДаннымЕГАИСПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ИнтеграцияИСКлиентПереопределяемый.ОбновитьКешированныеЗначенияДляУчетаСерий(Элемент, КэшированныеЗначения, ЭтаФорма, Копирование);
...
Процедура ОстаткиПоДаннымЕГАИСХарактеристикаСоздание(Элемент, СтандартнаяОбработка)
	...
	СобытияФормЕГАИСКлиентПереопределяемый.ХарактеристикаСоздание(ЭтаФорма, Элементы.ОстаткиПоДаннымЕГАИС.ТекущаяСтрока, Элемент, СтандартнаяОбработка);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	СобытияФормЕГАИСПереопределяемый.ДобавитьСлужебныеРеквизиты(ЭтаФорма, "Объект.ОстаткиПоДаннымЕГАИС");
...
Процедура ЗавершитьРедактированиеСтрокиТовары(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ИнтеграцияИСКлиент.НеобходимоОбновитьСтатусыСерий(ЭтаФорма, Элемент, Неопределено) Тогда
		...
		ИнтеграцияИСКлиентПереопределяемый.ОбновитьКешированныеЗначенияДляУчетаСерий(Элемент, КэшированныеЗначения, ЭтаФорма);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормИСПереопределяемый.УстановитьУсловноеОформлениеСерийНоменклатуры(
		ЭтаФорма, "ОстаткиПоДаннымЕГАИССерия");
			...
			Иначе
				...
				Иначе
					...
					ИнтеграцияЕГАИСКлиент.ВыполнитьОбмен(
						...
						Новый ОписаниеОповещения("ПроверитьОтветНаЗапросМарок", ЭтаФорма));
				КонецЕсли;
