...
Функция ЗагрузитьОтклоненияПоВидамПродукцииJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	Для Каждого Организация Из Организации Цикл
		...
		Для Каждого ВидПродукции Из ВидыПродукцииУчета Цикл
			...
			СообщениеJSON.ТекстСообщенияJSON  = ИнтерфейсМОТПСлужебный.ОбъектВТекстJSON(ПараметрыЗапроса, Истина);
...
Функция ЗапроситеОтчетJSON(ДокументСсылка, ДополнительныеПараметры = Неопределено)
	...
	ИначеЕсли Шапка.Операция = Перечисления.ВидыОперацийИСМП.ОтчетИСМПСписокКИНаБалансе Тогда
		...
		Если Товары.Количество() Тогда
			...
			ШтрихкодированиеИС.ЗаполнитьПроверяемыеGTIN(Товары, МассивGTIN, СоответствиеGTIN, Ложь);
		...
		ПараметрыJSON.Вставить("status", ИнтерфейсИСМПСлужебный.СтатусКодаМаркировки(Шапка.СтатусКодаМаркировки));
		...
		Если ВидыУпаковок.Количество() = 0 Тогда
			...
			Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетГрупповыеУпаковки(Шапка.ВидПродукции) Тогда
				ПараметрыОтбораВидУпаковки.Добавить("GROUP");
			КонецЕсли;
			Если ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетНаборы(Шапка.ВидПродукции) Тогда
		...
		Иначе
			...
			Для Каждого ВидУпаковки Из ВидыУпаковок Цикл
				Если ВидУпаковки = Перечисления.ВидыУпаковокИС.Логистическая Тогда
					ПараметрыОтбораВидУпаковки.Добавить("BOX");
				Иначе
					ПараметрыОтбораВидУпаковки.Добавить(ИнтерфейсИСМПСлужебный.ВидУпаковки(ВидУпаковки, Шапка.ВидПродукции));
				КонецЕсли;
			КонецЦикла;
	...
	ПараметрыЗапроса.Вставить("ПараметрыJSON", ИнтерфейсМОТПСлужебный.ОбъектВТекстJSON(ПараметрыJSON, Истина));
	СообщениеJSON.ПараметрыЗапроса = ПараметрыЗапроса;
