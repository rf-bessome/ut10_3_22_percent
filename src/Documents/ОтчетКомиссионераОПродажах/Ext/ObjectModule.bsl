...
Процедура ЗаполнитьТоварыУпр(ДокументПередачи = Неопределено) Экспорт
	...
	Иначе
		...
		|	ДокТабличнаяЧасть.ИдентификаторСтроки                      КАК ИдентификаторСтроки,
		|	ДокТабличнаяЧасть.ПрослеживаемыйТовар                      КАК ПрослеживаемыйТовар,
		|	ДокТабличнаяЧасть.СтранаПроисхождения                      КАК СтранаПроисхождения,
		...
		|	ТабличнаяЧастьДок.ИдентификаторСтроки              КАК ИдентификаторСтроки,
		|	ТабличнаяЧастьДок.ПрослеживаемыйТовар              КАК ПрослеживаемыйТовар,
		|	ТабличнаяЧастьДок.СтранаПроисхождения              КАК СтранаПроисхождения,
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(УсловияПрослеживаемости, СтруктураШапкиДокумента)
	...
	Запрос.Текст = Документы.ОтчетКомиссионераОПродажах.ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, УсловияПрослеживаемости);
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоУслугам, ТаблицаПоТоварам, ТаблицаПоДенежнымСредствам) Экспорт
	...
	ФормироватьСтруктуруПолей=Ложь;
	
	Если СтруктураШапкиДокумента.КонтролироватьДенежныеСредстваКомитента <> Неопределено Тогда 
		ФормироватьСтруктуруПолей = СтруктураШапкиДокумента.КонтролироватьДенежныеСредстваКомитента;
	КонецЕсли;
	
	Если Не ФормироватьСтруктуруПолей Тогда ФормироватьСтруктуруПолей = СтруктураШапкиДокумента.ОтражатьВБухгалтерскомУчете;
	КонецЕсли;
	
	Если ФормироватьСтруктуруПолей Тогда
