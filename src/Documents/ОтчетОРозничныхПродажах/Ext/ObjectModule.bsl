...
Функция ПечатьОтчетаОПродажахККМ()
	...
	ЗапросПоОплатам = Новый Запрос;
	ЗапросПоОплатам.УстановитьПараметр("ТекущийДокумент", ЭтотОбъект.Ссылка);
	
	ЗапросПоОплатам.Текст = 
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	ОтчетОРозничныхПродажахОплатаБанковскимиКредитами.Ссылка = &ТекущийДокумент
	|ИТОГИ
	|	СУММА(Сумма)
	|ПО
	|	ОБЩИЕ";
	
	Результат = ЗапросПоОплатам.Выполнить();

	Шапка = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Если Шапка.Следующий() Тогда
		СуммаБезналичнойОплаты = Шапка.Сумма;
	Иначе
	...
	ОбластьМакета.Параметры.СуммаВыручкиПрописью = ОбщегоНазначения.СформироватьСуммуПрописью
		(СуммаИтого - СуммаИтогоВозврат - СуммаБезналичнойОплаты, мВалютаРегламентированногоУчета);
...
Процедура ПриИзмененииНоменклатурыТоваров(СтрокаТабличнойЧасти) Экспорт
	...
	Если СтрокаТабличнойЧасти.ПрослеживаемыйТовар Тогда
		...
		Если Не ПустаяСтрока(СтрокаТабличнойЧасти.ИдентификаторСтроки) Тогда
			Отбор = Новый Структура;
			Отбор.Вставить("ИдентификаторСтроки", СтрокаТабличнойЧасти.ИдентификаторСтроки);
			СтрокиПрослеживаемостиНоменклатуры = СведенияПрослеживаемости.НайтиСтроки(Отбор);
			
			Если СтрокиПрослеживаемостиНоменклатуры.Количество() > 0 Тогда
				СтрокаТабличнойЧасти.АвтоподборРНПТ = Ложь;
			КонецЕсли;
...
Процедура ПриИзмененииВидаОплатыПлатежнымиКартами(ТекущаяСтрока, ЗаполнитьПроцентУступки = Истина) Экспорт
	...
	ТекущийДоговорЭквайринга = ?(ЗначениеЗаполнено(ТекущаяСтрока.ДоговорЭквайринга), ТекущаяСтрока.ДоговорЭквайринга, ДоговорЭквайринга);
	
	Если ЗначениеЗаполнено(ТекущийДоговорЭквайринга) Тогда
		Если ЗаполнитьПроцентУступки Тогда
			ТекущаяСтрока.ПроцентТорговойУступки = УправлениеРозничнойТорговлей.ПолучитьПроцентТорговойУступки(ТекущийДоговорЭквайринга, ТекущаяСтрока.ВидОплаты);
		КонецЕсли;
...
Процедура ПриИзмененииДоговораЭквайринга() Экспорт
	...
	Если ПересчитатьЗначенияТорговойУступки Тогда
		...
		Для Каждого СтрокаОплаты Из ОплатаПлатежнымиКартами Цикл
			Если Не ЗначениеЗаполнено(СтрокаОплаты.ДоговорЭквайринга) Тогда
...
Процедура ПроверитьЗаполнениеШапки(СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Если ОплатаПлатежнымиКартами.Количество() > 0 Тогда
		Для Каждого Строка Из ОплатаПлатежнымиКартами Цикл
			Если Не ЗначениеЗаполнено(Строка.ДоговорЭквайринга) Или Не ЗначениеЗаполнено(Строка.Эквайрер)
				Или Не ЗначениеЗаполнено(Строка.ДоговорВзаиморасчетовЭквайрера) Тогда
...
Процедура ПроверитьЗаполнениеТабличнойЧастиОплатаПлатежнымиКартами(ТаблицаПоПлатежнымКартам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	Для Каждого Строка Из ОплатаПлатежнымиКартами Цикл
		Если ЗначениеЗаполнено(Строка.ДоговорЭквайринга) Тогда
			Если Не ЗначениеЗаполнено(Строка.Эквайрер) Тогда
				ОбщегоНазначения.СообщитьОбОшибке("В строке номер """ + Строка.НомерСтроки + """ табличной части ""Оплата платежными картами"": "
													+ "Не заполнено значение реквизита ""Эквайрер""",
													Отказ,
													Заголовок);
			КонецЕсли;
			Если Не ЗначениеЗаполнено(Строка.ДоговорВзаиморасчетовЭквайрера) Тогда
				ОбщегоНазначения.СообщитьОбОшибке("В строке номер """ + Строка.НомерСтроки + """ табличной части ""Оплата платежными картами"": "
													+ "Не заполнено значение реквизита ""Договор взаиморасчетов эквайрера""",
													Отказ,
													Заголовок);
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(Строка.Эквайрер) Тогда
			ОбщегоНазначения.СообщитьОбОшибке("В строке номер """ + Строка.НомерСтроки + """ табличной части ""Оплата платежными картами"": "
												+ "Не заполнено значение реквизита ""Договор эквайринга""",
												Отказ,
												Заголовок);
		КонецЕсли;
		Если ЗначениеЗаполнено(Строка.ДоговорВзаиморасчетовЭквайрера) И Строка.ДоговорВзаиморасчетовЭквайрера.Организация <> Организация Тогда
			ОбщегоНазначения.СообщитьОбОшибке(СтрШаблон("Организация, указанная в договоре взаиморасчетов с эквайрером ""%1"" не соответствует организации, указанной в документе!",
														Строка.ДоговорВзаиморасчетовЭквайрера),
												Отказ,
												Заголовок);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоСкидкам, 
	...
	ДвиженияПоРегистрамПрослеживаемыхТоваров(СтруктураШапкиДокумента);
...
Процедура ДвиженияРегистраОперативныхРасчетов(РежимПроведения, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	МассивДоговоровВестиПоДокументамРасчетовСЭквайрером = ПолучитьМассивДоговоровВестиПоДокументамРасчетовСЭквайрером();
	
	Если НЕ СтруктураШапкиДокумента.ВестиПоДокументамРасчетовСЭквайрером И МассивДоговоровВестиПоДокументамРасчетовСЭквайрером.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	Если МассивДоговоровВестиПоДокументамРасчетовСЭквайрером.Количество() = 0 Тогда
	...
	Иначе
		...
		Для Каждого Договор Из МассивДоговоровВестиПоДокументамРасчетовСЭквайрером Цикл
			
			Отбор = Новый Структура;
			Отбор.Вставить("ДоговорВзаиморасчетовЭквайрера", Договор);
			
			НайденныеСтроки = ОплатаПлатежнымиКартами.НайтиСтроки(Отбор);
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если НайденнаяСтрока.Сумма <> 0 Тогда
					...
					Движение.ДоговорКонтрагента            = НайденнаяСтрока.ДоговорВзаиморасчетовЭквайрера;
					Движение.Контрагент            		   = НайденнаяСтрока.Эквайрер;
					Движение.Организация            	   = Организация;
					...
					Движение.СуммаВзаиморасчетов = НайденнаяСтрока.Сумма;
					Движение.СуммаРегл           = НайденнаяСтрока.Сумма;
		...
		Если СтруктураШапкиДокумента.ВестиПоДокументамРасчетовСЭквайрером Тогда
			
			Отбор = Новый Структура;
			Отбор.Вставить("ДоговорЭквайринга", Справочники.ДоговорыЭквайринга.ПустаяСсылка());
			
			НайденныеСтроки = ОплатаПлатежнымиКартами.НайтиСтроки(Отбор);
			СуммаВзаиморасчетов = 0;
			
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				СуммаВзаиморасчетов = СуммаВзаиморасчетов + НайденнаяСтрока.Сумма;
			КонецЦикла;
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоСкидкам,
	...
	Если ЕстьБезналичныеРасчеты Тогда
		...
		ЕстьНеПустойЭквайерВСтроках = Ложь;
		Для Каждого Строка Из ОплатаПлатежнымиКартами Цикл
			Если ЗначениеЗаполнено(Строка.Эквайрер) Тогда
				ЕстьНеПустойЭквайерВСтроках = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		
		Если ЕстьНеПустойЭквайерВСтроках Тогда
			
			Для Каждого Строка Из ОплатаПлатежнымиКартами Цикл
				
				СуммаВзаиморасчетовПоКарте = Строка.Сумма;
				
				Если СуммаВзаиморасчетовПоКарте <> 0 Тогда
					
					СуммаУпрПоКарте = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(
					СуммаВзаиморасчетовПоКарте, 
					мВалютаРегламентированногоУчета, СтруктураШапкиДокумента.ВалютаУправленческогоУчета,
					1, СтруктураШапкиДокумента.КурсВалютыУправленческогоУчета,	
					1, СтруктураШапкиДокумента.КратностьВалютыУправленческогоУчета);
					...
					СтрокаВзаиморасчеты.ДоговорКонтрагента  = ?(ЗначениеЗаполнено(Строка.ДоговорВзаиморасчетовЭквайрера), Строка.ДоговорВзаиморасчетовЭквайрера, ДоговорВзаиморасчетовЭквайрера);
					СтрокаВзаиморасчеты.Контрагент          = ?(ЗначениеЗаполнено(Строка.Эквайрер), Строка.Эквайрер, Эквайрер);
					СтрокаВзаиморасчеты.Организация         = Организация;
					СтрокаВзаиморасчеты.Сделка              = Неопределено;
					СтрокаВзаиморасчеты.СуммаВзаиморасчетов = СуммаВзаиморасчетовПоКарте;
					СтрокаВзаиморасчеты.СуммаУпр            = СуммаУпрПоКарте;
					
					СтрокаРасчеты = ТаблицаРасчеты.Добавить();
					
					СтрокаРасчеты.ДоговорКонтрагента  = ?(ЗначениеЗаполнено(Строка.ДоговорВзаиморасчетовЭквайрера), Строка.ДоговорВзаиморасчетовЭквайрера, ДоговорВзаиморасчетовЭквайрера);
					СтрокаРасчеты.Контрагент  		  = ?(ЗначениеЗаполнено(Строка.Эквайрер), Строка.Эквайрер, Эквайрер);
					...
					СтрокаРасчеты.СуммаВзаиморасчетов = СуммаВзаиморасчетовПоКарте;
					СтрокаРасчеты.СуммаУпр            = СуммаУпрПоКарте;
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(СтруктураШапкиДокумента)
	...
	Запрос.Текст = Документы.ОтчетОРозничныхПродажах.ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения);
...
// Функция возвращает массив договоров контрагентов из табличной части ОплатаПлатежнымиКартами, 
// у которых установлен признак ВестиПоДокументамРасчетовСКонтрагентом.
//
// Возвращаемое значение:
//  Массив - массив договоров с флагом ВестиПоДокументамРасчетовСКонтрагентом.
//
Функция ПолучитьМассивДоговоровВестиПоДокументамРасчетовСЭквайрером()
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ОтчетОРозничныхПродажахОплатаПлатежнымиКартами.ДоговорВзаиморасчетовЭквайрера КАК ДоговорВзаиморасчетовЭквайрера
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах.ОплатаПлатежнымиКартами КАК ОтчетОРозничныхПродажахОплатаПлатежнымиКартами
	|ГДЕ
	|	ОтчетОРозничныхПродажахОплатаПлатежнымиКартами.Ссылка = &Ссылка
	|	И ОтчетОРозничныхПродажахОплатаПлатежнымиКартами.ДоговорВзаиморасчетовЭквайрера.ВестиПоДокументамРасчетовСКонтрагентом";
	
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		МассивДоговоров.Добавить(Выборка.ДоговорВзаиморасчетовЭквайрера);
	КонецЦикла;
