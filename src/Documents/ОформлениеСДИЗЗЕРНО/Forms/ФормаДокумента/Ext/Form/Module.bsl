...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДокументОснование.ДоступныеТипы = ОпределяемыеТипы().ОснованиеОформлениеСДИЗЗЕРНО.Тип;
...
Процедура ОбработкаНавигационнойСсылки(НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если СтрНачинаетсяС(НавигационнаяСсылкаФорматированнойСтроки, "ОткрытьОбщуюФорму") Тогда
		...
		ОбработкаОповещения = Новый ОписаниеОповещения("ОбновитьДанные", ЭтаФорма, ИмяТабличнойЧасти);
		...
		ОткрытьФорму("ОбщаяФорма." + ИмяОбщейФормы, ПараметрыОткрытияФормы, ЭтаФорма, УникальныйИдентификатор,,, ОбработкаОповещения);
		Возврат;
...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	ОбновитьПодсказкиВвода(ЭтаФорма);
...
Процедура ПодразделениеПриИзменении(Элемент)
	...
	ОбновитьПодсказкиВводаПодразделения(ЭтаФорма);
	...
	Если ВыбранноеЗначение.Свойство("Действие") Тогда
		Если ВыбранноеЗначение.Действие = "ОткрытьФорму" Тогда
			ОткрытьФорму(СтрШаблон("%1.%2.%3",
				...
				ЭтаФорма,,,,
				Новый ОписаниеОповещения("ПунктНазначенияОкончаниеВыбора", ЭтаФорма),
...
Процедура ЛабораторияПриИзменении(Элемент)
	НастроитьЗависимыеЭлементыФормы(ЭтаФорма,, "Лаборатория");
	...
	Если Поле = Элементы.ТоварыДокументыСертификацииПредставление Тогда
		...
		ОткрытьФорму(
			"ОбщаяФорма.ДокументыСертификацииЗЕРНО",
			 ...
			 Новый ОписаниеОповещения("ОбновитьДанные", ЭтаФорма, ПараметрыОповещения));
...
Процедура ТоварыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		КонецЕсли;
		УстановитьПредставлениеДокументовСертификацииПоСтроке(ЭтаФорма, ТекущиеДанные);
...
Процедура ТоварыПослеУдаления(Элемент)
	...
	НастроитьЗависимыеЭлементыФормы(ЭтаФорма, , "Товары");
	ОтключитьОтметкуНезаполненного();
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	НастроитьЗависимыеЭлементыФормы(ЭтаФорма,, "ТоварыДатаПротоколаИспытаний");
	ОтключитьОтметкуНезаполненного();
...
Процедура ТоварыЛабораторияПриИзменении(Элемент)
	НастроитьЗависимыеЭлементыФормы(ЭтаФорма,, "ТоварыЛаборатория");
	ОтключитьОтметкуНезаполненного();
КонецПроцедуры
...
Процедура МаршрутВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Перевозчики = ИнтеграцияЗЕРНОКлиентСервер.УникальныеПеревозчики(ЭтаФорма,, Истина);
	...
	ИнтеграцияЗЕРНОСлужебныйКлиент.ОткрытьФормуУказанияТранспортныхСредств(
		ПараметрыОткрытияФормы,
		ЭтаФорма,
		Новый ОписаниеОповещения("ВыборМаршрутаЗавершение", ЭтаФорма, ПараметрыОповещения));
...
Процедура МаршрутПередУдалением(Элемент, Отказ)
	...
	ИнтеграцияЗЕРНОКлиентСервер.ОчиститьДанныеТабличныхЧастейПоСтроке(
		ЭтаФорма, ТекущиеДанные.Перевозчик, "Перевозчик", СвязанныеТаблицы);
...
Процедура ОткрытьПодборТоваров(Команда)
	...
	СобытияФормЗЕРНОКлиентПереопределяемый.ОткрытьФормуПодбораНоменклатуры(ЭтаФорма);
...
Процедура ИнициализироватьСлужебныеРеквизитыТоваров()
	...
	Для Каждого ТекущаяСтрока Из Объект.Товары Цикл
		...
		УстановитьПредставлениеДокументовСертификацииПоСтроке(ЭтаФорма, ТекущаяСтрока);
	...
	Маршрут.Очистить();
	Перевозчики = ИнтеграцияЗЕРНОКлиентСервер.УникальныеПеревозчики(ЭтаФорма,, ПодразделенияИспользуются);
	Для Каждого КлючИЗначение Из Перевозчики Цикл
		...
		Если ПодразделенияИспользуются Тогда
			...
			Для Каждого ЭлементКоллекции Из КлючИЗначение.Значение Цикл
				...
				УстановитьПредставлениеТранспортныхСредствПоСтроке(ЭтаФорма, СтрокаМаршрута);
				УстановитьПредставлениеМестПерегрузкиПоСтроке(ЭтаФорма, СтрокаМаршрута);
			КонецЦикла;
...
Процедура ОбновитьСтатусИПараметрыДополнения(УстановитьПараметрыОбновленияСтатуса = Истина)
	...
	Если УстановитьПараметрыОбновленияСтатуса Тогда
		ПараметрыОбновленияСтатуса = ПараметрыОбновленияСтатуса(ПараметрыДополненияСтатусаЗЕРНО(ЭтаФорма));
	КонецЕсли;
...
Процедура ОбновитьДанныеНаСервере(Результат, ДополнительныеПараметры, КэшированныеЗначения)
	...
	Если ИмяОбновляемогоПоля = "ТоварыПартия"
		...
		УстановитьПредставлениеДокументовСертификацииПоСтроке(ЭтаФорма, ИзменяемаяСтрока);
...
Процедура ОбновитьДанные(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ИмяОбновляемогоПоля = "ТоварыКоличество" Тогда
		...
		СобытияФормЗЕРНОКлиентПереопределяемый.ПриИзмененииКоличества(
			ЭтаФорма, ИзменяемаяСтрока, ЕдиницаИзмеренияКилограмм, КэшированныеЗначения);
	...
	Если ИмяОбновляемогоПоля = "Операция" Тогда
		...
		ЗаполнитьСтатусУказанияДокументовСертификации(ЭтаФорма);
	...
	Если ИмяОбновляемогоПоля = "ДокументыСертификации"
		...
		Для Каждого СтрокаЗаполнения Из Результат Цикл
			...
			ОбновитьКэшСертификацииКлиентСервер(ЭтаФорма, НоваяСтрока, ИзменяемаяСтрока.Номенклатура);
...
Процедура ВыборМаршрутаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	УстановитьПредставлениеТранспортныхСредствПоСтроке(ЭтаФорма, СтрокаМаршрут);
	...
	УстановитьПредставлениеМестПерегрузкиПоСтроке(ЭтаФорма, СтрокаМаршрут);
...
Функция ПунктНазначенияСписокАвтоподбораОбщий(ТекстДляАвтоПодбора)
	...
	СписокАвтоподбора = ИнтеграцияЗЕРНОВызовСервера.СписокАвтоподбораПоОписаниюТипов(
		Новый ОписаниеТипов(ОпределяемыеТипы().ПунктНазначенияЗЕРНО.Тип,, ИсключаемыеТипы),
		ТекстДляАвтоПодбора);
...
Процедура СоздатьКэшСертификацииСервер()
	...
	Для Каждого СтрокаТовары Из Объект.Товары Цикл
		...
		Для Каждого СтрокаСертификации Из СтрокиДокументовСертификации Цикл 
			...
			ОбновитьКэшСертификацииКлиентСервер(ЭтаФорма, ДанныеЗаполнения, СтрокаТовары.Номенклатура);
		КонецЦикла;
...
Процедура ЗаполнитьСертфикацию(ИзменяемыеСтроки)
	...
	Если МассивНоменклатуры.Количество() Тогда
		ДобавитьСертификатыНоменклатурыВКэш(ЭтаФорма, МассивНоменклатуры);
