...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДокументОснование.ДоступныеТипы = ОпределяемыеТипы().ОснованиеПеремаркировкиТоваровИСМП.Тип;
	...
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(
		ЭтаФорма, "ТоварыХарактеристика");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(
		ЭтаФорма, "ТоварыНоваяХарактеристика", "Элементы.Товары.ТекущиеДанные.НоваяНоменклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(
		ЭтаФорма, "ТоварыМарка", "Элементы.Товары.ТекущиеДанные.Номенклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСНоменклатурой(
		ЭтаФорма, "ТоварыНоваяМарка", "Элементы.Товары.ТекущиеДанные.НоваяНоменклатура");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСХарактеристикой(
		ЭтаФорма, "ТоварыМарка", "Элементы.Товары.ТекущиеДанные.Характеристика");
	СобытияФормИСПереопределяемый.УстановитьСвязиПараметровВыбораСХарактеристикой(
		ЭтаФорма, "ТоварыНоваяМарка", "Элементы.Товары.ТекущиеДанные.НоваяХарактеристика");
	...
	Если (Не ЗначениеЗаполнено(Объект.Ссылка)) Или (Не Объект.Проведен) Тогда
		...
		ОписаниеОповещенияВопрос = Новый ОписаниеОповещения(
			"СтатусПредставлениеОбработкаНавигационнойСсылкиЗавершение",
				ЭтаФорма, Новый Структура("НавигационнаяСсылкаФорматированнойСтроки",
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	ИначеЕсли Элемент.ТекущийЭлемент = Элементы.ТоварыПредставлениеПервичногоДокумента Тогда
		ИнтеграцияИСМПКлиент.ОбновитьСписокВыбораПервичногоДокумента(ЭтаФорма);
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормИСПереопределяемый.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(
		ЭтаФорма, "ТоварыНоваяХарактеристика", "Объект.Товары.НоваяХарактеристикаИспользуется");
...
Процедура ОбновитьСтатусИСМП()
	...
	ИнтеграцияИСКлиентСервер.УстановитьДоступностьЭлементовФормы(ЭтаФорма,
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	СобытияФормИСКлиентСерверПереопределяемый.УстановитьПараметрыВыбораНоменклатуры(
		ЭтаФорма, Объект.ВидПродукции, "ТоварыНоваяНоменклатура");
...
Процедура ЗаполнениеСертификацииЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ОбновитьКэшСертификацииКлиентСервер(
		ЭтаФорма, ВидСертификации, НомерСертификации, ДатаСертификации);
...
Процедура ЗаполнениеПервичногоДокументаЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	ОбновитьКэшПервичныхДокументов(ЭтаФорма, РезультатВыбора);
	ИнтеграцияИСМПКлиент.ОбновитьСписокВыбораПервичногоДокумента(ЭтаФорма);
...
Процедура ОткрытьФормуВводаКодовМаркировки(ИсходныеДанныеСтроки)
	...
	Если ТипЗнч(ИсходныеДанныеСтроки) = Тип("Структура") Тогда
		...
		ПараметрыОткрытия.Вставить("АдресДанныеШтрихкода",
			ПоместитьВоВременноеХранилище(ИсходныеДанныеСтроки.ДанныеШтрихкода, ЭтаФорма.УникальныйИдентификатор));
	...
	ОписаниеОповещенияЗакрытиеФормыВводаКодаМаркировки = Новый ОписаниеОповещения(
		"ОповещениеЗакрытиеФормыВводаКода", ЭтаФорма, ДополнительныеПараметры);
		
	Форма = ОткрытьФорму(
		"Обработка.РаботаСКМПриПеремаркировкеТоваровИСМП.Форма.ФормаВводаКодовМаркировки",
		ПараметрыОткрытия,
		ЭтаФорма,,,, ОписаниеОповещенияЗакрытиеФормыВводаКодаМаркировки);
...
Процедура ОповещениеЗакрытиеФормыВводаКода(Результат, ДополнительныеПараметры) Экспорт
	...
	ОбновитьКэшСертификацииКлиентСервер(ЭтаФорма,
	...
	ОбновитьКэшПервичныхДокументов(ЭтаФорма, Результат);
	ИнтеграцияИСМПКлиент.ОбновитьСписокВыбораПервичногоДокумента(ЭтаФорма);
...
Функция ЭтоРедактируемоеПолеФормы(Поле)
	...
	Возврат РедактируемыеПоляФормы(ЭтаФорма).Найти(Поле) <> Неопределено;
