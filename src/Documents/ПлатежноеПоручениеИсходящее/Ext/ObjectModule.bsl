...
Функция ПечатьПлатежногоПоручения() Экспорт
	...
	Если Дата < '20120709' Тогда
		Если Прав(НомерПечать,3)="000" Тогда
			Сообщить("Номер платежного поручения не может оканчиваться на ""000""!", СтатусСообщения.Важное);
			ЕстьОшибки = Истина;
		КонецЕсли;
	ИначеЕсли Прав(НомерПечать,6) = "000000" Тогда
		Сообщить("Номер платежного поручения не может оканчиваться на ""000000""!", СтатусСообщения.Важное);
		ЕстьОшибки = Истина;
	КонецЕсли;
	...
	ПлательщикИНН           = ?(ПустаяСтрока(ИННПлательщика), Организация.ИНН, СокрЛП(ИННПлательщика));
	ПлательщикКПП           = ?(ПустаяСтрока(КППплательщика),?(ВидОперации=Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога,"0",""),СокрЛП(КППплательщика));	
	...
	Если ВидОперации=Перечисления.ВидыОперацийППИсходящее.ПеречислениеНалога Тогда
		ТекстПлательщикПечать = ?(ПустаяСтрока(ТекстПлательщика) ИЛИ СокрЛП(ТекстПлательщика) = СокрЛП(Организация.НаименованиеПлательщикаПриПеречисленииНалогов),
			?(ПустаяСтрока(Организация.НаименованиеПлательщикаПриПеречисленииНалогов),
				СокрЛП(Организация.НаименованиеПолное), СокрЛП(Организация.НаименованиеПлательщикаПриПеречисленииНалогов) + СтрКорреспондент),
			СокрЛП(ТекстПлательщика));
	Иначе
	...
	Если НЕ ВидОперации=Перечисления.ВидыОперацийППИсходящее.ПереводНаДругойСчет Тогда
		...
		ПолучательИНН           = ?(ПустаяСтрока(ИННПолучателя), Контрагент.ИНН, СокрЛП(ИННПолучателя));
		ПолучательКПП           = ?(ПустаяСтрока(КПППолучателя),"",СокрЛП(КПППолучателя));
		Обл.Параметры.Получатель              = ?(ПустаяСтрока(ТекстПолучателя),Контрагент.НаименованиеПолное,СокрЛП(ТекстПолучателя));
		
	Иначе
		
		ПолучательИНН           = ?(ПустаяСтрока(ИННПолучателя), Организация.ИНН, СокрЛП(ИННПолучателя));
		ПолучательКПП           = ?(ПустаяСтрока(КПППолучателя),"",СокрЛП(КПППолучателя));
		Обл.Параметры.Получатель              = ?(ПустаяСтрока(ТекстПолучателя),Организация.НаименованиеПолное,СокрЛП(ТекстПолучателя));
		
	КонецЕсли;
	
	Если Лев(ПлательщикИНН, 2) = "00" Тогда
		Сообщить("Первые две цифры ИНН плательщика не могут быть ""00""", СтатусСообщения.Важное);
		ЕстьОшибки = Истина;
	КонецЕсли;
	
	Если Лев(ПолучательИНН, 2) = "00" Тогда
		Сообщить("Первые две цифры ИНН получателя не могут быть ""00""", СтатусСообщения.Важное);
		ЕстьОшибки = Истина;
	КонецЕсли;
	
	Если Лев(ПлательщикКПП, 2) = "00" Тогда
		Сообщить("КПП плательщика не должен начинаться на 00", СтатусСообщения.Важное);
		ЕстьОшибки = Истина;
	КонецЕсли;
	
	Если Лев(ПолучательКПП, 2) = "00" Тогда
		Сообщить("КПП получателя не должен начинаться на 00", СтатусСообщения.Важное);
		ЕстьОшибки = Истина;
	КонецЕсли;
	...
	Обл.Параметры.ПлательщикИНН = "ИНН " + ПлательщикИНН;
	Обл.Параметры.ПлательщикКПП = "КПП " + ПлательщикКПП;
	Обл.Параметры.ПолучательИНН = "ИНН " + ПолучательИНН;
	Обл.Параметры.ПолучательКПП = "КПП " + ПолучательКПП;
	
	Обл.Параметры.БанкПолучателя          = "" + ?(ЗначениеЗаполнено(БанкКонтрагента.БИКРКЦ), БанкКонтрагента.БИКРКЦ.Наименование + "//", "") + БанкКонтрагента + " " + БанкКонтрагента.Город;
	...
	Если ПеречислениеВБюджет Тогда
		...
		Если ПлатежиВБюджетКлиентСервер.ДействуетПриказ107н(ЭтотОбъект.Дата) Тогда
			Если Дата >= ПлатежиВБюджетКлиентСервер.НачалоДействияУказания3844У() Тогда
				...
				РеквизитыПлатежаВБюджет.ТипПлатежа = "";
			ИначеЕсли Дата >= ПлатежиВБюджетКлиентСервер.НачалоДействияПриказа126н() Тогда
	...
	Иначе
		...
		Если ЗначениеЗаполнено(КодВыплат) Тогда
			
			Обл.Параметры.ПоказательТипа = КодВыплат;
			
		КонецЕсли;
		
		Если Дата >= УправлениеДенежнымиСредствами.НачалоДействияУказанияБанкаРоссии5286У()
			И ЗначениеЗаполнено(КодВидаДохода) Тогда
