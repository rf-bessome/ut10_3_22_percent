...
Функция ПечатьТОРГ12ЗаПоставщика(БезУслуг = Ложь)
	...
	|(ВЫБРАТЬ
	|	ПоступлениеТоваровУслуг.Номенклатура                      КАК Номенклатура,
	|	ПоступлениеТоваровУслуг.Цена * &КурсДокумента / &КратностьДокумента         КАК Цена,
	|	СУММА(ПоступлениеТоваровУслуг.Количество)                 КАК Количество,
	|	СУММА(ПоступлениеТоваровУслуг.Сумма * &КурсДокумента / &КратностьДокумента) КАК Сумма,
	|	МИНИМУМ(ПоступлениеТоваровУслуг.НомерСтроки)              КАК НомерСтроки
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.ВозвратнаяТара КАК ПоступлениеТоваровУслуг
	|ГДЕ
...
Функция ПечатьТОРГ4()
	...
	|	ВложенныйЗапрос.Серия,
	|	ВложенныйЗапрос.НомерСтроки КАК НомерСтроки,
	|	ВложенныйЗапрос.Масса КАК МассаНетто,
	|	ВложенныйЗапрос.Масса КАК МассаБрутто,
	|	ВложенныйЗапрос.Метка       КАК Метка
	|
	|ИЗ
	|	(
	...
	|		ПоступлениеТоваровУслуг.СерияНоменклатуры          КАК Серия,
	|		МИНИМУМ(ПоступлениеТоваровУслуг.НомерСтроки)       КАК НомерСтроки,
	|		СУММА(ПоступлениеТоваровУслуг.ЕдиницаИзмерения.Вес * ПоступлениеТоваровУслуг.Количество) КАК Масса,
	|		0                                                  КАК Метка
	|	ИЗ
	|		Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслуг
	|
	...
	|	ПоступлениеТоваровУслуг.Цена  * &КурсДокумента / &КратностьДокумента КАК Цена,
	|	СУММА(ПоступлениеТоваровУслуг.Сумма * &КурсДокумента / &КратностьДокумента) КАК Сумма,
	|	СУММА(ПоступлениеТоваровУслуг.Номенклатура.ЕдиницаХраненияОстатков.Вес * ПоступлениеТоваровУслуг.Количество) КАК Масса,
	|	МИНИМУМ(ПоступлениеТоваровУслуг.НомерСтроки) КАК НомерСтроки
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.ВозвратнаяТара КАК ПоступлениеТоваровУслуг
	|
	...
	Если ВалютаДокумента <> мВалютаРегламентированногоУчета
		...
		ПараметрыПересчета.СуммаВключаетНДС     = Шапка.СуммаВключаетНДС;
		ПараметрыПересчета.ДатаПересчета        = Дата;
		УчетНДС.ПерезаполнитьСуммыПоУЕвРублях(ЗапросТовары, ПараметрыПересчета, Новый Структура("Сумма, Количество", "Стоимость", "КоличествоШтук"));
	...
	Для Каждого ВыборкаСтрокТовары Из ЗапросТовары Цикл
		...
		ОбластьМакета.Параметры.Заполнить(ВыборкаСтрокТовары);
		СуммаСНДС = ?(СуммаВключаетНДС, ВыборкаСтрокТовары.Стоимость, ВыборкаСтрокТовары.Стоимость + ВыборкаСтрокТовары.СуммаНДС);
		ОбластьМакета.Параметры.Стоимость = СуммаСНДС;
		Цена = ?(ВыборкаСтрокТовары.КоличествоШтук = 0, СуммаСНДС, СуммаСНДС/ВыборкаСтрокТовары.КоличествоШтук);
		...
		ИтогоМассаБруттоПоСтранице = ИтогоМассаБруттоПоСтранице + ВыборкаСтрокТовары.МассаБрутто;
		ИтогоМассаНеттоПоСтранице  = ИтогоМассаНеттоПоСтранице  + ВыборкаСтрокТовары.МассаНетто;
...
Функция ПечатьМ4()
	...
	|	ПоступлениеТоваровУслуг.Номенклатура." + ТоварКод + " КАК ТоварКод,
	|	ПоступлениеТоваровУслуг.Номенклатура.ЕдиницаХраненияОстатков.Представление               КАК ЕдиницаИзмеренияНаименование,
	|	ПоступлениеТоваровУслуг.Номенклатура.ЕдиницаХраненияОстатков.ЕдиницаПоКлассификатору.Код КАК ЕдиницаИзмеренияКод,
	|	ПоступлениеТоваровУслуг.СтавкаНДС                     КАК СтавкаНДС,
	|	ПоступлениеТоваровУслуг.Количество                    КАК КоличествоПринято,
	|	ПоступлениеТоваровУслуг.Цена * &Курс / &Кратность     КАК Цена,
	|	ПоступлениеТоваровУслуг.Сумма    * &Курс / &Кратность КАК Стоимость,
	|	ПоступлениеТоваровУслуг.СуммаНДС * &Курс / &Кратность КАК СуммаНДС,
	|	NULL												  КАК Характеристика,
	|	NULL												  КАК Серия,
	|	ПоступлениеТоваровУслуг.НомерСтроки                   КАК НомерСтроки,
	...
	|	Цена     * &КурсДокумента / &КратностьДокумента КАК Цена,
	|	СУММА(Сумма    * &КурсДокумента / &КратностьДокумента) КАК Сумма,
	|	МИНИМУМ(НомерСтроки) КАК НомерСтроки
	|ИЗ
	|	Документ.ПоступлениеТоваровУслуг.ВозвратнаяТара КАК ПоступлениеТоваровУслуг
	|ГДЕ
	...
	Если ВалютаДокумента <> мВалютаРегламентированногоУчета
		...
		ПараметрыПересчета.СуммаВключаетНДС     = Шапка.СуммаВключаетНДС;
		ПараметрыПересчета.ДатаПересчета        = Дата;
		УчетНДС.ПерезаполнитьСуммыПоУЕвРублях(ЗапросТовары, ПараметрыПересчета, Новый Структура("Сумма, Количество", "Стоимость", "КоличествоПринято"));
...
Процедура ЗаполнитьТоварыПоОснованиюУпр(ДокументОснование, ТабличнаяЧасть) Экспорт
	...
	Запрос.УстановитьПараметр("Склад",        ?(ТипЗнч(ДокументОснование) = Тип("СправочникСсылка.Склады"), ДокументОснование, ДокументОснование.Склад));
...
Процедура ЗаполнитьТабличныеЧастиПередПроведениемУпр() Экспорт
	...
	Для Сч = 0 По 1 Цикл
		...
		КонецЦикла; 
	КонецЦикла;
	Товары.Свернуть("Номенклатура,ХарактеристикаНоменклатуры,СерияНоменклатуры,ЕдиницаИзмерения,ЕдиницаИзмеренияМест,Коэффициент,Цена,СтавкаНДС,Заказ,Склад,ПриходныйОрдер,ЗаказПоставщику,КлючСвязи,ПрослеживаемыйТовар,СтранаПроисхождения","Количество,КоличествоМест,Сумма,СуммаНДС");
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(УсловияПрослеживаемости, СтруктураШапкиДокумента)
	...
	Запрос.УстановитьПараметр("ЭтоИмпорт", 
	?(ЗначениеЗаполнено(СтруктураШапкиДокумента.Контрагент.СтранаРегистрации) И СтруктураШапкиДокумента.Контрагент.СтранаРегистрации = Справочники.КлассификаторСтранМира.Россия, Истина, Ложь));
	...
	СтруктураШапкиДокумента.Вставить("СтранаРегистрации", ?(ЗначениеЗаполнено(СтруктураШапкиДокумента.Контрагент.СтранаРегистрации),
	СтруктураШапкиДокумента.Контрагент.СтранаРегистрации, Справочники.КлассификаторСтранМира.Россия));
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Если Основание.Проведен Тогда
			...
			УчетРНПТ.ЗаполнитьРНПТПоОснованию(Основание, СведенияПрослеживаемости);
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Не заполенены реквизиты ""Контрагент""");
		Отказ = Истина;
		Возврат;
	КонецЕСли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Перем ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоРНПТ;
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров") И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(СтруктураШапкиДокумента.Дата) Тогда
		
		Если Не СтруктураШапкиДокумента.Контрагент.СтранаРегистрации.УчастникЕАЭС Тогда
			РезультатЗапросаПоРНПТ = УчетРНПТ.СформироватьЗапросПоТабличнойЧастиДляПроверкиКоличества(ЭтотОбъект, "Товары", "Номенклатура");
	
			Отказ = Не РезультатЗапросаПоРНПТ.Пустой();
	
			Если Отказ Тогда 
				ОбщегоНазначения.СообщитьОбОшибке( "Количество по РНПТ по прослеживаемым товарам не соответствует количеству, указанному для номенклатуры.", Отказ, Заголовок);
				Возврат;
			КонецЕсли;	
	...
	Если ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(СтруктураШапкиДокумента.Дата)
			И ПрослеживаемостьБП.ЭтоКонтрагентИзЕАЭС(СтруктураШапкиДокумента.Контрагент) Тогда
		
		РегистрыСведений.РегистрацияПрослеживаемыхТоваров.ОбновитьСостояние(
			СтруктураШапкиДокумента.Ссылка, СтруктураШапкиДокумента.Ссылка);
