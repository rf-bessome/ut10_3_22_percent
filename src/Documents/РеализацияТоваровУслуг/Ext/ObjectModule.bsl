...
Перем мЕстьПравоСозданиеЭТрН Экспорт;
...
Функция ПечатьДокумента(СуммыВРублях = Ложь)
	...
	УниверсальныеМеханизмы.ОпределитьКурсыДокументаДляПечати(ЭтотОбъект, Запрос, Не СуммыВРублях);
	...
	Если СуммыВРублях И Шапка.ВалютаДокумента <> мВалютаРегламентированногоУчета Тогда
		...
		ПараметрыПересчета.ВалютаВзаиморасчетов = ВалютаДокумента;
		...
		ПараметрыПересчета.ДатаПересчета        = Дата;
...
Функция ПечатьМ15()
	...
	УниверсальныеМеханизмы.ОпределитьКурсыДокументаДляПечати(ЭтотОбъект, ЗапросПоТоварам);
	...
	|	ВЫРАЗИТЬ(РеализацияТоваровУслугУслуги.Номенклатура.НаименованиеПолное КАК СТРОКА(1000)),
	...
	|	РеализацияТоваровУслугУслуги.Цена * &Курс / &Кратность,
	|	РеализацияТоваровУслугУслуги.Сумма * &Курс / &Кратность,
	|	РеализацияТоваровУслугУслуги.СуммаНДС * &Курс / &Кратность,
	...
	Если ВалютаДокумента <> мВалютаРегламентированногоУчета
		И (ДоговорКонтрагента.РасчетыВУсловныхЕдиницах ИЛИ Дата >= '20090101000000')
		И ДоговорКонтрагента.ВестиПоДокументамРасчетовСКонтрагентом Тогда

		ПараметрыПересчета = УчетНДС.СтруктураПараметровДляПересчетаСуммВРубли();
		ПараметрыПересчета.СуммаАвансаВалИт     = ЗапросПоТоварам.Параметры.СуммаАвансаВал;
		ПараметрыПересчета.СуммаАвансаРубИт     = ЗапросПоТоварам.Параметры.СуммаАвансаРуб;
		ПараметрыПересчета.СуммаРеализацииВалИт = УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект);
		ПараметрыПересчета.ВалютаВзаиморасчетов = ВалютаДокумента;
		ПараметрыПересчета.КурсРеализации       = ЗаполнениеДокументов.КурсДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета);
		ПараметрыПересчета.КратностьРеализации  = ЗаполнениеДокументов.КратностьДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета);
		ПараметрыПересчета.СуммаВключаетНДС     = Шапка.СуммаВключаетНДС;
		ПараметрыПересчета.ДатаПересчета        = Дата;
		УчетНДС.ПерезаполнитьСуммыПоУЕвРублях(ЗапросТовары, ПараметрыПересчета, Новый Структура("Количество", "КоличествоПринято"));

	КонецЕсли;
	
	МассивТЧ = Новый Массив();
	МассивТЧ.Добавить(0);
	УниверсальныеМеханизмы.ПодготовитьТаблицуДокументаДляПечати(ЗапросТовары, МассивТЧ);
	...
	ОбластьПодвала.Параметры.ИтогНДС       = ОбщегоНазначения.ФорматСумм(ИтогоВсегоСНДС - ИтогоСуммаБезНДС);
...
Функция ПечатьАктаОбОказанииУслуг(СуммыВРублях = Ложь)
	...
	|		РеализацияТоваровУслуг.СтавкаНДС КАК СтавкаНДС,
	|		СУММА(РеализацияТоваровУслуг.СуммаНДС * &Курс / &Кратность) КАК СуммаНДС,
	|		РеализацияТоваровУслуг.ПроцентСкидкиНаценки + РеализацияТоваровУслуг.ПроцентАвтоматическихСкидок КАК Скидка,
	|		МИНИМУМ(РеализацияТоваровУслуг.НомерСтроки) КАК НомерСтроки,
	|		0 КАК Метка
	|	ИЗ
	|		Документ.РеализацияТоваровУслуг.Товары КАК РеализацияТоваровУслуг
	...
	|		РеализацияТоваровУслуг.Цена,
	|		ВЫРАЗИТЬ(РеализацияТоваровУслуг.Номенклатура.НаименованиеПолное КАК СТРОКА(1000)),
	|		РеализацияТоваровУслуг.ПроцентСкидкиНаценки + РеализацияТоваровУслуг.ПроцентАвтоматическихСкидок,
	|		РеализацияТоваровУслуг.Цена * &Курс / &Кратность) КАК ВложенныйЗапрос";
	...
	УниверсальныеМеханизмы.ОпределитьКурсыДокументаДляПечати(ЭтотОбъект, ЗапросУслуги, Не СуммыВРублях);
	
	ТаблицаУслуги = ЗапросУслуги.Выполнить().Выгрузить();

	Если СуммыВРублях И Шапка.ВалютаДокумента <> мВалютаРегламентированногоУчета Тогда
		ПараметрыПересчета = УчетНДС.СтруктураПараметровДляПересчетаСуммВРубли();
		ПараметрыПересчета.СуммаАвансаВалИт     = ЗапросУслуги.Параметры.СуммаАвансаВал;
		ПараметрыПересчета.СуммаАвансаРубИт     = ЗапросУслуги.Параметры.СуммаАвансаРуб;
		ПараметрыПересчета.СуммаРеализацииВалИт = УчетНДС.ПолучитьСуммуДокументаСНДС(ЭтотОбъект);
		ПараметрыПересчета.ВалютаВзаиморасчетов = ВалютаДокумента;
		ПараметрыПересчета.КурсРеализации       = ЗаполнениеДокументов.КурсДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета);
		ПараметрыПересчета.КратностьРеализации  = ЗаполнениеДокументов.КратностьДокумента(ЭтотОбъект, мВалютаРегламентированногоУчета);
		ПараметрыПересчета.СуммаВключаетНДС     = Шапка.СуммаВключаетНДС;
		ПараметрыПересчета.ДатаПересчета        = Дата;
		УчетНДС.ПерезаполнитьСуммыПоУЕвРублях(ТаблицаУслуги, ПараметрыПересчета);
	КонецЕсли;

	МассивТЧ = Новый Массив();
	МассивТЧ.Добавить(1);
	УниверсальныеМеханизмы.ПодготовитьТаблицуДокументаДляПечати(ТаблицаУслуги, МассивТЧ);
	...
	Для Каждого СтрокаТабличнойЧасти Из ТаблицаУслуги Цикл	
		...
		Если ЕстьСкидки Тогда
			...
			ОбластьСтроки.Параметры.Цена = ОбластьСтроки.Параметры.СуммаБезСкидки/СтрокаТабличнойЧасти.Количество;
		КонецЕсли;
...
Функция ПечатьТТН()
	...
	ИначеЕсли Дата < Дата(2021, 01, 01) Тогда
		ОбработкаПечати = Обработки.ПечатьТТН1208.ПолучитьФорму("Форма");
	ИначеЕсли Дата < Дата(2022, 03, 01) Тогда
		ОбработкаПечати = Обработки.ПечатьТТН1208.ПолучитьФорму("Форма2200");
	Иначе
		ОбработкаПечати = Обработки.ПечатьТТН1208.ПолучитьФорму("Форма2116");
	КонецЕсли;
...
Функция ПечатьАктаНаПередачуПрав() 
	...
	// Внимание
	Если ЗначениеЗаполнено(ДанныеДляПечати.ОтсрочкаПлатежаДней) И ЗначениеЗаполнено(ДанныеДляПечати.ПроцентШтрафаЗаДеньПросрочки) Тогда
...
Процедура ЗаполнитьТабличнуюЧастьПоЗаказуПокупателяУпр(ТабличнаяЧасть, ДокументОснование) Экспорт
	...
	|	Остатки.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар КАК ПрослеживаемыйТовар,
	|	Остатки.Номенклатура.СтранаПроисхождения КАК СтранаПроисхождения,
	...
	|	ОстаткиНаСкладе.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар        КАК ПрослеживаемыйТовар,
	|	ОстаткиНаСкладе.Номенклатура.СтранаПроисхождения                 КАК СтранаПроисхождения,	
	|	0                                                                КАК Цена,
			...
			Для Сч = 1 По 2 Цикл
				...
				Для каждого СтрокаОстатков Из СтрокиОстатков Цикл
					...
					Если ТабличнаяЧасть = Товары Тогда
						...
						СтрокаТабличнойЧасти.ПрослеживаемыйТовар                 = СтрокаЗаказа.ПрослеживаемыйТовар;
						СтрокаТабличнойЧасти.СтранаПроисхождения                 = СтрокаЗаказа.СтранаПроисхождения;
...
Процедура РаспределитьСуммуПродажиПоПрослеживаемымТоварам(РеализацияПрослеживаемыхТоваров, РеализацияТаблицаДокумента)
	...
	Для каждого ТекСтрокаРеализацияПрослеживаемыхТоваров Из РеализацияПрослеживаемыхТоваров Цикл
		...
		Для каждого ТекущаяСтрока Из НайденныеСтроки Цикл
			...
			Если ОстатокКоличество = ТекСтрокаРеализацияПрослеживаемыхТоваров.Количество Тогда
				...
				ТекСтрокаРеализацияПрослеживаемыхТоваров.Сумма = ТекущаяСтрока.СуммаБезНДС;
			Иначе
				
				СуммаБезНДСРуб = ТекущаяСтрока.СуммаБезНДС;
...
Функция ПодготовитьТаблицуТоваровРеализованныхВЕАЭС(РезультатЗапросаПоТоварам, РеализацияТаблицаДокумента, СтруктураШапкиДокумента)
	
	Если НЕ ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(СтруктураШапкиДокумента.Дата)
		ИЛИ НЕ СтруктураШапкиДокумента.РеализацияВЕАЭС Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Параметры = ПодготовитьПараметрыПрослеживаемыхТоваров(РезультатЗапросаПоТоварам.Выгрузить(), РеализацияТаблицаДокумента);
	
	ТаблицаТоваров = Параметры.ПрослеживаемыеТовары;
	ТаблицаТоваров.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
	ТаблицаТоваров.Колонки.Добавить("Период",  Новый ОписаниеТипов("Дата"));
	...
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл
		...
		СтрокаТаблицы.Период = СтруктураШапкиДокумента.Дата;
		
	КонецЦикла;
	
	РаспределитьСуммуПродажиПоПрослеживаемымТоварам(ТаблицаТоваров, Параметры.РеализацияТаблицаДокумента);
	
	Возврат ТаблицаТоваров;
	
КонецФункции // ПодготовитьТаблицуТоваров()

Функция СформироватьЗапросПоПрослеживаемымТоварам(ЭтотОбъект)
...
Функция ПодготовитьПараметрыПрослеживаемыхТоваров(ТаблицаТовары, РеализацияТаблицаДокумента)
...
Параметры = Новый Структура;
	...
	Параметры.Вставить("ПрослеживаемыеТовары", ПрослеживаемостьБП.ПолучитьТаблицуПараметровПроведения(
		ТаблицаТовары, СписокОбязательныхКолонок));
		
	СписокОбязательныхКолонок = ""
	+ "Номенклатура,"     // <СправочникСсылка.Номенклатура>
	+ "Количество,"       // <Число,15,3> - количество
	+ "СуммаБезНДС,"      // <Число,26,11> - Сумма продажи
	+ "НомерСтроки";      // <Число> - Номер строки документа в табличной части
	
	Параметры.Вставить("РеализацияТаблицаДокумента", ПрослеживаемостьБП.ПолучитьТаблицуПараметровПроведения(
...
Процедура ПроверитьЗаполнениеТабличнойЧастиТовары(ТаблицаПоТоварам, СтруктураШапкиДокумента, Отказ, Заголовок)
	...
	// проверка заполнения страны для прослеживаемых товаров
	УчетРНПТ.ПроверитьЗаполнениеСтраныДляПрослеживаемыхТоваров(ЭтотОбъект, "Товары", Отказ);
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Если ОтражатьВБухгалтерскомУчете Тогда
		...
		УсловияПрослеживаемости.Вставить("ЕстьТовары", ТаблицаПоТоварам.Количество() > 0);
...
Процедура ДвижениеПоРегистрамПрослеживаемости(ТаблицаПоПрослеживаемымТоварам)
	
	// Движения по реализации прослеживаемых товаров в страны ЕАЭС
	СформироватьДвиженияПрослеживаемыхТоваровВЕАЭС(ТаблицаПоПрослеживаемымТоварам);
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(УсловияПрослеживаемости, СтруктураШапкиДокумента)
	...
	ЗапросТЧ.УстановитьПараметр("Ссылка", СтруктураШапкиДокумента.Ссылка);
	...
	ПлательщикНДС = СтруктураШапкиДокумента.УчитыватьНДС;
	...
	Запрос.УстановитьПараметр("ПрослеживаемыеОперацииОтображаютсяВДекларацииНДС",
		ЕстьНДС ИЛИ СтруктураШапкиДокумента.ПрименяетсяОсвобождениеОтУплатыНДС);
	Запрос.УстановитьПараметр("ЭтоДоговорСКомиссионером", 
	...
	Запрос.УстановитьПараметр("ЭтоЭкспорт",
		ЗначениеЗаполнено(СтруктураШапкиДокумента.Контрагент.СтранаРегистрации)
		И СтруктураШапкиДокумента.Контрагент.СтранаРегистрации <> Справочники.КлассификаторСтранМира.Россия);
		
	НомераТаблиц = Новый Структура;
	ПараметрыПроведения = Новый Структура;
	
	Запрос.Текст = Документы.РеализацияТоваровУслуг.ТекстЗапросаПрослеживаемыеТовары(НомераТаблиц, ПараметрыПроведения, УсловияПрослеживаемости);
	...
	Если ПараметрыПроведения.ПрослеживаемыеОперации <> Неопределено Тогда
		Если НЕ ПараметрыПроведения.ПрослеживаемыеОперации.Колонки.Найти("НомерДокумента") = Неопределено Тогда
			Для Каждого СтрокаТаблицы Из ПараметрыПроведения.ПрослеживаемыеОперации Цикл
				СтрокаТаблицы.НомерДокумента = ОбщегоНазначения.ПолучитьНомерНаПечать(Ссылка);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьДвиженияПрослеживаемыхТоваровВЕАЭС(РеализацияПрослеживаемыхТоваровВЕАЭС) Экспорт
...
Процедура ПодготовитьСтруктуруШапкиДокумента(Заголовок, СтруктураШапкиДокумента, РежимПроведения = Неопределено) Экспорт
	...
	СтруктураШапкиДокумента.Вставить("РеализацияВЕАЭС", 
		ПрослеживаемостьБП.ЭтоКонтрагентИзЕАЭС(СтруктураШапкиДокумента.Контрагент)); //Это реализация в ЕАЭС
...
Процедура ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоПрослеживаемымТоварам) Экспорт
	...
	РезультатЗапросаПоТоварам = СформироватьЗапросПоПрослеживаемымТоварам(ЭтотОбъект);
	
	ТаблицаПоПрослеживаемымТоварам = ПодготовитьТаблицуТоваровРеализованныхВЕАЭС(РезультатЗапросаПоТоварам, ТаблицаПоТоварам, СтруктураШапкиДокумента);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если (ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров") И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(Дата)) Тогда
			...
			Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				...
				Для Каждого СтрокаТоваров Из ЭтотОбъект.Товары Цикл
					...
					Если (СтрокаТоваров.ПрослеживаемыйТовар) И (СтрокаТоваров.АвтоподборРНПТ) Тогда
						...
						СтрокиПрослеживаемостиТовара = ЭтотОбъект.СведенияПрослеживаемости.НайтиСтроки(Отбор);	
						...
						Если СтрокиПрослеживаемостиТовара.Количество() > 0 Тогда
							...
							Для Каждого СтрокаПрослеживаемостиТовара Из СтрокиПрослеживаемостиТовара Цикл
								ЭтотОбъект.СведенияПрослеживаемости.Удалить(СтрокаПрослеживаемостиТовара);
							КонецЦикла;
...
Процедура ПодготовитьПараметрыУчетнойПолитикиРегл(Отказ, СтруктураШапкиДокумента)
	...
	УчетнаяПолитикаРегл = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитикиНалоговыйУчет(Дата, Отказ, Организация);
	...
	Иначе
		...
		СтруктураШапкиДокумента.Вставить("ПрименяетсяОсвобождениеОтУплатыНДС", УчетнаяПолитикаРегл.ПрименяетсяОсвобождениеОтУплатыНДС);
	КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Перем Заголовок, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоПрослеживаемымТоварам;
	...
	ПодготовитьТаблицыДокумента(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоПрослеживаемымТоварам);
	...
	Если Не Отказ Тогда
		...
		ДвижениеПоРегистрамПрослеживаемости(ТаблицаПоПрослеживаемымТоварам);
	КонецЕсли;
...
мЕстьПравоСозданиеЭТрН = ПравоДоступа("Изменение", Метаданные.Документы.ЭлектроннаяТранспортнаяНакладная);
