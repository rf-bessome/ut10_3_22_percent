...
Процедура ПроверкаРеквизитовТЧ(Отказ, Заголовок, ТаблицаМатериалов) Экспорт
	...
	УчетРНПТ.ПроверитьЗаполнениеСтраныДляПрослеживаемыхТоваров(ЭтотОбъект, "Материалы", Отказ);
...
Процедура ДвиженияПоРегистрамПрослеживаемыхТоваров(УсловияПрослеживаемости, СтруктураШапкиДокумента)
	...
	Отказ = Ложь;
	УчетнаяПолитикаРегл = ОбщегоНазначения.ПолучитьПараметрыУчетнойПолитики(Дата, Отказ, Организация, "Нал");
...
Процедура ОбработкаЗаполнения(Основание)
	...
	Если ТипЗнч(Основание) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
		...
		Для Каждого ТекСтрокаТовары Из Основание.Товары Цикл
			...
			НоваяСтрока.ИдентификаторСтроки        = ТекСтрокаТовары.ИдентификаторСтроки;
			НоваяСтрока.ПрослеживаемыйТовар        = ТекСтрокаТовары.ПрослеживаемыйТовар;
			...
			НоваяСтрока.АвтоподборРНПТ = Истина;
	...
	Если (ПолучитьФункциональнуюОпцию("ВестиУчетПрослеживаемыхТоваров") И ПрослеживаемостьБРУ.ВедетсяУчетПрослеживаемыхТоваров(Дата)) Тогда
		...
		Если ЕстьПрослеживаемыйТовар Тогда
			...
			Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
				...
				Для Каждого СтрокаТоваров Из ЭтотОбъект.Материалы Цикл
...
мПараметрыСвязиСтрокТЧ.Вставить("Материалы", Новый Структура("СвободныйКлюч, ФлагМодификации", Неопределено, Ложь));
