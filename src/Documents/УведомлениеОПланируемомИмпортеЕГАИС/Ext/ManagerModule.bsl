...
Функция ПоследовательностьОпераций(ДокументСсылка) Экспорт
	...
	ИнтеграцияЕГАИС.ДобавитьОперациюВПоследовательность(Таблица, 0, Исходящий, ПередачаУведомления(), ДокументСсылка);
...
Функция УведомлениеОПланируемомИмпортеЕГАИСXML(ДокументСсылка)
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		XDTO = ИнтеграцияЕГАИС.ОбъектXDTO(ПространствоИмен, "InvoicePlannedImportType");
		
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(XDTO, "Identity", Шапка.Идентификатор, СообщениеXML, 3);
		
		XDTO.Header = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(XDTO, "Header");
		Содержимое = XDTO.Header;
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Содержимое, "Number",           НомерУведомления,          СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Содержимое, "Date",              Шапка.Дата,               СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Содержимое, "Note",              Шапка.Комментарий,        СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Содержимое, "TotalQuantity",     СтрокаТЧ.Количество,      СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(Содержимое, "TotalQuantityDal",  СтрокаТЧ.ОбъемДАЛ,        СообщениеXML);
		...
		Содержимое.Product = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(XDTO.Header, "Product");
		...
		XDTO.Content = ИнтеграцияИС.ОбъектXDTOПоИмениТипа(XDTO, "Content");
		
		ТекстСообщенияXML = ИнтеграцияЕГАИС.ОбъектXDTOВXML(XDTO, Шапка.ИдентификаторФСРАР, ПространствоИмен, ИмяТипа);
...
Функция ЗапросНаОтменуПроведенияXML(ДокументСсылка)
	...
	НомерСтроки = 0;
	ФорматОбмена = ИнтеграцияЕГАИСКлиентСервер.ФорматОбмена(Товары.ФорматОбмена);
	...
	Пока Товары.Следующий() Цикл
		...
		XDTO = ИнтеграцияЕГАИС.ОбъектXDTO(ПространствоИмен, "RequestRepealIPI");
		
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(XDTO, "ClientID",      Товары.ИдентификаторФСРАР,       СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(XDTO, "RequestNumber", НомерУведомления,                СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(XDTO, "RequestDate",   ДатаРаспроведения,               СообщениеXML);
		ИнтеграцияЕГАИС.ЗаполнитьСвойствоXDTO(XDTO, "IPIRegId",      Товары.ИдентификаторУведомления, СообщениеXML);
		
		ТекстСообщенияXML = ИнтеграцияЕГАИС.ОбъектXDTOВXML(XDTO, Товары.ИдентификаторФСРАР, ПространствоИмен, ИмяТипа);
