...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		КомандаПодписиОтправки.Доступность = НЕ СтатусОтклонен И Не ЭДОЗакрыт
			...
			И Не (Не ЭтоИзвещениеОПолучении И ЭлектронныеДокументыСлужебный.ВторойТитулПодписан(ПрисоединенныйФайлСсылка));
...
Процедура ИнициализацияДереваНаСервере()
	...
	ЭлектронныеДокументыСлужебный.СформироватьДеревьяЭД(ДеревоОбъект,СоответствиеВладельцевИЭД, Неопределено, Ложь, ОтключитьВыводДерева);
