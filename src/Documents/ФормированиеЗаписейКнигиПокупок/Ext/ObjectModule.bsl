		...
		Если ВерсияКодовВидовОпераций > 1 Тогда		
			...
			Если СчетФактураПоДокументу <> Неопределено Тогда 
				...
				Если НЕ ЗначениеЗаполнено(СтрокаСчетФактура.КодВидаОперации)
					ИЛИ СтрокаСчетФактура.КодВидаОперации = "01" 
					ИЛИ СтрокаСчетФактура.КодВидаОперации = "25;01" Тогда
					...
					Если СтрокаСчетФактура.КодВидаОперации = "25;01" Тогда
						КодВидаОперации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '25;%1'"), КодВидаОперации);
					КонецЕсли;
					СтрокаСчетФактура.КодВидаОперации = КодВидаОперации;
				КонецЕсли;
				
				Если НЕ ПредъявленНДСКВычету0 Тогда
					СтрокаСчетФактура.ДатаПолученияСчетаФактуры = СчетФактураПоДокументу.СчетФактураДата;
				КонецЕсли;
...
Функция ТекстЗапросаАвансыПолученные()
	...
	|	И СчетФактураВыданный.ВидСчетаФактуры В (ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.НаАванс), ЗНАЧЕНИЕ(Перечисление.ВидСчетаФактурыВыставленного.КорректировочныйНаАванс))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОснование
	|;
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента,
	ТаблицаПоВычетам,
	ТаблицапоАвансам,
	ТаблицаПоАвансамВыданным,
	ТаблицаПоВычетамПриИзмененииСтоимости,
	Отказ,
	Заголовок)
...
Процедура ДвиженияПоВычетамНДСсАвансов(СтруктураШапкиДокумента, ТаблицаПоАвансам, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ПредъявленНДСКВычету0 Тогда
		...
		СоответствиеКолонок1 = Новый Структура("ДоговорКонтрагента","Договор");
		УчетНДС.ПереименованиеКолонок(ТаблицаДвижений_НДСЗаписиКнигиПокупок,СоответствиеКолонок1);
		СоответствиеКолонок2 = Новый Структура("ДоговорАванса","ДоговорКонтрагента");
		УчетНДС.ПереименованиеКолонок(ТаблицаДвижений_НДСЗаписиКнигиПокупок,СоответствиеКолонок2);
		ОбщегоНазначения.ЗагрузитьВТаблицуЗначений(ТаблицаПоАвансам,ТаблицаДвижений_НДСЗаписиКнигиПокупок);
		УчетНДС.ПереименованиеКолонок(ТаблицаДвижений_НДСЗаписиКнигиПокупок, СоответствиеКолонок2, Истина);
		УчетНДС.ПереименованиеКолонок(ТаблицаДвижений_НДСЗаписиКнигиПокупок, СоответствиеКолонок1, Истина);
...
КонецПроцедуры // ДвиженияПоВычетамНДСсАвансовВыданных()
...
Процедура ДвиженияПоВычетамПриИзмененииСтоимости(СтруктураШапкиДокумента, ТаблицаПоВычетамПриИзмененииСтоимости, Отказ, Заголовок)
	...
	ТаблицаПоВычетамПриИзмененииСтоимости.Свернуть(
		"Организация, Событие, ДатаСчетаФактуры, ДатаИсправленногоСчетаФактуры, ДатаСобытия, Поставщик, СчетФактура, СтавкаНДС, 
		|ВидЦенности, ИсправленныйСчетФактура, Содержание, Период, Регистратор, КодВидаОперации", 
		"СуммаБезНДС, НДС");
...
КонецПроцедуры // ДвиженияПоВычетамПриИзмененииСтоимости()
