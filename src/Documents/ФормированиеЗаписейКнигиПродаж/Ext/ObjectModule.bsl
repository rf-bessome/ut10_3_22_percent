...
Функция ЗаполнитьНДСНачисленныйДаннымиПоНДСВосстановленномуПоАвансам()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
				   |	НДСсАвансовОстатки.НДСОстаток - ЕСТЬNULL(НДСПредъявленныйОстатки.НДСОстаток, 0) > 0";
...
Функция ПодготовитьТаблицуЗаписей(СтруктураШапкиДокумента)
	...
	иначе
		...
		|	ТаблицаЗаписей.ДоговорАванса КАК ДоговорКонтрагента,
		|	NULL,
		|	NULL,
		|	""""
		|ИЗ
...
Процедура ДвиженияПоРегистрамНачисления(СтруктураШапкиДокумента, ТаблицаПоНачислению, Отказ, Заголовок)
	...
	Если НачалоКвартала(Дата) >= '20141001' Тогда
		
		Для Каждого СтрокаТаблицыДвижений Из ТаблицаДвижений_НДСЗаписиКнигиПродаж Цикл
			
			Если (СтрокаТаблицыДвижений.ЗаписьДополнительногоЛиста
				ИЛИ СтрокаТаблицыДвижений.СторнирующаяЗаписьДопЛиста)
					И НачалоКвартала(СтрокаТаблицыДвижений.КорректируемыйПериод) < '20141001' Тогда
				
				Продолжить;
				
			КонецЕсли;
			
			СтрокаТаблицыДвижений.ДокументОплаты = Неопределено;
			СтрокаТаблицыДвижений.ДатаОплаты = '00010101';
			СтрокаТаблицыДвижений.ДатаСобытия = Дата;
	...
	ТаблицаДвижений_НДСЗаписиКнигиПродаж.Свернуть("Период, Активность, Организация, СчетФактура, ИсправленныйСчетФактура, ДокументОплаты, 
		|ДатаОплаты, СтавкаНДС, Покупатель, ДоговорКонтрагента, ДатаСобытия, Событие, ВидЦенности, ЗаписьДополнительногоЛиста, 
