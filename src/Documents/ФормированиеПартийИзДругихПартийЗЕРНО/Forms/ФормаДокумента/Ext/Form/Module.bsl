...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДокументОснование.ДоступныеТипы = ОпределяемыеТипы().ОснованиеФормированиеПартииИзДругихПартийЗЕРНО.Тип;
...
Процедура ХарактеристикаПриИзменении(Элемент)
	...
	СобытияФормЗЕРНОКлиентПереопределяемый.ПриИзмененииХарактеристики(ЭтаФорма, ЭтаФорма, КэшированныеЗначения);
...
Процедура ОКПД2НачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияЗЕРНОКлиент.ОткрытьФормуПодбораОКПД2(ЭтаФорма, ДанныеПартии, Элемент);
...
Процедура МестоположениеКлючАдресаПриИзменении(Элемент)
	...
	МестоположениеКлючАдресаОбновитьНадпись(ЭтаФорма);
КонецПроцедуры
...
Процедура МестоположениеКлючАдресаОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.КлючиАдресовЗЕРНО.ФормаОбъекта", ПараметрыОткрытия, ЭтаФорма,,,,
		Новый ОписаниеОповещения("МестоположениеКлючАдресаОкончаниеРедактирования", ЭтаФорма, ДополнительныеПараметры),
...
Процедура МестоположениеНадписьНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		ПоказатьВыборИзСписка(
			Новый ОписаниеОповещения("МестоположениеКлючАдресаОкончаниеВыбораИзСписка", ЭтаФорма),
			ОператорыАдреса,
			Элемент,
			ОператорыАдреса[0]);
...
Процедура УстановитьУсловноеОформление()
	...
	СобытияФормЗЕРНО.УстановитьУсловноеОформлениеКоличестваДляПустойНоменклатуры(
		ЭтаФорма, "Количество", "Номенклатура");
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ИнтеграцияЗЕРНОКлиентСервер.УстановитьПараметрыВыбораНоменклатуры(ЭтаФорма, Объект.ВидПродукции, "Номенклатура");
	
	ЗаполнитьДоступныеОперации(ЭтаФорма);
...
Процедура ИнициализироватьСлужебныеРеквизиты()
	...
	Если ЗначениеЗаполнено(ОКПД2) Тогда
		...
		ИнтеграцияЗЕРНО.ИнициализироватьКэшВидовСельхозКультур(ЭтаФорма, ОКПД2);
...
Процедура ОбработкаРезультатаПодбораПартийНаСервере(Партия, ИдентификаторСтроки, ПараметрыОбработки)
	...
	Для Каждого СтрокаПартии Из ТаблицаПодобранныхПартий Цикл
		...
		Если ПараметрыОбработки.ЗаполнитьОКПД2 Тогда
			...
			ЗаполнитьЗначенияСвойств(ЭтаФорма, СтрокаПартии, "ОКПД2, ОКПД2Представление, КодТНВЭД, Номенклатура, Характеристика, Серия");
...
Процедура ОбновитьДанные(Результат = Неопределено, ДополнительныеПараметры) Экспорт
	...
	Если ИмяОбновляемогоПоля = "ВидПродукции" Тогда
		...
		ОчиститьРеквизитыОбъекта(ЭтаФорма);
...
Процедура ОКПД2ПриИзмененииНаКлиенте(ОКПД2, ДополнительныеПараметры)
	...
	Если ЗначениеЗаполнено(ОКПД2) Тогда
		...
		СвойстваЗаполненыПоДаннымКэша = ИнтеграцияЗЕРНОКлиентСервер.ЗаполнитьПотребительскиеСвойстваПоДаннымКеша(ЭтаФорма, ОКПД2);
		ДополнительныеПараметры.Вставить("ОбновитьПотребительскиеСвойства", Не СвойстваЗаполненыПоДаннымКэша);
...
Функция МестоположениеКлючАдресаСписокАвтоподбора(ТекстДляАвтоПодбора, КлючВладельца, КэшированныеЗначенияПоОрганизации)
	...
	ОписаниеТипов = ОпределяемыеТипы().СкладКонтрагентЗЕРНО.Тип;
