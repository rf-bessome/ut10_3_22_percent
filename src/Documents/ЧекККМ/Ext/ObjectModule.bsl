...
Функция ПровестиИРаспечататьЧек(Ответ, Отказ, ФормаДокумента, РучнойРежимПечати = Ложь, УникальныйИдентификатор, ПараметрыККТ = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	НЕ ЕСТЬNULL(УчетнаяПолитикаНалоговыйУчетСрезПоследних.ОрганизацияНеЯвляетсяПлательщикомНДС, ИСТИНА) КАК ОплачиваетНДС,
			|	УчетнаяПолитикаНалоговыйУчетСрезПоследних.Организация,
			|	МАКСИМУМ(УчетнаяПолитикаНалоговыйУчетСрезПоследних.Период) КАК Период
			|ПОМЕСТИТЬ вт_ДанныеПоОрганизации
			|ИЗ
			|	РегистрСведений.УчетнаяПолитикаНалоговыйУчет.СрезПоследних(, ) КАК УчетнаяПолитикаНалоговыйУчетСрезПоследних
			|
			|СГРУППИРОВАТЬ ПО
			|	УчетнаяПолитикаНалоговыйУчетСрезПоследних.Организация,
			|	НЕ ЕСТЬNULL(УчетнаяПолитикаНалоговыйУчетСрезПоследних.ОрганизацияНеЯвляетсяПлательщикомНДС, ИСТИНА)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	вт_Предытог.Номенклатура,
			|	вт_Предытог.ХарактеристикаНоменклатуры,
			|	вт_Предытог.СерияНоменклатуры,
			|	вт_Предытог.ЕдиницаИзмерения,
			|	вт_Предытог.Цена,
			|	вт_Предытог.Количество,
			|	вт_Предытог.ПроцентСкидкиНаценки,
			|	вт_Предытог.СтавкаНДС,
			|	вт_Предытог.Штрихкод,
			|	вт_Предытог.АлкогольнаяПродукция,
			|	вт_Предытог.Крепость,
			|	вт_Предытог.Маркируемая,
			|	вт_Предытог.ШтрихкодМарки,
			|	вт_Предытог.ИННПроизводителя,
			|	вт_Предытог.КПППроизводителя,
			|	вт_Предытог.Объем,
			|	вт_Предытог.КодВидаПродукции,
			|	вт_ДанныеПоОрганизации.ОплачиваетНДС КАК ОплачиваетНДС
			|ИЗ
			|	вт_Предытог КАК вт_Предытог
			|		ЛЕВОЕ СОЕДИНЕНИЕ вт_ДанныеПоОрганизации КАК вт_ДанныеПоОрганизации
			|		ПО вт_Предытог.Организация = вт_ДанныеПоОрганизации.Организация");
			...
			// +++ Лупонос Д.В. 2026-01-26 Притащил из УТ 11.5. Как хранится структура в атолле. Справочно 
			СистемыНалогообложения = Новый Соответствие();
			СистемыНалогообложения.Вставить("ОСН", 0);
			СистемыНалогообложения.Вставить("УСНДоход", 1);
			СистемыНалогообложения.Вставить("УСНДоходРасход", 2);
			СистемыНалогообложения.Вставить("ЕНВД", 3);
			СистемыНалогообложения.Вставить("ЕСН", 4);
			СистемыНалогообложения.Вставить("Патент", 5);

			СтавкаНДСДЛЯУСН = Константы.СтавкаНДСДляККМПоУСН.Получить(); 
 			// --- Лупонос Д.В. 2026-01-26 Притащил из УТ 11.5. Как хранится структура в атолле. Справочно. А вот для усн из константы
			...
			Пока Выборка.Следующий() Цикл
				...
				// +++ Лупонос Д.В. - ставка НДС не ноль, а значение константы для налогового режима без НДС 
				//Товар.СтавкаНДС    = 0;
				// захардкодим УСН 6% 
				Если Выборка.ОплачиваетНДС Тогда 
					Товар.СтавкаНДС    = 0;
				Иначе
					Товар.СтавкаНДС    = СтавкаНДСДЛЯУСН; 		
				КонецЕсли; 
				// --- Лупонос Д.В. - ставка НДС не ноль, а значение константы для налогового режима без НДС   
...
Функция МожноРассчитыватьАвтоматическиеСкидки() Экспорт
	...
	Если мРассчитыватьАвтоматическиеСкидки = Неопределено Тогда
		// +++ Лупонос Д.В. 2026-01-26 
		ЗначенияУчетнойПолитики = ПолучитьЗначениеУчетнойПолитики(); 
		Если ЗначенияУчетнойПолитики.Свойство("ИспользоватьСкидкиПоКоличествуТовара") И ЗначенияУчетнойПолитики.ИспользоватьСкидкиПоКоличествуТовара
		 Или ЗначенияУчетнойПолитики.Свойство("ИспользоватьСкидкиПоСуммеДокумента") И ЗначенияУчетнойПолитики.ИспользоватьСкидкиПоСуммеДокумента
		 Или ЗначенияУчетнойПолитики.Свойство("ИспользоватьСкидкиПоВидуОплаты") И ЗначенияУчетнойПолитики.ИспользоватьСкидкиПоВидуОплаты
		 Или ЗначенияУчетнойПолитики.Свойство("ИспользоватьСкидкиПоДисконтнойКарте") И ЗначенияУчетнойПолитики.ИспользоватьСкидкиПоДисконтнойКарте Тогда
...
Процедура ПодготовитьДанныеДляФискализацииЧека(ЧекККМ, ОбщиеПараметры, ДопДанные, ТО) Экспорт
	...
	// +++ Лупонос Д.В. Корректировка таблицы оплат
	//Если ОбщиеПараметры.СистемаНалогообложения = 0 Тогда 
...
Функция ФискализироватьЧек(ФУ, ДопДанные, Ответ, Отказ, ОшибкаТО, ПараметрыЧека)
	...
	// +++ Лупонос Д.В. 2026-01-26 Притащил из УТ 11.5. Как хранится структура в атолле. Справочно 
	СистемыНалогообложения = Новый Соответствие();
	СистемыНалогообложения.Вставить("ОБЩАЯ", 22);
	СистемыНалогообложения.Вставить("УСН ДОХОД", 0);
	СистемыНалогообложения.Вставить("УСНДоходРасход", 0);
	СистемыНалогообложения.Вставить("ЕНВД", 0);
	СистемыНалогообложения.Вставить("ЕСН", 0);
	СистемыНалогообложения.Вставить("ПАТЕНТ", 0);
	СистемыНалогообложения.Вставить("НЕ ОПРЕДЕЛЕНО", 22);

	СтрокаСистемаНалогообложения = СистемыНалогообложения.Получить(ВРег(ПараметрыЧека.СистемаНалогообложения)); 
	
	Для Каждого ПозицияЧека Из ПараметрыЧека.ПозицииЧека Цикл 
		Если ПозицияЧека.Свойство("СтавкаНДС") Тогда 
			ПозицияЧека.СтавкаНДС 		= СтрокаСистемаНалогообложения; 
			Если ПозицияЧека.СтавкаНДС 	= 0 Тогда 
				ПозицияЧека.СуммаНДС 	= 0; 
				ПозицияЧека.СтавкаНДС 	= 0; 
			КонецЕсли; 
		КонецЕсли; 
	КонецЦикла; 
	// --- Лупонос Д.В. 2026-01-26 Притащил из УТ 11.5. Как хранится структура в атолле. Справочно. А вот для усн из константы
