...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ФормаБезОбработки") = Ложь И ЭтаФорма.ЗапретитьИзменение = Ложь Тогда
		Элементы.СсылкаВалютаШтрафа.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка." + ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьИмяПрикладногоСправочника("Валюты"));
	КонецЕсли;
...
Процедура СсылкаВалютаШтрафаПриИзменении(Элемент)
	...
	ОбменСГИСЭПДКлиентСервер.ЗаполнитьРеквизитыПоСсылке(Элемент, ЭтаФорма, Элементы.ТитулПеревозчикаПриемкаРасчетыШтрафов.ТекущиеДанные);
