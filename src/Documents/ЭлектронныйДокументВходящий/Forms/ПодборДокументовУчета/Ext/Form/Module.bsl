...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		Если ЭлектронныйДокумент.ВидЭД = Перечисления.ВидыЭД.МашиночитаемаяДоверенность Тогда
			ЭД = ЭлектронныеДокументыСлужебный.ПрисоединенныйФайл(ЭлектронныйДокумент.Ссылка);
			ФайлДанныхСсылка = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьДанныеЭД(ЭД);
			ДвоичныеДанныеАрхива = ПолучитьИзВременногоХранилища(ФайлДанныхСсылка);
			УдалитьИзВременногоХранилища(ФайлДанныхСсылка);
			ВерсияФорматаМЧД = МашиночитаемыеДоверенности.ВерсияФорматаПоДаннымЭлектронногоДокумента(ДвоичныеДанныеАрхива);
			Если ВерсияФорматаМЧД = МашиночитаемыеДоверенностиКлиентСервер.ФорматМЧД_003() Тогда
				СписокТиповДокументов = МашиночитаемыеДоверенностиКлиентСервер.СпособыОтраженияВУчетеМЧД003();
			КонецЕсли;
		КонецЕсли;		
		...
		ИначеЕсли ЭлектронныйДокумент.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД Тогда
			СписокТиповДокументовТН = ЭлектронныеДокументыСлужебный.СписокОперацийВидаЭД(Перечисления.ВидыЭД.ТОРГ12Продавец);
			...
			Для Каждого ЭлементСпискаСФ Из СписокТиповДокументов Цикл
				Для Каждого ЭлементСпискаТН Из СписокТиповДокументовТН Цикл
					...
					НоваяКоманда = Команды.Добавить("Создать_" + ЭлементСпискаСФ.Значение + "_И_" + ЭлементСпискаТН.Значение);
		...
		ИначеЕсли СписокТиповДокументов.Количество() = 1 Тогда
			...
			НоваяКоманда = Команды.Добавить("Создать_" + СписокТиповДокументов[0].Значение);
			НоваяКоманда.Действие = "СоздатьДокументУчета";
			Элементы.ФормаСоздатьДокумент.ИмяКоманды = "Создать_" + СписокТиповДокументов[0].Значение;

			НоваяКоманда = Команды.Добавить("Прикрепить_" + СписокТиповДокументов[0].Значение);
			НоваяКоманда.Действие = "ПодобратьДокумент";
		...
		Иначе
			...
			Для Каждого ЭлементСписка Из СписокТиповДокументов Цикл
				...
				НоваяКоманда = Команды.Добавить("Создать_" + ЭлементСписка.Значение);
				НоваяКоманда.Действие = "СоздатьДокументУчета";
				...
				НоваяКоманда = Команды.Добавить("Прикрепить_" + ЭлементСписка.Значение);
				НоваяКоманда.Действие = "ПодобратьДокумент";
...
Процедура ПерезаполнитьТекущий(Команда)
	...
	Если ТекСтрока <> Неопределено Тогда
		...
		Если Результат = КодВозвратаДиалога.Да Тогда
			ЭлектронныеДокументыКлиент.ПерезаполнитьДокумент(ТекСтрока.ДокументОснование, , , ЭлектронныйДокумент.Ссылка, СпособОбработки);
			ОповеститьВладельца();
			
			ОповещениеОЗавершении = Новый ОписаниеОповещения("СоздатьДокументУчетаЗавершение", ЭтаФорма);
			МашиночитаемыеДоверенностиКлиент.ПослеПерезаполненияОбъектаУчета(ОповещениеОЗавершении,
				ЭлектронныйДокумент.ВидЭД, ТекСтрока.ДокументОснование, Неопределено);			
...
Процедура ПодобратьДокумент(Команда)
	...
	Если ЗначениеЗаполнено(ИмяТипа) Тогда
		...
		ВвестиЗначение(Результат, Подсказка, ИмяТипа);  
...
Процедура СоздатьДокументУчета(Команда)
	...
	ОповещениеОЗавершении = Новый ОписаниеОповещения("СоздатьДокументУчетаЗавершение", ЭтаФорма);
	МашиночитаемыеДоверенностиКлиент.ПослеОтраженияВУчете(ОповещениеОЗавершении, ЭлектронныйДокумент.ВидЭД, ЭлектронныйДокумент.ДокументыОснования, 
		Неопределено);	
...
Процедура УдалитьСвязьСДокументом(Команда)
	...
	Если ТекСтрока <> Неопределено Тогда
		...
		Если Результат = КодВозвратаДиалога.Да Тогда
			УдалитьСвязьСДокументомНаСервере(ТекСтрока.ДокументОснование, ЭлектронныйДокумент.Ссылка);
			Прочитать();
			ОповеститьВладельца();
			УправлениеДоступностью();
		КонецЕсли;
...
Процедура ОповеститьВладельца()
	...
	Оповестить("ЭлектронныйДокументВходящий_ПодборДокументаУчета",ЭлектронныйДокумент.Ссылка, ВладелецФормы);
...
Процедура СоздатьДокументУчетаНаСервере(СпособОбработки)
	...
	КонецЕсли;
	СтруктураПараметров.Вставить("ФайлДанныхСсылка", ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьДанныеЭД(ЭД));
	...
	Если ЗначениеЗаполнено(ДокументыУчета) Тогда
		...
		Для каждого Основание Из ДокументыУчета Цикл
			...
			Если ТипЗнч(СпособОбработки) = Тип("Структура") Тогда												
				Если ЭлектронныеДокументыВнутренний.ДокументЯвляетсяСчетомФактурой(Основание) Тогда
			...
			ЭлектронныеДокументыСлужебный.УстановитьСсылкуДляВладельцаВРегистреСостояний(Основание, ЭлектронныйДокумент.Ссылка);
		КонецЦикла;
...
Процедура УдалитьСвязьСДокументомНаСервере(СсылкаНаОбъект,ЭД)
	...
	Если ЭлектронныйДокумент.ВидЭД <> Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
		ЭлектронныеДокументыСлужебный.УдалитьСсылкуДляВладельцаВРегистреСостояний(СсылкаНаОбъект,ЭД);
	КонецЕсли;
...
Процедура ПерепривязатьЭлектронныйДокумент(ВыбранноеЗначение,СпособОбработки)
	...
	Если ЭлектронныйДокумент.ВидЭД <> Перечисления.ВидыЭД.ПроизвольныйЭД Тогда
		ЭлектронныеДокументыСлужебный.УстановитьСсылкуДляВладельцаВРегистреСостояний(ВыбранноеЗначение,ЭлектронныйДокумент.Ссылка);
	КонецЕсли;
