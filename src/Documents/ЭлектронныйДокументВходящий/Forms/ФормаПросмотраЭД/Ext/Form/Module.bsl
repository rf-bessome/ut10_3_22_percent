...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		Если ТипЗнч(Параметры.Ключ) = Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Тогда
			ПрисоединенныйФайлСсылка = ЭлектронныеДокументыСлужебный.ПрисоединенныйФайл(Параметры.Ключ,,Истина);
	...
	Если ЗначениеЗаполнено(ПрисоединенныйФайлСсылка) Тогда
		СвойстваФайлаЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайлСсылка,
			"НаправлениеЭД, ДополнительнаяИнформация, Расширение");
		Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Интеркампани Тогда
			ОформитьКолонкуПодписант = Ложь;
		КонецЕсли;
		
		Если ЭтоПЭД И СвойстваФайлаЭД.Расширение = "xml" Тогда
			ДанныеФайла = ПрисоединенныеФайлы.ПолучитьДвоичныеДанныеФайла(ПрисоединенныйФайлСсылка);
			Если ДанныеФайла <> Неопределено Тогда
				ИмяФайлаXML = ПолучитьИмяВременногоФайла("xml");
				ДанныеФайла.Записать(ИмяФайлаXML);
...
Процедура ТекстДокументИБНажатие()
	...
	Если КоличествоДокументов = 1 Тогда
		...
		Если Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.КаталогТоваров") Тогда
			ЭлектронныеДокументыКлиент.ПерезаполнитьДокумент(МассивДокументов[0], , , Объект.Ссылка);
		Иначе
...
&НаКлиенте
Процедура ГиперссылкаТекстПодборНажатие(Элемент, СтандартнаяОбработка)
...
Процедура ТекстВложениеНажатие(Элемент, СтандартнаяОбработка)
	...
	ЭлектронныеДокументыКлиент.ОткрытьФайл(ДанныеФайла, Ложь);
...
Процедура Завершить(Команда)
	...
	ВвестиЗначение(Результат, ВводСтрокиЗаголовок, Тип("Строка"));
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Если Результат <> Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Для закрытия документооборота необходимо указать причину.");
		КонецЕсли;
		
		Возврат;
	КонецЕсли;
	
	ЗавершитьЭДОНаСервере(Результат);
...
Процедура ИнициализацияДанныхНаСервере(Отказ)
	...
	Если ЭлектронныеДокументыСлужебныйВызовСервера.ВыполнятьКриптооперацииНаСервере() Тогда
		МассивОтпечатков = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьМассивСтруктурСертификатов(Истина);
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	СвойстваФайлаЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайлСсылка,
		"ЭлектронныйДокументВладелец,СтатусЭД,НаправлениеЭД,ТипЭлементаВерсииЭД,ДополнительнаяИнформация");
	...
	МожноОтклонитьЭтотЭД = ЕстьПравоОбработки И ЭлектронныеДокументыСлужебныйВызовСервера.МожноОтклонитьЭтотЭД(СсылкаНаЭД);
	МожноАннулироватьЭтотЭД = ЕстьПравоОбработки И ЭлектронныеДокументыСлужебныйВызовСервера.МожноАннулироватьЭтотЭД(СсылкаНаЭД);
	...
	Если ПараметрыВыводаПредупреждения = Неопределено Тогда
		Если СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД 
			Или СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.МашиночитаемаяДоверенность Тогда
	...
	Если СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий Тогда
		...
		Элементы.КомандаУтвердить.Видимость = СвойстваЭД.СостояниеЭДО = Перечисления.СостоянияВерсийЭД.НаУтверждении
		...
		КомандаПодписиОтправки.Доступность = НЕ СтатусОтклонен И Не ЭДОЗакрыт
			И (СтатусЭД = Перечисления.СтатусыЭД.Получен ИЛИ СтатусЭД = Перечисления.СтатусыЭД.Утвержден);
		...
		Если НЕ ЭтоСлужебный И НЕ Объект.ТребуетсяПодтверждение И (СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.СчетФактура
	...
	ИначеЕсли СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		КомандаПодписиОтправки.Видимость = ЕстьВозможностьПодписания И (ТребуетсяПодпись(Объект.Ссылка) ИЛИ ЭтоИзвещениеОПолучении) 
		...
		Если НЕ ЭДОЗакрыт И НЕ КомандаПодписиОтправки.Видимость
			И ЗначениеЗаполнено(ДокументыКОтправке(ДеревоПодчиненныеЭД, ПрисоединенныйФайлСсылка)) Тогда
			Элементы.КомандаОтправить.Видимость   = Истина;
			Элементы.КомандаОтправить.Доступность = Истина;
		Иначе
...
Процедура ОбработатьПредложениеОбАннулировании(ОтклонитьАннулирование)
	...
	СтруктураВладельцаЭД = ПолучитьЭлектронныйДокументВладелец();
	ЭлектронныеДокументыСлужебныйКлиент.ОбработатьПредложениеОбАннулировании(СтруктураВладельцаЭД.Ссылка, ОтклонитьАннулирование);
...
Функция ПолучитьЭлектронныйДокументВладелец()
	...
	Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаЭД, "Организация");
	
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("Ссылка",      СсылкаНаЭД);
...
Процедура ОтклонитьАннулироватьЭД(Отклонить = Ложь)
	...
	СтруктураВладельцаЭД = ПолучитьЭлектронныйДокументВладелец();
	ПераметрыЭД = Новый Структура("Отклонить, Организация", Отклонить, СтруктураВладельцаЭД.Организация);
		
	Если ЭлектронныеДокументыСлужебныйКлиент.ОбработатьОтклонениеАннулированиеЭД(СтруктураВладельцаЭД.Ссылка, ПераметрыЭД) Тогда
		ИзменитьВидимостьДоступность();
		ОбновитьСостояниеЭД();
	КонецЕсли;
...
&НаСервере
Процедура ЗавершитьЭДОНаСервере(Результат)
...
Процедура ИнициализацияДереваНаСервере()
	...
	РеквизитыНастройки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.НастройкаЭДО, "СпособОбменаЭД");
	
	НастройкиОбмена = Новый Структура;
	НастройкиОбмена.Вставить("ВидЭД",               Объект.ВидЭД);
	НастройкиОбмена.Вставить("НаправлениеЭД",       Перечисления.НаправленияЭД.Входящий);
	...
	НастройкиОбмена.Вставить("СпособОбменаЭД",      РеквизитыНастройки.СпособОбменаЭД);
	...
	НастройкиОбмена.Вставить("ТребуетсяИзвещение",     Объект.ТребуетсяИзвещение);
	НастройкиОбмена.Вставить("ТребуетсяПодтверждение", Объект.ТребуетсяПодтверждение);
...
Процедура ВывестиДокументыУчета()
	...
	Если Объект.ВидЭД = Перечисления.ВидыЭД.МашиночитаемаяДоверенность Тогда
	...
	Если КоличествоДокументов = 0 Тогда
		...
		Иначе
			...
			Если АктуальныеВидыЭД.Найти(Объект.ВидЭД) = Неопределено Тогда
				Элементы.ГруппаНевозможноОтразитьВУчете.Видимость = Истина;
