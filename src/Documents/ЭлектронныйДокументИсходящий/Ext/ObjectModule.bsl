...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ДанныеЗаполнения <> Неопределено И ОбщегоНазначения.ЭтоСсылка(ТипЗначенияДанныеЗаполнения) Тогда
		...
		Иначе
			...
			Иначе
				ОснованиеСсылка = ДанныеЗаполнения;
			КонецЕсли;
			СтруктураРеквизитов = ЭлектронныеДокументыСлужебный.ЗаполнитьПараметрыЭДПоИсточнику(ОснованиеСсылка);
		...
		Если Метаданные.Документы.Содержит(ОснованиеСсылка.Метаданные()) Тогда
			...
			Запрос.УстановитьПараметр("ОсновныеТипыЭлектронныхДокументов", ЭлектронныеДокументыКлиентСервер.ОсновныеТипыЭД());
...
Процедура ПоместитьВложениеВоВременноеХранилище(ПрисоединенныйФайлСсылка, ВладелецФайла)
	...
	ЭлектронныеДокументыСлужебный.ПоместитьПараметрВПараметрыКлиентаНаСервере(ВладелецФайла, АдресХранилища);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтотОбъект.ЭтоНовый() Тогда
		...
		Если Не ЗначениеЗаполнено(Ответственный) Тогда
			Ответственный = ЭлектронныеДокументыПереопределяемый.ПолучитьОтветственногоПоЭД(Контрагент, НастройкаЭДО);
