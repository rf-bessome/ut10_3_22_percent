...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ТипЭД = Неопределено;		
		Если Параметры.Свойство("ТипЭД", ТипЭД) И ТипЭД = Перечисления.ТипыЭД.МашиночитаемаяДоверенность Тогда
			Доверенность = Неопределено;
			Параметры.Свойство("Ссылка", Доверенность);
			
			РеквизитыДоверенности = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Доверенность, "ДатаВыдачи, НомерДоверенности");
			
			РезультатВыгрузки = МашиночитаемыеДоверенности.ВыгрузитьДанныеДоверенности(Доверенность);
			
			Если РезультатВыгрузки.Ошибка Тогда				
				ТекстОшибки = РезультатВыгрузки.ТекстОшибки;
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
				Отказ = Истина;
				Возврат;
			КонецЕсли;  
			
			АдресКаталога = ЭлектронныеДокументыСлужебный.РабочийКаталог(, Доверенность.УникальныйИдентификатор());
			ПолноеИмяФайла = АдресКаталога + РезультатВыгрузки.ОписаниеФайла.ИмяФайла;
			ДвоичныеДанныеМЧД = РезультатВыгрузки.ОписаниеФайла.ДвоичныеДанные;	
			ДвоичныеДанныеМЧД.Записать(ПолноеИмяФайла);

			АдресХранилища = ПоместитьВоВременноеХранилище(ДвоичныеДанныеМЧД, УникальныйИдентификатор); 
			
			ФайлДоверенности = Новый Файл(ПолноеИмяФайла);
			
			СтруктураФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ПолноеИмяФайла);
			СтруктураФайла.Вставить("Размер", ФайлДоверенности.Размер());
			СтруктураФайла.Вставить("ДатаМодификацииУниверсальная", ФайлДоверенности.ПолучитьУниверсальноеВремяИзменения());
			
			ПолучитьФайлИзХранилища(АдресХранилища, СтруктураФайла);
			
			ЭлектронныеДокументыСлужебный.УдалитьПапкуНаСервере(ПолноеИмяФайла);
			
			Объект.ТипДокумента = ТипЭД;  
			Объект.НаименованиеДокументаОтправителя = НСтр("ru = 'Машиночитаемая доверенность'");
			Объект.НомерДокументаОтправителя = РеквизитыДоверенности.НомерДоверенности;
			Объект.ДатаДокументаОтправителя = ТекущаяДатаСеанса();
			Объект.СуммаДокумента = 0; 
					
			Основание = Объект.ДокументыОснования.Добавить();
			Основание.ДокументОснование = Доверенность;
			
			ДокументУчета = Доверенность;
	...
	Если ЗначениеЗаполнено(ПрисоединенныйФайлСсылка) Тогда
		СвойстваФайлаЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ПрисоединенныйФайлСсылка,
			"НаправлениеЭД, ДополнительнаяИнформация");
		...
		Если ЭтоПЭД Тогда
			...
			ЭтоПЭД = ПараметрыПроизвольногоДокумента = Неопределено;
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД") Тогда
		...
		Если Объект.ТипДокумента =  ПредопределенноеЗначение("Перечисление.ТипыЭД.МашиночитаемаяДоверенность") Тогда
			Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.МашиночитаемаяДоверенность");
		КонецЕсли;		
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД 
		Или Объект.ВидЭД = Перечисления.ВидыЭД.МашиночитаемаяДоверенность Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ВидЭД = Перечисления.ВидыЭД.ПроизвольныйЭД
		...
		Если ПараметрыПроизвольногоДокумента <> Неопределено Тогда
			КоллекцияЭлементов = ДеревоПодчиненныеЭД.ПолучитьЭлементы();
			Если КоллекцияЭлементов.Количество()
				И ЗначениеЗаполнено(КоллекцияЭлементов[0].ИмяРеквизита)
				И Элементы.СтраницыЭД.ТекущаяСтраница = Элементы.СтраницаПрисоединенныйФайл Тогда 
				Элементы.СтраницыЭД.ТекущаяСтраница = Элементы["Страница" + КоллекцияЭлементов[0].ИмяРеквизита];
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.ПроизвольныйЭД") 
		Или Объект.ВидЭД = ПредопределенноеЗначение("Перечисление.ВидыЭД.МашиночитаемаяДоверенность") Тогда
...
Процедура ТекстВложениеНажатие(Элемент, СтандартнаяОбработка)
	...
	ЭлектронныеДокументыКлиент.ОткрытьФайл(ДанныеФайла, ФайлРедактируется);
...
Процедура ЭППриАктивизацииСтроки(Элемент)
	...
	Элементы.ПроверкаДоверенностиВыполненаВручную.Видимость = Ложь;
	...
	Если ЭтоВходящаяПодпись И ЭтоПодписьПоДоверенности Тогда
		...
		Если ДоступнаКомандаРучнойПроверки Тогда
			Элементы.ПроверкаДоверенностиВыполненаВручную.Видимость = ДоступнаКомандаРучнойПроверки;
...
Процедура ЭПВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ЭПДоверенностьПредставление Тогда	
	...
	ИначеЕсли Поле = Элементы.ЭПСтатус Тогда 
...
Процедура ПроверитьПодписьПоДоверенностиВручную(Команда)
	...
	Если Элементы.ПроверкаДоверенностиВыполненаВручную.Пометка Тогда
		ВыполнитьПроверкуПодписей();
	Иначе
		ВыбраннаяПодпись = Элементы.ТаблицаЭП.ТекущиеДанные; 
		
		ДанныеПодписи = ЭлектронныеДокументыКлиентСервер.НовыеДанныеПодписиСУчетомДоверенности();
...
Процедура РедактироватьЗавершить()
	...
	ЭлектронныеДокументыКлиент.ОткрытьФайл(ДанныеФайла, Истина);
...
Процедура ДобавитьВложениеИзФайлаНаДиске(Команда)
	...
	Если ПоместитьФайл(АдресВременногоХранилища, "", ВыбранныйФайл, Истина, УникальныйИдентификатор) Тогда
		...
		СкорректированноеИмяФайла = СкорректироватьИмяФайлаСУчетомКодировки(СтруктураФайла.Имя);
		
		Если Не ЗначениеЗаполнено(СкорректированноеИмяФайла) Тогда
			Возврат;
		КонецЕсли;
		
		СкорректированноеПолноеИмяФайла = ОбщегоНазначенияКлиентСервер.ПолучитьПолноеИмяФайла(СтруктураФайла.Путь, СкорректированноеИмяФайла);		
		СтруктураФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(СкорректированноеПолноеИмяФайла);
		...
		ПолучитьФайлИзХранилища(АдресВременногоХранилища, СтруктураФайла);				
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьПакетЭД(Команда)

	ЭлектронныеДокументыКлиент.ОткрытьПакетЭД(ПрисоединенныйФайлСсылка);
...
Процедура ОбновитьСостояниеКомандыРучнойПроверкиПодписи(РезультатПроверкиПоМЧД)
	...
	Элементы.ПроверкаДоверенностиВыполненаВручную.Пометка = 
...
&НаСервере
Процедура ПолучитьФайлИзХранилища(АдресВременногоХранилища, СтруктураФайла)
...
Процедура ОтправитьПакетыЭД()
	...
	Если ЗначениеЗаполнено(ПакетыЭД) Тогда
		ЭлектронныеДокументыСлужебныйКлиент.ОтправитьМассивПакетовЭД(ПакетыЭД);
	КонецЕсли;
...
Функция НайтиСоздатьПакетыЭлектронныхДокументов(Знач ДеревоПодчиненныеЭД, Знач ПрисоединенныйФайлСсылка, Знач ПризнакПодписи, Знач СостояниеЭДО)
	...
	Если СостояниеЭДО = Перечисления.СостоянияВерсийЭД.ОжидаетсяОтправка Тогда
		...
		ДокументыКОтправке.Добавить(ПрисоединенныйФайлСсылка);
	Иначе
	...
	Если ЗначениеЗаполнено(ДокументыКОтправке) Тогда
		ДанныеПакетовЭД = ЭлектронныеДокументыСлужебный.СоздатьДокументыПакетовЭД(ДокументыКОтправке, ПризнакПодписи);
...
Функция ДокументыКОтправке(ДеревоПодчиненныеЭД, ТекущийЭД)
	...
	СтекЭД = Новый Массив;
	СтекЭД.Добавить(ДеревоПодчиненныеЭД);
...
Процедура ОбновитьСостояниеЭД()
	...
	ЭлектронныеДокументыИзменены = Новый Массив;
	ЭлектронныеДокументыИзменены.Добавить(Объект.Ссылка);
	
	ПараметрыОповещения = Новый Структура;
	ПараметрыОповещения.Вставить("ЭлектронныеДокументы", ЭлектронныеДокументыИзменены);
...
Процедура ОткрытьФормуПодбора()
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ТекстВопроса = НСтр("ru = 'Для выполнения операции необходимо сохранить документ. Продолжить?'");
		Результат = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
...
Процедура ВывестиДокументыУчета()
	...
	Элементы.ГруппаТекстДокументИБ.Видимость = Истина;
	...
	Если КоличествоДокументов = 0 Тогда
		...
		ИначеЕсли Объект.ТипДокумента = Перечисления.ТипыЭД.МашиночитаемаяДоверенность Тогда
			Элементы.ГруппаТекстДокументИБ.Видимость = Ложь;
		Иначе
	...
	ИначеЕсли КоличествоДокументов = 1 Тогда
		...
		Если ТипЗнч(МассивДокументов[0]) = Тип("СправочникСсылка.СоглашенияОбИспользованииЭД") Тогда
			Элементы.ТекстДокументИБ.Заголовок = НСтр("ru = 'Настройка ЭДО'");  
		ИначеЕсли МашиночитаемыеДоверенностиКлиентСервер.ЭтоСсылкаНаДоверенность(МассивДокументов[0]) Тогда
			Элементы.ТекстДокументИБ.Заголовок = НСтр("ru = 'Доверенность'");  
		КонецЕсли;
...
Процедура ИзменитьВидимостьДоступностьНаСервере()
	...
	СвойстваЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Ссылка,
		"ВидЭД,ПрофильНастроекЭДО,НастройкаЭДО,ПричинаОтклонения,СостояниеЭДО");
	...
	ИначеЕсли СвойстваФайлаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Исходящий Тогда
		...
		КомандаПодписиОтправки.Доступность = 
			НЕ СтатусОтклонен И Не ЭДОЗакрыт
			И (ОтраженВУчете ИЛИ ЭтоСлужебный)
			И (СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.Сформирован
				ИЛИ СвойстваФайлаЭД.СтатусЭД = Перечисления.СтатусыЭД.Утвержден);
...
Процедура ЗаполнитьТаблицуЭП()
	...
	Если ТаблицаЭП.НайтиСтроки(Новый Структура("ПодписьВерна", Ложь)).Количество() = 0 
		И ТаблицаЭП.НайтиСтроки(Новый Структура("ЭтоПодписьПоДоверенности, ПодписьВернаПоДоверенности", Истина, Ложь)).Количество() = 0 Тогда
	...
	Если ТаблицаЭП.НайтиСтроки(Новый Структура("ЭтоПодписьПоДоверенности", Истина)).Количество() = 0 Тогда
		Элементы.ЭПДоверенностьПредставление.Видимость = Ложь;
	Иначе
		Элементы.ЭПДоверенностьПредставление.Видимость = Истина;
	КонецЕсли;	
...
Процедура ПоказатьСертификат(НомерСтроки, Отпечаток)
	...
	Если СтруктураСертификата <> Неопределено Тогда
		...
		ЭтоПодписьКонтрагента = ЭлектронныеДокументыСлужебныйВызовСервера.ИнформацияОСертификатеПодписи(Отпечаток) = Неопределено;
...
Функция ДанныеФайлаВложения(АдресВременногоХранилищаВложения, ПрисоединенныйФайлСсылка, УникальныйИдентификатор)
	...
	ИначеЕсли ЗначениеЗаполнено(ПрисоединенныйФайлСсылка) Тогда
		// Файл записан в ИБ
		ДанныеФайла = ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьДанныеФайлаСВизуализацией(ПрисоединенныйФайлСсылка, УникальныйИдентификатор);
...
Процедура ДобавитьОбновитьПрисоединенныеФайлыКСообщению(АдресВХранилище = Неопределено)
	...
	Иначе
		...
		Для Каждого Строка Из Объект.ДокументыОснования Цикл
			ЭлектронныеДокументыСлужебныйВызовСервера.УстановитьНовуюВерсиюЭД(Строка.ДокументОснование);
		КонецЦикла;			
...
Процедура ОбновитьДанныеОВложении()
	...
	Если ЭтоАдресВременногоХранилища(АдресВременногоХранилищаВложения) Тогда
		...
		Если ТипЗнч(СтруктураФайла) = Тип("Структура") И СтруктураФайла.Свойство("СсылкаНаДвоичныеДанныеФайла") Тогда
			...
			ОбновитьПараметрыПроизвольногоДокумента(СтруктураФайла.СсылкаНаДвоичныеДанныеФайла);
			Если ПараметрыПроизвольногоДокумента = Неопределено Тогда
				Объект.ТипДокумента = Перечисления.ТипыЭД.Прочее;
				Объект.НаименованиеДокументаОтправителя = "";
				Объект.НомерДокументаОтправителя = "";
				Объект.ДатаДокументаОтправителя  = '00010101';
				Объект.СуммаДокумента  = 0;
			Иначе
				ЗначениеСвойства = Неопределено;
				Объект.ТипДокумента = ?(ПараметрыПроизвольногоДокумента.Свойство("ТипДокумента", ЗначениеСвойства),
					ЗначениеСвойства, Перечисления.ТипыЭД.Прочее);
				ПараметрыПроизвольногоДокумента.Свойство("НаименованиеДокументаОтправителя", Объект.НаименованиеДокументаОтправителя);
				ПараметрыПроизвольногоДокумента.Свойство("НомерДокументаОтправителя", Объект.НомерДокументаОтправителя);
				ПараметрыПроизвольногоДокумента.Свойство("ДатаДокументаОтправителя", Объект.ДатаДокументаОтправителя);
				ПараметрыПроизвольногоДокумента.Свойство("СуммаДокумента", Объект.СуммаДокумента);
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Результат.Следующий() Тогда
			...
			ОбновитьПараметрыПроизвольногоДокумента();
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьПараметрыПроизвольногоДокумента(АдресДанных = Неопределено)
	
	Если ПрисоединенныйФайлРасширение <> "xml" Тогда
		ПараметрыПроизвольногоДокумента = Неопределено;
		Возврат;
	КонецЕсли;
	
	Если АдресДанных <> Неопределено Тогда
		ДанныеФайлаXML = ПолучитьИзВременногоХранилища(АдресДанных);
	ИначеЕсли ЗначениеЗаполнено(ПрисоединенныйФайлСсылка) Тогда
		ДанныеФайлаXML = ПрисоединенныеФайлы.ПолучитьДвоичныеДанныеФайла(ПрисоединенныйФайлСсылка);
	Иначе
		ПараметрыПроизвольногоДокумента = Неопределено;
		Возврат;
	КонецЕсли;
	...
	ДанныеФайлаXML.Записать(ИмяФайлаXML);
	ПараметрыПроизвольногоДокумента = ЭлектронныеДокументыСлужебный.ПараметрыФайлаПроизвольногоДокумента(ИмяФайлаXML);
	ЭлектронныеДокументыВнутренний.УдалитьВременныеФайлы(ИмяФайлаXML);
...
&НаСервереБезКонтекста
Функция ЗаменитьНедопустимыеСимволыВИмениФайла(Знач Значение)
	
	Результат = СтрЗаменить(Значение, "и" + Символ(774), "й");
	Результат = СтрЗаменить(Результат, "е" + Символ(776), "е");
...
//
&НаКлиенте
Функция СкорректироватьИмяФайлаСУчетомКодировки(Знач ИмяФайла, Кодировка = "windows-1251") Экспорт 
	...
	Если РезультатПроверки.ИмеютсяНекорректныеСимволы Тогда 
		...
		ДополнительныеПараметры = ЭлектронныеДокументыСлужебныйКлиент.ДополнительныеПараметрыВводаСтроки();
		...
		ИмяФайлаИсправленноеПользователем = ЭлектронныеДокументыСлужебныйКлиент.ВвестиСтрокуБЭД(ДополнительныеПараметры);
		Если Не ПустаяСтрока(ИмяФайлаИсправленноеПользователем) Тогда
			Возврат СкорректироватьИмяФайлаСУчетомКодировки(ИмяФайлаИсправленноеПользователем, Кодировка);
		КонецЕсли;
	Иначе
		Возврат ИмяФайла;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаЭП.ЭтоПодписьПоДоверенности");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
    ОтборЭлемента.ПравоеЗначение = Истина;
	
	ОтборЭлемента = ГруппаОтбора1.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаЭП.ПодписьВернаПоДоверенности");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
