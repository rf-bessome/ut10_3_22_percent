...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ВидДокумента) = Ложь Тогда
		ВидДокумента = ОбменСГИСЭПД.ВидДокументаПоТипу(Перечисления.ВидыЭД.ЭПЛ);
	КонецЕсли;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ТекущийТитул = ПредопределенноеЗначение("Перечисление.ТипыЭлементовВерсииЭД.ЭПЛ_Титул1");
...
Процедура ОпределитьРольУчастника()
	...
	РольУчастника = 0;
	Если ТипЗнч(СсылкаТитулОформлениеОформитель) = Тип("СправочникСсылка." + ЭлектронныеДокументыСлужебныйВызовСервера.ПолучитьИмяПрикладногоСправочника("Организации")) Тогда
		РольУчастника = 1;
