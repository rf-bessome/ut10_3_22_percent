...
Перем глПодключаемоеОборудованиеСобытиеОбработаноДанные Экспорт;
...
// Глобальная переменная эмуляции системного перечисления
...
Процедура ПриНачалеРаботыСистемы()
	...
	РаботаСДиалогамиДополнительный.ВключитьИнтерфейсГосИС();
	...
	ДатаПоказаФормыПереходаНаУТ11 = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("Обработка.ПомощникПереходаНаУТ11","ДатаПереходаНаУТ11",ТекущаяДатаСеанса());
	ПоказыватьПриНачалеРаботы     = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("Обработка.ПомощникПереходаНаУТ11","ПоказыватьПомощникПереходаНаУТ11ПриНачалеРаботы", Истина);
	Если ЭтоБазовая и РольДоступна("ПолныеПрава") и ПоказыватьПриНачалеРаботы и ДатаПоказаФормыПереходаНаУТ11 <= ТекущаяДатаСеанса() Тогда
		ОткрытьФорму("Обработка.ПомощникПереходаНаУТ11.Форма.Форма");
	КонецЕсли;
	...
	Если УправлениеПользователями.ПолучитьЗначениеПоУмолчанию(мТекущийПользователь, "ЗагружатьАктуальныеКурсыВалютПриЗапускеСистемы") = Истина 
	   И (Не ПервыйЗапуск) Тогда
		Если ПравоДоступа("Использование", Метаданные.Обработки.КурсыВалютРБК) Тогда
			...
			ОбработкаЗагрузкиКурсовВалют.ЗагрузитьКурсыВалют();
		Иначе
			ОбщегоНазначения.СообщитьОбОшибке("Недостаточно прав доступа к обработке ""Загрузка курсов валют с РБК"". Курсы валют не загружены. Обратитесь к администратору пользователей.");
		КонецЕсли; 
	КонецЕсли;
	...
	Если ПользовательИБ <> Неопределено
	   ...
	   И ПользовательИБ.ОсновнойИнтерфейс.Имя = "ИнтерфейсКассира" Тогда //Задействуем интерфейс кассира
		...
		Если Не Отказ Тогда
			...
			Иначе
				Предупреждение("У кассы: """ + КассаККМ + """, для компьютера: """ + ПолучитьСерверТО().ПолучитьИмяКомпьютераТО() + """, фискальное устройство не установлено!");
				Отказ = Истина;
