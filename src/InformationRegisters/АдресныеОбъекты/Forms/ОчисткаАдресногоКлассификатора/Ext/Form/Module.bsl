...
Процедура ПрерватьОчистку(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПослеВопросаПрерватьОчистку", ЭтаФорма);
...
Процедура ОчиститьКлассификатор()
	...
	Обработчик = Новый ОписаниеОповещения("ПослеФоновойОчистки", ЭтаФорма);
...
Процедура ПослеФоновойОчистки(Задание, ДополнительныеПараметры) Экспорт
	...
	Элементы.ШагиОчистки.ТекущаяСтраница = Элементы.УспешноеЗавершение;
		...
		Оповестить("ОчищенАдресныйКлассификатор", КлассификаторСодержитЗагруженныеАдресныеСведения(), ЭтаФорма);
