...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Запись.АдресУТМ)
		...
		ОбновитьТекстСопоставления(ЭтаФорма);
...
Процедура ПередЗакрытием(Отказ,  СтандартнаяОбработка)
	...
	Если ПодключениеНастроеноКорректно
		...
		ПоказатьВопрос(
			Новый ОписаниеОповещения("ОбработатьОтветНаВопросОЗапросеДанныхОрганизацииЕГАИСПередЗаписью", ЭтаФорма),
			СтрШаблон(
...
Процедура ТекстСопоставлениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "КомандаЗапроситьОрганизациюЕГАИС" Тогда
		...
		Если Модифицированность Тогда
			...
			ПоказатьВопрос(
				Новый ОписаниеОповещения("ОбработатьОтветНаВопросОЗаписиНастроекПередЗапросомОрганизацииЕГАИС", ЭтаФорма),
...
Процедура ОбработатьОтветНаВопросОЗапросеДанныхОрганизацииЕГАИСПередЗаписью(Результат, ДополнительныеПараметры) Экспорт
	...
	НачатьФормированиеЗапросаНаЗагрузкуОрганизации(
		Новый ОписаниеОповещения("ПослеЗакрытияФормыЗапросаДанныхОрганизацииПередЗаписью", ЭтаФорма));
...
Процедура ПроверитьПодключениеКУТМ()
	...
	ПолучитьКодФСРАРИзУТМ(
		Новый ОписаниеОповещения("ПослеПроверкиПодключенияКУТМ", ЭтаФорма, ДополнительныеПараметры), 1);
...
Процедура ОбработатьНавигационнуюСсылку(Знач НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "ПроверитьПодключениеКУТМ" Тогда
		...
		ПолучитьКодФСРАРИзУТМ(
			Новый ОписаниеОповещения("ПослеПроверкиПодключенияКУТМ", ЭтаФорма, ДополнительныеПараметры), Запись.Таймаут)
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ОткрытьWebИнтерфейсУТМ" Тогда
		...
		ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(
			...
			Новый ОписаниеОповещения("ПослеОткрытияWebИнтерфейсаУТМ", ЭтаФорма));
...
Процедура ПроверитьЗаполнениеПолучитьКодФСРАРИзУТМ()
	...
	ПолучитьКодФСРАРИзУТМ(
		Новый ОписаниеОповещения("ПолучениеКодаФСРАР_Завершение", ЭтаФорма),
		Запись.Таймаут);
