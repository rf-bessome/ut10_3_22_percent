...
Процедура ОбменНаСервереПриИзменении(Элемент)
	...
	ПриИзмененииВариантаПодключения(ЭтаФорма);
...
Процедура ПроизводственныйОбъектПриИзменении(Элемент)
	...
	СобытияФормИСМПКлиентПереопределяемый.ПриИзмененииПроизводственногоОбъекта(ЭтаФорма);
...
Процедура ПолучитьИдентификаторСоединенияЧерезИнтернетПоддержку(Команда)
	...
	ИначеЕсли ТребуетсяПодключениеИнтернетПоддержки Тогда
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
			...
			ОповещениеОЗакрытии = Новый ОписаниеОповещения("Подключаемый_ИдентификаторСоединения", ЭтаФорма, "ПолучитьИдентификаторСоединенияЧерезИнтернетПоддержку");
...
Процедура НастроитьРасписание(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ИзменитьРасписаниеОбмена", ЭтаФорма);
...
Процедура ЗапроситьКлючСессииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ПроверитьПодключениеСУЗ(
			Новый ОписаниеОповещения("ПослеПроверкиПодключенияСУЗ", ЭтаФорма, ДополнительныеПараметры),
			Запись.Таймаут,
			Истина);
	КонецЕсли;
...
Процедура ПроверкаПодключениеКСУЗОбработчикОжидания()
	...
	ПроверитьПодключениеСУЗ(
		Новый ОписаниеОповещения("ПослеПроверкиПодключенияСУЗ", ЭтаФорма, ДополнительныеПараметры), 1);
...
Процедура Подключаемый_ИдентификаторСоединения(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ОписаниеПриЗавершении = Новый ОписаниеОповещения(
		"Подключаемый_ПослеПолученияИдентификатораСоединения", ЭтаФорма, ПараметрыОбработкиДокументов);
...
Процедура НачатьПолучениеКодаФИАС()
		...
		Иначе
			ПоказатьВопрос(
				Новый ОписаниеОповещения(
					"ВопросВыполнитьПоискПоАдресномуКлассифкаторуПриЗавершении", ЭтаФорма),
				НСтр("ru = 'Учет по видам продукции, использующим код ФИАС отключен.
	...
	Иначе
		...
		ПоказатьВыборИзСписка(
			Новый ОписаниеОповещения(
				"ВыборВидаПродукцииПриЗавершении", ЭтаФорма),
			СписокВыбора,
...
Процедура ПолучитьМестаОсуществленияДеятельности(ВидПродукции)
	...
	ОписаниеПриЗавершении = Новый ОписаниеОповещения(
		"ПослеПолученияМестОсуществленияДеятельности", ЭтаФорма, ПараметрыОповещения);
...
Процедура ПослеПолученияМестОсуществленияДеятельности(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		ПоказатьВыборИзСписка(
			Новый ОписаниеОповещения(
				"ВыборМестаОсуществленияДеятельностиПриЗавершении", ЭтаФорма),
			СписокМОД,
			Элементы.КодФИАС);
	КонецЕсли;
